...
public class EntityAIMoveThroughVillage extends EntityAIBase {
	...
	public boolean shouldExecute() {
		...
		Village village = theEntity.worldObj.villageCollectionObj
				.findNearestVillage(MathHelper.floor_double(theEntity.posX),
		...
		field_48291_c = theEntity.getNavigator().getPathToXYZ(doorInfo.posX,
		...
		Vec3D vec3d = RandomPositionGenerator
				.func_48620_a(theEntity, 10, 7, Vec3D.createVector(
		...
		if (vec3d == null) {
			...
			theEntity.getNavigator().setBreakDoors(false);
			field_48291_c = theEntity.getNavigator().getPathToXYZ(vec3d.xCoord,
	...
	private VillageDoorInfo func_48284_a(Village par1Village) {
		...
		do {
			...
			VillageDoorInfo villagedoorinfo1 = (VillageDoorInfo) iterator
					.next();
			int j = villagedoorinfo1.getDistanceSquared(
	...
	private boolean func_48285_a(VillageDoorInfo par1VillageDoorInfo) {
		for (Iterator iterator = doorList.iterator(); iterator.hasNext();) {
			...
			if (par1VillageDoorInfo.posX == villagedoorinfo.posX
					&& par1VillageDoorInfo.posY == villagedoorinfo.posY
					&& par1VillageDoorInfo.posZ == villagedoorinfo.posZ) {
				return true;
			}
		}
