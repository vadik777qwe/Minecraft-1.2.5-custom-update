...
public class EntityAITasks {
	...
	public void onUpdateTasks() {
		...
		do {
			...
			EntityAITaskEntry entityaitaskentry = (EntityAITaskEntry) iterator
					.next();
			boolean flag1 = executingTasks.contains(entityaitaskentry);

			if (flag1) {
				if (func_46116_a(entityaitaskentry)
						&& entityaitaskentry.action.continueExecuting()) {
					continue;
				}
			...
			if (func_46116_a(entityaitaskentry)
					&& entityaitaskentry.action.shouldExecute()) {
		...
		for (Iterator iterator1 = arraylist.iterator(); iterator1.hasNext(); entityaitaskentry1.action
				.startExecuting()) {
			entityaitaskentry1 = (EntityAITaskEntry) iterator1.next();

			if (flag) {
				System.out.println((new StringBuilder())
						.append(entityaitaskentry1.action.toString())
						.append(", ").toString());
			}
		}
		...
		for (Iterator iterator2 = executingTasks.iterator(); iterator2
				.hasNext(); entityaitaskentry2.action.updateTask()) {
	...
	private boolean func_46116_a(EntityAITaskEntry par1EntityAITaskEntry) {
		label0: {
			...
			do {
				...
				} while (!executingTasks.contains(entityaitaskentry)
						|| areTasksCompatible(par1EntityAITaskEntry,
								entityaitaskentry));

				return false;
			} while (!executingTasks.contains(entityaitaskentry)
					|| entityaitaskentry.action.isContinuous());
