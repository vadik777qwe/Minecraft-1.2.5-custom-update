...
public abstract class EntityAITarget extends EntityAIBase {
	...
	public EntityAITarget(EntityLiving par1EntityLiving, float par2,
			boolean par3) {
	...
	protected boolean func_48376_a(EntityLiving par1EntityLiving, boolean par2) {
		...
		if (par1EntityLiving.boundingBox.maxY <= taskOwner.boundingBox.minY
				|| par1EntityLiving.boundingBox.minY >= taskOwner.boundingBox.maxY) {
			return false;
		}
		...
		if ((taskOwner instanceof EntityTameable)
				&& ((EntityTameable) taskOwner).isTamed()) {
		...
		if (field_48380_e
				&& !taskOwner.func_48090_aM().canSee(par1EntityLiving)) {
			return false;
		}
	...
	private boolean func_48375_a(EntityLiving par1EntityLiving) {
		...
		PathEntity pathentity = taskOwner.getNavigator().func_48679_a(
				par1EntityLiving);
