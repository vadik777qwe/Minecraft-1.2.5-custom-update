...
public class RenderPainting extends Render {
	...
	public void func_158_a(EntityPainting par1EntityPainting, double par2,
		...
		float f = 0.0625F;
		GL11.glScalef(f, f, f);
		func_159_a(par1EntityPainting, enumart.sizeX, enumart.sizeY,
				enumart.offsetX, enumart.offsetY);
		GL11.glDisable(GL12.GL_RESCALE_NORMAL);
		GL11.glPopMatrix();
	}

	private void func_159_a(EntityPainting par1EntityPainting, int par2,
		...
		for (int i = 0; i < par2 / 16; i++) {
			for (int j = 0; j < par3 / 16; j++) {
				...
				func_160_a(par1EntityPainting, (f4 + f5) / 2.0F,
						(f6 + f7) / 2.0F);
	...
	private void func_160_a(EntityPainting par1EntityPainting, float par2,
			float par3) {
		int i = MathHelper.floor_double(par1EntityPainting.posX);
		int j = MathHelper.floor_double(par1EntityPainting.posY
				+ (double) (par3 / 16F));
		int k = MathHelper.floor_double(par1EntityPainting.posZ);

		if (par1EntityPainting.direction == 0) {
			i = MathHelper.floor_double(par1EntityPainting.posX
					+ (double) (par2 / 16F));
		}

		if (par1EntityPainting.direction == 1) {
			k = MathHelper.floor_double(par1EntityPainting.posZ
					- (double) (par2 / 16F));
		}
		...
		int l = renderManager.worldObj.getLightBrightnessForSkyBlocks(i, j, k,
				0);
		int i1 = l % 0x10000;
		int j1 = l / 0x10000;
