...
public class BlockPistonBase extends Block {
	...
	/**
	 * Return the either 106 or 107 as the texture index depending on the
	 * isSticky flag. This will actually never get called by
	...
	public int getBlockTextureFromSideAndMetadata(int par1, int par2) {
		...
		if (par1 == i) {
			if (isExtended(par2) || minX > 0.0D || minY > 0.0D || minZ > 0.0D
	 ...
	 */
	public boolean blockActivated(World par1World, int par2, int par3, int i,
	...
	private boolean isIndirectlyPowered(World par1World, int par2, int par3,
			int par4, int par5) {
		if (par5 != 0
				&& par1World.isBlockIndirectlyProvidingPowerTo(par2, par3 - 1,
						par4, 0)) {
			return true;
		}
		...
		if (par5 != 2
				&& par1World.isBlockIndirectlyProvidingPowerTo(par2, par3,
						par4 - 1, 2)) {
			return true;
		}
		...
		if (par5 != 5
				&& par1World.isBlockIndirectlyProvidingPowerTo(par2 + 1, par3,
						par4, 5)) {
			return true;
		}
		...
		if (par1World
				.isBlockIndirectlyProvidingPowerTo(par2, par3 + 2, par4, 1)) {
			return true;
		}

		if (par1World.isBlockIndirectlyProvidingPowerTo(par2, par3 + 1,
				par4 - 1, 2)) {
			return true;
		}
		...
		if (par1World.isBlockIndirectlyProvidingPowerTo(par2 - 1, par3 + 1,
				par4, 4)) {
			return true;
		}

		return par1World.isBlockIndirectlyProvidingPowerTo(par2 + 1, par3 + 1,
				par4, 5);
	}

	public void powerBlock(World par1World, int par2, int par3, int par4,
			int par5, int par6) {
		ignoreUpdates = true;
		int i = par6;
		...
		if (par5 == 0) {
			...
			TileEntity tileentity = par1World.getBlockTileEntity(par2
					+ Facing.offsetsXForSide[i], par3
					+ Facing.offsetsYForSide[i], par4
			...
			par1World.setBlockAndMetadata(par2, par3, par4,
					Block.pistonMoving.blockID, i);
			par1World
					.setBlockTileEntity(par2, par3, par4, BlockPistonMoving
							.getTileEntity(blockID, i, i, false, true));
			...
			if (isSticky) {
				...
				if (i1 == Block.pistonMoving.blockID) {
					TileEntity tileentity1 = par1World.getBlockTileEntity(j, k,
							l);

					if (tileentity1 != null
							&& (tileentity1 instanceof TileEntityPiston)) {
						TileEntityPiston tileentitypiston = (TileEntityPiston) tileentity1;

						if (tileentitypiston.getPistonOrientation() == i
				...
				if (!flag
						&& i1 > 0
						&& canPushBlock(i1, par1World, j, k, l, false)
						&& (Block.blocksList[i1].getMobilityFlag() == 0
								|| i1 == Block.pistonBase.blockID || i1 == Block.pistonStickyBase.blockID)) {
					par2 += Facing.offsetsXForSide[i];
					par3 += Facing.offsetsYForSide[i];
					par4 += Facing.offsetsZForSide[i];
					par1World.setBlockAndMetadata(par2, par3, par4,
							Block.pistonMoving.blockID, j1);
					par1World.setBlockTileEntity(par2, par3, par4,
							BlockPistonMoving.getTileEntity(i1, j1, i, false,
									false));
					ignoreUpdates = false;
					...
					ignoreUpdates = false;
					par1World.setBlockWithNotify(par2
							+ Facing.offsetsXForSide[i], par3
							+ Facing.offsetsYForSide[i], par4
							+ Facing.offsetsZForSide[i], 0);
					ignoreUpdates = true;
				}
			} else {
				ignoreUpdates = false;
				par1World.setBlockWithNotify(par2 + Facing.offsetsXForSide[i],
						par3 + Facing.offsetsYForSide[i], par4
								+ Facing.offsetsZForSide[i], 0);
				ignoreUpdates = true;
			}
	...
	public void getCollidingBoundingBoxes(World par1World, int par2, int par3,
		...
		setBlockBounds(0.0F, 0.0F, 0.0F, 1.0F, 1.0F, 1.0F);
		super.getCollidingBoundingBoxes(par1World, par2, par3, par4,
				par5AxisAlignedBB, par6ArrayList);
	}
	...
	private boolean tryExtend(World par1World, int par2, int par3, int par4,
		...
		do {
			...
			if (Block.blocksList[j1].getMobilityFlag() == 1) {
				Block.blocksList[j1].dropBlockAsItem(par1World, i, j, k,
						par1World.getBlockMetadata(i, j, k), 0);
		...
		for (; i != par2 || j != par3 || k != par4; k = l1) {
			...
			if (i2 == blockID && i1 == par2 && k1 == par3 && l1 == par4) {
				par1World.setBlockAndMetadata(i, j, k,
						Block.pistonMoving.blockID, par5 | (isSticky ? 8 : 0));
				par1World.setBlockTileEntity(i, j, k, BlockPistonMoving
						.getTileEntity(Block.pistonExtension.blockID, par5
								| (isSticky ? 8 : 0), par5, true, false));
			} else {
				par1World.setBlockAndMetadata(i, j, k,
						Block.pistonMoving.blockID, j2);
				par1World.setBlockTileEntity(i, j, k, BlockPistonMoving
						.getTileEntity(i2, j2, par5, true, false));
			}
