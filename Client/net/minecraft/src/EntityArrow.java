...
public class EntityArrow extends Entity {
	...
	public EntityArrow(World par1World, EntityLiving par2EntityLiving,
		...
		if (d3 < 9.9999999999999995E-008D) {
			...
			double d4 = d / d3;
			double d5 = d2 / d3;
			setLocationAndAngles(par2EntityLiving.posX + d4, posY,
	...
	public void onUpdate() {
		...
		if (i > 0) {
			Block.blocksList[i].setBlockBoundsBasedOnState(worldObj, xTile,
					yTile, zTile);
			AxisAlignedBB axisalignedbb = Block.blocksList[i]
					.getCollisionBoundingBoxFromPool(worldObj, xTile, yTile,
							zTile);

			if (axisalignedbb != null
					&& axisalignedbb.isVecInside(Vec3D.createVector(posX, posY,
							posZ))) {
				inGround = true;
			}
		}
		...
		Vec3D vec3d1 = Vec3D.createVector(posX + motionX, posY + motionY, posZ
				+ motionZ);
		MovingObjectPosition movingobjectposition = worldObj
		...
		for (int l = 0; l < list.size(); l++) {
			...
			float f5 = 0.3F;
			AxisAlignedBB axisalignedbb1 = entity1.boundingBox.expand(f5, f5,
					f5);
			...
			if (movingobjectposition.entityHit != null) {
				...
				if (movingobjectposition.entityHit.attackEntityFrom(
					...
					if (movingobjectposition.entityHit instanceof EntityLiving) {
						...
						if (field_46027_au > 0) {
							float f7 = MathHelper.sqrt_double(motionX * motionX
									+ motionZ * motionZ);
		...
		for (rotationPitch = (float) ((Math.atan2(motionY, f3) * 180D) / Math.PI); rotationPitch
	...
	public void onCollideWithPlayer(EntityPlayer par1EntityPlayer) {
		...
		if (inGround
				...
				&& par1EntityPlayer.inventory
						.addItemStackToInventory(new ItemStack(Item.arrow, 1))) {
