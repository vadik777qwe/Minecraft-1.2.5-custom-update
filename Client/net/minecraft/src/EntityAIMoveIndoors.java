...
public class EntityAIMoveIndoors extends EntityAIBase {
	...
	public boolean shouldExecute() {
		if (entityObj.worldObj.isDaytime() && !entityObj.worldObj.isRaining()
				|| entityObj.worldObj.worldProvider.hasNoSky) {
			return false;
		}
		...
		Village village = entityObj.worldObj.villageCollectionObj
				.findNearestVillage(MathHelper.floor_double(entityObj.posX),
						MathHelper.floor_double(entityObj.posY),
						MathHelper.floor_double(entityObj.posZ), 14);

		if (village == null) {
			return false;
		} else {
			doorInfo = village.findNearestDoorUnrestricted(
	...
	public void startExecuting() {
		...
		if (entityObj.getDistanceSq(doorInfo.getInsidePosX(), doorInfo.posY,
				doorInfo.getInsidePosZ()) > 256D) {
			Vec3D vec3d = RandomPositionGenerator.func_48620_a(entityObj, 14,
					3, Vec3D.createVector(
							(double) doorInfo.getInsidePosX() + 0.5D,
							doorInfo.getInsidePosY(),
							(double) doorInfo.getInsidePosZ() + 0.5D));

			if (vec3d != null) {
				entityObj.getNavigator().func_48666_a(vec3d.xCoord,
						vec3d.yCoord, vec3d.zCoord, 0.3F);
			}
		} else {
			entityObj.getNavigator().func_48666_a(
					(double) doorInfo.getInsidePosX() + 0.5D,
					doorInfo.getInsidePosY(),
					(double) doorInfo.getInsidePosZ() + 0.5D, 0.3F);
		}
	}
