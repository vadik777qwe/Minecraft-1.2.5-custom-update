...
public class GuiScreen extends Gui {
	...
	public static String getClipboardString() {
		try {
			java.awt.datatransfer.Transferable transferable = java.awt.Toolkit
	 ...
	 */
	public void setWorldAndResolution(Minecraft par1Minecraft, int par2,
			int par3) {
	...
	public void handleMouseInput() {
		if (Mouse.getEventButtonState()) {
			...
			int k = height - (Mouse.getEventY() * height) / mc.displayHeight
					- 1;
			mouseClicked(i, k, Mouse.getEventButton());
		} else {
			int j = (Mouse.getEventX() * width) / mc.displayWidth;
			int l = height - (Mouse.getEventY() * height) / mc.displayHeight
					- 1;
	...
	public void drawBackground(int par1) {
		...
		tessellator.addVertexWithUV(0.0D, height, 0.0D, 0.0D, (float) height
				/ f + (float) par1);
		tessellator.addVertexWithUV(width, height, 0.0D, (float) width / f,
				(float) height / f + (float) par1);
