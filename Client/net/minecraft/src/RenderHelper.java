...
public class RenderHelper {
	...
	private static FloatBuffer colorBuffer = GLAllocation
	...
	public static void enableStandardItemLighting() {
		...
		float f1 = 0.6F;
		float f2 = 0.0F;
		Vec3D vec3d = Vec3D.createVector(0.20000000298023224D, 1.0D,
				-0.69999998807907104D).normalize();
		GL11.glLight(GL11.GL_LIGHT0, GL11.GL_POSITION,
				setColorBuffer(vec3d.xCoord, vec3d.yCoord, vec3d.zCoord, 0.0D));
		GL11.glLight(GL11.GL_LIGHT0, GL11.GL_DIFFUSE,
				setColorBuffer(f1, f1, f1, 1.0F));
		GL11.glLight(GL11.GL_LIGHT0, GL11.GL_AMBIENT,
				setColorBuffer(0.0F, 0.0F, 0.0F, 1.0F));
		GL11.glLight(GL11.GL_LIGHT0, GL11.GL_SPECULAR,
				setColorBuffer(f2, f2, f2, 1.0F));
		vec3d = Vec3D.createVector(-0.20000000298023224D, 1.0D,
				0.69999998807907104D).normalize();
		...
		GL11.glLight(GL11.GL_LIGHT1, GL11.GL_AMBIENT,
				setColorBuffer(0.0F, 0.0F, 0.0F, 1.0F));
		GL11.glLight(GL11.GL_LIGHT1, GL11.GL_SPECULAR,
				setColorBuffer(f2, f2, f2, 1.0F));
		GL11.glShadeModel(GL11.GL_FLAT);
		GL11.glLightModel(GL11.GL_LIGHT_MODEL_AMBIENT,
				setColorBuffer(f, f, f, 1.0F));
	}

	/**
	 * Update and return colorBuffer with the RGBA values passed as arguments
	 */
	private static FloatBuffer setColorBuffer(double par0, double par2,
			double par4, double par6) {
		return setColorBuffer((float) par0, (float) par2, (float) par4,
				(float) par6);
	}
	 ...
	 * Update and return colorBuffer with the RGBA values passed as arguments
	 */
	private static FloatBuffer setColorBuffer(float par0, float par1,
