...
public class PostHttp {
	...
	public static String func_52016_a(Map par0Map) {
		...
		do {
			...
			try {
				stringbuilder.append(URLEncoder.encode((String) entry.getKey(),
						"UTF-8"));
	...
	public static String func_52017_a(URL par0URL, String par1Str, boolean par2) {
		try {
			String s = par1Str;
			HttpURLConnection httpurlconnection = (HttpURLConnection) par0URL
					.openConnection();
			...
			httpurlconnection.setRequestProperty("Content-Length",
					(new StringBuilder()).append("")
							.append(s.getBytes().length).toString());
			...
			if (!par2) {
				Logger.getLogger("Minecraft").log(
						Level.SEVERE,
						(new StringBuilder()).append("Could not post to ")
								.append(par0URL).toString(), exception);
			}
		}
