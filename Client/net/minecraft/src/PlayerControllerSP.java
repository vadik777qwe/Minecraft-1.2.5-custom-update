...
public class PlayerControllerSP extends PlayerController {
	...
	public boolean onPlayerDestroyBlock(int par1, int par2, int par3, int par4) {
		...
		if (flag && flag1) {
			Block.blocksList[i].harvestBlock(mc.theWorld, mc.thePlayer, par1,
					par2, par3, j);
		}
	...
	public void clickBlock(int par1, int par2, int par3, int par4) {
		...
		if (i > 0 && curBlockDamage == 0.0F) {
			Block.blocksList[i].onBlockClicked(mc.theWorld, par1, par2, par3,
					mc.thePlayer);
		}
	...
	public void setPartialTime(float par1) {
		if (curBlockDamage <= 0.0F) {
			...
			float f = prevBlockDamage + (curBlockDamage - prevBlockDamage)
					* par1;
