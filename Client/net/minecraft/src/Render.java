...
public abstract class Render {
	 ...
	 */
	public abstract void doRender(Entity entity, double d, double d1,
	 ...
	 */
	protected boolean loadDownloadableImageTexture(String par1Str,
			String par2Str) {
		RenderEngine renderengine = renderManager.renderEngine;
	 ...
	 */
	private void renderShadow(Entity par1Entity, double par2, double par4,
			double par6, float par8, float par9) {
		GL11.glEnable(GL11.GL_BLEND);
		GL11.glBlendFunc(GL11.GL_SRC_ALPHA, GL11.GL_ONE_MINUS_SRC_ALPHA);
		RenderEngine renderengine = renderManager.renderEngine;
		renderengine.bindTexture(renderengine
				.getTexture("%clamp%/misc/shadow.png"));
		...
		double d = par1Entity.lastTickPosX
				+ (par1Entity.posX - par1Entity.lastTickPosX) * (double) par9;
		double d1 = par1Entity.lastTickPosY
				+ (par1Entity.posY - par1Entity.lastTickPosY) * (double) par9
				+ (double) par1Entity.getShadowSize();
		double d2 = par1Entity.lastTickPosZ
				+ (par1Entity.posZ - par1Entity.lastTickPosZ) * (double) par9;
			...
			for (int l1 = k; l1 <= l; l1++) {
				for (int i2 = i1; i2 <= j1; i2++) {
					...
					if (j2 > 0 && world.getBlockLightValue(k1, l1, i2) > 3) {
						renderShadowOnBlock(Block.blocksList[j2], par2, par4
								+ (double) par1Entity.getShadowSize(), par6,
								k1, l1, i2, par8, f, d3, d4
										+ (double) par1Entity.getShadowSize(),
								d5);
					}
				}
	 ...
	 */
	private void renderShadowOnBlock(Block par1Block, double par2, double par4,
			double par6, int par8, int par9, int par10, float par11,
			float par12, double par13, double par15, double par17) {
		Tessellator tessellator = Tessellator.instance;
		...
		double d = ((double) par11 - (par4 - ((double) par9 + par15)) / 2D)
				* 0.5D
				* (double) getWorldFromRenderManager().getLightBrightness(par8,
						par9, par10);
	...
	public static void renderOffsetAABB(AxisAlignedBB par0AxisAlignedBB,
		...
		tessellator.addVertex(par0AxisAlignedBB.minX, par0AxisAlignedBB.maxY,
				par0AxisAlignedBB.minZ);
		tessellator.addVertex(par0AxisAlignedBB.maxX, par0AxisAlignedBB.maxY,
				par0AxisAlignedBB.minZ);
		tessellator.addVertex(par0AxisAlignedBB.maxX, par0AxisAlignedBB.minY,
				par0AxisAlignedBB.minZ);
		tessellator.addVertex(par0AxisAlignedBB.minX, par0AxisAlignedBB.minY,
				par0AxisAlignedBB.minZ);
	...
	public void doRenderShadowAndFire(Entity par1Entity, double par2,
		...
		if (renderManager.options.fancyGraphics && shadowSize > 0.0F) {
			double d = renderManager.getDistanceToCamera(par1Entity.posX,
					par1Entity.posY, par1Entity.posZ);
