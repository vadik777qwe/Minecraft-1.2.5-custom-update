...
public class Block {
	 ...
	 * on graphics settings. Mostly seems to come from terrain.png, and the
	 * index is 0-based (grass is 0).
	 */
	...
	/**
	 * If this field is true, the block is counted for statistics (mined or
	 * placed)
	 */
	...
	protected Block(int par1, Material par2Material) {
		...
		if (blocksList[par1] != null) {
			throw new IllegalArgumentException((new StringBuilder())
					.append("Slot ").append(par1)
					.append(" is already occupied by ")
					.append(blocksList[par1]).append(" when adding ")
	...
	/**
	 * Blocks with this attribute will not notify all near blocks when it's
	 * metadata change. The default behavior is always notify every neightbor
	 * block when anything changes.
	 */
	 ...
	 */
	public void setBlockBounds(float par1, float par2, float par3, float par4,
	...
	public int getMixedBrightnessForBlock(IBlockAccess par1IBlockAccess,
		...
		return par1IBlockAccess.getLightBrightnessForSkyBlocks(par2, par3,
				par4, lightValue[blockID]);
	}
	...
	public int getBlockTexture(IBlockAccess par1IBlockAccess, int par2,
			int par3, int par4, int par5) {
		return getBlockTextureFromSideAndMetadata(par5,
	...
	public AxisAlignedBB getSelectedBoundingBoxFromPool(World par1World,
			int par2, int par3, int par4) {
		return AxisAlignedBB.getBoundingBoxFromPool((double) par2 + minX,
				(double) par3 + minY, (double) par4 + minZ, (double) par2
						+ maxX, (double) par3 + maxY, (double) par4 + maxZ);
	}
	...
	public void getCollidingBoundingBoxes(World par1World, int par2, int par3,
		...
		AxisAlignedBB axisalignedbb = getCollisionBoundingBoxFromPool(
				par1World, par2, par3, par4);

		if (axisalignedbb != null
				&& par5AxisAlignedBB.intersectsWith(axisalignedbb)) {
	...
	public float blockStrength(EntityPlayer par1EntityPlayer) {
		...
		if (!par1EntityPlayer.canHarvestBlock(this)) {
			...
			return par1EntityPlayer.getCurrentPlayerStrVsBlock(this)
					/ blockHardness / 30F;
		}
	}
	...
	protected void dropBlockAsItem_do(World par1World, int par2, int par3,
		...
		if (par1World.isRemote) {
			...
			float f = 0.7F;
			double d = (double) (par1World.rand.nextFloat() * f)
					+ (double) (1.0F - f) * 0.5D;
			...
			EntityItem entityitem = new EntityItem(par1World,
					(double) par2 + d, (double) par3 + d1, (double) par4 + d2,
					par5ItemStack);
	...
	public MovingObjectPosition collisionRayTrace(World par1World, int par2,
		...
		if (vec3d != null
				&& (vec3d6 == null || par5Vec3D.distanceTo(vec3d) < par5Vec3D
						.distanceTo(vec3d6))) {
			vec3d6 = vec3d;
		}

		if (vec3d1 != null
				&& (vec3d6 == null || par5Vec3D.distanceTo(vec3d1) < par5Vec3D
						.distanceTo(vec3d6))) {
			vec3d6 = vec3d1;
		}

		if (vec3d2 != null
				&& (vec3d6 == null || par5Vec3D.distanceTo(vec3d2) < par5Vec3D
						.distanceTo(vec3d6))) {
			vec3d6 = vec3d2;
		}

		if (vec3d3 != null
				&& (vec3d6 == null || par5Vec3D.distanceTo(vec3d3) < par5Vec3D
						.distanceTo(vec3d6))) {
			vec3d6 = vec3d3;
		}

		if (vec3d4 != null
				&& (vec3d6 == null || par5Vec3D.distanceTo(vec3d4) < par5Vec3D
						.distanceTo(vec3d6))) {
			vec3d6 = vec3d4;
		}

		if (vec3d5 != null
				&& (vec3d6 == null || par5Vec3D.distanceTo(vec3d5) < par5Vec3D
						.distanceTo(vec3d6))) {
			vec3d6 = vec3d5;
		}
		...
		return new MovingObjectPosition(par2, par3, par4, byte0,
				vec3d6.addVector(par2, par3, par4));
	}
	...
	private boolean isVecInsideYZBounds(Vec3D par1Vec3D) {
		if (par1Vec3D == null) {
			...
			return par1Vec3D.yCoord >= minY && par1Vec3D.yCoord <= maxY
	...
	public void harvestBlock(World par1World, EntityPlayer par2EntityPlayer,
		...
		if (func_50074_q()
				&& EnchantmentHelper
						.getSilkTouchModifier(par2EntityPlayer.inventory)) {
			...
			int i = EnchantmentHelper
					.getFortuneModifier(par2EntityPlayer.inventory);
	...
	protected ItemStack createStackedBlock(int par1) {
		...
		if (blockID >= 0 && blockID < Item.itemsList.length
	...
	public Block setBlockName(String par1Str) {
		blockName = (new StringBuilder()).append("tile.").append(par1Str)
				.toString();
		return this;
	}
	 ...
	 * StatCollector.translateToLocal. Used for the statistic page.
	 */
	 ...
	 */
	public void onFallenUpon(World world, int i, int j, int k, Entity entity,
			float f) {
	}
