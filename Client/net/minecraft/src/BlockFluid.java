...
public abstract class BlockFluid extends Block {
	protected BlockFluid(int par1, Material par2Material) {
		super(par1, (par2Material != Material.lava ? 12 : 14) * 16 + 13,
				par2Material);
		float f = 0.0F;
		float f1 = 0.0F;
		setBlockBounds(0.0F + f1, 0.0F + f, 0.0F + f1, 1.0F + f1, 1.0F + f,
				1.0F + f1);
	 ...
	 */
	protected int getEffectiveFlowDecay(IBlockAccess par1IBlockAccess,
	 ...
	 */
	private Vec3D getFlowVector(IBlockAccess par1IBlockAccess, int par2,
			int par3, int par4) {
		...
		for (int j = 0; j < 4; j++) {
			...
			if (j1 < 0) {
				if (par1IBlockAccess.getBlockMaterial(k, l, i1)
						.blocksMovement()) {
					continue;
				}
				...
				if (j1 >= 0) {
					int k1 = j1 - (i - 8);
					vec3d = vec3d.addVector((k - par2) * k1, (l - par3) * k1,
							(i1 - par4) * k1);
				}
			...
			if (j1 >= 0) {
				int l1 = j1 - i;
				vec3d = vec3d.addVector((k - par2) * l1, (l - par3) * l1,
						(i1 - par4) * l1);
			}
		}
	...
	public void velocityToAddToEntity(World par1World, int par2, int par3,
			int par4, Entity par5Entity, Vec3D par6Vec3D) {
	...
	public int getMixedBrightnessForBlock(IBlockAccess par1IBlockAccess,
			int par2, int par3, int par4) {
		int i = par1IBlockAccess.getLightBrightnessForSkyBlocks(par2, par3,
				par4, 0);
		int j = par1IBlockAccess.getLightBrightnessForSkyBlocks(par2, par3 + 1,
				par4, 0);
		int k = i & 0xff;
		int l = j & 0xff;
		int i1 = i >> 16 & 0xff;
	...
	public void randomDisplayTick(World par1World, int par2, int par3,
		...
		if (blockMaterial == Material.water) {
			if (par5Random.nextInt(10) == 0) {
				...
				if (i <= 0 || i >= 8) {
					par1World.spawnParticle("suspended", (float) par2
							+ par5Random.nextFloat(),
							(float) par3 + par5Random.nextFloat(), (float) par4
			...
			for (int j = 0; j < 0; j++) {
				...
				if (par1World.getBlockMaterial(i1, par3, j1) != Material.air
						|| !par1World.getBlockMaterial(i1, par3 - 1, j1)
								.blocksMovement()
						&& !par1World.getBlockMaterial(i1, par3 - 1, j1)
								.isLiquid()) {
					continue;
		...
		if (blockMaterial == Material.water && par5Random.nextInt(64) == 0) {
			...
			if (k > 0 && k < 8) {
				par1World.playSoundEffect((float) par2 + 0.5F,
		...
		if (blockMaterial == Material.lava
			...
			if (par5Random.nextInt(100) == 0) {
				...
				par1World.playSoundEffect(d, d2, d4, "liquid.lavapop",
						0.2F + par5Random.nextFloat() * 0.2F,
						0.9F + par5Random.nextFloat() * 0.15F);
			}

			if (par5Random.nextInt(200) == 0) {
				par1World.playSoundEffect(par2, par3, par4, "liquid.lava",
						0.2F + par5Random.nextFloat() * 0.2F,
						0.9F + par5Random.nextFloat() * 0.15F);
			}
		}
		...
		if (par5Random.nextInt(10) == 0
			...
			if (blockMaterial == Material.water) {
				par1World.spawnParticle("dripWater", d1, d3, d5, 0.0D, 0.0D,
						0.0D);
			} else {
				par1World.spawnParticle("dripLava", d1, d3, d5, 0.0D, 0.0D,
						0.0D);
			}
		}
	}

	public static double func_293_a(IBlockAccess par0IBlockAccess, int par1,
			int par2, int par3, Material par4Material) {
		Vec3D vec3d = null;

		if (par4Material == Material.water) {
			vec3d = ((BlockFluid) Block.waterMoving).getFlowVector(
					par0IBlockAccess, par1, par2, par3);
		}

		if (par4Material == Material.lava) {
			vec3d = ((BlockFluid) Block.lavaMoving).getFlowVector(
					par0IBlockAccess, par1, par2, par3);
		}
	...
	protected void triggerLavaMixEffects(World par1World, int par2, int par3,
			int par4) {
		par1World
				.playSoundEffect((float) par2 + 0.5F, (float) par3 + 0.5F,
		...
		for (int i = 0; i < 8; i++) {
			par1World.spawnParticle("largesmoke",
					(double) par2 + Math.random(), (double) par3 + 1.2D,
