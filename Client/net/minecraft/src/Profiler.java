...
public class Profiler {
	...
	public static void startSection(String par0Str) {
		...
		if (profilingSection.length() > 0) {
			profilingSection = (new StringBuilder()).append(profilingSection)
					.append(".").toString();
		}

		profilingSection = (new StringBuilder()).append(profilingSection)
				.append(par0Str).toString();
	...
	public static void endSection() {
		...
		long l1 = ((Long) timestampList.remove(timestampList.size() - 1))
				.longValue();
		...
		if (profilingMap.containsKey(profilingSection)) {
			profilingMap.put(
					profilingSection,
					Long.valueOf(((Long) profilingMap.get(profilingSection))
							.longValue() + l2));
		} else {
			profilingMap.put(profilingSection, Long.valueOf(l2));
		}

		profilingSection = sectionList.size() <= 0 ? "" : (String) sectionList
				.get(sectionList.size() - 1);

		if (l2 > 0x5f5e100L) {
			System.out.println((new StringBuilder()).append(profilingSection)
	...
	public static List getProfilingData(String par0Str) {
		...
		String s = par0Str;
		long l = profilingMap.containsKey("root") ? ((Long) profilingMap
				.get("root")).longValue() : 0L;
		long l1 = profilingMap.containsKey(par0Str) ? ((Long) profilingMap
				.get(par0Str)).longValue() : -1L;
		ArrayList arraylist = new ArrayList();

		if (par0Str.length() > 0) {
			par0Str = (new StringBuilder()).append(par0Str).append(".")
					.toString();
		}
		...
		do {
			...
			if (s1.length() > par0Str.length() && s1.startsWith(par0Str)
					&& s1.indexOf(".", par0Str.length() + 1) < 0) {
		...
		do {
			...
			if (s2.length() > par0Str.length() && s2.startsWith(par0Str)
					&& s2.indexOf(".", par0Str.length() + 1) < 0) {
		...
		for (Iterator iterator2 = profilingMap.keySet().iterator(); iterator2
				.hasNext(); profilingMap
				.put(s3, Long.valueOf((((Long) profilingMap.get(s3))
						.longValue() * 999L) / 1000L))) {
			s3 = (String) iterator2.next();
		}

		if ((float) l2 > f) {
			arraylist.add(new ProfilerResult("unspecified",
					((double) ((float) l2 - f) * 100D) / (double) l2,
					((double) ((float) l2 - f) * 100D) / (double) l));
		}

		Collections.sort(arraylist);
		arraylist.add(0, new ProfilerResult(s, 100D, ((double) l2 * 100D)
				/ (double) l));
		return arraylist;
	}
