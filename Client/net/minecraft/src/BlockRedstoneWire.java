...
public class BlockRedstoneWire extends Block {
	...
	private void updateAndPropagateCurrentStrength(World par1World, int par2,
		...
		for (int i = 0; i < arraylist.size(); i++) {
			...
			par1World.notifyBlocksOfNeighborChange(chunkposition.x,
					chunkposition.y, chunkposition.z, blockID);
		}
	}
	...
	private void calculateCurrentChanges(World par1World, int par2, int par3,
		...
		boolean flag = par1World.isBlockIndirectlyGettingPowered(par2, par3,
				par4);
		wiresProvidePower = true;
		...
		if (flag) {
			...
			for (int k = 0; k < 4; k++) {
				...
				if (par1World.isBlockNormalCube(i1, par3, k1)
						&& !par1World.isBlockNormalCube(par2, par3 + 1, par4)) {
					if (i1 != par5 || par3 + 1 != par6 || k1 != par7) {
						j = getMaxCurrentStrength(par1World, i1, par3 + 1, k1,
								j);
					}
		...
		if (i != j) {
			...
			for (int l = 0; l < 4; l++) {
				...
				if (j2 >= 0 && j2 != j) {
					calculateCurrentChanges(par1World, j1, par3, l1, par2,
							par3, par4);
				}
				...
				if (j2 >= 0 && j2 != j) {
					calculateCurrentChanges(par1World, j1, i2, l1, par2, par3,
							par4);
				}
			}
			...
			if (i < j || j == 0) {
				...
				blocksNeedingUpdate
						.add(new ChunkPosition(par2 + 1, par3, par4));
				blocksNeedingUpdate
						.add(new ChunkPosition(par2, par3 - 1, par4));
				blocksNeedingUpdate
						.add(new ChunkPosition(par2, par3 + 1, par4));
				blocksNeedingUpdate
						.add(new ChunkPosition(par2, par3, par4 - 1));
				blocksNeedingUpdate
						.add(new ChunkPosition(par2, par3, par4 + 1));
			}
		}
	...
	public void onBlockAdded(World par1World, int par2, int par3, int par4) {
		...
		if (par1World.isBlockNormalCube(par2 - 1, par3, par4)) {
			notifyWireNeighborsOfNeighborChange(par1World, par2 - 1, par3 + 1,
					par4);
		} else {
			notifyWireNeighborsOfNeighborChange(par1World, par2 - 1, par3 - 1,
					par4);
	 ...
	 * z, strength)
	 */
	private int getMaxCurrentStrength(World par1World, int par2, int par3,
			int par4, int par5) {
		if (par1World.getBlockId(par2, par3, par4) != blockID) {
			return par5;
		}
	...
	public boolean isPoweringTo(IBlockAccess par1IBlockAccess, int par2,
		...
		boolean flag = isPoweredOrRepeater(par1IBlockAccess, par2 - 1, par3,
				par4, 1)
				|| !par1IBlockAccess.isBlockNormalCube(par2 - 1, par3, par4)
				&& isPoweredOrRepeater(par1IBlockAccess, par2 - 1, par3 - 1,
						par4, -1);
		boolean flag1 = isPoweredOrRepeater(par1IBlockAccess, par2 + 1, par3,
				par4, 3)
				|| !par1IBlockAccess.isBlockNormalCube(par2 + 1, par3, par4)
				&& isPoweredOrRepeater(par1IBlockAccess, par2 + 1, par3 - 1,
						par4, -1);
		boolean flag2 = isPoweredOrRepeater(par1IBlockAccess, par2, par3,
				par4 - 1, 2)
				|| !par1IBlockAccess.isBlockNormalCube(par2, par3, par4 - 1)
				&& isPoweredOrRepeater(par1IBlockAccess, par2, par3 - 1,
						par4 - 1, -1);
		boolean flag3 = isPoweredOrRepeater(par1IBlockAccess, par2, par3,
				par4 + 1, 0)
				|| !par1IBlockAccess.isBlockNormalCube(par2, par3, par4 + 1)
				&& isPoweredOrRepeater(par1IBlockAccess, par2, par3 - 1,
						par4 + 1, -1);
	...
	public void randomDisplayTick(World par1World, int par2, int par3,
		...
		if (i > 0) {
			double d = (double) par2 + 0.5D
					+ ((double) par5Random.nextFloat() - 0.5D)
	...
	public static boolean isPowerProviderOrWire(IBlockAccess par0IBlockAccess,
		...
		if (i == Block.redstoneRepeaterIdle.blockID
				|| i == Block.redstoneRepeaterActive.blockID) {
			int j = par0IBlockAccess.getBlockMetadata(par1, par2, par3);
			return par4 == (j & 3)
					|| par4 == Direction.footInvisibleFaceRemap[j & 3];
		}
