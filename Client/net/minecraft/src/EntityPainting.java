...
public class EntityPainting extends Entity {
	...
	public EntityPainting(World par1World, int par2, int par3, int par4,
			int par5) {
		this(par1World);
		xPosition = par2;
		yPosition = par3;
	...
	public void func_412_b(int par1) {
		...
		float f7 = -0.00625F;
		boundingBox.setBounds(f3 - f - f7, f4 - f1 - f7, f5 - f2 - f7, f3 + f
				+ f7, f4 + f1 + f7, f5 + f2 + f7);
	}
		...
		if (tickCounter1++ == 100 && !worldObj.isRemote) {
			...
			if (!isDead && !onValidSurface()) {
				...
				worldObj.spawnEntityInWorld(new EntityItem(worldObj, posX,
						posY, posZ, new ItemStack(Item.painting)));
			}
		}
	}
		...
		for (int j1 = 0; j1 < i; j1++) {
			for (int k1 = 0; k1 < j; k1++) {
				...
				if (direction == 0 || direction == 2) {
					material = worldObj.getBlockMaterial(k + j1, l + k1,
							zPosition);
				} else {
					material = worldObj.getBlockMaterial(xPosition, l + k1, i1
							+ j1);
				}
	...
	public boolean attackEntityFrom(DamageSource par1DamageSource, int par2) {
		if (!isDead && !worldObj.isRemote) {
			...
			worldObj.spawnEntityInWorld(new EntityItem(worldObj, posX, posY,
					posZ, new ItemStack(Item.painting)));
		}
