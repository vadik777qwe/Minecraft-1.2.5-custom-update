	...
	private void spawnHeartParticles(EntityLiving par1EntityLiving) {
		...
		for (int i = 0; i < 5; i++) {
			...
			worldObj.spawnParticle(
					"heart",
					(par1EntityLiving.posX + (double) (random.nextFloat()
							* par1EntityLiving.width * 2.0F))
							- (double) par1EntityLiving.width,
					par1EntityLiving.posY
							+ 1.0D
							+ (double) (random.nextFloat() * par1EntityLiving.height),
					(par1EntityLiving.posZ + (double) (random.nextFloat()
							* par1EntityLiving.width * 2.0F))
							- (double) par1EntityLiving.width, d, d1, d2);
		}
	}
}
