		...
		if (movingobjectposition.typeOfHit == EnumMovingObjectType.TILE) {
			...
			if (isFull == 0) {
				...
				if (par2World.isAirBlock(i, j, k)
						|| !par2World.getBlockMaterial(i, j, k).isSolid()) {
					if (par2World.worldProvider.isHellWorld
						...
						par2World
								.playSoundEffect(
										...
										"random.fizz",
										0.5F,
										2.6F + (par2World.rand.nextFloat() - par2World.rand
												.nextFloat()) * 0.8F);

						for (int l = 0; l < 8; l++) {
							par2World.spawnParticle("largesmoke", (double) i
									+ Math.random(),
									(double) j + Math.random(), (double) k
											+ Math.random(), 0.0D, 0.0D, 0.0D);
						}
					} else {
						par2World.setBlockAndMetadataWithNotify(i, j, k,
								isFull, 0);
					}
