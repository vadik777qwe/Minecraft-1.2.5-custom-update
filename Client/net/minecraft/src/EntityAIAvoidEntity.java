...
public class EntityAIAvoidEntity extends EntityAIBase {
	...
	public boolean shouldExecute() {
		if (targetEntityClass == (net.minecraft.src.EntityPlayer.class)) {
			...
			field_48240_d = theEntity.worldObj.getClosestPlayerToEntity(
					theEntity, field_48241_e);

			if (field_48240_d == null) {
				return false;
			}
		} else {
			List list = theEntity.worldObj.getEntitiesWithinAABB(
					targetEntityClass, theEntity.boundingBox.expand(
							field_48241_e, 3D, field_48241_e));
		...
		Vec3D vec3d = RandomPositionGenerator.func_48623_b(theEntity, 16, 7,
				Vec3D.createVector(field_48240_d.posX, field_48240_d.posY,
						field_48240_d.posZ));
		...
		if (field_48240_d.getDistanceSq(vec3d.xCoord, vec3d.yCoord,
				vec3d.zCoord) < field_48240_d.getDistanceSqToEntity(theEntity)) {
			return false;
		}

		field_48238_f = entityPathNavigate.getPathToXYZ(vec3d.xCoord,
