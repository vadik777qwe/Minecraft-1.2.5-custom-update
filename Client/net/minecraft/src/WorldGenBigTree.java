...
public class WorldGenBigTree extends WorldGenerator {
	...
	void generateLeafNodeList() {
		...
		int i = (int) (1.3819999999999999D + Math.pow(
				(leafDensity * (double) heightLimit) / 13D, 2D));
		...
		while (i1 >= 0) {
			...
			if (f < 0.0F) {
				...
				for (; j1 < i; j1++) {
					double d1 = scaleWidth
							* ((double) f * ((double) rand.nextFloat() + 0.32800000000000001D));
	...
	void genTreeLayer(int par1, int par2, int par3, float par4, byte par5,
		...
		for (; j <= i; j++) {
			...
			for (int l = -i; l <= i;) {
				double d = Math.sqrt(Math.pow((double) Math.abs(j) + 0.5D, 2D)
						+ Math.pow((double) Math.abs(l) + 0.5D, 2D));
				...
				if (d > (double) par4) {
					...
					if (i1 != 0 && i1 != 18) {
						...
						setBlockAndMetadata(worldObj, ai1[0], ai1[1], ai1[2],
								par6, 0);
						l++;
					}
				}
	...
	void placeBlockLine(int par1ArrayOfInteger[], int par2ArrayOfInteger[],
		...
		for (int k = ai[i] + byte3; j != k; j += byte3) {
			ai1[i] = MathHelper
					.floor_double((double) (par1ArrayOfInteger[i] + j) + 0.5D);
			ai1[byte1] = MathHelper
					.floor_double((double) par1ArrayOfInteger[byte1]
							+ (double) j * d + 0.5D);
			ai1[byte2] = MathHelper
					.floor_double((double) par1ArrayOfInteger[byte2]
							+ (double) j * d1 + 0.5D);
