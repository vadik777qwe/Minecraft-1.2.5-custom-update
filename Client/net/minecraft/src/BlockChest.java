...
public class BlockChest extends BlockContainer {
	...
	public void unifyAdjacentChests(World par1World, int par2, int par3,
		...
		if (i == blockID || j == blockID) {
			int i1 = par1World.getBlockId(par2 - 1, par3,
					i != blockID ? par4 + 1 : par4 - 1);
			int k1 = par1World.getBlockId(par2 + 1, par3,
					i != blockID ? par4 + 1 : par4 - 1);
			byte0 = 5;
			int i2 = -1;
			...
			if ((Block.opaqueCubeLookup[k] || Block.opaqueCubeLookup[i1])
					&& !Block.opaqueCubeLookup[l]
					&& !Block.opaqueCubeLookup[k1]) {
				byte0 = 5;
			}

			if ((Block.opaqueCubeLookup[l] || Block.opaqueCubeLookup[k1])
					&& !Block.opaqueCubeLookup[k]
					&& !Block.opaqueCubeLookup[i1]) {
				byte0 = 4;
			}
		} else if (k == blockID || l == blockID) {
			int j1 = par1World.getBlockId(k != blockID ? par2 + 1 : par2 - 1,
					par3, par4 - 1);
			int l1 = par1World.getBlockId(k != blockID ? par2 + 1 : par2 - 1,
					par3, par4 + 1);
			byte0 = 3;
			int j2 = -1;
			...
			if ((Block.opaqueCubeLookup[i] || Block.opaqueCubeLookup[j1])
					&& !Block.opaqueCubeLookup[j]
					&& !Block.opaqueCubeLookup[l1]) {
				byte0 = 3;
			}

			if ((Block.opaqueCubeLookup[j] || Block.opaqueCubeLookup[l1])
					&& !Block.opaqueCubeLookup[i]
	...
	public int getBlockTexture(IBlockAccess par1IBlockAccess, int par2,
		...
		if (i == blockID || j == blockID) {
			...
			int k1 = par1IBlockAccess.getBlockId(par2 - 1, par3,
					i != blockID ? par4 + 1 : par4 - 1);
			int i2 = par1IBlockAccess.getBlockId(par2 + 1, par3,
					i != blockID ? par4 + 1 : par4 - 1);
			...
			if ((Block.opaqueCubeLookup[k] || Block.opaqueCubeLookup[k1])
					&& !Block.opaqueCubeLookup[l]
			...
			return (par5 != byte1 ? blockIndexInTexture + 32
					: blockIndexInTexture + 16) + i1;
		}
		...
		if (k == blockID || l == blockID) {
			...
			int l1 = par1IBlockAccess.getBlockId(k != blockID ? par2 + 1
					: par2 - 1, par3, par4 - 1);
			int j2 = par1IBlockAccess.getBlockId(k != blockID ? par2 + 1
					: par2 - 1, par3, par4 + 1);
			...
			if ((Block.opaqueCubeLookup[i] || Block.opaqueCubeLookup[l1])
					&& !Block.opaqueCubeLookup[j]
					&& !Block.opaqueCubeLookup[j2]) {
				byte2 = 3;
			}

			if ((Block.opaqueCubeLookup[j] || Block.opaqueCubeLookup[j2])
					&& !Block.opaqueCubeLookup[i]
					&& !Block.opaqueCubeLookup[l1]) {
				byte2 = 2;
			}

			return (par5 != byte2 ? blockIndexInTexture + 32
					: blockIndexInTexture + 16) + j1;
		}
			...
			for (int i = 0; i < tileentitychest.getSizeInventory(); i++) {
				...
				while (itemstack.stackSize > 0) {
					...
					EntityItem entityitem = new EntityItem(par1World,
							(float) par2 + f, (float) par3 + f1, (float) par4
									+ f2, new ItemStack(itemstack.itemID, j,
	...
	public boolean blockActivated(World par1World, int par2, int par3,
		...
		Object obj = (TileEntityChest) par1World.getBlockTileEntity(par2, par3,
				par4);
		...
		if (par1World.getBlockId(par2 - 1, par3, par4) == blockID
				&& (par1World.isBlockNormalCube(par2 - 1, par3 + 1, par4) || func_50075_j(
		...
		if (par1World.getBlockId(par2, par3, par4 - 1) == blockID
				&& (par1World.isBlockNormalCube(par2, par3 + 1, par4 - 1) || func_50075_j(
		...
		if (par1World.getBlockId(par2 - 1, par3, par4) == blockID) {
			obj = new InventoryLargeChest("Large chest",
					(TileEntityChest) par1World.getBlockTileEntity(par2 - 1,
							par3, par4), ((IInventory) (obj)));
		}
		...
		if (par1World.getBlockId(par2, par3, par4 - 1) == blockID) {
			obj = new InventoryLargeChest("Large chest",
					(TileEntityChest) par1World.getBlockTileEntity(par2, par3,
							par4 - 1), ((IInventory) (obj)));
		}
