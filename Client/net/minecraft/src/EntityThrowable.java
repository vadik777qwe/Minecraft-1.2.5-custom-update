...
public abstract class EntityThrowable extends Entity {
	...
	public EntityThrowable(World par1World, EntityLiving par2EntityLiving) {
		...
		motionZ = MathHelper.cos((rotationYaw / 180F) * (float) Math.PI)
				* MathHelper.cos((rotationPitch / 180F) * (float) Math.PI) * f;
		motionY = -MathHelper.sin(((rotationPitch + func_40074_d()) / 180F)
				* (float) Math.PI)
				* f;
		setThrowableHeading(motionX, motionY, motionZ, func_40077_c(), 1.0F);
	}

	public EntityThrowable(World par1World, double par2, double par4,
			double par6) {
		super(par1World);
		xTile = -1;
		yTile = -1;
	...
	public void onUpdate() {
		...
		if (!worldObj.isRemote) {
			...
			for (int k = 0; k < list.size(); k++) {
				...
				float f4 = 0.3F;
				AxisAlignedBB axisalignedbb = entity1.boundingBox.expand(f4,
						f4, f4);
