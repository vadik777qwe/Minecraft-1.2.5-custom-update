...
public class PlayerControllerMP extends PlayerController {
	...
	public void clickBlock(int par1, int par2, int par3, int par4) {
		if (creativeMode) {
			netClientHandler.addToSendQueue(new Packet14BlockDig(0, par1, par2,
					par3, par4));
			PlayerControllerCreative.clickBlockCreative(mc, this, par1, par2,
					par3, par4);
			blockHitDelay = 5;
		} else if (!isHittingBlock || par1 != currentBlockX
				|| par2 != currentBlockY || par3 != currentblockZ) {
			netClientHandler.addToSendQueue(new Packet14BlockDig(0, par1, par2,
					par3, par4));
			int i = mc.theWorld.getBlockId(par1, par2, par3);

			if (i > 0 && curBlockDamageMP == 0.0F) {
				Block.blocksList[i].onBlockClicked(mc.theWorld, par1, par2,
						par3, mc.thePlayer);
			}

			if (i > 0
					&& Block.blocksList[i].blockStrength(mc.thePlayer) >= 1.0F) {
	...
	public void onPlayerDamageBlock(int par1, int par2, int par3, int par4) {
		...
		if (par1 == currentBlockX && par2 == currentBlockY
				&& par3 == currentblockZ) {
			int i = mc.theWorld.getBlockId(par1, par2, par3);
			...
			if (stepSoundTickCounter % 4F == 0.0F && block != null) {
				mc.sndManager.playSound(block.stepSound.getStepSound(),
						(float) par1 + 0.5F, (float) par2 + 0.5F,
						(float) par3 + 0.5F,
			...
			if (curBlockDamageMP >= 1.0F) {
				isHittingBlock = false;
				netClientHandler.addToSendQueue(new Packet14BlockDig(2, par1,
						par2, par3, par4));
				onPlayerDestroyBlock(par1, par2, par3, par4);
				curBlockDamageMP = 0.0F;
				prevBlockDamageMP = 0.0F;
	...
	public void setPartialTime(float par1) {
		if (curBlockDamageMP <= 0.0F) {
			...
			float f = prevBlockDamageMP
					+ (curBlockDamageMP - prevBlockDamageMP) * par1;
	...
	public boolean onPlayerRightClick(EntityPlayer par1EntityPlayer,
		...
		netClientHandler.addToSendQueue(new Packet15Place(par4, par5, par6,
				par7, par1EntityPlayer.inventory.getCurrentItem()));
		int i = par2World.getBlockId(par4, par5, par6);

		if (i > 0
				&& Block.blocksList[i].blockActivated(par2World, par4, par5,
						par6, par1EntityPlayer)) {
			return true;
		}
		...
		if (creativeMode) {
			...
			boolean flag = par3ItemStack.useItem(par1EntityPlayer, par2World,
					par4, par5, par6, par7);
			par3ItemStack.setItemDamage(j);
			par3ItemStack.stackSize = k;
			return flag;
		} else {
			return par3ItemStack.useItem(par1EntityPlayer, par2World, par4,
					par5, par6, par7);
		}
	}
	 ...
	 */
	public boolean sendUseItem(EntityPlayer par1EntityPlayer, World par2World,
			ItemStack par3ItemStack) {
		syncCurrentPlayItem();
		netClientHandler.addToSendQueue(new Packet15Place(-1, -1, -1, 255,
				par1EntityPlayer.inventory.getCurrentItem()));
		boolean flag = super.sendUseItem(par1EntityPlayer, par2World,
				par3ItemStack);
		return flag;
	}
	 ...
	 */
	public void interactWithEntity(EntityPlayer par1EntityPlayer,
			Entity par2Entity) {
		syncCurrentPlayItem();
		netClientHandler.addToSendQueue(new Packet7UseEntity(
	...
	public ItemStack windowClick(int par1, int par2, int par3, boolean par4,
			EntityPlayer par5EntityPlayer) {
		short word0 = par5EntityPlayer.craftingInventory
				.getNextTransactionID(par5EntityPlayer.inventory);
		ItemStack itemstack = super.windowClick(par1, par2, par3, par4,
				par5EntityPlayer);
		netClientHandler.addToSendQueue(new Packet102WindowClick(par1, par2,
				par3, par4, itemstack, word0));
		return itemstack;
	}
