...
public class PotionEffect {
	...
	public void combine(PotionEffect par1PotionEffect) {
		...
		if (par1PotionEffect.amplifier > amplifier) {
			amplifier = par1PotionEffect.amplifier;
			duration = par1PotionEffect.duration;
		} else if (par1PotionEffect.amplifier == amplifier
				&& duration < par1PotionEffect.duration) {
			duration = par1PotionEffect.duration;
		}
	}
	...
	public void performEffect(EntityLiving par1EntityLiving) {
		if (duration > 0) {
			Potion.potionTypes[potionID].performEffect(par1EntityLiving,
					amplifier);
		}
	}
	...
	public String toString() {
		...
		if (getAmplifier() > 0) {
			s = (new StringBuilder()).append(getEffectName()).append(" x ")
					.append(getAmplifier() + 1).append(", Duration: ")
					.append(getDuration()).toString();
		} else {
			s = (new StringBuilder()).append(getEffectName())
	...
	public boolean equals(Object par1Obj) {
		if (!(par1Obj instanceof PotionEffect)) {
			...
			return potionID == potioneffect.potionID
