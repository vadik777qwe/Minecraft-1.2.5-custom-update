...
public class MapStorage {
	...
	public WorldSavedData loadData(Class par1Class, String par2Str) {
		WorldSavedData worldsaveddata = (WorldSavedData) loadedDataMap
				.get(par2Str);
		...
		if (saveHandler != null) {
			try {
				...
				if (file != null && file.exists()) {
					try {
						worldsaveddata = (WorldSavedData) par1Class
								.getConstructor(
										new Class[] { java.lang.String.class })
								.newInstance(new Object[] { par2Str });
					} catch (Exception exception1) {
						throw new RuntimeException((new StringBuilder())
								.append("Failed to instantiate ")
								.append(par1Class.toString()).toString(),
								exception1);
					}
	...
	public void saveAllData() {
		for (int i = 0; i < loadedDataList.size(); i++) {
			WorldSavedData worldsaveddata = (WorldSavedData) loadedDataList
					.get(i);
	...
	public int getUniqueDataId(String par1Str) {
		...
		if (short1 == null) {
			...
			Short short2 = short1;
			Short short3 = short1 = Short
					.valueOf((short) (short1.shortValue() + 1));
			Short _tmp = short2;
		}
		...
		try {
			...
			if (file != null) {
				...
				for (Iterator iterator = idCounts.keySet().iterator(); iterator
						.hasNext(); nbttagcompound.setShort(s, word0)) {
