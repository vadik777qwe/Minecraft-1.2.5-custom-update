...
import net.minecraft.render.EffectRenderer;
...
import net.minecraft.enums.EnumOS2;
import net.minecraft.enums.EnumOSMappingHelper;
import net.minecraft.enums.EnumOptions;
import net.minecraft.render.FontRenderer;
import net.minecraft.src.GLAllocation;
import net.minecraft.src.GameSettings;
import net.minecraft.src.GameWindowListener;
import net.minecraft.gui.GuiAchievement;
import net.minecraft.gui.GuiChat;
import net.minecraft.gui.GuiConflictWarning;
import net.minecraft.gui.GuiErrorScreen;
import net.minecraft.gui.GuiGameOver;
import net.minecraft.gui.GuiIngame;
import net.minecraft.gui.GuiIngameMenu;
import net.minecraft.gui.GuiInventory;
import net.minecraft.gui.GuiMemoryErrorScreen;
import net.minecraft.gui.GuiParticle;
import net.minecraft.gui.GuiScreen;
import net.minecraft.gui.GuiSleepMP;
...
import net.minecraft.entity.player.PlayerUsageSnooper;
import net.minecraft.src.Profiler;
import net.minecraft.src.ProfilerResult;
import net.minecraft.render.RenderBlocks;
import net.minecraft.render.RenderEngine;
import net.minecraft.render.RenderGlobal;
import net.minecraft.render.RenderManager;
...
import net.minecraft.textures.TextureCompassFX;
import net.minecraft.textures.TextureFlamesFX;
import net.minecraft.textures.TextureLavaFX;
import net.minecraft.textures.TextureLavaFlowFX;
import net.minecraft.textures.TexturePackList;
import net.minecraft.textures.TexturePortalFX;
import net.minecraft.textures.TextureWatchFX;
import net.minecraft.textures.TextureWaterFX;
import net.minecraft.textures.TextureWaterFlowFX;
import net.minecraft.threads.ThreadCheckHasPaid;
import net.minecraft.threads.ThreadClientSleep;
import net.minecraft.threads.ThreadDownloadResources;
...
public abstract class Minecraft implements Runnable {
	...
	/**
	 * time in milliseconds when TheadCheckHasPaid determined you have not paid.
	 * 0 if you have paid. Used in GuiAchievement whether to display the nag
	 * text.
	 */
	...
	public Minecraft(Component par1Component, Canvas par2Canvas,
		...
		if (par3MinecraftApplet == null
				|| "true".equals(par3MinecraftApplet
						.getParameter("stand-alone"))) {
			hideQuitButton = false;
		}
	...
	public void startGame() throws LWJGLException {
		...
		System.out.println((new StringBuilder()).append("LWJGL Version: ")
				.append(Sys.getVersion()).toString());
		...
		if (gameSettings.language != null) {
			...
			fontRenderer.setBidiFlag(StringTranslate
					.isBidrectional(gameSettings.language));
		}

		ColorizerWater.setWaterBiomeColorizer(renderEngine
				.getTextureContents("/misc/watercolor.png"));
		ColorizerGrass.setGrassBiomeColorizer(renderEngine
				.getTextureContents("/misc/grasscolor.png"));
		ColorizerFoliage.getFoilageBiomeColorizer(renderEngine
	...
	private void loadScreen() throws LWJGLException {
		...
		GL11.glMatrixMode(GL11.GL_PROJECTION);
		GL11.glLoadIdentity();
		GL11.glOrtho(0.0D, scaledresolution.scaledWidthD,
				scaledresolution.scaledHeightD, 0.0D, 1000D, 3000D);
		GL11.glMatrixMode(GL11.GL_MODELVIEW);
		GL11.glLoadIdentity();
		...
		tessellator.setColorOpaque_I(0xffffff);
		scaledTessellator((scaledresolution.getScaledWidth() - c) / 2,
	 ...
	 */
	public void scaledTessellator(int par1, int par2, int par3, int par4,
			int par5, int par6) {
		float f = 0.00390625F;
		float f1 = 0.00390625F;
		Tessellator tessellator = Tessellator.instance;
		tessellator.startDrawingQuads();
		tessellator.addVertexWithUV(par1 + 0, par2 + par6, 0.0D,
				(float) (par3 + 0) * f, (float) (par4 + par6) * f1);
		tessellator.addVertexWithUV(par1 + par5, par2 + par6, 0.0D,
				(float) (par3 + par5) * f, (float) (par4 + par6) * f1);
		tessellator.addVertexWithUV(par1 + par5, par2 + 0, 0.0D,
				(float) (par3 + par5) * f, (float) (par4 + 0) * f1);
		tessellator.addVertexWithUV(par1 + 0, par2 + 0, 0.0D,
	...
	private void runGameLoop() {
		...
		while (System.currentTimeMillis() >= debugUpdateTime + 1000L) {
			debug = (new StringBuilder()).append(fpsCounter).append(" fps, ")
	...
	private void updateDebugProfilerName(int par1) {
		...
		} else if (!(--par1 >= list.size() || ((ProfilerResult) list.get(par1)).name
				.equals("unspecified"))) {
	...
	private void displayDebugInfo(long par1) {
		...
		frameTimes[numRecordedFrameTimes++ & frameTimes.length - 1] = l1
				- prevFrameTime;
		prevFrameTime = l1;
		GL11.glClear(256);
		GL11.glMatrixMode(GL11.GL_PROJECTION);
		...
		for (int i1 = 0; i1 < frameTimes.length; i1++) {
			int k1 = ((i1 - numRecordedFrameTimes & frameTimes.length - 1) * 255)
					/ frameTimes.length;
			int j2 = (k1 * k1) / 255;
			j2 = (j2 * j2) / 255;
			int i3 = (j2 * j2) / 255;
			...
			tessellator.addVertex((float) i1 + 0.5F,
					(float) ((long) displayHeight - l3) + 0.5F, 0.0D);
			tessellator.addVertex((float) i1 + 0.5F,
					(float) displayHeight + 0.5F, 0.0D);
			tessellator.setColorOpaque_I(0xff000000 + j2 * 0x10000 + j2 * 256
					+ j2 * 1);
			tessellator.addVertex((float) i1 + 0.5F,
					(float) ((long) displayHeight - l3) + 0.5F, 0.0D);
			tessellator.addVertex((float) i1 + 0.5F,
					(float) ((long) displayHeight - (l3 - l4)) + 0.5F, 0.0D);
		}
		...
		tessellator.addVertex((float) i2 - (float) j1 * 1.1F, (float) k2
				- (float) j1 * 0.6F - 16F, 0.0D);
		tessellator
				.addVertex((float) i2 - (float) j1 * 1.1F, k2 + j1 * 2, 0.0D);
		tessellator
				.addVertex((float) i2 + (float) j1 * 1.1F, k2 + j1 * 2, 0.0D);
		tessellator.addVertex((float) i2 + (float) j1 * 1.1F, (float) k2
				- (float) j1 * 0.6F - 16F, 0.0D);
		...
		for (int j3 = 0; j3 < list.size(); j3++) {
			...
			tessellator
					.setColorOpaque_I((profilerresult1.getDisplayColor() & 0xfefefe) >> 1);

			for (int i5 = i4; i5 >= 0; i5--) {
				float f1 = (float) (((d + (profilerresult1.sectionPercentage * (double) i5)
						/ (double) i4)
						* Math.PI * 2D) / 100D);
				...
				tessellator.addVertex((float) i2 + f3, ((float) k2 - f5) + 10F,
						0.0D);
			}
		...
		if (profilerresult.name.length() == 0) {
			...
			s = (new StringBuilder()).append(s).append(profilerresult.name)
					.append(" ").toString();
		}
		...
		fontRenderer.drawStringWithShadow(
				s = (new StringBuilder())
						.append(decimalformat
								.format(profilerresult.globalPercentage))
						.append("%").toString(),
				(i2 + j1) - fontRenderer.getStringWidth(s), k2 - j1 / 2 - 16,
				j4);
		...
		for (int k3 = 0; k3 < list.size(); k3++) {
			...
			if (!profilerresult2.name.equals("unspecified")) {
				s1 = (new StringBuilder()).append(s1).append("[")
						.append(k3 + 1).append("] ").toString();
			} else {
				s1 = (new StringBuilder()).append(s1).append("[?] ").toString();
			}

			s1 = (new StringBuilder()).append(s1).append(profilerresult2.name)
					.toString();
			fontRenderer.drawStringWithShadow(s1, i2 - j1, k2 + j1 / 2 + k3 * 8
					+ 20, profilerresult2.getDisplayColor());
			fontRenderer.drawStringWithShadow(
					s1 = (new StringBuilder())
							.append(decimalformat
									.format(profilerresult2.sectionPercentage))
							.append("%").toString(), (i2 + j1) - 50
							- fontRenderer.getStringWidth(s1), k2 + j1 / 2 + k3
							* 8 + 20, profilerresult2.getDisplayColor());
			fontRenderer.drawStringWithShadow(
					s1 = (new StringBuilder())
							.append(decimalformat
									.format(profilerresult2.globalPercentage))
							.append("%").toString(),
					(i2 + j1) - fontRenderer.getStringWidth(s1), k2 + j1 / 2
							+ k3 * 8 + 20, profilerresult2.getDisplayColor());
		}
	}
	...
	private void sendClickBlockToController(int par1, boolean par2) {
		...
		if (par2 && objectMouseOver != null
			...
			playerController.onPlayerDamageBlock(i, j, k,
					objectMouseOver.sideHit);

			if (thePlayer.canPlayerEdit(i, j, k)) {
				effectRenderer.addBlockHitEffects(i, j, k,
						objectMouseOver.sideHit);
				thePlayer.swingItem();
			}
		} else {
	...
	private void clickMouse(int par1) {
		...
		if (objectMouseOver == null) {
			...
			if (par1 == 0) {
				...
				if (playerController.onPlayerRightClick(thePlayer, theWorld,
						itemstack2, i, j, k, l)) {
					flag = false;
					thePlayer.swingItem();
				}
				...
				} else if (itemstack2.stackSize != i1
	...
	public void runTick() {
		...
		if (currentScreen == null || currentScreen.allowUserInput) {
					...
					if (Keyboard.getEventKey() == 87) {
						...
						if (currentScreen != null) {
							...
							if (Keyboard.getEventKey() == 33
									&& Keyboard.isKeyDown(61)) {
								boolean flag = Keyboard.isKeyDown(42)
										| Keyboard.isKeyDown(54);
								gameSettings.setOptionValue(
										EnumOptions.RENDER_DISTANCE, flag ? -1
												: 1);
							}
							...
							if (Keyboard.getEventKey() == 61) {
								...
								gameSettings.field_50119_G = !GuiScreen
										.func_50049_m();
							}
			...
			for (; gameSettings.keyBindInventory.isPressed(); displayGuiScreen(new GuiInventory(
					thePlayer))) {
			}

			for (; gameSettings.keyBindDrop.isPressed(); thePlayer
					.dropOneItem()) {
			}
			...
			if (isMultiplayerWorld() && currentScreen == null
					&& (Keyboard.isKeyDown(53) || Keyboard.isKeyDown(181))) {
	...
	/**
	 * Checks if the current world is a multiplayer world, returns true if it
	 * is, false otherwise.
	 */
	 ...
	 */
	public void startWorld(String par1Str, String par2Str,
		...
		if (saveLoader.isOldMapFormat(par1Str)) {
			...
			if (loadingScreen != null) {
				loadingScreen.printText(StatCollector
						.translateToLocal("menu.switchingLevel"));
				loadingScreen.displayLoadingString("");
			}

			net.minecraft.src.ISaveHandler isavehandler = saveLoader
					.getSaveLoader(par1Str, false);
			World world = null;
			...
			if (world.isNewWorld) {
				...
				changeWorld2(world,
						StatCollector.translateToLocal("menu.generatingLevel"));
			} else {
				statFileWriter.readStat(StatList.loadWorldStat, 1);
				statFileWriter.readStat(StatList.startGameStat, 1);
				changeWorld2(world,
						StatCollector.translateToLocal("menu.loadingLevel"));
			}
		}
	}
	...
	public void usePortal(int par1) {
		...
		if (thePlayer.dimension == -1) {
			...
			World world2 = null;
			world2 = new World(theWorld,
					WorldProvider.getProviderForDimension(thePlayer.dimension));
			ChunkCoordinates chunkcoordinates = world2
		...
		System.out.println((new StringBuilder()).append("Teleported to ")
				.append(theWorld.worldProvider.worldType).toString());
	...
	/**
	 * first argument is the world to change to, second one is a loading message
	 * and the third the player itself
	 */
	public void changeWorld(World par1World, String par2Str,
		...
		if (par1World != null) {
			...
			if (!isMultiplayerWorld()) {
				if (par3EntityPlayer == null) {
					thePlayer = (EntityPlayerSP) par1World
							.func_4085_a(net.minecraft.entity.EntityPlayerSP.class);
				}
			...
			net.minecraft.src.IChunkProvider ichunkprovider = par1World
					.getChunkProvider();
	...
	private void preloadWorld(String par1Str) {
		if (loadingScreen != null) {
			...
			loadingScreen.displayLoadingString(StatCollector
					.translateToLocal("menu.generatingTerrain"));
		}
		...
		if (ichunkprovider instanceof ChunkProviderLoadOrGenerate) {
			...
			chunkproviderloadorgenerate.setCurrentChunkOver(
					chunkcoordinates.posX >> 4, chunkcoordinates.posZ >> 4);
		}
		...
		for (int k = -c; k <= c; k += 16) {
			for (int l = -c; l <= c; l += 16) {
				...
				theWorld.getBlockId(chunkcoordinates.posX + k, 64,
						chunkcoordinates.posZ + l);
		...
		if (!playerController.func_35643_e()) {
			if (loadingScreen != null) {
				loadingScreen.displayLoadingString(StatCollector
						.translateToLocal("menu.simulating"));
			}
	...
	public String debugInfoEntities() {
		return (new StringBuilder()).append("P: ")
				...
				.append(theWorld.getDebugLoadedEntities()).toString();
	}
	...
	public void respawn(boolean par1, int par2, boolean par3) {
		...
		if (ichunkprovider instanceof ChunkProviderLoadOrGenerate) {
			ChunkProviderLoadOrGenerate chunkproviderloadorgenerate = (ChunkProviderLoadOrGenerate) ichunkprovider;
			chunkproviderloadorgenerate.setCurrentChunkOver(
					chunkcoordinates1.posX >> 4, chunkcoordinates1.posZ >> 4);
		}
		...
		if (flag) {
			...
			thePlayer.setLocationAndAngles(
					(float) chunkcoordinates1.posX + 0.5F,
					(float) chunkcoordinates1.posY + 0.1F,
					(float) chunkcoordinates1.posZ + 0.5F, 0.0F, 0.0F);
		}
	...
	public static void startMainThread(String par0Str, String par1Str,
			String par2Str) {
		boolean flag = false;
		String s = par0Str;
	...
	private void clickMiddleMouseButton() {
		if (objectMouseOver != null) {
			...
			if (Item.itemsList[i] != null && Item.itemsList[i].getHasSubtypes()) {
				j = theWorld.getBlockMetadata(objectMouseOver.blockX,
						objectMouseOver.blockY, objectMouseOver.blockZ);
				flag1 = true;
			}

			if (Item.itemsList[i] != null
					&& (Item.itemsList[i] instanceof ItemBlock)) {
			...
			if (flag) {
				int k = (thePlayer.inventorySlots.inventorySlots.size() - 9)
						+ thePlayer.inventory.currentItem;
				playerController.sendSlotPacket(thePlayer.inventory
						.getStackInSlot(thePlayer.inventory.currentItem), k);
			}
		}
	}
	...
	public static void func_52004_D() {
		...
		playerusagesnooper.func_52022_a("os_name",
				System.getProperty("os.name"));
		playerusagesnooper.func_52022_a("os_version",
				System.getProperty("os.version"));
		playerusagesnooper.func_52022_a("os_architecture",
				System.getProperty("os.arch"));
		playerusagesnooper.func_52022_a("memory_total",
				Long.valueOf(Runtime.getRuntime().totalMemory()));
		playerusagesnooper.func_52022_a("memory_max",
				Long.valueOf(Runtime.getRuntime().maxMemory()));
		playerusagesnooper.func_52022_a("java_version",
				System.getProperty("java.version"));
		playerusagesnooper.func_52022_a("opengl_version",
				GL11.glGetString(GL11.GL_VERSION));
		playerusagesnooper.func_52022_a("opengl_vendor",
