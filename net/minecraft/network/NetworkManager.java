...
import net.minecraft.threads.ThreadMonitorConnection;
import java.io.*;
import java.net.*;
import java.util.*;
...
public class NetworkManager {
	...
	/**
	 * Counter used to detect read timeouts after 1200 failed attempts to read a
	 * packet.
	 */
	...
	/**
	 * Counter used to prevent us from sending too many chunk data packets one
	 * after another. The delay appears to be set to 50.
	 */
	public int chunkDataSendCounter;
	private int field_20100_w;

	public NetworkManager(Socket par1Socket, String par2Str,
			NetHandler par3NetHandler) throws IOException {
		...
		socketOutputStream = new DataOutputStream(new BufferedOutputStream(
				par1Socket.getOutputStream(), 5120));
		readThread = new NetworkReaderThread(this, (new StringBuilder())
				.append(par2Str).append(" read thread").toString());
		writeThread = new NetworkWriterThread(this, (new StringBuilder())
				.append(par2Str).append(" write thread").toString());
		readThread.start();
		writeThread.start();
	}
	 ...
	 * if there is one and the counter is up, or does nothing. If it sends a
	 * packet, it sleeps for 10ms.
	 */
	...
	private void onNetworkError(Exception par1Exception) {
		...
		networkShutdown(
				...
				new Object[] { (new StringBuilder())
						.append("Internal exception: ")
						.append(par1Exception.toString()).toString() });
	}
	...
	public void processReadPackets() {
		...
		for (int i = 1000; !readPackets.isEmpty() && i-- >= 0; packet
				.processPacket(netHandler)) {
	 ...
	 */
	public static boolean isRunning(NetworkManager par0NetworkManager) {
	 ...
	 */
	static void sendError(NetworkManager par0NetworkManager,
			Exception par1Exception) {
	 ...
	 */
	public static Thread getReadThread(NetworkManager par0NetworkManager) {
	 ...
	 */
	public static Thread getWriteThread(NetworkManager par0NetworkManager) {
