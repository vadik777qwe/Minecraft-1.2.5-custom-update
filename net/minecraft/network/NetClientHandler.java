...
import net.minecraft.entity.player.PlayerControllerMP;
import net.minecraft.world.chunk.Chunk;
import net.minecraft.world.chunk.ChunkCoordinates;
import net.minecraft.gui.GuiDownloadTerrain;
import net.minecraft.gui.GuiDisconnected;
import net.minecraft.gui.GuiPlayerInfo;
import net.minecraft.gui.GuiWinGame;
import net.minecraft.item.ItemStack;
import net.minecraft.item.Item;
import net.minecraft.item.ItemMap;
import net.minecraft.world.map.MapStorage;
import net.minecraft.containers.Container;
import net.minecraft.world.WorldClient;
import net.minecraft.world.WorldProviderSurface;
import net.minecraft.entity.tiles.TileEntityMobSpawner;
import net.minecraft.entity.tiles.TileEntitySign;
import net.minecraft.entity.tiles.TileEntityFurnace;
import net.minecraft.entity.tiles.TileEntity;
import net.minecraft.entity.tiles.TileEntityBrewingStand;
...
import net.minecraft.entity.EntityCrit2FX;
...
import net.minecraft.entity.EntityFishHook;
...
import net.minecraft.packet.packets.Packet100OpenWindow;
import net.minecraft.packet.packets.Packet101CloseWindow;
import net.minecraft.packet.packets.Packet103SetSlot;
import net.minecraft.packet.packets.Packet104WindowItems;
import net.minecraft.packet.packets.Packet105UpdateProgressbar;
import net.minecraft.packet.packets.Packet106Transaction;
import net.minecraft.packet.packets.Packet10Flying;
import net.minecraft.packet.packets.Packet130UpdateSign;
import net.minecraft.packet.packets.Packet131MapData;
import net.minecraft.packet.packets.Packet132TileEntityData;
import net.minecraft.packet.packets.Packet17Sleep;
import net.minecraft.packet.packets.Packet18Animation;
import net.minecraft.packet.packets.Packet1Login;
import net.minecraft.packet.packets.Packet200Statistic;
import net.minecraft.packet.packets.Packet201PlayerInfo;
import net.minecraft.packet.packets.Packet202PlayerAbilities;
import net.minecraft.packet.packets.Packet20NamedEntitySpawn;
import net.minecraft.packet.packets.Packet21PickupSpawn;
import net.minecraft.packet.packets.Packet22Collect;
import net.minecraft.packet.packets.Packet23VehicleSpawn;
import net.minecraft.packet.packets.Packet24MobSpawn;
import net.minecraft.packet.packets.Packet255KickDisconnect;
import net.minecraft.packet.packets.Packet25EntityPainting;
import net.minecraft.packet.packets.Packet26EntityExpOrb;
import net.minecraft.packet.packets.Packet28EntityVelocity;
import net.minecraft.packet.packets.Packet29DestroyEntity;
import net.minecraft.packet.packets.Packet2Handshake;
import net.minecraft.packet.packets.Packet30Entity;
import net.minecraft.packet.packets.Packet34EntityTeleport;
import net.minecraft.packet.packets.Packet35EntityHeadRotation;
import net.minecraft.packet.packets.Packet38EntityStatus;
import net.minecraft.packet.packets.Packet39AttachEntity;
import net.minecraft.packet.packets.Packet3Chat;
import net.minecraft.packet.packets.Packet40EntityMetadata;
import net.minecraft.packet.packets.Packet41EntityEffect;
import net.minecraft.packet.packets.Packet42RemoveEntityEffect;
import net.minecraft.packet.packets.Packet43Experience;
import net.minecraft.packet.packets.Packet4UpdateTime;
import net.minecraft.packet.packets.Packet50PreChunk;
import net.minecraft.packet.packets.Packet51MapChunk;
import net.minecraft.packet.packets.Packet52MultiBlockChange;
import net.minecraft.packet.packets.Packet53BlockChange;
import net.minecraft.packet.packets.Packet54PlayNoteBlock;
import net.minecraft.packet.packets.Packet5PlayerInventory;
import net.minecraft.packet.packets.Packet60Explosion;
import net.minecraft.packet.packets.Packet61DoorChange;
import net.minecraft.packet.packets.Packet6SpawnPosition;
import net.minecraft.packet.packets.Packet70Bed;
import net.minecraft.packet.packets.Packet71Weather;
import net.minecraft.packet.packets.Packet8UpdateHealth;
...
public class NetClientHandler extends NetHandler {
	...
	public void handleLogin(Packet1Login par1Packet1Login) {
		...
		worldClient = new WorldClient(this, new WorldSettings(0L,
				par1Packet1Login.serverMode, false, false,
				par1Packet1Login.terrainType), par1Packet1Login.field_48170_e,
		...
		((PlayerControllerMP) mc.playerController)
				.setCreative(par1Packet1Login.serverMode == 1);
	}
	...
	public void handlePickupSpawn(Packet21PickupSpawn par1Packet21PickupSpawn) {
		...
		EntityItem entityitem = new EntityItem(worldClient, d, d1, d2,
				new ItemStack(par1Packet21PickupSpawn.itemID,
						par1Packet21PickupSpawn.count,
						par1Packet21PickupSpawn.itemDamage));
		...
		worldClient.addEntityToWorld(par1Packet21PickupSpawn.entityId,
				entityitem);
	}
	...
	public void handleVehicleSpawn(Packet23VehicleSpawn par1Packet23VehicleSpawn) {
		...
		if (par1Packet23VehicleSpawn.type == 10) {
			...
			obj = new EntityFireball(worldClient, d, d1, d2,
					(double) par1Packet23VehicleSpawn.speedX / 8000D,
					(double) par1Packet23VehicleSpawn.speedY / 8000D,
					(double) par1Packet23VehicleSpawn.speedZ / 8000D);
			par1Packet23VehicleSpawn.throwerEntityId = 0;
		} else if (par1Packet23VehicleSpawn.type == 64) {
			obj = new EntitySmallFireball(worldClient, d, d1, d2,
					(double) par1Packet23VehicleSpawn.speedX / 8000D,
					(double) par1Packet23VehicleSpawn.speedY / 8000D,
					(double) par1Packet23VehicleSpawn.speedZ / 8000D);
			par1Packet23VehicleSpawn.throwerEntityId = 0;
			...
			obj = new EntityPotion(worldClient, d, d1, d2,
		...
		if (obj != null) {
			...
			if (aentity != null) {
				int i = par1Packet23VehicleSpawn.entityId
						- ((Entity) (obj)).entityId;
			...
			worldClient.addEntityToWorld(par1Packet23VehicleSpawn.entityId,
					((Entity) (obj)));
	...
	public void handleEntityExpOrb(Packet26EntityExpOrb par1Packet26EntityExpOrb) {
		EntityXPOrb entityxporb = new EntityXPOrb(worldClient,
				par1Packet26EntityExpOrb.posX, par1Packet26EntityExpOrb.posY,
				par1Packet26EntityExpOrb.posZ, par1Packet26EntityExpOrb.xpValue);
		...
		worldClient.addEntityToWorld(par1Packet26EntityExpOrb.entityId,
				entityxporb);
	}
	...
	public void handleWeather(Packet71Weather par1Packet71Weather) {
		...
		if (par1Packet71Weather.isLightningBolt == 1) {
			entitylightningbolt = new EntityLightningBolt(worldClient, d, d1,
					d2);
		}
	 ...
	 * Packet handler
	 */
	public void handleEntityPainting(
			Packet25EntityPainting par1Packet25EntityPainting) {
		EntityPainting entitypainting = new EntityPainting(worldClient,
				par1Packet25EntityPainting.xPosition,
				par1Packet25EntityPainting.yPosition,
				par1Packet25EntityPainting.zPosition,
				par1Packet25EntityPainting.direction,
				par1Packet25EntityPainting.title);
		worldClient.addEntityToWorld(par1Packet25EntityPainting.entityId,
				entitypainting);
	}
	...
	public void handleEntityVelocity(
			Packet28EntityVelocity par1Packet28EntityVelocity) {
		...
		if (entity == null) {
			...
			entity.setVelocity(
					(double) par1Packet28EntityVelocity.motionX / 8000D,
					(double) par1Packet28EntityVelocity.motionY / 8000D,
					(double) par1Packet28EntityVelocity.motionZ / 8000D);
			return;
		}
	}
	...
	public void handleEntityMetadata(
			Packet40EntityMetadata par1Packet40EntityMetadata) {
		...
		if (entity != null && par1Packet40EntityMetadata.getMetadata() != null) {
			entity.getDataWatcher().updateWatchedObjectsFromList(
					par1Packet40EntityMetadata.getMetadata());
		}
	}

	public void handleNamedEntitySpawn(
			Packet20NamedEntitySpawn par1Packet20NamedEntitySpawn) {
		...
		EntityOtherPlayerMP entityotherplayermp = new EntityOtherPlayerMP(
				mc.theWorld, par1Packet20NamedEntitySpawn.name);
		...
		worldClient.addEntityToWorld(par1Packet20NamedEntitySpawn.entityId,
				entityotherplayermp);
	}
	...
	public void handleEntity(Packet30Entity par1Packet30Entity) {
		...
		if (entity == null) {
			...
			double d2 = (double) entity.serverPosZ / 32D;
			float f = par1Packet30Entity.rotating ? (float) (par1Packet30Entity.yaw * 360) / 256F
					: entity.rotationYaw;
			float f1 = par1Packet30Entity.rotating ? (float) (par1Packet30Entity.pitch * 360) / 256F
	...
	public void handleEntityHeadRotation(
			Packet35EntityHeadRotation par1Packet35EntityHeadRotation) {
	...
	public void handleDestroyEntity(
			Packet29DestroyEntity par1Packet29DestroyEntity) {
	...
	public void handlePreChunk(Packet50PreChunk par1Packet50PreChunk) {
		worldClient.doPreChunk(par1Packet50PreChunk.xPosition,
				par1Packet50PreChunk.yPosition, par1Packet50PreChunk.mode);
	}

	public void handleMultiBlockChange(
			Packet52MultiBlockChange par1Packet52MultiBlockChange) {
		...
		DataInputStream datainputstream = new DataInputStream(
				new ByteArrayInputStream(
						par1Packet52MultiBlockChange.metadataArray));
		...
		try {
			for (int k = 0; k < par1Packet52MultiBlockChange.size; k++) {
				...
				int k1 = word0 >> 8 & 0xf;
				int l1 = word0 & 0xff;
				worldClient.setBlockAndMetadataAndInvalidate(j1 + i, l1,
						k1 + j, l, i1);
			}
	...
	public void func_48487_a(Packet51MapChunk par1Packet51MapChunk) {
		worldClient.invalidateBlockReceiveRegion(par1Packet51MapChunk.xCh << 4,
				0, par1Packet51MapChunk.zCh << 4,
				(par1Packet51MapChunk.xCh << 4) + 15, 256,
				(par1Packet51MapChunk.zCh << 4) + 15);
		Chunk chunk = worldClient.getChunkFromChunkCoords(
				par1Packet51MapChunk.xCh, par1Packet51MapChunk.zCh);

		if (par1Packet51MapChunk.includeInitialize && chunk == null) {
			worldClient.doPreChunk(par1Packet51MapChunk.xCh,
					par1Packet51MapChunk.zCh, true);
			chunk = worldClient.getChunkFromChunkCoords(
					par1Packet51MapChunk.xCh, par1Packet51MapChunk.zCh);
		}

		if (chunk != null) {
			chunk.func_48494_a(par1Packet51MapChunk.chunkData,
					par1Packet51MapChunk.yChMin, par1Packet51MapChunk.yChMax,
					par1Packet51MapChunk.includeInitialize);
			worldClient.markBlocksDirty(par1Packet51MapChunk.xCh << 4, 0,
					par1Packet51MapChunk.zCh << 4,
					(par1Packet51MapChunk.xCh << 4) + 15, 256,
					(par1Packet51MapChunk.zCh << 4) + 15);

			if (!par1Packet51MapChunk.includeInitialize
					|| !(worldClient.worldProvider instanceof WorldProviderSurface)) {
	...
	public void handleBlockChange(Packet53BlockChange par1Packet53BlockChange) {
		worldClient.setBlockAndMetadataAndInvalidate(
				par1Packet53BlockChange.xPosition,
				par1Packet53BlockChange.yPosition,
				par1Packet53BlockChange.zPosition,
				par1Packet53BlockChange.type, par1Packet53BlockChange.metadata);
	}

	public void handleKickDisconnect(
			Packet255KickDisconnect par1Packet255KickDisconnect) {
		netManager.networkShutdown("disconnect.kicked", new Object[0]);
		disconnected = true;
		mc.changeWorld1(null);
		mc.displayGuiScreen(new GuiDisconnected("disconnect.disconnected",
				"disconnect.genericReason",
				new Object[] { par1Packet255KickDisconnect.reason }));
	}
	...
	public void handleErrorMessage(String par1Str, Object par2ArrayOfObj[]) {
		if (disconnected) {
			...
			disconnected = true;
			mc.changeWorld1(null);
			mc.displayGuiScreen(new GuiDisconnected("disconnect.lost", par1Str,
					par2ArrayOfObj));
			return;
		}
	}
	...
	public void handleCollect(Packet22Collect par1Packet22Collect) {
		...
		if (entity != null) {
			...
			mc.effectRenderer.addEffect(new EntityPickupFX(mc.theWorld, entity,
					((Entity) (obj)), -0.5F));
			worldClient
					.removeEntityFromWorld(par1Packet22Collect.collectedEntityId);
		}
	}
	...
	public void handleAnimation(Packet18Animation par1Packet18Animation) {
		...
		if (par1Packet18Animation.animate == 1) {
			...
			EntityCrit2FX entitycrit2fx = new EntityCrit2FX(mc.theWorld,
					entity, "magicCrit");
	...
	public void handleSleep(Packet17Sleep par1Packet17Sleep) {
		...
		if (par1Packet17Sleep.field_22046_e == 0) {
			EntityPlayer entityplayer = (EntityPlayer) entity;
			entityplayer.sleepInBedAt(par1Packet17Sleep.bedX,
					par1Packet17Sleep.bedY, par1Packet17Sleep.bedZ);
		}
	}
			...
			try {
				URL url = new URL(
						(new StringBuilder())
								...
								.append(mc.session.username)
								.append("&sessionId=")
								.append(mc.session.sessionId)
								.append("&serverId=")
								.append(par1Packet2Handshake.username)
								.toString());
				...
				netManager.networkShutdown(
						"disconnect.genericReason",
						new Object[] { (new StringBuilder())
								.append("Internal client error: ")
	...
	public void handleMobSpawn(Packet24MobSpawn par1Packet24MobSpawn) {
		...
		EntityLiving entityliving = (EntityLiving) EntityList.createEntityByID(
				par1Packet24MobSpawn.type, mc.theWorld);
		...
		worldClient.addEntityToWorld(par1Packet24MobSpawn.entityId,
				entityliving);
	...
	public void handleSpawnPosition(
			Packet6SpawnPosition par1Packet6SpawnPosition) {
		mc.thePlayer.setSpawnChunk(new ChunkCoordinates(
				par1Packet6SpawnPosition.xPosition,
				par1Packet6SpawnPosition.yPosition,
				par1Packet6SpawnPosition.zPosition));
		mc.theWorld.getWorldInfo().setSpawnPosition(
				par1Packet6SpawnPosition.xPosition,
				par1Packet6SpawnPosition.yPosition,
				par1Packet6SpawnPosition.zPosition);
	}
	...
	/**
	 * Recieves player health from the server and then proceeds to set it
	...
	public void handleUpdateHealth(Packet8UpdateHealth par1Packet8UpdateHealth) {
		...
		mc.thePlayer.getFoodStats().setFoodSaturationLevel(
				par1Packet8UpdateHealth.foodSaturation);
	}
	...
	public void handleExperience(Packet43Experience par1Packet43Experience) {
		mc.thePlayer.setXPStats(par1Packet43Experience.experience,
				par1Packet43Experience.experienceTotal,
				par1Packet43Experience.experienceLevel);
	}
	...
	public void handleRespawn(Packet9Respawn par1Packet9Respawn) {
		if (par1Packet9Respawn.respawnDimension != mc.thePlayer.dimension) {
			field_1210_g = false;
			worldClient = new WorldClient(this, new WorldSettings(0L,
					par1Packet9Respawn.creativeMode, false, false,
					par1Packet9Respawn.terrainType),
					par1Packet9Respawn.respawnDimension,
					par1Packet9Respawn.difficulty);
			worldClient.isRemote = true;
			mc.changeWorld1(worldClient);
		...
		((PlayerControllerMP) mc.playerController)
				.setCreative(par1Packet9Respawn.creativeMode == 1);
	}

	public void handleExplosion(Packet60Explosion par1Packet60Explosion) {
		Explosion explosion = new Explosion(mc.theWorld, null,
				par1Packet60Explosion.explosionX,
				par1Packet60Explosion.explosionY,
				par1Packet60Explosion.explosionZ,
				par1Packet60Explosion.explosionSize);
	...
	public void handleOpenWindow(Packet100OpenWindow par1Packet100OpenWindow) {
		...
		case 0:
			entityplayersp.displayGUIChest(new InventoryBasic(
					par1Packet100OpenWindow.windowTitle,
					par1Packet100OpenWindow.slotsCount));
		...
		case 1:
			entityplayersp.displayWorkbenchGUI(
		...
		case 4:
			entityplayersp.displayGUIEnchantment(
					MathHelper.floor_double(entityplayersp.posX),
					MathHelper.floor_double(entityplayersp.posY),
					MathHelper.floor_double(entityplayersp.posZ));
			entityplayersp.craftingInventory.windowId = par1Packet100OpenWindow.windowId;
	...
	public void handleSetSlot(Packet103SetSlot par1Packet103SetSlot) {
		...
		if (par1Packet103SetSlot.windowId == -1) {
			((EntityPlayer) (entityplayersp)).inventory
					.setItemStack(par1Packet103SetSlot.myItemStack);
		} else if (par1Packet103SetSlot.windowId == 0
				&& par1Packet103SetSlot.itemSlot >= 36
				&& par1Packet103SetSlot.itemSlot < 45) {
			ItemStack itemstack = ((EntityPlayer) (entityplayersp)).inventorySlots
					.getSlot(par1Packet103SetSlot.itemSlot).getStack();

			if (par1Packet103SetSlot.myItemStack != null
					&& (itemstack == null || itemstack.stackSize < par1Packet103SetSlot.myItemStack.stackSize)) {
				par1Packet103SetSlot.myItemStack.animationsToGo = 5;
			}

			((EntityPlayer) (entityplayersp)).inventorySlots.putStackInSlot(
					par1Packet103SetSlot.itemSlot,
					par1Packet103SetSlot.myItemStack);
		} else if (par1Packet103SetSlot.windowId == ((EntityPlayer) (entityplayersp)).craftingInventory.windowId) {
			((EntityPlayer) (entityplayersp)).craftingInventory.putStackInSlot(
					par1Packet103SetSlot.itemSlot,
					par1Packet103SetSlot.myItemStack);
		}
	}
	...
	public void handleTransaction(Packet106Transaction par1Packet106Transaction) {
		...
		if (container != null) {
			if (par1Packet106Transaction.accepted) {
				...
				addToSendQueue(new Packet106Transaction(
						par1Packet106Transaction.windowId,
						par1Packet106Transaction.shortWindowId, true));
			}
		}
	}
	...
	public void handleWindowItems(Packet104WindowItems par1Packet104WindowItems) {
		...
		if (par1Packet104WindowItems.windowId == 0) {
			((EntityPlayer) (entityplayersp)).inventorySlots
					.putStacksInSlots(par1Packet104WindowItems.itemStack);
		} else if (par1Packet104WindowItems.windowId == ((EntityPlayer) (entityplayersp)).craftingInventory.windowId) {
			((EntityPlayer) (entityplayersp)).craftingInventory
					.putStacksInSlots(par1Packet104WindowItems.itemStack);
		}
	}
	...
	public void handleUpdateSign(Packet130UpdateSign par1Packet130UpdateSign) {
		if (mc.theWorld.blockExists(par1Packet130UpdateSign.xPosition,
	...
	public void handleTileEntityData(
			Packet132TileEntityData par1Packet132TileEntityData) {
		if (mc.theWorld.blockExists(par1Packet132TileEntityData.xPosition,
				par1Packet132TileEntityData.yPosition,
				par1Packet132TileEntityData.zPosition)) {
			TileEntity tileentity = mc.theWorld.getBlockTileEntity(
					par1Packet132TileEntityData.xPosition,
					par1Packet132TileEntityData.yPosition,
					par1Packet132TileEntityData.zPosition);

			if (tileentity != null
					&& par1Packet132TileEntityData.actionType == 1
					&& (tileentity instanceof TileEntityMobSpawner)) {
				((TileEntityMobSpawner) tileentity)
						.setMobID(EntityList
								.getStringFromID(par1Packet132TileEntityData.customParam1));
			}
		}
	}

	public void handleUpdateProgressbar(
			Packet105UpdateProgressbar par1Packet105UpdateProgressbar) {
		EntityPlayerSP entityplayersp = mc.thePlayer;
		registerPacket(par1Packet105UpdateProgressbar);

		if (((EntityPlayer) (entityplayersp)).craftingInventory != null
				&& ((EntityPlayer) (entityplayersp)).craftingInventory.windowId == par1Packet105UpdateProgressbar.windowId) {
			((EntityPlayer) (entityplayersp)).craftingInventory
					.updateProgressBar(
							par1Packet105UpdateProgressbar.progressBar,
							par1Packet105UpdateProgressbar.progressBarValue);
		}
	}

	public void handlePlayerInventory(
			Packet5PlayerInventory par1Packet5PlayerInventory) {
		Entity entity = getEntityByID(par1Packet5PlayerInventory.entityID);

		if (entity != null) {
			entity.outfitWithItem(par1Packet5PlayerInventory.slot,
					par1Packet5PlayerInventory.itemID,
					par1Packet5PlayerInventory.itemDamage);
		}
	}
	...
	public void handlePlayNoteBlock(
			Packet54PlayNoteBlock par1Packet54PlayNoteBlock) {
		mc.theWorld.playNoteAt(par1Packet54PlayNoteBlock.xLocation,
				par1Packet54PlayNoteBlock.yLocation,
				par1Packet54PlayNoteBlock.zLocation,
				par1Packet54PlayNoteBlock.instrumentType,
	...
	public void handleBed(Packet70Bed par1Packet70Bed) {
		...
		if (i >= 0 && i < Packet70Bed.bedChat.length
				&& Packet70Bed.bedChat[i] != null) {
		...
		if (i == 1) {
			...
			((PlayerControllerMP) mc.playerController)
					.setCreative(par1Packet70Bed.gameMode == 1);
		} else if (i == 4) {
	...
	/**
	 * Contains logic for handling packets containing arbitrary unique item
	 * data. Currently this is only for maps.
	 */
	public void handleMapData(Packet131MapData par1Packet131MapData) {
		if (par1Packet131MapData.itemID == Item.map.shiftedIndex) {
			ItemMap.getMPMapData(par1Packet131MapData.uniqueID, mc.theWorld)
					.func_28171_a(par1Packet131MapData.itemData);
		} else {
			System.out.println((new StringBuilder()).append("Unknown itemid: ")
					.append(par1Packet131MapData.uniqueID).toString());
		}
	}

	public void handleDoorChange(Packet61DoorChange par1Packet61DoorChange) {
		mc.theWorld.playAuxSFX(par1Packet61DoorChange.sfxID,
				par1Packet61DoorChange.posX, par1Packet61DoorChange.posY,
				par1Packet61DoorChange.posZ, par1Packet61DoorChange.auxData);
	}
	...
	public void handleStatistic(Packet200Statistic par1Packet200Statistic) {
		((EntityClientPlayerMP) mc.thePlayer).incrementStat(
				StatList.getOneShotStat(par1Packet200Statistic.statisticId),
				par1Packet200Statistic.amount);
	}
	...
	public void handleEntityEffect(Packet41EntityEffect par1Packet41EntityEffect) {
		...
		if (entity == null || !(entity instanceof EntityLiving)) {
			...
			((EntityLiving) entity).addPotionEffect(new PotionEffect(
					par1Packet41EntityEffect.effectId,
					par1Packet41EntityEffect.duration,
					par1Packet41EntityEffect.effectAmp));
			return;
		}
	}
	...
	public void handleRemoveEntityEffect(
			Packet42RemoveEntityEffect par1Packet42RemoveEntityEffect) {
		...
		if (entity == null || !(entity instanceof EntityLiving)) {
			...
			((EntityLiving) entity)
					.removePotionEffect(par1Packet42RemoveEntityEffect.effectId);
			return;
		}
	}
	...
	public void handlePlayerInfo(Packet201PlayerInfo par1Packet201PlayerInfo) {
		GuiPlayerInfo guiplayerinfo = (GuiPlayerInfo) playerInfoMap
				.get(par1Packet201PlayerInfo.playerName);

		if (guiplayerinfo == null && par1Packet201PlayerInfo.isConnected) {
			guiplayerinfo = new GuiPlayerInfo(
					par1Packet201PlayerInfo.playerName);
			playerInfoMap
					.put(par1Packet201PlayerInfo.playerName, guiplayerinfo);
	...
	public void func_50100_a(
			Packet202PlayerAbilities par1Packet202PlayerAbilities) {
		EntityPlayerSP entityplayersp = mc.thePlayer;
