...
public class TexturePackList {
	...
	public void updateAvaliableTexturePacks() {
		...
		if (texturePackDir.exists() && texturePackDir.isDirectory()) {
			...
			for (int j = 0; j < i; j++) {
				...
				if (file.isFile()
					...
					String s = (new StringBuilder()).append(file.getName())
							.append(":").append(file.length()).append(":")
							.append(file.lastModified()).toString();

					try {
						if (!field_6538_d.containsKey(s)) {
							TexturePackCustom texturepackcustom = new TexturePackCustom(
									file);
						...
						TexturePackBase texturepackbase1 = (TexturePackBase) field_6538_d
								.get(s);

						if (texturepackbase1.texturePackFileName
								.equals(currentTexturePack)) {
				...
				if (!file.isDirectory()
						|| !(new File(file, "pack.txt")).exists()) {
					continue;
				}

				String s1 = (new StringBuilder()).append(file.getName())
						.append(":folder:").append(file.lastModified())
						.toString();

				try {
					if (!field_6538_d.containsKey(s1)) {
						TexturePackFolder texturepackfolder = new TexturePackFolder(
								file);
					...
					TexturePackBase texturepackbase2 = (TexturePackBase) field_6538_d
							.get(s1);

					if (texturepackbase2.texturePackFileName
							.equals(currentTexturePack)) {
		...
		for (Iterator iterator = availableTexturePacks.iterator(); iterator
				.hasNext(); field_6538_d.remove(texturepackbase.texturePackID)) {
