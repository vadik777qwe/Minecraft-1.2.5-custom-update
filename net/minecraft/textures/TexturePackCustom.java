...
public class TexturePackCustom extends TexturePackBase {
	...
	/**
	 * The allocated OpenGL texture name for this texture pack, or -1 if it
	...
	public void func_6485_a(Minecraft par1Minecraft) throws IOException {
		...
		try {
			...
			try {
				inputstream = zipfile.getInputStream(zipfile
						.getEntry("pack.txt"));
				BufferedReader bufferedreader = new BufferedReader(
						new InputStreamReader(inputstream));
				firstDescriptionLine = truncateString(bufferedreader.readLine());
				secondDescriptionLine = truncateString(bufferedreader
						.readLine());
	...
	public void bindThumbnailTexture(Minecraft par1Minecraft) {
		if (texturePackThumbnail != null && texturePackName < 0) {
			texturePackName = par1Minecraft.renderEngine
					.allocateAndSetupTexture(texturePackThumbnail);
		}
		...
		if (texturePackThumbnail != null) {
			...
			GL11.glBindTexture(GL11.GL_TEXTURE_2D, par1Minecraft.renderEngine
					.getTexture("/gui/unknown_pack.png"));
		}
	}
	...
	/**
	 * Closes the zipfile associated to this texture pack. Does nothing for the
	...
	public InputStream getResourceAsStream(String par1Str) {
		try {
			java.util.zip.ZipEntry zipentry = texturePackZipFile
					.getEntry(par1Str.substring(1));
		...
		return (net.minecraft.textures.TexturePackBase.class)
