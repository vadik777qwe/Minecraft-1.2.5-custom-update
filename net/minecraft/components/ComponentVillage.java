...
public abstract class ComponentVillage extends StructureComponent {
	 ...
	 * rectangle).
	 */
	protected int getAverageGroundLevel(World par1World,
		...
		for (int k = boundingBox.minZ; k <= boundingBox.maxZ; k++) {
			for (int l = boundingBox.minX; l <= boundingBox.maxX; l++) {
				if (par2StructureBoundingBox.isVecInside(l, 64, k)) {
					i += Math.max(par1World.getTopSolidOrLiquidBlock(l, k),
							par1World.worldProvider.getAverageGroundLevel());
					j++;
				}
			}
		}
	 ...
	 */
	protected void spawnVillagers(World par1World,
		...
		do {
			...
			villagersSpawned++;
			EntityVillager entityvillager = new EntityVillager(par1World,
					getVillagerType(i));
			entityvillager.setLocationAndAngles((double) j + 0.5D, k,
					(double) l + 0.5D, 0.0F, 0.0F);
