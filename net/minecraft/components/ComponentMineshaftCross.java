...
public class ComponentMineshaftCross extends StructureComponent {
	...
	public ComponentMineshaftCross(int par1, Random par2Random,
	...
	public void buildComponent(StructureComponent par1StructureComponent,
		...
		case 2:
			StructureMineshaftPieces.getNextComponent(par1StructureComponent,
					par2List, par3Random, boundingBox.minX + 1,
					boundingBox.minY, boundingBox.minZ - 1, 2, i);
			StructureMineshaftPieces.getNextComponent(par1StructureComponent,
					par2List, par3Random, boundingBox.minX - 1,
					boundingBox.minY, boundingBox.minZ + 1, 1, i);
			StructureMineshaftPieces.getNextComponent(par1StructureComponent,
					par2List, par3Random, boundingBox.maxX + 1,
					boundingBox.minY, boundingBox.minZ + 1, 3, i);
			break;
	...
	public boolean addComponentParts(World par1World, Random par2Random,
		...
		if (isMultipleFloors) {
			fillWithBlocks(par1World, par3StructureBoundingBox,
			...
			fillWithBlocks(par1World, par3StructureBoundingBox,
					...
					boundingBox.maxZ - 1, 0, 0, false);
			fillWithBlocks(par1World, par3StructureBoundingBox,
					boundingBox.minX + 1, boundingBox.maxY - 2,
					boundingBox.minZ, boundingBox.maxX - 1, boundingBox.maxY,
					boundingBox.maxZ, 0, 0, false);
			fillWithBlocks(par1World, par3StructureBoundingBox,
					boundingBox.minX, boundingBox.maxY - 2,
					boundingBox.minZ + 1, boundingBox.maxX, boundingBox.maxY,
					boundingBox.maxZ - 1, 0, 0, false);
			fillWithBlocks(par1World, par3StructureBoundingBox,
					boundingBox.minX + 1, boundingBox.minY + 3,
					boundingBox.minZ + 1, boundingBox.maxX - 1,
					boundingBox.minY + 3, boundingBox.maxZ - 1, 0, 0, false);
		} else {
			fillWithBlocks(par1World, par3StructureBoundingBox,
					boundingBox.minX + 1, boundingBox.minY, boundingBox.minZ,
		...
		for (int i = boundingBox.minX; i <= boundingBox.maxX; i++) {
			for (int j = boundingBox.minZ; j <= boundingBox.maxZ; j++) {
				int k = getBlockIdAtCurrentPosition(par1World, i,
						boundingBox.minY - 1, j, par3StructureBoundingBox);

				if (k == 0) {
					placeBlockAtCurrentPosition(par1World,
							Block.planks.blockID, 0, i, boundingBox.minY - 1,
							j, par3StructureBoundingBox);
				}
			}
		}
