...
public class WorldGenSpikes extends WorldGenerator {
	/**
	 * The Block ID that the generator is allowed to replace while generating
	 * the terrain.
	 */
	...
	public boolean generate(World par1World, Random par2Random, int par3,
		...
		if (!par1World.isAirBlock(par3, par4, par5)
				|| par1World.getBlockId(par3, par4 - 1, par5) != replaceID) {
			return false;
		}
		...
		for (int k = par3 - j; k <= par3 + j; k++) {
			for (int i1 = par5 - j; i1 <= par5 + j; i1++) {
				...
				if (k1 * k1 + i2 * i2 <= j * j + 1
						&& par1World.getBlockId(k, par4 - 1, i1) != replaceID) {
					return false;
				}
			}
		}
			...
			for (int j1 = par3 - j; j1 <= par3 + j; j1++) {
				for (int l1 = par5 - j; l1 <= par5 + j; l1++) {
					...
					if (j2 * j2 + k2 * k2 <= j * j + 1) {
						par1World.setBlockWithNotify(j1, l, l1,
		...
		EntityEnderCrystal entityendercrystal = new EntityEnderCrystal(
				par1World);
		entityendercrystal.setLocationAndAngles((float) par3 + 0.5F, par4 + i,
				(float) par5 + 0.5F, par2Random.nextFloat() * 360F, 0.0F);
		par1World.spawnEntityInWorld(entityendercrystal);
		par1World.setBlockWithNotify(par3, par4 + i, par5,
