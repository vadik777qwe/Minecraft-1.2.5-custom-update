...
public final class SpawnerAnimals {
	...
	protected static ChunkPosition getRandomSpawningPointInChunk(
		...
		int j = par0World.rand.nextInt(chunk != null ? Math.max(128,
				chunk.getTopFilledSegment()) : 128);
		int k = par2 * 16 + par0World.rand.nextInt(16);
		return new ChunkPosition(i, j, k);
	}

	/**
	 * The main spawning algorithm, spawns three random creatures of types in
	 * the subclass array
	 */
	public static final int performSpawning(World par0World, boolean par1,
			boolean par2) {
		if (!par1 && !par2) {
			return 0;
		} else {
			...
			for (var3 = 0; var3 < par0World.playerEntities.size(); ++var3) {
				EntityPlayer var4 = (EntityPlayer) par0World.playerEntities
						.get(var3);
				...
				for (int var8 = -var7; var8 <= var7; ++var8) {
					for (int var9 = -var7; var9 <= var7; ++var9) {
						boolean var10 = var8 == -var7 || var8 == var7
								|| var9 == -var7 || var9 == var7;
						ChunkCoordIntPair var11 = new ChunkCoordIntPair(var8
								+ var5, var9 + var6);

						if (!var10) {
							eligibleChunksForSpawning.put(var11,
			...
			for (int var33 = 0; var33 < var6; ++var33) {
				...
				if ((!var34.getPeacefulCreature() || par2)
						...
						&& par0World.countEntities(var34.getCreatureClass()) <= var34
								.getMaxNumberOfCreature()
								* eligibleChunksForSpawning.size() / 256) {
					Iterator var35 = eligibleChunksForSpawning.keySet()
							.iterator();
					label108:

					while (var35.hasNext()) {
						ChunkCoordIntPair var37 = (ChunkCoordIntPair) var35
								.next();

						if (!((Boolean) eligibleChunksForSpawning.get(var37))
								.booleanValue()) {
							ChunkPosition var36 = getRandomSpawningPointInChunk(
									par0World, var37.chunkXPos, var37.chunkZPos);
							int var12 = var36.x;
							int var13 = var36.y;
							int var14 = var36.z;

							if (!par0World.isBlockNormalCube(var12, var13,
									var14)
									&& par0World.getBlockMaterial(var12, var13,
											var14) == var34
											.getCreatureMaterial()) {
								int var15 = 0;
								int var16 = 0;
								...
								while (var16 < 3) {
									...
									while (true) {
										if (var22 < 4) {
											label101: {
												var17 += par0World.rand
														.nextInt(var20)
														- par0World.rand
																.nextInt(var20);
												var18 += par0World.rand
														.nextInt(1)
														- par0World.rand
																.nextInt(1);
												var19 += par0World.rand
														.nextInt(var20)
														- par0World.rand
																.nextInt(var20);
												...
												if (canCreatureTypeSpawnAtLocation(
													...
													if (par0World
														...
														if (var29 >= 576.0F) {
															...
															try {
																var38 = (EntityLiving) var21.entityClass
																		.getConstructor(
																				new Class[] { World.class })
																		.newInstance(
																				new Object[] { par0World });
	...
	public static boolean canCreatureTypeSpawnAtLocation(
		...
		if (par0EnumCreatureType.getCreatureMaterial() == Material.water) {
			...
			return Block.isNormalCube(i) && i != Block.bedrock.blockID
	 ...
	 * different color
	 */
	private static void creatureSpecificInit(EntityLiving par0EntityLiving,
			World par1World, float par2, float par3, float par4) {
		if ((par0EntityLiving instanceof EntitySpider)
				&& par1World.rand.nextInt(100) == 0) {
			EntitySkeleton entityskeleton = new EntitySkeleton(par1World);
			entityskeleton.setLocationAndAngles(par2, par3, par4,
					par0EntityLiving.rotationYaw, 0.0F);
			...
			((EntitySheep) par0EntityLiving).setFleeceColor(EntitySheep
					.getRandomFleeceColor(par1World.rand));
		} else if ((par0EntityLiving instanceof EntityOcelot)
				&& par1World.rand.nextInt(7) == 0) {
			for (int i = 0; i < 2; i++) {
				EntityOcelot entityocelot = new EntityOcelot(par1World);
				entityocelot.setLocationAndAngles(par2, par3, par4,
						par0EntityLiving.rotationYaw, 0.0F);
	...
	public static void performWorldGenSpawning(World par0World,
		...
		while (par6Random.nextFloat() < par1BiomeGenBase.getSpawningChance()) {
			SpawnListEntry spawnlistentry = (SpawnListEntry) WeightedRandom
					.getRandomItem(par0World.rand, list);
			int i = spawnlistentry.minGroupCount
					+ par6Random.nextInt((1 + spawnlistentry.maxGroupCount)
			...
			while (j1 < i) {
				...
				for (int k1 = 0; !flag && k1 < 4; k1++) {
					...
					if (canCreatureTypeSpawnAtLocation(
							EnumCreatureType.creature, par0World, j, l1, k)) {
						...
						entityliving.setLocationAndAngles(f, f1, f2,
								par6Random.nextFloat() * 360F, 0.0F);
					...
					for (k += par6Random.nextInt(5) - par6Random.nextInt(5); j < par2
							|| j >= par2 + par4 || k < par3 || k >= par3 + par4; k = (i1 + par6Random
