...
public class WorldGenHugeTrees extends WorldGenerator {
	...
	public boolean generate(World par1World, Random par2Random, int par3,
		...
		if (k != Block.grass.blockID && k != Block.dirt.blockID
				|| par4 >= 256 - i - 1) {
			return false;
		}
		...
		for (int l = (par4 + i) - 2 - par2Random.nextInt(4); l > par4 + i / 2; l -= 2 + par2Random
				.nextInt(4)) {
			...
			for (int j3 = 0; j3 < 5; j3++) {
				...
				setBlockAndMetadata(par1World, i2, (l - 3) + j3 / 2, i3,
		...
		for (int j1 = 0; j1 < i; j1++) {
			...
			if (j2 == 0 || j2 == Block.leaves.blockID) {
				setBlockAndMetadata(par1World, par3, par4 + j1, par5,
						Block.wood.blockID, woodMetadata);

				if (j1 > 0) {
					if (par2Random.nextInt(3) > 0
							&& par1World.isAirBlock(par3 - 1, par4 + j1, par5)) {
						setBlockAndMetadata(par1World, par3 - 1, par4 + j1,
								par5, Block.vine.blockID, 8);
					}

					if (par2Random.nextInt(3) > 0
							&& par1World.isAirBlock(par3, par4 + j1, par5 - 1)) {
						setBlockAndMetadata(par1World, par3, par4 + j1,
								par5 - 1, Block.vine.blockID, 1);
					}
				}
			}
			...
			if (j2 == 0 || j2 == Block.leaves.blockID) {
				setBlockAndMetadata(par1World, par3 + 1, par4 + j1, par5,
						Block.wood.blockID, woodMetadata);

				if (j1 > 0) {
					if (par2Random.nextInt(3) > 0
							&& par1World.isAirBlock(par3 + 2, par4 + j1, par5)) {
						setBlockAndMetadata(par1World, par3 + 2, par4 + j1,
								par5, Block.vine.blockID, 2);
					}

					if (par2Random.nextInt(3) > 0
							&& par1World.isAirBlock(par3 + 1, par4 + j1,
									par5 - 1)) {
						setBlockAndMetadata(par1World, par3 + 1, par4 + j1,
								par5 - 1, Block.vine.blockID, 1);
					}
			...
			if (j2 == 0 || j2 == Block.leaves.blockID) {
				setBlockAndMetadata(par1World, par3 + 1, par4 + j1, par5 + 1,
						Block.wood.blockID, woodMetadata);

				if (j1 > 0) {
					if (par2Random.nextInt(3) > 0
							&& par1World.isAirBlock(par3 + 2, par4 + j1,
									par5 + 1)) {
						setBlockAndMetadata(par1World, par3 + 2, par4 + j1,
								par5 + 1, Block.vine.blockID, 2);
					}
			...
			setBlockAndMetadata(par1World, par3, par4 + j1, par5 + 1,
					Block.wood.blockID, woodMetadata);
			...
			if (par2Random.nextInt(3) > 0
					&& par1World.isAirBlock(par3 - 1, par4 + j1, par5 + 1)) {
				setBlockAndMetadata(par1World, par3 - 1, par4 + j1, par5 + 1,
						Block.vine.blockID, 8);
			}

			if (par2Random.nextInt(3) > 0
					&& par1World.isAirBlock(par3, par4 + j1, par5 + 2)) {
				setBlockAndMetadata(par1World, par3, par4 + j1, par5 + 2,
	...
	private void func_48192_a(World par1World, int par2, int par3, int par4,
		...
		for (int i = par4 - byte0; i <= par4; i++) {
			...
			for (int l = par2 - k; l <= par2 + k + 1; l++) {
				...
				for (int j1 = par3 - k; j1 <= par3 + k + 1; j1++) {
					...
					if ((i1 >= 0 || k1 >= 0 || i1 * i1 + k1 * k1 <= k * k)
							&& (i1 <= 0 && k1 <= 0 || i1 * i1 + k1 * k1 <= (k + 1)
									* (k + 1))
							&& (par6Random.nextInt(4) != 0 || i1 * i1 + k1 * k1 <= (k - 1)
									* (k - 1))
							&& !Block.opaqueCubeLookup[par1World.getBlockId(l,
									i, j1)]) {
						setBlockAndMetadata(par1World, l, i, j1,
