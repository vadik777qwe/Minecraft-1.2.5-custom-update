...
public class EntityAIArrowAttack extends EntityAIBase {
	...
	public EntityAIArrowAttack(EntityLiving par1EntityLiving, float par2,
			int par3, int par4) {
		rangedAttackTime = 0;
		field_48367_f = 0;
	...
	public void updateTask() {
		double d = 100D;
		double d1 = entityHost.getDistanceSq(attackTarget.posX,
		...
		entityHost.getLookHelper().setLookPositionWithEntity(attackTarget, 30F,
				30F);
	...
	private void doRangedAttack() {
		if (rangedAttackID == 1) {
			EntityArrow entityarrow = new EntityArrow(worldObj, entityHost,
					attackTarget, 1.6F, 12F);
			worldObj.playSoundAtEntity(entityHost, "random.bow", 1.0F,
					1.0F / (entityHost.getRNG().nextFloat() * 0.4F + 0.8F));
			worldObj.spawnEntityInWorld(entityarrow);
		} else if (rangedAttackID == 2) {
			EntitySnowball entitysnowball = new EntitySnowball(worldObj,
					entityHost);
			double d = attackTarget.posX - entityHost.posX;
			double d1 = (attackTarget.posY + (double) attackTarget
					.getEyeHeight())
					- 1.1000000238418579D
					- entitysnowball.posY;
			double d2 = attackTarget.posZ - entityHost.posZ;
			float f = MathHelper.sqrt_double(d * d + d2 * d2) * 0.2F;
			entitysnowball.setThrowableHeading(d, d1 + (double) f, d2, 1.6F,
					12F);
			worldObj.playSoundAtEntity(entityHost, "random.bow", 1.0F,
					1.0F / (entityHost.getRNG().nextFloat() * 0.4F + 0.8F));
