...
public class WorldGenTrees extends WorldGenerator {
	...
	public WorldGenTrees(boolean par1, int par2, int par3, int par4,
			boolean par5) {
		super(par1);
		field_48202_a = par2;
		field_48201_c = par3;
	...
	public boolean generate(World par1World, Random par2Random, int par3,
		...
		for (int k1 = (par4 - byte1) + i; k1 <= par4 + i; k1++) {
			...
			for (int l3 = par3 - j3; l3 <= par3 + j3; l3++) {
				...
				for (int l4 = par5 - j3; l4 <= par5 + j3; l4++) {
					...
					if ((Math.abs(j4) != j3 || Math.abs(i5) != j3 || par2Random
							.nextInt(2) != 0 && k2 != 0)
							&& !Block.opaqueCubeLookup[par1World.getBlockId(l3,
									k1, l4)]) {
						setBlockAndMetadata(par1World, l3, k1, l4,
								Block.leaves.blockID, field_48199_d);
					}
				}
			}
		}
		...
		for (int l1 = 0; l1 < i; l1++) {
			...
			setBlockAndMetadata(par1World, par3, par4 + l1, par5,
					Block.wood.blockID, field_48201_c);
			...
			if (par2Random.nextInt(3) > 0
					&& par1World.isAirBlock(par3 - 1, par4 + l1, par5)) {
				setBlockAndMetadata(par1World, par3 - 1, par4 + l1, par5,
			...
			if (par2Random.nextInt(3) > 0
					&& par1World.isAirBlock(par3, par4 + l1, par5 - 1)) {
				setBlockAndMetadata(par1World, par3, par4 + l1, par5 - 1,
				...
				for (int i4 = par3 - k3; i4 <= par3 + k3; i4++) {
					for (int k4 = par5 - k3; k4 <= par5 + k3; k4++) {
						...
						if (par2Random.nextInt(4) == 0
								&& par1World.getBlockId(i4 - 1, i2, k4) == 0) {
						...
						if (par2Random.nextInt(4) == 0
								&& par1World.getBlockId(i4, i2, k4 - 1) == 0) {
	...
	private void func_48198_a(World par1World, int par2, int par3, int par4,
			int par5) {
		setBlockAndMetadata(par1World, par2, par3, par4, Block.vine.blockID,
				par5);

		for (int i = 4; par1World.getBlockId(par2, --par3, par4) == 0 && i > 0; i--) {
			setBlockAndMetadata(par1World, par2, par3, par4,
					Block.vine.blockID, par5);
		}
	}
}
