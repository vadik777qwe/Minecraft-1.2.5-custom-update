...
public class Vec3D {
	...
	/**
	 * Next empty index in the vectorList. We don't ever seem to remove vectors
	...
	public static Vec3D createVector(double par0, double par2, double par4) {
		...
		return ((Vec3D) vectorList.get(nextVector++)).setComponents(par0, par2,
				par4);
	}
	...
	public Vec3D subtract(Vec3D par1Vec3D) {
		return createVector(par1Vec3D.xCoord - xCoord, par1Vec3D.yCoord
				- yCoord, par1Vec3D.zCoord - zCoord);
	}
	...
	public Vec3D normalize() {
		double d = MathHelper.sqrt_double(xCoord * xCoord + yCoord * yCoord
				+ zCoord * zCoord);
	...
	public double dotProduct(Vec3D par1Vec3D) {
		return xCoord * par1Vec3D.xCoord + yCoord * par1Vec3D.yCoord + zCoord
				* par1Vec3D.zCoord;
	}
	...
	public Vec3D crossProduct(Vec3D par1Vec3D) {
		return createVector(yCoord * par1Vec3D.zCoord - zCoord
				* par1Vec3D.yCoord, zCoord * par1Vec3D.xCoord - xCoord
				* par1Vec3D.zCoord, xCoord * par1Vec3D.yCoord - yCoord
				* par1Vec3D.xCoord);
	}
	...
	public double lengthVector() {
		return (double) MathHelper.sqrt_double(xCoord * xCoord + yCoord
	...
	public Vec3D getIntermediateWithXValue(Vec3D par1Vec3D, double par2) {
		...
		if (d3 < 0.0D || d3 > 1.0D) {
			...
			return createVector(xCoord + d * d3, yCoord + d1 * d3, zCoord + d2
					* d3);
		}
	}
	...
	public String toString() {
		return (new StringBuilder()).append("(").append(xCoord).append(", ")
				.append(yCoord).append(", ").append(zCoord).append(")")
				.toString();
	}
