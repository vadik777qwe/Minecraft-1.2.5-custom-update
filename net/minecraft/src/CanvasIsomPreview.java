...
public class CanvasIsomPreview extends Canvas implements KeyListener,
	...
	public File getWorkingDirectory(String par1Str) {
		...
		case 4:
			file = new File(s, (new StringBuilder())
					.append("Library/Application Support/").append(par1Str)
					.toString());
			break;

		default:
			file = new File(s, (new StringBuilder()).append(par1Str)
					.append('/').toString());
			break;
		}
	...
	public void loadLevel(String par1Str) {
		...
		level = new World(new SaveHandler(new File(workDir, "saves"), par1Str,
				false), par1Str, new WorldSettings((new Random()).nextLong(),
				0, true, false, WorldType.DEFAULT));
	...
	public void render(Graphics2D par1Graphics2D) {
		currentRender++;
		java.awt.geom.AffineTransform affinetransform = par1Graphics2D
				.getTransform();
		par1Graphics2D.setClip(0, 0, getWidth(), getHeight());
		par1Graphics2D.setRenderingHint(RenderingHints.KEY_INTERPOLATION,
		...
		int l = (rectangle.y + rectangle.height + 16 + 128) / byte0 + 1 + byte1
				* 2;
		...
		for (int i1 = k; i1 <= l; i1++) {
			for (int k1 = i; k1 <= j; k1++) {
				...
				if (!isoimagebuffer.noContent) {
					...
					par1Graphics2D
							.drawImage(isoimagebuffer.image, j2, k2, null);
				}
			}
		}
		...
		if (showHelp) {
			...
			par1Graphics2D
					.fillRect(4, getHeight() - 32 - 4, getWidth() - 8, 32);
			...
			par1Graphics2D.drawString(s, getWidth() / 2
					- par1Graphics2D.getFontMetrics().stringWidth(s) / 2,
					j1 + 20);
		}
