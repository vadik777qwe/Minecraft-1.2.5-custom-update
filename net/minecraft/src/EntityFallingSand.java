...
public class EntityFallingSand extends Entity {
	...
	public EntityFallingSand(World par1World, double par2, double par4,
		...
		if (onGround) {
			...
			if (worldObj.getBlockId(i, j, k) != Block.pistonMoving.blockID) {
				...
				if ((!worldObj.canBlockBePlacedAt(blockID, i, j, k, true, 1)
						|| BlockSand.canFallBelow(worldObj, i, j - 1, k) || !worldObj
							.setBlockWithNotify(i, j, k, blockID))
						&& !worldObj.isRemote) {
					dropItem(blockID, 1);
				}
			}
		} else if (fallTime > 100 && !worldObj.isRemote && (j < 1 || j > 256)
				|| fallTime > 600) {
			dropItem(blockID, 1);
			setDead();
		}
