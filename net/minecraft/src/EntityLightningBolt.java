...
public class EntityLightningBolt extends EntityWeatherEffect {
	...
	public EntityLightningBolt(World par1World, double par2, double par4,
			double par6) {
		super(par1World);
		boltVertex = 0L;
		...
		if (par1World.difficultySetting >= 2
			...
			for (int j = 0; j < 4; j++) {
				...
				if (par1World.getBlockId(l, j1, k1) == 0
						&& Block.fire.canPlaceBlockAt(par1World, l, j1, k1)) {
	...
	public void onUpdate() {
		...
		if (lightningState >= 0) {
			...
			List list = worldObj.getEntitiesWithinAABBExcludingEntity(this,
					AxisAlignedBB.getBoundingBoxFromPool(posX - d, posY - d,
							posZ - d, posX + d, posY + 6D + d, posZ + d));
