...
public class EntityBoat extends Entity {
	...
	public void onUpdate() {
		...
		for (int j = 0; j < i; j++) {
			double d2 = (boundingBox.minY + ((boundingBox.maxY - boundingBox.minY) * (double) (j + 0))
					/ (double) i) - 0.125D;
			double d8 = (boundingBox.minY + ((boundingBox.maxY - boundingBox.minY) * (double) (j + 1))
					/ (double) i) - 0.125D;
			AxisAlignedBB axisalignedbb = AxisAlignedBB.getBoundingBoxFromPool(
					boundingBox.minX, d2, boundingBox.minZ, boundingBox.maxX,
					d8, boundingBox.maxZ);
		...
		if (d1 > 0.14999999999999999D) {
			...
			for (int i1 = 0; (double) i1 < 1.0D + d1 * 60D; i1++) {
				...
				if (rand.nextBoolean()) {
					double d21 = (posX - d3 * d16 * 0.80000000000000004D) + d9
							* d19;
					double d23 = posZ - d9 * d16 * 0.80000000000000004D - d3
							* d19;
		...
		if (worldObj.isRemote) {
			if (boatPosRotationIncrements > 0) {
				double d4 = posX + (boatX - posX)
						/ (double) boatPosRotationIncrements;
				double d10 = posY + (boatY - posY)
						/ (double) boatPosRotationIncrements;
				double d13 = posZ + (boatZ - posZ)
						/ (double) boatPosRotationIncrements;
				double d17;
