...
public class BlockBed extends BlockDirectional {
	...
	public boolean blockActivated(World par1World, int par2, int par3,
		...
		if (!par1World.worldProvider.canRespawnHere()) {
			...
			par1World.newExplosion(null, (float) par2 + 0.5F,
					(float) par3 + 0.5F, (float) par4 + 0.5F, 5F, true);
			return true;
		}
		...
		if (isBedOccupied(i)) {
			...
			do {
				...
				if (entityplayer1.isPlayerSleeping()) {
					...
					if (chunkcoordinates.posX == par2
							&& chunkcoordinates.posY == par3
							&& chunkcoordinates.posZ == par4) {
	...
	public void onNeighborBlockChange(World par1World, int par2, int par3,
		...
		if (isBlockFootOfBed(i)) {
			if (par1World.getBlockId(par2 - headBlockToFootBlockMap[j][0],
					par3, par4 - headBlockToFootBlockMap[j][1]) != blockID) {
				par1World.setBlockWithNotify(par2, par3, par4, 0);
			}
		} else if (par1World.getBlockId(par2 + headBlockToFootBlockMap[j][0],
				par3, par4 + headBlockToFootBlockMap[j][1]) != blockID) {
			par1World.setBlockWithNotify(par2, par3, par4, 0);
	 ...
	 */
	public static void setBedOccupied(World par0World, int par1, int par2,
		...
		for (int k = 0; k <= 1; k++) {
			...
			for (int l1 = l; l1 <= j1; l1++) {
				for (int i2 = i1; i2 <= k1; i2++) {
					if (!par0World.isBlockNormalCube(l1, par2 - 1, i2)
							|| !par0World.isAirBlock(l1, par2, i2)
							|| !par0World.isAirBlock(l1, par2 + 1, i2)) {
						continue;
					}
