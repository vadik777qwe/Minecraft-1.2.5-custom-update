...
public class ActiveRenderInfo {
	...
	private static FloatBuffer modelview = GLAllocation
			.createDirectFloatBuffer(16);

	/** The current GL projection matrix */
	private static FloatBuffer projection = GLAllocation
			.createDirectFloatBuffer(16);

	/** The computed view object coordinates */
	private static FloatBuffer objectCoords = GLAllocation
			.createDirectFloatBuffer(3);
	...
	public static void updateRenderInfo(EntityPlayer par0EntityPlayer,
		...
		GLU.gluUnProject(f, f1, 0.0F, modelview, projection, viewport,
				objectCoords);
		...
		rotationX = MathHelper.cos((f3 * (float) Math.PI) / 180F)
				* (float) (1 - i * 2);
		rotationZ = MathHelper.sin((f3 * (float) Math.PI) / 180F)
				* (float) (1 - i * 2);
		rotationYZ = -rotationZ * MathHelper.sin((f2 * (float) Math.PI) / 180F)
				* (float) (1 - i * 2);
		rotationXY = rotationX * MathHelper.sin((f2 * (float) Math.PI) / 180F)
				* (float) (1 - i * 2);
		rotationXZ = MathHelper.cos((f2 * (float) Math.PI) / 180F);
	}

	/**
	 * Returns a vector representing the projection along the given entity's
	 * view for the given distance
	 */
	public static Vec3D projectViewFromEntity(EntityLiving par0EntityLiving,
			double par1) {
		double d = par0EntityLiving.prevPosX
				+ (par0EntityLiving.posX - par0EntityLiving.prevPosX) * par1;
		double d1 = par0EntityLiving.prevPosY
				+ (par0EntityLiving.posY - par0EntityLiving.prevPosY) * par1
				+ (double) par0EntityLiving.getEyeHeight();
		double d2 = par0EntityLiving.prevPosZ
				+ (par0EntityLiving.posZ - par0EntityLiving.prevPosZ) * par1;
	...
	/**
	 * Returns the block ID at the current camera location (either air or
	 * fluid), taking into account the height of fluid blocks
	 */
	public static int getBlockIdAtEntityViewpoint(World par0World,
			EntityLiving par1EntityLiving, float par2) {
		Vec3D vec3d = projectViewFromEntity(par1EntityLiving, par2);
		ChunkPosition chunkposition = new ChunkPosition(vec3d);
		int i = par0World.getBlockId(chunkposition.x, chunkposition.y,
				chunkposition.z);

		if (i != 0 && Block.blocksList[i].blockMaterial.isLiquid()) {
			float f = BlockFluid.getFluidHeightPercent(par0World
					.getBlockMetadata(chunkposition.x, chunkposition.y,
							chunkposition.z)) - 0.1111111F;
			float f1 = (float) (chunkposition.y + 1) - f;

			if (vec3d.yCoord >= (double) f1) {
				i = par0World.getBlockId(chunkposition.x, chunkposition.y + 1,
						chunkposition.z);
			}
		}
