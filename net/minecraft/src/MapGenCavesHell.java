...
public class MapGenCavesHell extends MapGenBase {
	 ...
	 */
	protected void generateLargeCaveNode(int par1, int par2,
			byte par3ArrayOfByte[], double par4, double par6, double par8) {
		generateCaveNode(par1, par2, par3ArrayOfByte, par4, par6, par8,
	 ...
	 */
	protected void generateCaveNode(int par1, int par2, byte par3ArrayOfByte[],
			double par4, double par6, double par8, float par10, float par11,
			float par12, int par13, int par14, double par15) {
		double d = par1 * 16 + 8;
		double d1 = par2 * 16 + 8;
		float f = 0.0F;
		float f1 = 0.0F;
		...
		for (; par13 < par14; par13++) {
			double d2 = 1.5D + (double) (MathHelper
					.sin(((float) par13 * (float) Math.PI) / (float) par14)
					* par10 * 1.0F);
			double d3 = d2 * par15;
			...
			f1 *= 0.9F;
			f *= 0.75F;
			f1 += (random.nextFloat() - random.nextFloat())
					* random.nextFloat() * 2.0F;
			f += (random.nextFloat() - random.nextFloat()) * random.nextFloat()
					* 4F;

			if (!flag && par13 == j && par10 > 1.0F) {
				generateCaveNode(par1, par2, par3ArrayOfByte, par4, par6, par8,
						random.nextFloat() * 0.5F + 0.5F, par11
								- ((float) Math.PI / 2F), par12 / 3F, par13,
						par14, 1.0D);
				generateCaveNode(par1, par2, par3ArrayOfByte, par4, par6, par8,
						random.nextFloat() * 0.5F + 0.5F, par11
								+ ((float) Math.PI / 2F), par12 / 3F, par13,
			...
			if (par4 < d - 16D - d2 * 2D || par8 < d1 - 16D - d2 * 2D
					|| par4 > d + 16D + d2 * 2D || par8 > d1 + 16D + d2 * 2D) {
				continue;
			}
			...
			for (int j1 = (int) d4; !flag2 && j1 < k; j1++) {
				for (int l1 = (int) d6; !flag2 && l1 < i1; l1++) {
					for (int i2 = l + 1; !flag2 && i2 >= d5 - 1; i2--) {
						...
						if (par3ArrayOfByte[j2] == Block.lavaMoving.blockID
								|| par3ArrayOfByte[j2] == Block.lavaStill.blockID) {
							flag2 = true;
						}

						if (i2 != d5 - 1 && j1 != d4 && j1 != k - 1 && l1 != d6
								&& l1 != i1 - 1) {
							i2 = (int) d5;
						}
					}
			...
			for (int k1 = (int) d4; k1 < k; k1++) {
				...
				for (int k2 = (int) d6; k2 < i1; k2++) {
					double d9 = (((double) (k2 + par2 * 16) + 0.5D) - par8)
							/ d2;
					...
					for (int i3 = l - 1; i3 >= d5; i3--) {
						...
						if (d10 > -0.69999999999999996D
								&& d8 * d8 + d10 * d10 + d9 * d9 < 1.0D) {
							byte byte0 = par3ArrayOfByte[l2];

							if (byte0 == Block.netherrack.blockID
									|| byte0 == Block.dirt.blockID
	...
	protected void recursiveGenerate(World par1World, int par2, int par3,
		...
		for (int j = 0; j < i; j++) {
			...
			for (int l = 0; l < k; l++) {
				...
				generateCaveNode(par4, par5, par6ArrayOfByte, d, d1, d2,
						f2 * 2.0F, f, f1, 0, 0, 0.5D);
			}
		}
	}
}
