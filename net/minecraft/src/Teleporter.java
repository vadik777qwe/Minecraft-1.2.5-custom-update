...
public class Teleporter {
			...
			for (int j1 = -2; j1 <= 2; j1++) {
				for (int k1 = -2; k1 <= 2; k1++) {
					for (int l1 = -1; l1 < 3; l1++) {
						...
						int k2 = (k + k1 * i1) - j1 * l;
						boolean flag = l1 < 0;
						par1World.setBlockWithNotify(i2, j2, k2,
								flag ? Block.obsidian.blockID : 0);
					}
				}
			}

			par2Entity.setLocationAndAngles(i, j, k, par2Entity.rotationYaw,
					0.0F);
	...
	public boolean placeInExistingPortal(World par1World, Entity par2Entity) {
		...
		if (d >= 0.0D) {
			...
			par2Entity.setLocationAndAngles(d2, d4, d6, par2Entity.rotationYaw,
					0.0F);
			par2Entity.motionX = par2Entity.motionY = par2Entity.motionZ = 0.0D;
			return true;
		} else {
						...
						for (int j9 = 0; j9 < 3; j9++) {
							for (int k10 = 0; k10 < 4; k10++) {
								for (int l11 = -1; l11 < 4; l11++) {
									...
									if (l11 < 0
											&& !par1World.getBlockMaterial(j12,
													l12, j13).isSolid()
											|| l11 >= 0
											&& !par1World.isAirBlock(j12, l12,
													j13)) {
										break label0;
									}
								}
						...
						for (int l5 = l1; l5 < l1 + 2; l5++) {
							...
							for (int k9 = 0; k9 < 4; k9++) {
								for (int l10 = -1; l10 < 4; l10++) {
									...
									if (l10 < 0
											&& !par1World.getBlockMaterial(i12,
													k12, i13).isSolid()
											|| l10 >= 0
											&& !par1World.isAirBlock(i12, k12,
													i13)) {
										break label1;
									}
								}
			...
			for (int i5 = -1; i5 <= 1; i5++) {
				for (int i6 = 1; i6 < 3; i6++) {
					for (int j7 = -1; j7 < 3; j7++) {
						...
						boolean flag = j7 < 0;
						par1World.setBlockWithNotify(k8, l9, i11,
								flag ? Block.obsidian.blockID : 0);
					}
				}
			}
		...
		for (int j5 = 0; j5 < 4; j5++) {
			...
			for (int j6 = 0; j6 < 4; j6++) {
				for (int k7 = -1; k7 < 4; k7++) {
					...
					par1World.setBlockWithNotify(l8, i10, j11,
							flag1 ? Block.obsidian.blockID
			...
			for (int k6 = 0; k6 < 4; k6++) {
				for (int l7 = -1; l7 < 4; l7++) {
					...
					int j10 = i3 + l7;
					int k11 = l3 + (k6 - 1) * j4;
					par1World.notifyBlocksOfNeighborChange(i9, j10, k11,
							par1World.getBlockId(i9, j10, k11));
				}
			}
		}
