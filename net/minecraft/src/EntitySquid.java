...
public class EntitySquid extends EntityWaterMob {
	...
	public void onLivingUpdate() {
		...
		if (isInWater()) {
			...
			float f1 = MathHelper.sqrt_double(motionX * motionX + motionZ
					* motionZ);
			renderYawOffset += ((-(float) Math.atan2(motionX, motionZ) * 180F)
					/ (float) Math.PI - renderYawOffset) * 0.1F;
			rotationYaw = renderYawOffset;
			field_21087_c = field_21087_c + (float) Math.PI * field_21079_m
					* 1.5F;
			field_21089_a += ((-(float) Math.atan2(f1, motionY) * 180F)
					/ (float) Math.PI - field_21089_a) * 0.1F;
		} else {
			tentacleAngle = MathHelper.abs(MathHelper.sin(field_21085_g))
					* (float) Math.PI * 0.25F;
