...
public class RandomPositionGenerator {
	private static Vec3D field_48624_a = Vec3D.createVectorHelper(0.0D, 0.0D,
			0.0D);

	public RandomPositionGenerator() {
	}

	public static Vec3D func_48622_a(EntityCreature par0EntityCreature,
			int par1, int par2) {
		return func_48621_c(par0EntityCreature, par1, par2, null);
	}

	public static Vec3D func_48620_a(EntityCreature par0EntityCreature,
			int par1, int par2, Vec3D par3Vec3D) {
	...
	public static Vec3D func_48623_b(EntityCreature par0EntityCreature,
			int par1, int par2, Vec3D par3Vec3D) {
	...
	private static Vec3D func_48621_c(EntityCreature par0EntityCreature,
			int par1, int par2, Vec3D par3Vec3D) {
		...
		for (int l = 0; l < 10; l++) {
			...
			if (par3Vec3D != null
					&& (double) i1 * par3Vec3D.xCoord + (double) k1
							* par3Vec3D.zCoord < 0.0D) {
				continue;
			}
