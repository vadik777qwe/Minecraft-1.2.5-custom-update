...
public class InventoryPlayer implements IInventory {
	...
	private int getInventorySlotContainItemAndDamage(int par1, int par2) {
		for (int i = 0; i < mainInventory.length; i++) {
			if (mainInventory[i] != null && mainInventory[i].itemID == par1
					&& mainInventory[i].getItemDamage() == par2) {
				return i;
			}
		}
	...
	private int storeItemStack(ItemStack par1ItemStack) {
		for (int i = 0; i < mainInventory.length; i++) {
			if (mainInventory[i] != null
					...
					&& ItemStack.func_46154_a(mainInventory[i], par1ItemStack)) {
				return i;
			}
		}
	...
	public void func_52006_a(Item par1Item, int par2) {
		if (par1Item != null) {
			int i = getInventorySlotContainItemAndDamage(par1Item.shiftedIndex,
					par2);
			...
			mainInventory[currentItem] = new ItemStack(
					Item.itemsList[par1Item.shiftedIndex], 1, par2);
		}
	}
	...
	private int storePartialItemStack(ItemStack par1ItemStack) {
		...
		if (mainInventory[l] == null) {
			mainInventory[l] = new ItemStack(i, 0,
					par1ItemStack.getItemDamage());

			if (par1ItemStack.hasTagCompound()) {
				mainInventory[l].setTagCompound((NBTTagCompound) par1ItemStack
						.getTagCompound().copy());
			}
		}
		...
		if (i1 > mainInventory[l].getMaxStackSize()
				- mainInventory[l].stackSize) {
			i1 = mainInventory[l].getMaxStackSize()
					- mainInventory[l].stackSize;
		}
	...
	public void decrementAnimations() {
		for (int i = 0; i < mainInventory.length; i++) {
			if (mainInventory[i] != null) {
				mainInventory[i].updateAnimation(player.worldObj, player, i,
						currentItem == i);
			}
		}
	}
	...
	public boolean hasItemStack(ItemStack par1ItemStack) {
		for (int i = 0; i < armorInventory.length; i++) {
			if (armorInventory[i] != null
					&& armorInventory[i].isStackEqual(par1ItemStack)) {
				return true;
			}
		}
		...
		for (int j = 0; j < mainInventory.length; j++) {
			if (mainInventory[j] != null
					&& mainInventory[j].isStackEqual(par1ItemStack)) {
				return true;
			}
		}
	...
	public void copyInventory(InventoryPlayer par1InventoryPlayer) {
		...
		for (int j = 0; j < armorInventory.length; j++) {
			armorInventory[j] = ItemStack
					.copyItemStack(par1InventoryPlayer.armorInventory[j]);
		}
	}
}
