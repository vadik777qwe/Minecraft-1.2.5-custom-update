...
public abstract class StructureComponent {
	...
	public static StructureComponent findIntersecting(List par0List,
		...
		for (Iterator iterator = par0List.iterator(); iterator.hasNext();) {
			StructureComponent structurecomponent = (StructureComponent) iterator
					.next();
	...
	public ChunkPosition getCenter() {
		return new ChunkPosition(boundingBox.getCenterX(),
				boundingBox.getCenterY(), boundingBox.getCenterZ());
	}

	/**
	 * 'checks the entire StructureBoundingBox for Liquids'
	 */
	protected boolean isLiquidInStructureBoundingBox(World par1World,
	...
	protected int getMetadataWithOffset(int par1, int par2) {
		...
		} else if (par1 == Block.stairCompactCobblestone.blockID
				|| par1 == Block.stairCompactPlanks.blockID
				|| par1 == Block.stairsNetherBrick.blockID
				|| par1 == Block.stairsStoneBrickSmooth.blockID) {
			if (coordBaseMode == 0) {
				if (par2 == 2) {
					return 3;
				}
	 ...
	 */
	protected void fillWithBlocks(World par1World,
			StructureBoundingBox par2StructureBoundingBox, int par3, int par4,
			int par5, int par6, int par7, int par8, int par9, int par10,
			boolean par11) {
		for (int i = par4; i <= par7; i++) {
			for (int j = par3; j <= par6; j++) {
				for (int k = par5; k <= par8; k++) {
					if (par11
							&& getBlockIdAtCurrentPosition(par1World, j, i, k,
									par2StructureBoundingBox) == 0) {
						continue;
					}

					if (i == par4 || i == par7 || j == par3 || j == par6
							|| k == par5 || k == par8) {
						placeBlockAtCurrentPosition(par1World, par9, 0, j, i,
								k, par2StructureBoundingBox);
					} else {
						placeBlockAtCurrentPosition(par1World, par10, 0, j, i,
								k, par2StructureBoundingBox);
					}
				}
			}
	 ...
	 */
	protected void fillWithRandomizedBlocks(World par1World,
			StructureBoundingBox par2StructureBoundingBox, int par3, int par4,
			int par5, int par6, int par7, int par8, boolean par9,
			Random par10Random,
			...
			for (int j = par3; j <= par6; j++) {
				for (int k = par5; k <= par8; k++) {
					if (!par9
							|| getBlockIdAtCurrentPosition(par1World, j, i, k,
									par2StructureBoundingBox) != 0) {
						par11StructurePieceBlockSelector.selectBlocks(
								par10Random, j, i, k, i == par4 || i == par7
										|| j == par3 || j == par6 || k == par5
										|| k == par8);
						placeBlockAtCurrentPosition(par1World,
								par11StructurePieceBlockSelector
										.getSelectedBlockId(),
								par11StructurePieceBlockSelector
										.getSelectedBlockMetaData(), j, i, k,
	 ...
	 * float randLimit, int minX, int minY, int minZ, int maxX, int maxY, int
	 * maxZ, int olaceBlockId, int replaceBlockId, boolean alwaysreplace'
	 */
	protected void randomlyFillWithBlocks(World par1World,
			StructureBoundingBox par2StructureBoundingBox, Random par3Random,
			float par4, int par5, int par6, int par7, int par8, int par9,
			int par10, int par11, int par12, boolean par13) {
		for (int i = par6; i <= par9; i++) {
			for (int j = par5; j <= par8; j++) {
				for (int k = par7; k <= par10; k++) {
					...
					if (i == par6 || i == par9 || j == par5 || j == par8
							|| k == par7 || k == par10) {
						placeBlockAtCurrentPosition(par1World, par11, 0, j, i,
								k, par2StructureBoundingBox);
					} else {
						placeBlockAtCurrentPosition(par1World, par12, 0, j, i,
								k, par2StructureBoundingBox);
					}
				}
			}
	 ...
	 * and Spiderwebs'
	 */
	protected void randomlyPlaceBlock(World par1World,
			StructureBoundingBox par2StructureBoundingBox, Random par3Random,
			float par4, int par5, int par6, int par7, int par8, int par9) {
		if (par3Random.nextFloat() < par4) {
			placeBlockAtCurrentPosition(par1World, par8, par9, par5, par6,
					par7, par2StructureBoundingBox);
		}
	}

	/**
	 * arguments: World worldObj, StructureBoundingBox structBB, int minX, int
	 * minY, int minZ, int maxX, int maxY, int maxZ, int placeBlockId, boolean
	 * alwaysreplace
	 */
	protected void randomlyRareFillWithBlocks(World par1World,
			StructureBoundingBox par2StructureBoundingBox, int par3, int par4,
			int par5, int par6, int par7, int par8, int par9, boolean par10) {
		float f = (par6 - par3) + 1;
	 ...
	 * hitting air'
	 */
	protected void clearCurrentPositionBlocksUpwards(World par1World, int par2,
			int par3, int par4, StructureBoundingBox par5StructureBoundingBox) {
		int i = getXWithOffset(par2, par4);
		int j = getYWithOffset(par3);
		int k = getZWithOffset(par2, par4);
	 ...
	 */
	protected void fillCurrentPositionBlocksDownwards(World par1World,
		...
		for (; (par1World.isAirBlock(i, j, k) || par1World.getBlockMaterial(i,
				j, k).isLiquid())
				&& j > 1; j--) {
			par1World.setBlockAndMetadata(i, j, k, par2, par3);
		}
	...
	protected void createTreasureChestAtCurrentPosition(World par1World,
			StructureBoundingBox par2StructureBoundingBox, Random par3Random,
			int par4, int par5, int par6,
			StructurePieceTreasure par7ArrayOfStructurePieceTreasure[], int par8) {
		int i = getXWithOffset(par4, par6);
		int j = getYWithOffset(par5);
		int k = getZWithOffset(par4, par6);

		if (par2StructureBoundingBox.isVecInside(i, j, k)
				&& par1World.getBlockId(i, j, k) != Block.chest.blockID) {
			par1World.setBlockWithNotify(i, j, k, Block.chest.blockID);
			TileEntityChest tileentitychest = (TileEntityChest) par1World
					.getBlockTileEntity(i, j, k);

			if (tileentitychest != null) {
				fillTreasureChestWithLoot(par3Random,
						par7ArrayOfStructurePieceTreasure, tileentitychest,
						par8);
			}
		}
	}

	private static void fillTreasureChestWithLoot(Random par0Random,
			StructurePieceTreasure par1ArrayOfStructurePieceTreasure[],
			TileEntityChest par2TileEntityChest, int par3) {
		for (int i = 0; i < par3; i++) {
			StructurePieceTreasure structurepiecetreasure = (StructurePieceTreasure) WeightedRandom
					.getRandomItem(par0Random,
							par1ArrayOfStructurePieceTreasure);
			int j = structurepiecetreasure.minItemStack
					+ par0Random
							.nextInt((structurepiecetreasure.maxItemStack - structurepiecetreasure.minItemStack) + 1);

			if (Item.itemsList[structurepiecetreasure.itemID]
					.getItemStackLimit() >= j) {
				par2TileEntityChest.setInventorySlotContents(par0Random
						.nextInt(par2TileEntityChest.getSizeInventory()),
						new ItemStack(structurepiecetreasure.itemID, j,
								structurepiecetreasure.itemMetadata));
				continue;
			}
			...
			for (int k = 0; k < j; k++) {
				par2TileEntityChest.setInventorySlotContents(par0Random
						.nextInt(par2TileEntityChest.getSizeInventory()),
						new ItemStack(structurepiecetreasure.itemID, 1,
								structurepiecetreasure.itemMetadata));
			}
		}
	}

	protected void placeDoorAtCurrentPosition(World par1World,
			StructureBoundingBox par2StructureBoundingBox, Random par3Random,
