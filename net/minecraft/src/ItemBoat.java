...
public class ItemBoat extends Item {
	...
	public ItemStack onItemRightClick(ItemStack par1ItemStack, World par2World,
		...
		float f2 = par3EntityPlayer.prevRotationYaw
				...
				* f;
		double d = par3EntityPlayer.prevPosX
		...
		double d2 = par3EntityPlayer.prevPosZ
		...
		MovingObjectPosition movingobjectposition = par2World
				.rayTraceBlocks_do(vec3d, vec3d1, true);
		...
		List list = par2World.getEntitiesWithinAABBExcludingEntity(
				par3EntityPlayer,
				par3EntityPlayer.boundingBox.addCoord(vec3d2.xCoord * d3,
						vec3d2.yCoord * d3, vec3d2.zCoord * d3).expand(f10,
						f10, f10));
		...
		for (int l = 0; l < list.size(); l++) {
			...
			AxisAlignedBB axisalignedbb = entity.boundingBox.expand(f11, f11,
					f11);
		...
		if (movingobjectposition.typeOfHit == EnumMovingObjectType.TILE) {
			...
			if (!par2World.isRemote) {
				...
				par2World.spawnEntityInWorld(new EntityBoat(par2World,
						(float) i + 0.5F, (float) j + 1.0F, (float) k + 0.5F));
			}
