...
public class VillageCollection {
	...
	public void tick() {
		...
		for (Iterator iterator = villageList.iterator(); iterator.hasNext(); village
				.tick(tickCounter)) {
	...
	public Village findNearestVillage(int par1, int par2, int par3, int par4) {
		...
		do {
			...
			float f1 = village1.getCenter()
					.getDistanceSquared(par1, par2, par3);
	...
	private void addNewDoorsToVillageOrCreateVillage() {
		for (int i = 0; i < newDoors.size(); i++) {
			...
			do {
				...
				Village village1 = (Village) iterator.next();
				int j = (int) village1.getCenter().getEuclideanDistanceTo(
						villagedoorinfo.posX, villagedoorinfo.posY,
	...
	private void addUnassignedWoodenDoorsAroundToNewDoorsList(
		...
		for (int i = par1ChunkCoordinates.posX - byte0; i < par1ChunkCoordinates.posX
				+ byte0; i++) {
			for (int j = par1ChunkCoordinates.posY - byte1; j < par1ChunkCoordinates.posY
					+ byte1; j++) {
				for (int k = par1ChunkCoordinates.posZ - byte2; k < par1ChunkCoordinates.posZ
						+ byte2; k++) {
	...
	private VillageDoorInfo getVillageDoorAt(int par1, int par2, int par3) {
		...
		for (Iterator iterator1 = villageList.iterator(); iterator1.hasNext();) {
			...
			VillageDoorInfo villagedoorinfo1 = village.getVillageDoorAt(par1,
					par2, par3);
	...
	private void addDoorToNewListIfAppropriate(int par1, int par2, int par3) {
		int i = ((BlockDoor) Block.doorWood).getDoorOrientation(worldObj, par1,
				par2, par3);
		...
		if (i == 0 || i == 2) {
			...
			if (j != 0) {
				newDoors.add(new VillageDoorInfo(par1, par2, par3, j <= 0 ? 2
						: -2, 0, tickCounter));
			}
		} else {
			int k = 0;
	...
	private boolean isVillagerPositionPresent(int par1, int par2, int par3) {
		for (Iterator iterator = villagerPositionsList.iterator(); iterator
				.hasNext();) {
			ChunkCoordinates chunkcoordinates = (ChunkCoordinates) iterator
					.next();

			if (chunkcoordinates.posX == par1 && chunkcoordinates.posY == par2
					&& chunkcoordinates.posZ == par3) {
				return true;
			}
		}
