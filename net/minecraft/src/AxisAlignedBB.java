...
public class AxisAlignedBB {
	 ...
	 * maxX, maxY, maxZ
	 */
	public static AxisAlignedBB getBoundingBox(double par0, double par2,
	...
	/**
	 * Sets the number of bounding boxes in use from the pool to 0 so they will
	 * be reused
	 */
	...
	/**
	 * Returns a bounding box with the specified bounds from the pool. Args:
	 * minX, minY, minZ, maxX, maxY, maxZ
	 */
	public static AxisAlignedBB getBoundingBoxFromPool(double par0,
			double par2, double par4, double par6, double par8, double par10) {
		if (numBoundingBoxesInUse >= boundingBoxes.size()) {
			boundingBoxes
					.add(getBoundingBox(0.0D, 0.0D, 0.0D, 0.0D, 0.0D, 0.0D));
		}

		return ((AxisAlignedBB) boundingBoxes.get(numBoundingBoxesInUse++))
				.setBounds(par0, par2, par4, par6, par8, par10);
	}

	private AxisAlignedBB(double par1, double par3, double par5, double par7,
			double par9, double par11) {
		minX = par1;
		minY = par3;
		minZ = par5;
		maxX = par7;
	 ...
	 * maxZ
	 */
	public AxisAlignedBB setBounds(double par1, double par3, double par5,
	 ...
	 * numbers are given it will shrink). Args: x, y, z
	 */
	public AxisAlignedBB getOffsetBoundingBox(double par1, double par3,
			double par5) {
		return getBoundingBoxFromPool(minX + par1, minY + par3, minZ + par5,
				maxX + par1, maxY + par3, maxZ + par5);
	}
	...
	public MovingObjectPosition calculateIntercept(Vec3D par1Vec3D,
			Vec3D par2Vec3D) {
		...
		if (vec3d != null
				&& (vec3d6 == null || par1Vec3D.squareDistanceTo(vec3d) < par1Vec3D
						.squareDistanceTo(vec3d6))) {
			vec3d6 = vec3d;
		}

		if (vec3d1 != null
				&& (vec3d6 == null || par1Vec3D.squareDistanceTo(vec3d1) < par1Vec3D
						.squareDistanceTo(vec3d6))) {
			vec3d6 = vec3d1;
		}

		if (vec3d2 != null
				&& (vec3d6 == null || par1Vec3D.squareDistanceTo(vec3d2) < par1Vec3D
						.squareDistanceTo(vec3d6))) {
			vec3d6 = vec3d2;
		}

		if (vec3d3 != null
				&& (vec3d6 == null || par1Vec3D.squareDistanceTo(vec3d3) < par1Vec3D
						.squareDistanceTo(vec3d6))) {
			vec3d6 = vec3d3;
		}
		...
		if (vec3d5 != null
				&& (vec3d6 == null || par1Vec3D.squareDistanceTo(vec3d5) < par1Vec3D
						.squareDistanceTo(vec3d6))) {
			vec3d6 = vec3d5;
		}
