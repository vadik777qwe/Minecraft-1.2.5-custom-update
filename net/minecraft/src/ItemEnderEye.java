...
public class ItemEnderEye extends Item {
	...
	public boolean onItemUse(ItemStack par1ItemStack,
		...
		if (par2EntityPlayer.canPlayerEdit(par4, par5, par6)
				&& i == Block.endPortalFrame.blockID
				&& !BlockEndPortalFrame.isEnderEyeInserted(j)) {
			...
			for (int k = 0; k < 16; k++) {
				double d = (float) par4 + (5F + itemRand.nextFloat() * 6F)
						/ 16F;
				double d1 = (float) par5 + 0.8125F;
				double d2 = (float) par6 + (5F + itemRand.nextFloat() * 6F)
						/ 16F;
				double d3 = 0.0D;
				double d4 = 0.0D;
				double d5 = 0.0D;
			...
			if (flag1 && j1 == i1 + 2) {
				...
				do {
					...
					if (i5 != Block.endPortalFrame.blockID
							|| !BlockEndPortalFrame.isEnderEyeInserted(i6)) {
						flag1 = false;
						break;
					}
				...
				for (int j2 = i1 - 1; j2 <= j1 + 1; j2 += 4) {
					...
					do {
						...
						if (j6 != Block.endPortalFrame.blockID
								|| !BlockEndPortalFrame.isEnderEyeInserted(k6)) {
							flag1 = false;
							continue label0;
						}
				...
				if (flag1) {
					for (int k2 = i1; k2 <= j1; k2++) {
						for (int k3 = 1; k3 <= 3; k3++) {
							...
							par3World.setBlockWithNotify(k4, par5, k5,
	...
	public ItemStack onItemRightClick(ItemStack par1ItemStack, World par2World,
		...
		MovingObjectPosition movingobjectposition = getMovingObjectPositionFromPlayer(
				par2World, par3EntityPlayer, false);

		if (movingobjectposition != null
				&& movingobjectposition.typeOfHit == EnumMovingObjectType.TILE) {
			int i = par2World.getBlockId(movingobjectposition.blockX,
		...
		if (!par2World.isRemote) {
			ChunkPosition chunkposition = par2World.findClosestStructure(
					"Stronghold", (int) par3EntityPlayer.posX,
					(int) par3EntityPlayer.posY, (int) par3EntityPlayer.posZ);

			if (chunkposition != null) {
				EntityEnderEye entityendereye = new EntityEnderEye(par2World,
						par3EntityPlayer.posX,
						(par3EntityPlayer.posY + 1.6200000000000001D)
								- (double) par3EntityPlayer.yOffset,
						par3EntityPlayer.posZ);
				entityendereye.func_40090_a(chunkposition.x, chunkposition.y,
						chunkposition.z);
				par2World.spawnEntityInWorld(entityendereye);
				par2World.playSoundAtEntity(par3EntityPlayer, "random.bow",
						0.5F, 0.4F / (itemRand.nextFloat() * 0.4F + 0.8F));
				par2World.playAuxSFXAtEntity(null, 1002,
