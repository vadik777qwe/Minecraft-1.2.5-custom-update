...
public class ItemBlock extends Item {
	...
	public boolean onItemUse(ItemStack par1ItemStack,
		...
		if (par3World
				.canBlockBePlacedAt(blockID, par4, par5, par6, false, par7)) {
			Block block = Block.blocksList[blockID];

			if (par3World.setBlockAndMetadataWithNotify(par4, par5, par6,
					blockID, getMetadata(par1ItemStack.getItemDamage()))) {
				if (par3World.getBlockId(par4, par5, par6) == blockID) {
					Block.blocksList[blockID].onBlockPlaced(par3World, par4,
							par5, par6, par7);
					Block.blocksList[blockID].onBlockPlacedBy(par3World, par4,
							par5, par6, par2EntityPlayer);
				}

				par3World.playSoundEffect((float) par4 + 0.5F,
						(float) par5 + 0.5F, (float) par6 + 0.5F,
