...
public class GuiVideoSettings extends GuiScreen {
	...
	public void initGui() {
		...
		for (int k = 0; k < j; k++) {
			...
			if (!enumoptions.getEnumFloat()) {
				controlList.add(new GuiSmallButton(enumoptions
						.returnEnumOrdinal(),
						(width / 2 - 155) + (i % 2) * 160, height / 6 + 24
								* (i >> 1), enumoptions, guiGameSettings
								.getKeyBinding(enumoptions)));
			} else {
				controlList.add(new GuiSlider(enumoptions.returnEnumOrdinal(),
						(width / 2 - 155) + (i % 2) * 160, height / 6 + 24
								* (i >> 1), enumoptions, guiGameSettings
								.getKeyBinding(enumoptions), guiGameSettings
		...
		is64bit = false;
		aobj = (new String[] { "sun.arch.data.model", "com.ibm.vm.bitmode",
				"os.arch" });
	...
	protected void actionPerformed(GuiButton par1GuiButton) {
		...
		if (par1GuiButton.id < 100 && (par1GuiButton instanceof GuiSmallButton)) {
			guiGameSettings.setOptionValue(
					((GuiSmallButton) par1GuiButton).returnEnumOptions(), 1);
			par1GuiButton.displayString = guiGameSettings
	...
	public void drawScreen(int par1, int par2, float par3) {
		...
		if (!is64bit && guiGameSettings.renderDistance == 0) {
			drawCenteredString(fontRenderer,
					StatCollector.translateToLocal("options.farWarning1"),
					width / 2, height / 6 + 144, 0xaf0000);
			drawCenteredString(fontRenderer,
					StatCollector.translateToLocal("options.farWarning2"),
					width / 2, height / 6 + 144 + 12, 0xaf0000);
		}
