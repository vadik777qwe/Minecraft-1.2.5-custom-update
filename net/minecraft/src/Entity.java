...
public abstract class Entity {
	...
	public final AxisAlignedBB boundingBox = AxisAlignedBB.getBoundingBox(0.0D,
	...
	public void setPosition(double par1, double par3, double par5) {
		...
		float f = width / 2.0F;
		float f1 = height;
		boundingBox.setBounds(par1 - (double) f, (par3 - (double) yOffset)
				+ (double) ySize, par5 - (double) f, par1 + (double) f,
				(par3 - (double) yOffset) + (double) ySize + (double) f1, par5
						+ (double) f);
	}
		...
		if (isSprinting() && !isInWater()) {
			...
			if (j1 > 0) {
				worldObj.spawnParticle(
						(new StringBuilder()).append("tilecrack_").append(j1)
								.toString(), posX
								+ ((double) rand.nextFloat() - 0.5D)
								* (double) width,
						boundingBox.minY + 0.10000000000000001D, posZ
								+ ((double) rand.nextFloat() - 0.5D)
								* (double) width, -motionX * 4D, 1.5D,
						-motionZ * 4D);
			}
		}
	 ...
	 */
	public boolean isOffsetPositionInLiquid(double par1, double par3,
			double par5) {
		AxisAlignedBB axisalignedbb = boundingBox.getOffsetBoundingBox(par1,
				par3, par5);
	...
	public void moveEntity(double par1, double par3, double par5) {
		...
		if (flag) {
			...
			for (; par1 != 0.0D
					&& worldObj.getCollidingBoundingBoxes(this,
							boundingBox.getOffsetBoundingBox(par1, -1D, 0.0D))
							.size() == 0; d2 = par1) {
				if (par1 < d5 && par1 >= -d5) {
					par1 = 0.0D;
					continue;
			...
			for (; par5 != 0.0D
					&& worldObj.getCollidingBoundingBoxes(this,
							boundingBox.getOffsetBoundingBox(0.0D, -1D, par5))
							.size() == 0; d4 = par5) {
				if (par5 < d5 && par5 >= -d5) {
					par5 = 0.0D;
					continue;
			...
			while (par1 != 0.0D
					&& par5 != 0.0D
					&& worldObj.getCollidingBoundingBoxes(this,
							boundingBox.getOffsetBoundingBox(par1, -1D, par5))
							.size() == 0) {
				if (par1 < d5 && par1 >= -d5) {
					par1 = 0.0D;
				} else if (par1 > 0.0D) {
		...
		for (int i = 0; i < list.size(); i++) {
			par3 = ((AxisAlignedBB) list.get(i)).calculateYOffset(boundingBox,
					par3);
		}
		...
		for (int j = 0; j < list.size(); j++) {
			par1 = ((AxisAlignedBB) list.get(j)).calculateXOffset(boundingBox,
					par1);
		}
		...
		for (int k = 0; k < list.size(); k++) {
			par5 = ((AxisAlignedBB) list.get(k)).calculateZOffset(boundingBox,
					par5);
		}
		...
		if (stepHeight > 0.0F && flag1 && (flag || ySize < 0.05F)
			...
			for (int j2 = 0; j2 < list1.size(); j2++) {
				par3 = ((AxisAlignedBB) list1.get(j2)).calculateYOffset(
						boundingBox, par3);
			}
			...
			for (int k2 = 0; k2 < list1.size(); k2++) {
				par1 = ((AxisAlignedBB) list1.get(k2)).calculateXOffset(
						boundingBox, par1);
			}
			...
			for (int l2 = 0; l2 < list1.size(); l2++) {
				par5 = ((AxisAlignedBB) list1.get(l2)).calculateZOffset(
						boundingBox, par5);
			}
			...
			if (!field_9293_aM && d3 != par3) {
				...
				for (int i3 = 0; i3 < list1.size(); i3++) {
					par3 = ((AxisAlignedBB) list1.get(i3)).calculateYOffset(
							boundingBox, par3);
				}
		...
		if (canTriggerWalking() && !flag && ridingEntity == null) {
			distanceWalkedModified += (double) MathHelper.sqrt_double(d7 * d7
					+ d9 * d9) * 0.59999999999999998D;
			int l = MathHelper.floor_double(posX);
			int j1 = MathHelper.floor_double(posY - 0.20000000298023224D
					- (double) yOffset);
			int l1 = MathHelper.floor_double(posZ);
			int j3 = worldObj.getBlockId(l, j1, l1);

			if (j3 == 0
					&& worldObj.getBlockId(l, j1 - 1, l1) == Block.fence.blockID) {
			...
			for (int j4 = i1; j4 <= k3; j4++) {
				for (int k4 = k1; k4 <= l3; k4++) {
					for (int l4 = i2; l4 <= i4; l4++) {
						...
						if (i5 > 0) {
							Block.blocksList[i5].onEntityCollidedWithBlock(
									worldObj, j4, k4, l4, this);
						}
					}
				}
			}
		...
		if (worldObj.isBoundingBoxBurning(boundingBox.contract(0.001D, 0.001D,
				0.001D))) {
			dealFireDamage(1);
		...
		if (flag2 && fire > 0) {
			worldObj.playSoundAtEntity(this, "random.fizz", 0.7F,
		...
		if (par3) {
			if (fallDistance > 0.0F) {
				if (this instanceof EntityLiving) {
					...
					if (l == 0
							&& worldObj.getBlockId(i, j - 1, k) == Block.fence.blockID) {
						l = worldObj.getBlockId(i, j - 1, k);
					}

					if (l > 0) {
						Block.blocksList[l].onFallenUpon(worldObj, i, j, k,
								this, fallDistance);
					}
				}
	...
	public boolean isInsideOfMaterial(Material par1Material) {
		...
		if (l != 0 && Block.blocksList[l].blockMaterial == par1Material) {
			float f = BlockFluid.getFluidHeightPercent(worldObj
					.getBlockMetadata(i, j, k)) - 0.1111111F;
	...
	public boolean handleLavaMovement() {
		return worldObj.isMaterialInBB(boundingBox.expand(
				-0.10000000149011612D, -0.40000000596046448D,
	...
	public void writeToNBT(NBTTagCompound par1NBTTagCompound) {
		par1NBTTagCompound.setTag("Pos", newDoubleNBTList(new double[] { posX,
				posY + (double) ySize, posZ }));
	...
	public EntityItem entityDropItem(ItemStack par1ItemStack, float par2) {
		EntityItem entityitem = new EntityItem(worldObj, posX, posY
				+ (double) par2, posZ, par1ItemStack);
	...
	public boolean isEntityInsideOpaqueBlock() {
		for (int i = 0; i < 8; i++) {
			...
			int k = MathHelper.floor_double(posY + (double) getEyeHeight()
					+ (double) f1);
	...
	public void updateRidden() {
		...
		entityRiderPitchDelta += ridingEntity.rotationPitch
				- ridingEntity.prevRotationPitch;
	...
	public void mountEntity(Entity par1Entity) {
		...
		if (ridingEntity == par1Entity) {
			ridingEntity.riddenByEntity = null;
			ridingEntity = null;
			setLocationAndAngles(par1Entity.posX, par1Entity.boundingBox.minY
					+ (double) par1Entity.height, par1Entity.posZ, rotationYaw,
					rotationPitch);
			return;
		}
	...
	/**
	 * Parameters: item slot, item ID, item damage. If slot >= 0 a new item will
	 * be generated with the specified item ID damage.
	 */
	...
	protected void setFlag(int par1, boolean par2) {
		...
		if (par2) {
			dataWatcher.updateObject(0,
					Byte.valueOf((byte) (byte0 | 1 << par1)));
		} else {
			dataWatcher.updateObject(0,
					Byte.valueOf((byte) (byte0 & ~(1 << par1))));
		}
