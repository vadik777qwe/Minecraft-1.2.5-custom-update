...
public class PathEntity {
	...
	public Vec3D getVectorFromIndex(Entity par1Entity, int par2) {
		double d = (double) points[par2].xCoord
				+ (double) (int) (par1Entity.width + 1.0F) * 0.5D;
		double d1 = points[par2].yCoord;
		double d2 = (double) points[par2].zCoord
				+ (double) (int) (par1Entity.width + 1.0F) * 0.5D;
	...
	public boolean func_48647_a(PathEntity par1PathEntity) {
		...
		for (int i = 0; i < points.length; i++) {
			if (points[i].xCoord != par1PathEntity.points[i].xCoord
					|| points[i].yCoord != par1PathEntity.points[i].yCoord
					|| points[i].zCoord != par1PathEntity.points[i].zCoord) {
				return false;
			}
		}
	...
	public boolean func_48639_a(Vec3D par1Vec3D) {
		...
		if (pathpoint == null) {
			...
			return pathpoint.xCoord == (int) par1Vec3D.xCoord
					&& pathpoint.zCoord == (int) par1Vec3D.zCoord;
		}
	}
}
