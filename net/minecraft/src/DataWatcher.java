...
public class DataWatcher {
	...
	public byte getWatchableObjectByte(int par1) {
		return ((Byte) ((WatchableObject) watchedObjects.get(Integer
				.valueOf(par1))).getObject()).byteValue();
	}

	public short getWatchableObjectShort(int par1) {
		return ((Short) ((WatchableObject) watchedObjects.get(Integer
				.valueOf(par1))).getObject()).shortValue();
	}
	...
	public int getWatchableObjectInt(int par1) {
		return ((Integer) ((WatchableObject) watchedObjects.get(Integer
				.valueOf(par1))).getObject()).intValue();
	}
	...
	public String getWatchableObjectString(int par1) {
		return (String) ((WatchableObject) watchedObjects.get(Integer
				.valueOf(par1))).getObject();
	}
	...
	public void updateObject(int par1, Object par2Obj) {
		WatchableObject watchableobject = (WatchableObject) watchedObjects
	...
	public static void writeObjectsInListToStream(List par0List,
		...
		if (par0List != null) {
			...
			for (Iterator iterator = par0List.iterator(); iterator.hasNext(); writeWatchableObject(
	...
	private static void writeWatchableObject(
			DataOutputStream par0DataOutputStream,
			WatchableObject par1WatchableObject) throws IOException {
		int i = (par1WatchableObject.getObjectType() << 5 | par1WatchableObject
				.getDataValueId() & 0x1f) & 0xff;
		par0DataOutputStream.writeByte(i);

		switch (par1WatchableObject.getObjectType()) {
		case 0:
			par0DataOutputStream.writeByte(((Byte) par1WatchableObject
					.getObject()).byteValue());
			break;

		case 1:
			par0DataOutputStream.writeShort(((Short) par1WatchableObject
					.getObject()).shortValue());
			break;

		case 2:
			par0DataOutputStream.writeInt(((Integer) par1WatchableObject
					.getObject()).intValue());
			break;

		case 3:
			par0DataOutputStream.writeFloat(((Float) par1WatchableObject
					.getObject()).floatValue());
			break;

		case 4:
			Packet.writeString((String) par1WatchableObject.getObject(),
		...
		case 6:
			ChunkCoordinates chunkcoordinates = (ChunkCoordinates) par1WatchableObject
					.getObject();
	...
	public static List readWatchableObjects(DataInputStream par0DataInputStream)
		...
		for (byte byte0 = par0DataInputStream.readByte(); byte0 != 127; byte0 = par0DataInputStream
				.readByte()) {
			if (arraylist == null) {
			...
			case 0:
				watchableobject = new WatchableObject(i, j,
						Byte.valueOf(par0DataInputStream.readByte()));
				break;

			case 1:
				watchableobject = new WatchableObject(i, j,
						Short.valueOf(par0DataInputStream.readShort()));
				break;

			case 2:
				watchableobject = new WatchableObject(i, j,
						Integer.valueOf(par0DataInputStream.readInt()));
				break;

			case 3:
				watchableobject = new WatchableObject(i, j,
						Float.valueOf(par0DataInputStream.readFloat()));
				break;

			case 4:
				watchableobject = new WatchableObject(i, j, Packet.readString(
						par0DataInputStream, 64));
				break;
			...
			case 5:
				...
				watchableobject = new WatchableObject(i, j, new ItemStack(
						word0, byte1, word1));
				break;
