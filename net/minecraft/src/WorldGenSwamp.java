	...
	public boolean generate(World par1World, Random par2Random, int par3,
			...
			for (int k1 = par3 - byte0; k1 <= par3 + byte0 && flag; k1++) {
				for (int k2 = par5 - byte0; k2 <= par5 + byte0 && flag; k2++) {
					if (j >= 0 && j < 128) {
						...
						if (j3 == Block.waterStill.blockID
								|| j3 == Block.waterMoving.blockID) {
							if (j > par4) {
								flag = false;
							}
						} else {
		...
		for (int l = (par4 - 3) + i; l <= par4 + i; l++) {
			...
			for (int k3 = par3 - l2; k3 <= par3 + l2; k3++) {
				...
				for (int k4 = par5 - l2; k4 <= par5 + l2; k4++) {
					...
					if ((Math.abs(i4) != l2 || Math.abs(l4) != l2 || par2Random
							.nextInt(2) != 0 && l1 != 0)
							&& !Block.opaqueCubeLookup[par1World.getBlockId(k3,
									l, k4)]) {
		...
		for (int i1 = 0; i1 < i; i1++) {
			...
			if (i2 == 0 || i2 == Block.leaves.blockID
					|| i2 == Block.waterMoving.blockID
					|| i2 == Block.waterStill.blockID) {
				func_50073_a(par1World, par3, par4 + i1, par5,
						Block.wood.blockID);
			}
		}
			...
			for (int l3 = par3 - i3; l3 <= par3 + i3; l3++) {
				for (int j4 = par5 - i3; j4 <= par5 + i3; j4++) {
					...
					if (par2Random.nextInt(4) == 0
							&& par1World.getBlockId(l3 - 1, j1, j4) == 0) {
					...
					if (par2Random.nextInt(4) == 0
							&& par1World.getBlockId(l3, j1, j4 - 1) == 0) {
