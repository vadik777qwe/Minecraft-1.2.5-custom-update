...
public abstract class StructureStart {
	...
	public void generateStructure(World par1World, Random par2Random,
		...
		do {
			...
			if (structurecomponent.getBoundingBox().intersectsWith(
					...
					&& !structurecomponent.addComponentParts(par1World,
							par2Random, par3StructureBoundingBox)) {
				iterator.remove();
			}
		} while (true);
	}
	...
	protected void updateBoundingBox() {
		...
		for (Iterator iterator = components.iterator(); iterator.hasNext(); boundingBox
				.expandTo(structurecomponent.getBoundingBox())) {
			structurecomponent = (StructureComponent) iterator.next();
		}
	}

	/**
	 * 'offsets the structure Bounding Boxes up to a certain height, typically
	 * 63 - 10'
	 */
	protected void markAvailableHeight(World par1World, Random par2Random,
		...
		for (Iterator iterator = components.iterator(); iterator.hasNext(); structurecomponent
				.getBoundingBox().offset(0, k, 0)) {
			structurecomponent = (StructureComponent) iterator.next();
		}
	}
