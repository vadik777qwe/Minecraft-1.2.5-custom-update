...
public class BlockRedstoneRepeater extends BlockDirectional {
	...
	public void updateTick(World par1World, int par2, int par3, int par4,
		...
		if (isRepeaterPowered && !flag) {
			par1World.setBlockAndMetadataWithNotify(par2, par3, par4,
					Block.redstoneRepeaterIdle.blockID, i);
		} else if (!isRepeaterPowered) {
			par1World.setBlockAndMetadataWithNotify(par2, par3, par4,
					Block.redstoneRepeaterActive.blockID, i);
	...
	public boolean isPoweringTo(IBlockAccess par1IBlockAccess, int par2,
		...
		int i = getDirection(par1IBlockAccess
	...
	private boolean ignoreTick(World par1World, int par2, int par3, int par4,
			int par5) {
		int i = getDirection(par5);

		switch (i) {
		case 0:
			return par1World.isBlockIndirectlyProvidingPowerTo(par2, par3,
					par4 + 1, 3)
					|| par1World.getBlockId(par2, par3, par4 + 1) == Block.redstoneWire.blockID
					&& par1World.getBlockMetadata(par2, par3, par4 + 1) > 0;
		...
		case 3:
			return par1World.isBlockIndirectlyProvidingPowerTo(par2 + 1, par3,
					par4, 5)
					|| par1World.getBlockId(par2 + 1, par3, par4) == Block.redstoneWire.blockID
					&& par1World.getBlockMetadata(par2 + 1, par3, par4) > 0;
	...
	public void randomDisplayTick(World par1World, int par2, int par3,
		...
		par1World.spawnParticle("reddust", d + d3, d1, d2 + d4, 0.0D, 0.0D,
				0.0D);
	}
}
