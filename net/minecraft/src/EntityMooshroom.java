...
public class EntityMooshroom extends EntityCow {
	...
	public boolean interact(EntityPlayer par1EntityPlayer) {
		...
		if (itemstack != null
				&& itemstack.itemID == Item.bowlEmpty.shiftedIndex
				&& getGrowingAge() >= 0) {
			...
			if (par1EntityPlayer.inventory
					.addItemStackToInventory(new ItemStack(Item.bowlSoup))
					&& !par1EntityPlayer.capabilities.isCreativeMode) {
				par1EntityPlayer.inventory.decrStackSize(
						par1EntityPlayer.inventory.currentItem, 1);
				return true;
			}
		}
		...
		if (itemstack != null && itemstack.itemID == Item.shears.shiftedIndex
			...
			worldObj.spawnParticle("largeexplode", posX, posY
					+ (double) (height / 2.0F), posZ, 0.0D, 0.0D, 0.0D);

			if (!worldObj.isRemote) {
				EntityCow entitycow = new EntityCow(worldObj);
				entitycow.setLocationAndAngles(posX, posY, posZ, rotationYaw,
						rotationPitch);
				...
				for (int i = 0; i < 5; i++) {
					worldObj.spawnEntityInWorld(new EntityItem(worldObj, posX,
							posY + (double) height, posZ, new ItemStack(
									Block.mushroomRed)));
				}
			}
