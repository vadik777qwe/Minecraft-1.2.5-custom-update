...
public class ThreadDownloadResources extends Thread {
		...
		try {
			...
			for (int i = 0; i < 2; i++) {
				for (int j = 0; j < nodelist.getLength(); j++) {
					...
					String s = ((Element) element.getElementsByTagName("Key")
							.item(0)).getChildNodes().item(0).getNodeValue();
					long l = Long.parseLong(((Element) element
	...
	private void loadResource(File par1File, String par2Str) {
		...
		for (int i = 0; i < afile.length; i++) {
			if (afile[i].isDirectory()) {
				loadResource(afile[i], (new StringBuilder()).append(par2Str)
						.append(afile[i].getName()).append("/").toString());
				continue;
			}

			try {
				mc.installResource((new StringBuilder()).append(par2Str)
						.append(afile[i].getName()).toString(), afile[i]);
			} catch (Exception exception) {
				System.out.println((new StringBuilder())
						.append("Failed to add ").append(par2Str)
	 ...
	 */
	private void downloadAndInstallResource(URL par1URL, String par2Str,
			long par3, int par5) {
		try {
	 ...
	 */
	private void downloadResource(URL par1URL, File par2File, long par3)
			throws IOException {
