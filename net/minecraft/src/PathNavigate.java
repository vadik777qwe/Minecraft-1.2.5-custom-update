...
public class PathNavigate {
	...
	public PathEntity getPathToXYZ(double par1, double par3, double par5) {
		if (!canNavigate()) {
			...
			return worldObj.getEntityPathToXYZ(theEntity,
					MathHelper.floor_double(par1), (int) par3,
					MathHelper.floor_double(par5), pathSearchRange,
					canPassOpenWoodenDoors, canPassClosedWoodenDoors,
					avoidsWater, canSwim);
		}
	}

	public boolean func_48666_a(double par1, double par3, double par5,
			float par7) {
		PathEntity pathentity = getPathToXYZ(MathHelper.floor_double(par1),
				(int) par3, MathHelper.floor_double(par5));
	...
	public PathEntity func_48679_a(EntityLiving par1EntityLiving) {
		if (!canNavigate()) {
			...
			return worldObj.getPathEntityToEntity(theEntity, par1EntityLiving,
					pathSearchRange, canPassOpenWoodenDoors,
	...
	public void onUpdateNavigation() {
		...
		if (vec3d == null) {
			...
			theEntity.getMoveHelper().setMoveTo(vec3d.xCoord, vec3d.yCoord,
					vec3d.zCoord, speed);
			return;
		}
	}
	...
	private void pathFollow() {
		...
		for (int j = currentPath.getCurrentPathIndex(); j < i; j++) {
			if (vec3d.squareDistanceTo(currentPath.getVectorFromIndex(
					theEntity, j)) < (double) f) {
		...
		do {
			...
			if (isDirectPathBetweenPoints(vec3d,
					currentPath.getVectorFromIndex(theEntity, j1), k, l, i1)) {
	...
	private Vec3D getEntityPosition() {
		return Vec3D.createVector(theEntity.posX, getPathableYPos(),
				theEntity.posZ);
	}
	...
	private int getPathableYPos() {
		...
		int j = worldObj.getBlockId(MathHelper.floor_double(theEntity.posX), i,
				MathHelper.floor_double(theEntity.posZ));
		int k = 0;

		while (j == Block.waterMoving.blockID || j == Block.waterStill.blockID) {
			i++;
			j = worldObj.getBlockId(MathHelper.floor_double(theEntity.posX), i,
					MathHelper.floor_double(theEntity.posZ));
	...
	private void removeSunnyPath() {
		...
		for (int i = 0; i < currentPath.getCurrentPathLength(); i++) {
			...
			if (worldObj.canBlockSeeTheSky(pathpoint.xCoord, pathpoint.yCoord,
					pathpoint.zCoord)) {
	 ...
	 */
	private boolean isDirectPathBetweenPoints(Vec3D par1Vec3D, Vec3D par2Vec3D,
		...
		if (!isSafeToStandAt(i, (int) par1Vec3D.yCoord, j, par3, par4, par5,
				par1Vec3D, d, d1)) {
			return false;
		}
		...
		for (int j1 = l - j; i1 * byte0 > 0 || j1 * byte1 > 0;) {
			...
			if (!isSafeToStandAt(i, (int) par1Vec3D.yCoord, j, par3, par4,
					par5, par1Vec3D, d, d1)) {
				return false;
			}
		}
	...
	private boolean isSafeToStandAt(int par1, int par2, int par3, int par4,
			int par5, int par6, Vec3D par7Vec3D, double par8, double par10) {
		int i = par1 - par4 / 2;
		int j = par3 - par6 / 2;

		if (!isPositionClear(i, par2, j, par4, par5, par6, par7Vec3D, par8,
				par10)) {
			return false;
		}
		...
		for (int i = par1; i < par1 + par4; i++) {
			for (int j = par2; j < par2 + par5; j++) {
				for (int k = par3; k < par3 + par6; k++) {
					...
					if (l > 0
							&& !Block.blocksList[l].getBlocksMovement(worldObj,
									i, j, k)) {
						return false;
					}
				}
