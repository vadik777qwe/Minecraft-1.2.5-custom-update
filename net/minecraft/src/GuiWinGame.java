...
public class GuiWinGame extends GuiScreen {
	...
	public void updateScreen() {
		updateCounter++;
		float f = (float) (field_41042_d + height + height + 24)
				/ field_41043_e;
	...
	private void respawnPlayer() {
		if (mc.theWorld.isRemote) {
			...
			entityclientplayermp.sendQueue.addToSendQueue(new Packet9Respawn(
					entityclientplayermp.dimension,
					(byte) mc.theWorld.difficultySetting, mc.theWorld
							.getWorldInfo().getTerrainType(), mc.theWorld
							.getHeight(), 0));
		} else {
	...
	public void initGui() {
		...
		try {
			...
			BufferedReader bufferedreader = new BufferedReader(
					new InputStreamReader((net.minecraft.src.GuiWinGame.class)
							.getResourceAsStream("/title/win.txt"), Charset
							.forName("UTF-8")));
			...
			while ((s = bufferedreader.readLine()) != null) {
				...
				for (s = s.replaceAll("PLAYERNAME", mc.session.username); s
						.indexOf(s1) >= 0; s = (new StringBuilder()).append(s2)
						.append("\247f\247k")
						.append("XXXXXXXX".substring(0, random.nextInt(4) + 3))
						.append(s3).toString()) {
					int i = s.indexOf(s1);
					s2 = s.substring(0, i);
			...
			bufferedreader = new BufferedReader(
					new InputStreamReader((net.minecraft.src.GuiWinGame.class)
							.getResourceAsStream("/title/credits.txt"), Charset
							.forName("UTF-8")));
	...
	private void func_41040_b(int par1, int par2, float par3) {
		...
		GL11.glBindTexture(GL11.GL_TEXTURE_2D,
				mc.renderEngine.getTexture("%blur%/gui/background.png"));
		tessellator.startDrawingQuads();
		tessellator.setColorRGBA_F(1.0F, 1.0F, 1.0F, 1.0F);
		int i = width;
		float f = 0.0F - ((float) updateCounter + par3) * 0.5F * field_41043_e;
		float f1 = (float) height - ((float) updateCounter + par3) * 0.5F
				* field_41043_e;
		float f2 = 0.015625F;
		float f3 = (((float) updateCounter + par3) - 0.0F) * 0.02F;
		float f4 = (float) (field_41042_d + height + height + 24)
				/ field_41043_e;
	...
	public void drawScreen(int par1, int par2, float par3) {
		...
		for (int l = 0; l < lines.size(); l++) {
			...
			if ((float) k + f + 12F + 8F > 0.0F
					&& (float) k + f < (float) height) {
				String s = (String) lines.get(l);

				if (s.startsWith("[C]")) {
					fontRenderer.drawStringWithShadow(s.substring(3), i
							+ (c - fontRenderer.getStringWidth(s.substring(3)))
							/ 2, k, 0xffffff);
				} else {
					fontRenderer.fontRandom.setSeed((long) l * 0xfca99533L
							+ (long) (updateCounter / 4));
