...
public class WorldGenBigMushroom extends WorldGenerator {
	...
	public boolean generate(World par1World, Random par2Random, int par3,
		...
		setBlockAndMetadata(par1World, par3, par4 - 1, par5,
		...
		for (int k1 = i1; k1 <= par4 + j; k1++) {
			...
			for (int i3 = par3 - j2; i3 <= par3 + j2; i3++) {
				for (int j3 = par5 - j2; j3 <= par5 + j2; j3++) {
					...
					if (i == 0 || k1 < par4 + j) {
						if ((i3 == par3 - j2 || i3 == par3 + j2)
								&& (j3 == par5 - j2 || j3 == par5 + j2)) {
							continue;
						}
					...
					if ((k3 != 0 || par4 >= (par4 + j) - 1)
							&& !Block.opaqueCubeLookup[par1World.getBlockId(i3,
									k1, j3)]) {
						setBlockAndMetadata(par1World, i3, k1, j3,
								Block.mushroomCapBrown.blockID + i, k3);
					}
				}
			}
		}
		...
		for (int l1 = 0; l1 < j; l1++) {
			...
			if (!Block.opaqueCubeLookup[k2]) {
				setBlockAndMetadata(par1World, par3, par4 + l1, par5,
						Block.mushroomCapBrown.blockID + i, 10);
			}
		}
