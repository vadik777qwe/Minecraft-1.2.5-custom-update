...
public class ChunkProviderEnd implements IChunkProvider {
	...
	public void func_40380_a(int par1, int par2, byte par3ArrayOfByte[],
			BiomeGenBase par4ArrayOfBiomeGenBase[]) {
		byte byte0 = 2;
		int i = byte0 + 1;
		byte byte1 = 33;
		int j = byte0 + 1;
		densities = func_40379_a(densities, par1 * byte0, 0, par2 * byte0, i,
				byte1, j);
		...
		for (int k = 0; k < byte0; k++) {
			for (int l = 0; l < byte0; l++) {
				for (int i1 = 0; i1 < 32; i1++) {
					double d = 0.25D;
					double d1 = densities[((k + 0) * j + (l + 0)) * byte1
							+ (i1 + 0)];
					double d2 = densities[((k + 0) * j + (l + 1)) * byte1
							+ (i1 + 0)];
					double d3 = densities[((k + 1) * j + (l + 0)) * byte1
							+ (i1 + 0)];
					double d4 = densities[((k + 1) * j + (l + 1)) * byte1
							+ (i1 + 0)];
					double d5 = (densities[((k + 0) * j + (l + 0)) * byte1
							+ (i1 + 1)] - d1)
							* d;
					double d6 = (densities[((k + 0) * j + (l + 1)) * byte1
							+ (i1 + 1)] - d2)
							* d;
					double d7 = (densities[((k + 1) * j + (l + 0)) * byte1
							+ (i1 + 1)] - d3)
							* d;
					double d8 = (densities[((k + 1) * j + (l + 1)) * byte1
							+ (i1 + 1)] - d4)
							* d;
	...
	public void func_40381_b(int par1, int par2, byte par3ArrayOfByte[],
			BiomeGenBase par4ArrayOfBiomeGenBase[]) {
		for (int i = 0; i < 16; i++) {
			for (int j = 0; j < 16; j++) {
				int k = 1;
	...
	public Chunk provideChunk(int par1, int par2) {
		endRNG.setSeed((long) par1 * 0x4f9939f508L + (long) par2
				* 0x1ef1565bd5L);
	...
	private double[] func_40379_a(double par1ArrayOfDouble[], int par2,
	...
	public void populate(IChunkProvider par1IChunkProvider, int par2, int par3) {
		...
		int i = par2 * 16;
		int j = par3 * 16;
		BiomeGenBase biomegenbase = endWorld.getBiomeGenForCoords(i + 16,
				j + 16);
