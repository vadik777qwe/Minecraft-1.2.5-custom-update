...
public class ItemSlab extends ItemBlock {
	...
	public String getItemNameIS(ItemStack par1ItemStack) {
		...
		return (new StringBuilder()).append(super.getItemName()).append(".")
				.append(BlockStep.blockStepTypes[i]).toString();
	}
	...
	public boolean onItemUse(ItemStack par1ItemStack,
		...
		if ((par7 == 1 && !flag || par7 == 0 && flag)
				...
				&& k == par1ItemStack.getItemDamage()) {
			if (par3World.checkIfAABBIsClear(Block.stairDouble
					.getCollisionBoundingBoxFromPool(par3World, par4, par5,
							par6))
					&& par3World.setBlockAndMetadataWithNotify(par4, par5,
							par6, Block.stairDouble.blockID, k)) {
				par3World
						.playSoundEffect(
								(float) par4 + 0.5F,
								(float) par5 + 0.5F,
		...
		if (func_50087_b(par1ItemStack, par2EntityPlayer, par3World, par4,
				par5, par6, par7)) {
			return true;
		} else {
	...
	private static boolean func_50087_b(ItemStack par0ItemStack,
			EntityPlayer par1EntityPlayer, World par2World, int par3, int par4,
			int par5, int par6) {
		if (par6 == 0) {
			par4--;
		}
		...
		if (i == Block.stairSingle.blockID
				&& k == par0ItemStack.getItemDamage()) {
			if (par2World.checkIfAABBIsClear(Block.stairDouble
					.getCollisionBoundingBoxFromPool(par2World, par3, par4,
							par5))
					&& par2World.setBlockAndMetadataWithNotify(par3, par4,
							par5, Block.stairDouble.blockID, k)) {
				par2World
						.playSoundEffect(
								(float) par3 + 0.5F,
								(float) par4 + 0.5F,
