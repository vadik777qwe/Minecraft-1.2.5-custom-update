...
public class Explosion {
	...
	public Explosion(World par1World, Entity par2Entity, double par3,
	...
	public void doExplosionA() {
		...
		explosionSize *= 2.0F;
		int k = MathHelper.floor_double(explosionX - (double) explosionSize
				- 1.0D);
		int i1 = MathHelper.floor_double(explosionX + (double) explosionSize
				+ 1.0D);
		int k1 = MathHelper.floor_double(explosionY - (double) explosionSize
				- 1.0D);
		int l1 = MathHelper.floor_double(explosionY + (double) explosionSize
				+ 1.0D);
		int i2 = MathHelper.floor_double(explosionZ - (double) explosionSize
				- 1.0D);
		int j2 = MathHelper.floor_double(explosionZ + (double) explosionSize
				+ 1.0D);
		...
		for (int k2 = 0; k2 < list.size(); k2++) {
			...
			if (d4 <= 1.0D) {
				...
				double d11 = MathHelper.sqrt_double(d6 * d6 + d8 * d8 + d10
						* d10);
				d6 /= d11;
				d8 /= d11;
				d10 /= d11;
				double d12 = worldObj
						.getBlockDensity(vec3d, entity.boundingBox);
				double d13 = (1.0D - d4) * d12;
				entity.attackEntityFrom(DamageSource.explosion, (int) (((d13
						* d13 + d13) / 2D)
						* 8D * (double) explosionSize + 1.0D));
				double d14 = d13;
	...
	public void doExplosionB(boolean par1) {
		...
		for (int i = arraylist.size() - 1; i >= 0; i--) {
			...
			if (par1) {
				...
				d7 *= worldObj.rand.nextFloat() * worldObj.rand.nextFloat()
						+ 0.3F;
				d3 *= d7;
				d4 *= d7;
				d5 *= d7;
			...
			if (i2 > 0) {
				Block.blocksList[i2].dropBlockAsItemWithChance(worldObj, k, i1,
						k1, worldObj.getBlockMetadata(k, i1, k1), 0.3F, 0);
		...
		if (isFlaming) {
			for (int j = arraylist.size() - 1; j >= 0; j--) {
				...
				if (j2 == 0 && Block.opaqueCubeLookup[k2]
