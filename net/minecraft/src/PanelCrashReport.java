...
public class PanelCrashReport extends Panel {
	public PanelCrashReport(UnexpectedThrowable par1UnexpectedThrowable) {
		...
		par1UnexpectedThrowable.exception.printStackTrace(new PrintWriter(
				stringwriter));
		...
		try {
			...
			s2 = (new StringBuilder()).append(s2).append("VM: ")
					...
					.toString();
			s2 = (new StringBuilder()).append(s2).append("LWJGL: ")
					.append(Sys.getVersion()).append("\n").toString();
			s1 = GL11.glGetString(GL11.GL_VENDOR);
			s2 = (new StringBuilder()).append(s2).append("OpenGL: ")
					.append(GL11.glGetString(GL11.GL_RENDERER))
					.append(" version ")
					.append(GL11.glGetString(GL11.GL_VERSION)).append(", ")
					.append(GL11.glGetString(GL11.GL_VENDOR)).append("\n")
					.toString();
		} catch (Throwable throwable) {
			s2 = (new StringBuilder()).append(s2)
					.append("[failed to get system properties (")
		...
		if (s.contains("Pixel format not accelerated")) {
			...
			if (s1.toLowerCase().contains("nvidia")) {
				...
				s3 = (new StringBuilder())
						.append(s3)
						.append("You might be able to find drivers for your video card here:\n")
						.toString();
				s3 = (new StringBuilder()).append(s3)
						.append("  http://www.nvidia.com/\n").toString();
				...
				s3 = (new StringBuilder())
						...
						.toString();
				s3 = (new StringBuilder()).append(s3)
						.append("  http://www.amd.com/\n").toString();
			}
		} else {
			s3 = (new StringBuilder()).append(s3)
					.append("      Minecraft has crashed!      \n").toString();
			s3 = (new StringBuilder()).append(s3)
					.append("      ----------------------      \n").toString();
			s3 = (new StringBuilder()).append(s3).append("\n").toString();
			s3 = (new StringBuilder())
		...
		s3 = (new StringBuilder()).append(s3).append("\n").toString();
		s3 = (new StringBuilder()).append(s3).append("\n").toString();
		s3 = (new StringBuilder()).append(s3).append("--- BEGIN ERROR REPORT ")
				.append(Integer.toHexString(s3.hashCode()))
				.append(" --------\n").toString();
		s3 = (new StringBuilder()).append(s3).append(s2).toString();
		s3 = (new StringBuilder()).append(s3).append("--- END ERROR REPORT ")
				.append(Integer.toHexString(s3.hashCode()))
				.append(" ----------\n").toString();
		s3 = (new StringBuilder()).append(s3).append("\n").toString();
		s3 = (new StringBuilder()).append(s3).append("\n").toString();
