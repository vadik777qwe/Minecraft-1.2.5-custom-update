...
public class RenderLiving extends Render {
	...
	public void doRenderLiving(EntityLiving par1EntityLiving, double par2,
		...
		try {
			float f = func_48418_a(par1EntityLiving.prevRenderYawOffset,
					par1EntityLiving.renderYawOffset, par9);
			float f1 = func_48418_a(par1EntityLiving.prevRotationYawHead,
					par1EntityLiving.rotationYawHead, par9);
			float f2 = par1EntityLiving.prevRotationPitch
			...
			float f5 = par1EntityLiving.field_705_Q
					+ (par1EntityLiving.field_704_R - par1EntityLiving.field_705_Q)
					* par9;
			float f6 = par1EntityLiving.field_703_S
					- par1EntityLiving.field_704_R * (1.0F - par9);
			...
			for (int i = 0; i < 4; i++) {
				...
				renderPassModel.setLivingAnimations(par1EntityLiving, f6, f5,
						par9);
				renderPassModel.render(par1EntityLiving, f6, f5, f3, f1 - f,
						f2, f4);
				...
				if (j == 15) {
					...
					for (int i1 = 0; i1 < 2; i1++) {
						...
						renderPassModel.render(par1EntityLiving, f6, f5, f3, f1
								- f, f2, f4);
					}
			...
			if ((k >> 24 & 0xff) > 0 || par1EntityLiving.hurtTime > 0
				...
				if (par1EntityLiving.hurtTime > 0
					...
					mainModel.render(par1EntityLiving, f6, f5, f3, f1 - f, f2,
							f4);

					for (int l = 0; l < 4; l++) {
						if (inheritRenderPass(par1EntityLiving, l, par9) >= 0) {
							GL11.glColor4f(f7, 0.0F, 0.0F, 0.4F);
							renderPassModel.render(par1EntityLiving, f6, f5,
									f3, f1 - f, f2, f4);
						}
					}
				}
	...
	protected void renderModel(EntityLiving par1EntityLiving, float par2,
		...
		loadDownloadableImageTexture(par1EntityLiving.skinUrl,
				par1EntityLiving.getTexture());
	...
	protected void rotateCorpse(EntityLiving par1EntityLiving, float par2,
		...
		if (par1EntityLiving.deathTime > 0) {
			...
			GL11.glRotatef(f * getDeathMaxRotation(par1EntityLiving), 0.0F,
					0.0F, 1.0F);
		}
	}

	protected float renderSwingProgress(EntityLiving par1EntityLiving,
			float par2) {
	 ...
	 */
	protected float handleRotationFloat(EntityLiving par1EntityLiving,
			float par2) {
	...
	protected int inheritRenderPass(EntityLiving par1EntityLiving, int par2,
			float par3) {
