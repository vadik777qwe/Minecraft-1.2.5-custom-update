...
public class TerrainTextureManager {
	...
	public TerrainTextureManager() {
		...
		try {
			BufferedImage bufferedimage = ImageIO
					.read((net.minecraft.src.TerrainTextureManager.class)
							.getResource("/terrain.png"));
		...
		for (int i = 0; i < 4096; i++) {
			if (Block.blocksList[i] != null) {
				textures[i * 3 + 0] = Block.blocksList[i]
						.getBlockTextureFromSide(1);
				textures[i * 3 + 1] = Block.blocksList[i]
						.getBlockTextureFromSide(2);
				textures[i * 3 + 2] = Block.blocksList[i]
	...
	public void render(IsoImageBuffer par1IsoImageBuffer) {
		...
		int k = i + 16;
		int l = j + 16;
		Chunk chunk = world.getChunkFromChunkCoords(par1IsoImageBuffer.x,
				par1IsoImageBuffer.y);
			...
			for (int j1 = k - 1; j1 >= i; j1--) {
				...
				for (int j2 = 0; j2 < 128; j2++) {
					...
					if (block.blockMaterial == Material.water) {
						...
						if (l2 != 0
								&& Block.blocksList[l2].blockMaterial == Material.water) {
							continue;
						}

						float f1 = ((float) j2 / 127F) * 0.6F + 0.4F;
						float f2 = world.getLightBrightness(j1, j2 + 1, i1)
								* f1;
					...
					if (k2 >= 0 && k2 < 544) {
						...
						float f3 = (world.getLightBrightness(j1, j2 + 1, i1) * 0.8F + 0.2F)
								* f;
						int j4 = k3;
						...
						if (i2 >= 0) {
							...
							if (zBuf[i3] <= j2) {
								zBuf[i3] = j2;
								pixels[i3] = 0xff000000
										| (int) (texCols[j4 * 3 + 0] * f5) << 16
										| (int) (texCols[j4 * 3 + 1] * f5) << 8
										| (int) (texCols[j4 * 3 + 2] * f5);
							}
						}
						...
						if (i2 < 31) {
							...
							if (zBuf[i3 + 1] <= j2) {
								zBuf[i3 + 1] = j2;
								pixels[i3 + 1] = 0xff000000
										| (int) (texCols[j4 * 3 + 0] * f6) << 16
										| (int) (texCols[j4 * 3 + 1] * f6) << 8
										| (int) (texCols[j4 * 3 + 2] * f6);
							}
						}
					}
					...
					if (i2 >= 0) {
						...
						if (zBuf[j3] <= j2 - 1) {
							zBuf[j3] = j2 - 1;
							pixels[j3] = 0xff000000
									| (int) (texCols[l3 * 3 + 0] * f8) << 16
									| (int) (texCols[l3 * 3 + 1] * f8) << 8
									| (int) (texCols[l3 * 3 + 2] * f8);
						}
					}
					...
					if (zBuf[j3 + 1] <= j2 - 1) {
						zBuf[j3 + 1] = j2 - 1;
						pixels[j3 + 1] = 0xff000000
								| (int) (texCols[k4 * 3 + 0] * f9) << 16
								| (int) (texCols[k4 * 3 + 1] * f9) << 8
								| (int) (texCols[k4 * 3 + 2] * f9);
					}
				}
			}
		}
