...
public class RailLogic {
	...
	/**
	 * A boolean value that is true if the rail is powered, and false if its
	 * not.
	 */
	...
	public RailLogic(BlockRail par1BlockRail, World par2World, int par3,
			int par4, int par5) {
		rail = par1BlockRail;
	...
	private void setConnections(int par1) {
		...
		if (par1 == 0) {
			...
			connectedTracks.add(new ChunkPosition(trackX - 1, trackY, trackZ));
			connectedTracks.add(new ChunkPosition(trackX + 1, trackY + 1,
					trackZ));
		} else if (par1 == 3) {
			connectedTracks.add(new ChunkPosition(trackX - 1, trackY + 1,
					trackZ));
			...
			connectedTracks.add(new ChunkPosition(trackX, trackY + 1,
					trackZ - 1));
			connectedTracks.add(new ChunkPosition(trackX, trackY, trackZ + 1));
		} else if (par1 == 5) {
			connectedTracks.add(new ChunkPosition(trackX, trackY, trackZ - 1));
	...
	private void refreshConnectedTracks() {
		for (int i = 0; i < connectedTracks.size(); i++) {
			RailLogic raillogic = getMinecartTrackLogic((ChunkPosition) connectedTracks
					.get(i));

			if (raillogic == null || !raillogic.isConnectedTo(this)) {
				connectedTracks.remove(i--);
			} else {
				connectedTracks.set(i, new ChunkPosition(raillogic.trackX,
						raillogic.trackY, raillogic.trackZ));
			}
		}
	}
	...
	private RailLogic getMinecartTrackLogic(ChunkPosition par1ChunkPosition) {
		if (BlockRail.isRailBlockAt(worldObj, par1ChunkPosition.x,
				par1ChunkPosition.y, par1ChunkPosition.z)) {
			return new RailLogic(rail, worldObj, par1ChunkPosition.x,
					par1ChunkPosition.y, par1ChunkPosition.z);
		}
	...
	private boolean isConnectedTo(RailLogic par1RailLogic) {
		for (int i = 0; i < connectedTracks.size(); i++) {
			ChunkPosition chunkposition = (ChunkPosition) connectedTracks
					.get(i);

			if (chunkposition.x == par1RailLogic.trackX
					&& chunkposition.z == par1RailLogic.trackZ) {
				return true;
			}
		}
	...
	private boolean canConnectTo(RailLogic par1RailLogic) {
		...
		return par1RailLogic.trackY != trackY || chunkposition.y != trackY ? true
				: true;
	}
	...
	private void connectToNeighbor(RailLogic par1RailLogic) {
		connectedTracks.add(new ChunkPosition(par1RailLogic.trackX,
				par1RailLogic.trackY, par1RailLogic.trackZ));
		...
		if (byte0 == 0) {
			if (BlockRail.isRailBlockAt(worldObj, trackX, trackY + 1,
					trackZ - 1)) {
				byte0 = 4;
			}
		...
		if (byte0 == 1) {
			if (BlockRail.isRailBlockAt(worldObj, trackX + 1, trackY + 1,
					trackZ)) {
				byte0 = 2;
			}
	...
	private boolean canConnectFrom(int par1, int par2, int par3) {
		RailLogic raillogic = getMinecartTrackLogic(new ChunkPosition(par1,
				par2, par3));
