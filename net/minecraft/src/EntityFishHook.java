...
public class EntityFishHook extends Entity {
	...
	public EntityFishHook(World par1World, EntityPlayer par2EntityPlayer) {
		...
		setSize(0.25F, 0.25F);
		setLocationAndAngles(par2EntityPlayer.posX,
				(par2EntityPlayer.posY + 1.6200000000000001D)
						- (double) par2EntityPlayer.yOffset,
				par2EntityPlayer.posZ, par2EntityPlayer.rotationYaw,
		...
		motionX = -MathHelper.sin((rotationYaw / 180F) * (float) Math.PI)
				* MathHelper.cos((rotationPitch / 180F) * (float) Math.PI) * f;
	...
	public void calculateVelocity(double par1, double par3, double par5,
			float par7, float par8) {
		float f = MathHelper.sqrt_double(par1 * par1 + par3 * par3 + par5
				* par5);
		par1 /= f;
		par3 /= f;
		par5 /= f;
	...
	public void onUpdate() {
		...
		if (fishPosRotationIncrements > 0) {
			double d = posX + (fishX - posX)
					/ (double) fishPosRotationIncrements;
			double d1 = posY + (fishY - posY)
					/ (double) fishPosRotationIncrements;
			double d2 = posZ + (fishZ - posZ)
					/ (double) fishPosRotationIncrements;
			double d4;
			...
			rotationPitch += (fishPitch - (double) rotationPitch)
					/ (double) fishPosRotationIncrements;
		...
		if (!worldObj.isRemote) {
			...
			if (bobber != null) {
				if (bobber.isDead) {
					...
					posX = bobber.posX;
					posY = bobber.boundingBox.minY + (double) bobber.height
		...
		Vec3D vec3d1 = Vec3D.createVector(posX + motionX, posY + motionY, posZ
				+ motionZ);
		...
		for (int j = 0; j < list.size(); j++) {
			...
			if (!entity1.canBeCollidedWith() || entity1 == angler
					&& ticksInAir < 5) {
				continue;
			}
		...
		for (int l = 0; l < k; l++) {
			double d8 = ((boundingBox.minY + ((boundingBox.maxY - boundingBox.minY) * (double) (l + 0))
					/ (double) k) - 0.125D) + 0.125D;
			double d9 = ((boundingBox.minY + ((boundingBox.maxY - boundingBox.minY) * (double) (l + 1))
					/ (double) k) - 0.125D) + 0.125D;
			AxisAlignedBB axisalignedbb1 = AxisAlignedBB
					.getBoundingBoxFromPool(boundingBox.minX, d8,
			...
			if (ticksCatchable > 0) {
				...
				if (rand.nextInt(c) == 0) {
					...
					for (int i1 = 0; (float) i1 < 1.0F + width * 20F; i1++) {
						...
						worldObj.spawnParticle("bubble", posX + (double) f4,
								f3 + 1.0F, posZ + (double) f6, motionX, motionY
					...
					for (int j1 = 0; (float) j1 < 1.0F + width * 20F; j1++) {
						...
						worldObj.spawnParticle("splash", posX + (double) f5,
								f3 + 1.0F, posZ + (double) f7, motionX,
								motionY, motionZ);
					}
				}
			}
		...
		if (ticksCatchable > 0) {
			motionY -= (double) (rand.nextFloat() * rand.nextFloat() * rand
					.nextFloat()) * 0.20000000000000001D;
		}
	...
	public int catchFish() {
		...
		if (bobber != null) {
			...
			bobber.motionX += d * d8;
			bobber.motionY += d2 * d8 + (double) MathHelper.sqrt_double(d6)
			...
			entityitem.motionY = d3 * d9 + (double) MathHelper.sqrt_double(d7)
					* 0.080000000000000002D;
