	...
	public void func_6473_b(EntityPlayer par1EntityPlayer) {
		...
		for (int i = 0; i < 9; i++) {
			if (par1EntityPlayer.inventory.mainInventory[i] == null) {
				par1EntityPlayer.inventory.mainInventory[i] = new ItemStack(
						(Block) Session.registeredBlocksList.get(i));
			}
		}
	}

	/**
	 * Called from a PlayerController when the player is hitting a block with an
	 * item in Creative mode. Args: Minecraft instance, player controller, x, y,
	 * z, side
	 */
	public static void clickBlockCreative(Minecraft par0Minecraft,
			PlayerController par1PlayerController, int par2, int par3,
			int par4, int par5) {
		if (!par0Minecraft.theWorld.func_48457_a(par0Minecraft.thePlayer, par2,
				par3, par4, par5)) {
