...
public class ItemPotion extends Item {
	...
	public ItemStack onFoodEaten(ItemStack par1ItemStack, World par2World,
		...
		if (!par2World.isRemote) {
			...
			if (list != null) {
				...
				for (Iterator iterator = list.iterator(); iterator.hasNext(); par3EntityPlayer
		...
		if (isSplash(par1ItemStack.getItemDamage())) {
			...
			if (!par2World.isRemote) {
				par2World.spawnEntityInWorld(new EntityPotion(par2World,
						par3EntityPlayer, par1ItemStack.getItemDamage()));
			}
	...
	public String getItemDisplayName(ItemStack par1ItemStack) {
		...
		if (isSplash(par1ItemStack.getItemDamage())) {
			s = (new StringBuilder())
					.append(StatCollector.translateToLocal(
							"potion.prefix.grenade").trim()).append(" ")
					.toString();
		}
		...
		if (list != null && !list.isEmpty()) {
			...
			return (new StringBuilder()).append(s)
					.append(StatCollector.translateToLocal(s1).trim())
					.toString();
		} else {
			String s2 = PotionHelper
					.func_40359_b(par1ItemStack.getItemDamage());
			return (new StringBuilder())
					.append(StatCollector.translateToLocal(s2).trim())
					.append(" ")
					.append(super.getItemDisplayName(par1ItemStack)).toString();
		}
	}
		...
		if (list != null && !list.isEmpty()) {
			for (Iterator iterator = list.iterator(); iterator.hasNext();) {
				...
				String s1 = StatCollector.translateToLocal(
						potioneffect.getEffectName()).trim();
				...
				if (potioneffect.getDuration() > 20) {
					s1 = (new StringBuilder()).append(s1).append(" (")
							.append(Potion.getDurationString(potioneffect))
							.append(")").toString();
				}

				if (Potion.potionTypes[potioneffect.getPotionID()]
						.isBadEffect()) {
					par2List.add((new StringBuilder()).append("\247c")
							.append(s1).toString());
				} else {
					par2List.add((new StringBuilder()).append("\2477")
							.append(s1).toString());
				}
			}
		} else {
			String s = StatCollector.translateToLocal("potion.empty").trim();
			par2List.add((new StringBuilder()).append("\2477").append(s)
					.toString());
		}
	}
