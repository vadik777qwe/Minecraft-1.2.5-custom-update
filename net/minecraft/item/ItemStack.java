...
public final class ItemStack {
	...
	public ItemStack splitStack(int par1) {
		...
		if (stackTagCompound != null) {
			itemstack.stackTagCompound = (NBTTagCompound) stackTagCompound
					.copy();
		}
	...
	/**
	 * Uses the item stack by the player. Gives the coordinates of the block its
	 * being used against and the side. Args: player, world, x, y, z, side
	 */
	public boolean useItem(EntityPlayer par1EntityPlayer, World par2World,
			int par3, int par4, int par5, int par6) {
		boolean flag = getItem().onItemUse(this, par1EntityPlayer, par2World,
				par3, par4, par5, par6);
	...
	public void damageItem(int par1, EntityLiving par2EntityLiving) {
		...
		if (par1 > 0 && (par2EntityLiving instanceof EntityPlayer)) {
			int i = EnchantmentHelper
					.getUnbreakingModifier(((EntityPlayer) par2EntityLiving).inventory);
	...
	public void hitEntity(EntityLiving par1EntityLiving,
			EntityPlayer par2EntityPlayer) {
		boolean flag = Item.itemsList[itemID].hitEntity(this, par1EntityLiving,
				par2EntityPlayer);
	...
	public void onDestroyBlock(int par1, int par2, int par3, int par4,
			EntityPlayer par5EntityPlayer) {
		boolean flag = Item.itemsList[itemID].onBlockDestroyed(this, par1,
	...
	/**
	 * Called when a given item stack is about to be destroyed due to its damage
	 * level expiring when used on a block or entity. Typically used by tools.
	 */
	...
	public static boolean func_46154_a(ItemStack par0ItemStack,
		...
		if (par0ItemStack.stackTagCompound == null
				&& par1ItemStack.stackTagCompound != null) {
			return false;
		}

		return par0ItemStack.stackTagCompound == null
	...
	private boolean isItemStackEqual(ItemStack par1ItemStack) {
		...
		return stackTagCompound == null
				|| stackTagCompound.equals(par1ItemStack.stackTagCompound);
	}
	...
	public void onCrafting(World par1World, EntityPlayer par2EntityPlayer,
			int par3) {
	...
	public void onPlayerStoppedUsing(World par1World,
			EntityPlayer par2EntityPlayer, int par3) {
		getItem().onPlayerStoppedUsing(this, par1World, par2EntityPlayer, par3);
	}

	/**
	 * Returns true if the ItemStack have a NBTTagCompound. Used currently to
	 * store enchantments.
	 */
	...
	/**
	 * gets a list of strings representing the item name and successive extra
	 * data, eg Enchantments and potion effects
	 */
		...
		if (hasTagCompound()) {
			...
			if (nbttaglist != null) {
				for (int i = 0; i < nbttaglist.tagCount(); i++) {
					short word0 = ((NBTTagCompound) nbttaglist.tagAt(i))
							.getShort("id");
					short word1 = ((NBTTagCompound) nbttaglist.tagAt(i))
							.getShort("lvl");

					if (Enchantment.enchantmentsList[word0] != null) {
						arraylist.add(Enchantment.enchantmentsList[word0]
								.getTranslatedName(word1));
					}
				}
			}
		}
