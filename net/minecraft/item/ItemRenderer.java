...
import net.minecraft.world.map.MapItemRenderer;
import net.minecraft.world.map.MapData;
import net.minecraft.enums.EnumAction;
import net.minecraft.render.RenderPlayer;
...
import net.minecraft.render.Render;
...
public class ItemRenderer {
	...
	public ItemRenderer(Minecraft par1Minecraft) {
		...
		equippedItemSlot = -1;
		mc = par1Minecraft;
		mapItemRenderer = new MapItemRenderer(par1Minecraft.fontRenderer,
	...
	public void renderItem(EntityLiving par1EntityLiving,
		...
		if (par2ItemStack.itemID < 256
				&& RenderBlocks
						.renderItemIn3d(Block.blocksList[par2ItemStack.itemID]
								.getRenderType())) {
			GL11.glBindTexture(GL11.GL_TEXTURE_2D,
					mc.renderEngine.getTexture("/terrain.png"));
			renderBlocksInstance.renderBlockAsItem(
					Block.blocksList[par2ItemStack.itemID],
	 ...
	 */
	private void renderItemIn2D(Tessellator par1Tessellator, float par2,
	...
	public void renderItemInFirstPerson(float par1) {
		float f = prevEquippedProgress
				+ (equippedProgress - prevEquippedProgress) * par1;
		EntityPlayerSP entityplayersp = mc.thePlayer;
		float f1 = ((EntityPlayer) (entityplayersp)).prevRotationPitch
				+ (((EntityPlayer) (entityplayersp)).rotationPitch - ((EntityPlayer) (entityplayersp)).prevRotationPitch)
				* par1;
		GL11.glPushMatrix();
		...
		if (entityplayersp instanceof EntityPlayerSP) {
			...
			float f2 = entityplayersp1.prevRenderArmPitch
					+ (entityplayersp1.renderArmPitch - entityplayersp1.prevRenderArmPitch)
					* par1;
			float f4 = entityplayersp1.prevRenderArmYaw
					+ (entityplayersp1.renderArmYaw - entityplayersp1.prevRenderArmYaw)
					* par1;
			GL11.glRotatef(
					(((EntityPlayer) (entityplayersp)).rotationPitch - f2) * 0.1F,
					1.0F, 0.0F, 0.0F);
			GL11.glRotatef(
					(((EntityPlayer) (entityplayersp)).rotationYaw - f4) * 0.1F,
					0.0F, 1.0F, 0.0F);
		}

		ItemStack itemstack = itemToRender;
		float f3 = mc.theWorld
				.getLightBrightness(
						MathHelper
								.floor_double(((EntityPlayer) (entityplayersp)).posX),
						MathHelper
								.floor_double(((EntityPlayer) (entityplayersp)).posY),
						MathHelper
								.floor_double(((EntityPlayer) (entityplayersp)).posZ));
		f3 = 1.0F;
		int i = mc.theWorld
		...
		if (itemstack != null) {
			int j = Item.itemsList[itemstack.itemID].getColorFromDamage(
		...
		if (itemstack != null && itemstack.itemID == Item.map.shiftedIndex) {
			...
			float f20 = MathHelper.sin(MathHelper.sqrt_float(f9)
					* (float) Math.PI);
			GL11.glTranslatef(
					-f20 * 0.4F,
					MathHelper.sin(MathHelper.sqrt_float(f9) * (float) Math.PI
							* 2.0F) * 0.2F, -f14 * 0.2F);
			f9 = (1.0F - f1 / 45F) + 0.1F;
			...
			GL11.glTranslatef(0.0F,
					(0.0F * f5 - (1.0F - f) * 1.2F - f9 * 0.5F) + 0.04F, -0.9F
							* f5);
			GL11.glRotatef(90F, 0.0F, 1.0F, 0.0F);
			GL11.glRotatef(f9 * -85F, 0.0F, 0.0F, 1.0F);
			GL11.glEnable(GL12.GL_RESCALE_NORMAL);
			GL11.glBindTexture(GL11.GL_TEXTURE_2D, mc.renderEngine
					.getTextureForDownloadableImage(mc.thePlayer.skinUrl,
							mc.thePlayer.getTexture()));
			...
			for (f14 = 0; f14 < 2; f14++) {
				...
				Render render1 = RenderManager.instance
			...
			float f27 = MathHelper.sin(MathHelper.sqrt_float(f14)
					* (float) Math.PI);
			...
			mc.renderEngine.bindTexture(mc.renderEngine
					.getTexture("/misc/mapbg.png"));
			Tessellator tessellator = Tessellator.instance;
			GL11.glNormal3f(0.0F, 0.0F, -1F);
			tessellator.startDrawingQuads();
			byte byte0 = 7;
			tessellator.addVertexWithUV(0 - byte0, 128 + byte0, 0.0D, 0.0D,
					1.0D);
			tessellator.addVertexWithUV(128 + byte0, 128 + byte0, 0.0D, 1.0D,
					1.0D);
			tessellator.addVertexWithUV(128 + byte0, 0 - byte0, 0.0D, 1.0D,
					0.0D);
			...
			if (entityplayersp.getItemInUseCount() > 0) {
				...
				if (enumaction == EnumAction.eat
					...
					GL11.glTranslatef(
							0.0F,
							MathHelper.abs(MathHelper.cos((f15 / 4F)
									* (float) Math.PI) * 0.1F)
									* (float) ((double) f28 <= 0.20000000000000001D ? 0
											: 1), 0.0F);
				...
				float f22 = MathHelper.sin(MathHelper.sqrt_float(f10)
						* (float) Math.PI);
				GL11.glTranslatef(
						-f22 * 0.4F,
						MathHelper.sin(MathHelper.sqrt_float(f10)
								* (float) Math.PI * 2.0F) * 0.2F, -f16 * 0.2F);
			}

			GL11.glTranslatef(0.7F * f6, -0.65F * f6 - (1.0F - f) * 0.6F, -0.9F
					* f6);
			GL11.glRotatef(45F, 0.0F, 1.0F, 0.0F);
			GL11.glEnable(GL12.GL_RESCALE_NORMAL);
			float f11 = entityplayersp.getSwingProgress(par1);
			float f17 = MathHelper.sin(f11 * f11 * (float) Math.PI);
			float f23 = MathHelper.sin(MathHelper.sqrt_float(f11)
					* (float) Math.PI);
			...
			if (entityplayersp.getItemInUseCount() > 0) {
				...
				if (enumaction1 == EnumAction.block) {
					...
					float f24 = (float) itemstack.getMaxItemUseDuration()
					...
					if (f29 > 0.1F) {
						GL11.glTranslatef(0.0F,
								MathHelper.sin((f24 - 0.1F) * 1.3F) * 0.01F
										* (f29 - 0.1F), 0.0F);
					}
			...
			float f26 = MathHelper.sin(MathHelper.sqrt_float(f12)
					* (float) Math.PI);
			GL11.glTranslatef(
					-f26 * 0.3F,
					MathHelper.sin(MathHelper.sqrt_float(f12) * (float) Math.PI
							* 2.0F) * 0.4F, -f18 * 0.4F);
			GL11.glTranslatef(0.8F * f7, -0.75F * f7 - (1.0F - f) * 0.6F, -0.9F
					* f7);
			GL11.glRotatef(45F, 0.0F, 1.0F, 0.0F);
			GL11.glEnable(GL12.GL_RESCALE_NORMAL);
	...
	public void renderOverlays(float par1) {
		...
		if (mc.thePlayer.isEntityInsideOpaqueBlock()) {
			...
			if (mc.theWorld.isBlockNormalCube(j, l, i1)) {
				renderInsideOfBlock(par1,
						Block.blocksList[k1].getBlockTextureFromSide(2));
			} else {
				for (int l1 = 0; l1 < 8; l1++) {
					float f = ((float) ((l1 >> 0) % 2) - 0.5F)
							* mc.thePlayer.width * 0.9F;
					float f1 = ((float) ((l1 >> 1) % 2) - 0.5F)
							* mc.thePlayer.height * 0.2F;
					float f2 = ((float) ((l1 >> 2) % 2) - 0.5F)
							* mc.thePlayer.width * 0.9F;
	...
	public void updateEquippedItem() {
		...
		EntityPlayerSP entityplayersp = mc.thePlayer;
		ItemStack itemstack = ((EntityPlayer) (entityplayersp)).inventory
				.getCurrentItem();
		ItemStack itemstack1 = itemstack;
		boolean flag = equippedItemSlot == ((EntityPlayer) (entityplayersp)).inventory.currentItem
		...
		if (itemstack1 != null && itemToRender != null
				&& itemstack1 != itemToRender
