...
public class ItemDye extends Item {
	...
	public static final int dyeColors[] = { 0x1e1b1b, 0xb3312c, 0x3b511a,
	...
	public String getItemNameIS(ItemStack par1ItemStack) {
		...
		return (new StringBuilder()).append(super.getItemName()).append(".")
				.append(dyeColorNames[i]).toString();
	}
		...
		if (par1ItemStack.getItemDamage() == 15) {
			...
			if (i == Block.sapling.blockID) {
				if (!par3World.isRemote) {
					((BlockSapling) Block.sapling).growTree(par3World, par4,
							par5, par6, par3World.rand);
			...
			if (i == Block.mushroomBrown.blockID
					|| i == Block.mushroomRed.blockID) {
				if (!par3World.isRemote
						&& ((BlockMushroom) Block.blocksList[i])
								.fertilizeMushroom(par3World, par4, par5, par6,
										par3World.rand)) {
					par1ItemStack.stackSize--;
				}
			...
			if (i == Block.melonStem.blockID || i == Block.pumpkinStem.blockID) {
				if (!par3World.isRemote) {
					((BlockStem) Block.blocksList[i]).fertilizeStem(par3World,
							par4, par5, par6);
					par1ItemStack.stackSize--;
				}
			...
			if (i == Block.crops.blockID) {
				if (!par3World.isRemote) {
					((BlockCrops) Block.crops).fertilize(par3World, par4, par5,
							par6);
					par1ItemStack.stackSize--;
				}
				...
				if (!par3World.isRemote) {
					...
					for (int j = 0; j < 128; j++) {
						...
						for (int j1 = 0; j1 < j / 16; j1++) {
							...
							l += ((itemRand.nextInt(3) - 1) * itemRand
									.nextInt(3)) / 2;
							i1 += itemRand.nextInt(3) - 1;

							if (par3World.getBlockId(k, l - 1, i1) != Block.grass.blockID
									|| par3World.isBlockNormalCube(k, l, i1)) {
								continue label0;
							}
						}
						...
						if (itemRand.nextInt(10) != 0) {
							par3World.setBlockAndMetadataWithNotify(k, l, i1,
