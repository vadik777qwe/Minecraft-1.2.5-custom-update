...
public class ItemMap extends ItemMapBase {
	...
	public static MapData getMPMapData(short par0, World par1World) {
		...
		MapData mapdata = (MapData) par1World.loadItemData(
				net.minecraft.world.map.MapData.class,
				(new StringBuilder()).append("map_").append(par0).toString());

		if (mapdata == null) {
			int i = par1World.getUniqueDataId("map");
			String s1 = (new StringBuilder()).append("map_").append(i)
					.toString();
	...
	public MapData getMapData(ItemStack par1ItemStack, World par2World) {
		String s = (new StringBuilder()).append("map_")
				.append(par1ItemStack.getItemDamage()).toString();
		MapData mapdata = (MapData) par2World.loadItemData(
				net.minecraft.world.map.MapData.class,
				(new StringBuilder()).append("map_")
						.append(par1ItemStack.getItemDamage()).toString());

		if (mapdata == null) {
			par1ItemStack.setItemDamage(par2World.getUniqueDataId("map"));
			String s1 = (new StringBuilder()).append("map_")
					.append(par1ItemStack.getItemDamage()).toString();
			mapdata = new MapData(s1);
	...
	public void updateMapData(World par1World, Entity par2Entity,
			MapData par3MapData) {
		...
		int l = MathHelper.floor_double(par2Entity.posX - (double) j) / i + c
				/ 2;
		int i1 = MathHelper.floor_double(par2Entity.posZ - (double) k) / i + c1
				/ 2;
		int j1 = 128 / i;
			...
			for (int j2 = i1 - j1 - 1; j2 < i1 + j1; j2++) {
					...
					for (int j5 = 0; j5 < i; j5++) {
						for (int l5 = 0; l5 < i; l5++) {
							...
							if (j6 > 1) {
								...
								do {
									flag1 = true;
									l6 = chunk.getBlockID(j5 + j4, j6 - 1, l5
											+ k4);
									...
									if (!flag1) {
										j6--;
										l6 = chunk.getBlockID(j5 + j4, j6 - 1,
												l5 + k4);
									}
								} while (j6 > 0 && !flag1);
								...
								if (j6 > 0
										&& l6 != 0
										&& Block.blocksList[l6].blockMaterial
												.isLiquid()) {
									int i7 = j6 - 1;
									int k7 = 0;

									do {
										k7 = chunk.getBlockID(j5 + j4, i7--, l5
												+ k4);
										l4++;
									} while (i7 > 0
											&& k7 != 0
											&& Block.blocksList[k7].blockMaterial
													.isLiquid());
								}
							}
				...
				if (i6 > 0) {
					...
					if (mapcolor == MapColor.waterColor) {
						double d3 = (double) l4 * 0.10000000000000001D
								+ (double) (k1 + j2 & 1) * 0.20000000000000001D;
						byte0 = 1;
				...
				if (j2 < 0 || k2 * k2 + l2 * l2 >= j1 * j1 || flag
						&& (k1 + j2 & 1) == 0) {
					continue;
				}
