...
public class BiomeDecorator {
	...
	protected void decorate() {
		...
		for (int i = 0; i < sandPerChunk2; i++) {
			...
			sandGen.generate(currentWorld, randomGenerator, i1,
					currentWorld.getTopSolidOrLiquidBlock(i1, k5), k5);
		}

		for (int j = 0; j < clayPerChunk; j++) {
			int j1 = chunk_X + randomGenerator.nextInt(16) + 8;
			int l5 = chunk_Z + randomGenerator.nextInt(16) + 8;
			clayGen.generate(currentWorld, randomGenerator, j1,
					currentWorld.getTopSolidOrLiquidBlock(j1, l5), l5);
		}

		for (int k = 0; k < sandPerChunk; k++) {
			int k1 = chunk_X + randomGenerator.nextInt(16) + 8;
			int i6 = chunk_Z + randomGenerator.nextInt(16) + 8;
			sandGen.generate(currentWorld, randomGenerator, k1,
					currentWorld.getTopSolidOrLiquidBlock(k1, i6), i6);
		}
		...
		for (int l1 = 0; l1 < l; l1++) {
			...
			WorldGenerator worldgenerator = biome
					.getRandomWorldGenForTrees(randomGenerator);
			worldgenerator.setScale(1.0D, 1.0D, 1.0D);
			worldgenerator.generate(currentWorld, randomGenerator, j6,
					currentWorld.getHeightValue(j6, k10), k10);
		}
		...
		for (int i2 = 0; i2 < bigMushroomsPerChunk; i2++) {
			...
			bigMushroomGen.generate(currentWorld, randomGenerator, k6,
					currentWorld.getHeightValue(k6, l10), l10);
		}
		...
		for (int j2 = 0; j2 < flowersPerChunk; j2++) {
			...
			plantYellowGen
					.generate(currentWorld, randomGenerator, l6, i11, l14);
		...
		for (int k2 = 0; k2 < grassPerChunk; k2++) {
			...
			WorldGenerator worldgenerator1 = biome
					.func_48410_b(randomGenerator);
			worldgenerator1.generate(currentWorld, randomGenerator, j7, k11,
					j15);
		}
		...
		for (int l2 = 0; l2 < deadBushPerChunk; l2++) {
			...
			(new WorldGenDeadBush(Block.deadBush.blockID)).generate(
					currentWorld, randomGenerator, k7, l11, k15);
		}
		...
		for (int i3 = 0; i3 < waterlilyPerChunk; i3++) {
			...
			for (l15 = randomGenerator.nextInt(128); l15 > 0
					&& currentWorld.getBlockId(l7, l15 - 1, i12) == 0; l15--) {
			}
		...
		for (int j3 = 0; j3 < mushroomsPerChunk; j3++) {
			if (randomGenerator.nextInt(4) == 0) {
				...
				mushroomBrownGen.generate(currentWorld, randomGenerator, i8,
						i16, j12);
			}
			...
			if (randomGenerator.nextInt(8) == 0) {
				...
				mushroomRedGen.generate(currentWorld, randomGenerator, j8, j16,
						k12);
			}
		}
		...
		if (randomGenerator.nextInt(4) == 0) {
			...
			mushroomBrownGen.generate(currentWorld, randomGenerator, k3, k8,
					l12);
		}
		...
		if (randomGenerator.nextInt(32) == 0) {
			...
			(new WorldGenPumpkin()).generate(currentWorld, randomGenerator, k4,
					k9, l13);
		}
		...
		if (generateLakes) {
			for (int i5 = 0; i5 < 50; i5++) {
				...
				int j14 = randomGenerator
						.nextInt(randomGenerator.nextInt(120) + 8);
				int j17 = chunk_Z + randomGenerator.nextInt(16) + 8;
				(new WorldGenLiquids(Block.waterMoving.blockID)).generate(
						currentWorld, randomGenerator, i10, j14, j17);
			}

			for (int j5 = 0; j5 < 20; j5++) {
				int j10 = chunk_X + randomGenerator.nextInt(16) + 8;
				int k14 = randomGenerator.nextInt(randomGenerator
						.nextInt(randomGenerator.nextInt(112) + 8) + 8);
				int k17 = chunk_Z + randomGenerator.nextInt(16) + 8;
				(new WorldGenLiquids(Block.lavaMoving.blockID)).generate(
						currentWorld, randomGenerator, j10, k14, k17);
			}
		}
	}
	 ...
	 */
	protected void genStandardOre1(int par1, WorldGenerator par2WorldGenerator,
			int par3, int par4) {
	 ...
	 */
	protected void genStandardOre2(int par1, WorldGenerator par2WorldGenerator,
			int par3, int par4) {
		for (int i = 0; i < par1; i++) {
			int j = chunk_X + randomGenerator.nextInt(16);
			int k = randomGenerator.nextInt(par4)
					+ randomGenerator.nextInt(par4) + (par3 - par4);
			int l = chunk_Z + randomGenerator.nextInt(16);
			par2WorldGenerator.generate(currentWorld, randomGenerator, j, k, l);
		}
	}
