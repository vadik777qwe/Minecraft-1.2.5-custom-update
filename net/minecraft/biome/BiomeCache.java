...
public class BiomeCache {
	...
	public BiomeCacheBlock getBiomeCacheBlock(int par1, int par2) {
		...
		BiomeCacheBlock biomecacheblock = (BiomeCacheBlock) cacheMap
				.getValueByKey(l);
	...
	public void cleanupCache() {
		...
		if (l1 > 7500L || l1 < 0L) {
			...
			for (int i = 0; i < cache.size(); i++) {
				BiomeCacheBlock biomecacheblock = (BiomeCacheBlock) cache
						.get(i);
				long l2 = l - biomecacheblock.lastAccessTime;

				if (l2 > 30000L || l2 < 0L) {
					cache.remove(i--);
					long l3 = (long) biomecacheblock.xPosition
							& 0xffffffffL
							| ((long) biomecacheblock.zPosition & 0xffffffffL) << 32;
					cacheMap.remove(l3);
				}
			}
		}
