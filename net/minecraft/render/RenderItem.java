...
public class RenderItem extends Render {
	...
	public void doRenderItem(EntityItem par1EntityItem, double par2,
		...
		GL11.glPushMatrix();
		float f = MathHelper.sin(((float) par1EntityItem.age + par9) / 10F
				+ par1EntityItem.field_804_d) * 0.1F + 0.1F;
		float f1 = (((float) par1EntityItem.age + par9) / 20F + par1EntityItem.field_804_d)
				* (180F / (float) Math.PI);
		byte byte0 = 1;
		...
		if (itemstack.itemID < 256
			...
			for (int i = 0; i <= 1; i++) {
				int l = itemstack.getItem().func_46057_a(
						itemstack.getItemDamage(), i);
				float f3 = 1.0F;

				if (field_27004_a) {
					int k1 = Item.itemsList[itemstack.itemID]
							.getColorFromDamage(itemstack.getItemDamage(), i);
	...
	public void drawItemIntoGui(FontRenderer par1FontRenderer,
			RenderEngine par2RenderEngine, int par3, int par4, int par5,
			int par6, int par7) {
		if (par3 < 256
				&& RenderBlocks.renderItemIn3d(Block.blocksList[par3]
						.getRenderType())) {
			int i = par3;
			par2RenderEngine.bindTexture(par2RenderEngine
			...
			for (int j = 0; j <= 1; j++) {
				...
				renderTexturedQuad(par6, par7, (l % 16) * 16, (l / 16) * 16,
						16, 16);
			}
			...
			renderTexturedQuad(par6, par7, (par5 % 16) * 16, (par5 / 16) * 16,
					16, 16);
			GL11.glEnable(GL11.GL_LIGHTING);
		}
	...
	public void renderItemIntoGUI(FontRenderer par1FontRenderer,
		...
		drawItemIntoGui(par1FontRenderer, par2RenderEngine,
				par3ItemStack.itemID, par3ItemStack.getItemDamage(),
		...
		if (par3ItemStack != null && par3ItemStack.hasEffect()) {
			...
			func_40266_a(par4 * 0x19b4ca14 + par5 * 0x1eafff1, par4 - 2,
					par5 - 2, 20, 20);
	...
	private void func_40266_a(int par1, int par2, int par3, int par4, int par5) {
		for (int i = 0; i < 2; i++) {
			...
			tessellator.startDrawingQuads();
			tessellator.addVertexWithUV(par2 + 0, par3 + par5, zLevel,
					(f2 + (float) par5 * f4) * f, (f3 + (float) par5) * f1);
			tessellator.addVertexWithUV(par2 + par4, par3 + par5, zLevel, (f2
					+ (float) par4 + (float) par5 * f4)
					* f, (f3 + (float) par5) * f1);
			tessellator.addVertexWithUV(par2 + par4, par3 + 0, zLevel,
					(f2 + (float) par4) * f, (f3 + 0.0F) * f1);
			tessellator.addVertexWithUV(par2 + 0, par3 + 0, zLevel, (f2 + 0.0F)
					* f, (f3 + 0.0F) * f1);
			tessellator.draw();
		}
	}
	...
	public void renderItemOverlayIntoGUI(FontRenderer par1FontRenderer,
		...
		if (par3ItemStack.stackSize > 1) {
			String s = (new StringBuilder()).append("")
					.append(par3ItemStack.stackSize).toString();
			GL11.glDisable(GL11.GL_LIGHTING);
			GL11.glDisable(GL11.GL_DEPTH_TEST);
			par1FontRenderer.drawStringWithShadow(s, (par4 + 19) - 2
					- par1FontRenderer.getStringWidth(s), par5 + 6 + 3,
					0xffffff);
			GL11.glEnable(GL11.GL_LIGHTING);
		...
		if (par3ItemStack.isItemDamaged()) {
			int i = (int) Math.round(13D
					- ((double) par3ItemStack.getItemDamageForDisplay() * 13D)
					/ (double) par3ItemStack.getMaxDamage());
			int j = (int) Math.round(255D
					- ((double) par3ItemStack.getItemDamageForDisplay() * 255D)
					/ (double) par3ItemStack.getMaxDamage());
			GL11.glDisable(GL11.GL_LIGHTING);
			GL11.glDisable(GL11.GL_DEPTH_TEST);
