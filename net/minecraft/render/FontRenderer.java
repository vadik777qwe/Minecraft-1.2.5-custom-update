...
public class FontRenderer {
	private static final Pattern field_52015_r = Pattern
	...
	/**
	 * The currently bound GL texture ID. Avoids unnecessary glBindTexture() for
	 * the same texture if it's already bound.
	 */
	...
	public FontRenderer(GameSettings par1GameSettings, String par2Str,
			RenderEngine par3RenderEngine, boolean par4) {
		charWidth = new int[256];
		fontTextureName = 0;
		FONT_HEIGHT = 8;
		fontRandom = new Random();
		...
		try {
			bufferedimage = ImageIO.read((net.minecraft.render.RenderEngine.class)
					.getResourceAsStream(par2Str));
			InputStream inputstream = (net.minecraft.render.RenderEngine.class)
		...
		fontTextureName = par3RenderEngine
				.allocateAndSetupTexture(bufferedimage);
	...
	private void loadGlyphTexture(int par1) {
		String s = String.format("/font/glyph_%02X.png",
		...
		glyphTextureName[par1] = renderEngine
				.allocateAndSetupTexture(bufferedimage);
	...
	private String bidiReorder(String par1Str) {
		if (par1Str == null
				|| !Bidi.requiresBidi(par1Str.toCharArray(), 0,
						par1Str.length())) {
			return par1Str;
		}
	...
	private void renderStringAtPos(String par1Str, boolean par2) {
		...
		for (int i = 0; i < par1Str.length(); i++) {
			...
			if (c == '\247' && i + 1 < par1Str.length()) {
				int j = "0123456789abcdefklmnor".indexOf(par1Str.toLowerCase()
						.charAt(i + 1));
				...
				if (j < 16) {
					...
					int l = colorCode[j];
					GL11.glColor3f((float) (l >> 16) / 255F,
							(float) (l >> 8 & 0xff) / 255F,
							(float) (l & 0xff) / 255F);
				} else if (j == 16) {
					flag = true;
					...
					flag3 = false;
					flag2 = false;
					GL11.glColor4f(field_50115_n, field_50116_o, field_50118_p,
							field_50117_q);
				}
			...
			if (flag4) {
				...
				tessellator.addVertex(posX, posY + (float) (FONT_HEIGHT / 2),
						0.0D);
				tessellator.addVertex(posX + f, posY
						+ (float) (FONT_HEIGHT / 2), 0.0D);
				tessellator.addVertex(posX + f,
						(posY + (float) (FONT_HEIGHT / 2)) - 1.0F, 0.0D);
				tessellator.addVertex(posX,
						(posY + (float) (FONT_HEIGHT / 2)) - 1.0F, 0.0D);
				tessellator.draw();
			...
			if (flag3) {
				...
				int j1 = flag3 ? -1 : 0;
				tessellator1.addVertex(posX + (float) j1, posY
						+ (float) FONT_HEIGHT, 0.0D);
				tessellator1.addVertex(posX + f, posY + (float) FONT_HEIGHT,
						0.0D);
				tessellator1.addVertex(posX + f,
						(posY + (float) FONT_HEIGHT) - 1.0F, 0.0D);
				tessellator1.addVertex(posX + (float) j1,
						(posY + (float) FONT_HEIGHT) - 1.0F, 0.0D);
				tessellator1.draw();
				GL11.glEnable(GL11.GL_TEXTURE_2D);
			}
	...
	public int func_50101_a(String par1Str, int par2, int par3, int par4,
			boolean par5) {
		if (par1Str != null) {
			boundTextureName = 0;
	...
	/**
	 * renders a multi-line string with wordwrap (maximum length is parameter k)
	 * by means of renderSplitString
	 */
	private void renderSplitStringNoShadow(String par1Str, int par2, int par3,
			int par4, int par5) {
	 ...
	 * (maximum length is parameter k) and with darkre drop shadow color if flag
	 * is set
	 */
	private void renderSplitString(String par1Str, int par2, int par3,
		...
		do {
			...
			for (s1 = (new StringBuilder()).append(s).append(as1[j++])
					.append(" ").toString(); j < as1.length
					&& getStringWidth((new StringBuilder()).append(s1)
							.append(as1[j]).toString()) < par4; s1 = (new StringBuilder())
					.append(s1).append(as1[j++]).append(" ").toString()) {
			}

			int k;

			for (; getStringWidth(s1) > par4; s1 = (new StringBuilder())
					.append(s).append(s1.substring(k)).toString()) {
				...
				if (s2.lastIndexOf("\247") >= 0) {
					s = (new StringBuilder()).append("\247")
							.append(s2.charAt(s2.lastIndexOf("\247") + 1))
							.toString();
				}
			...
			if (getStringWidth(s1.trim()) > 0) {
				if (s1.lastIndexOf("\247") >= 0) {
					s = (new StringBuilder()).append("\247")
							.append(s1.charAt(s1.lastIndexOf("\247") + 1))
							.toString();
				}
	...
	public int splitStringWidth(String par1Str, int par2) {
		...
		do {
			...
			for (s = (new StringBuilder()).append(as1[k++]).append(" ")
					.toString(); k < as1.length
					&& getStringWidth((new StringBuilder()).append(s)
							.append(as1[k]).toString()) < par2; s = (new StringBuilder())
					.append(s).append(as1[k++]).append(" ").toString()) {
			}
	...
	String func_50113_d(String par1Str, int par2) {
		...
		if (par1Str.length() <= i) {
			...
			String s1 = (new StringBuilder())
					.append(func_50114_c(s))
					.append(par1Str.substring(i
							+ (par1Str.charAt(i) != ' ' ? 0 : 1))).toString();
	...
	private static String func_50114_c(String par0Str) {
		...
		do {
			...
			if (i < j - 1) {
				...
				if (func_50110_b(c)) {
					s = (new StringBuilder()).append("\247").append(c)
							.toString();
				} else if (func_50109_c(c)) {
					s = (new StringBuilder()).append(s).append("\247")
							.append(c).toString();
				}
			}
		} while (true);
