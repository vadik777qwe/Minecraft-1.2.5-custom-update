...
public class EffectRenderer {
	...
	public void renderParticles(Entity par1Entity, float par2) {
		...
		EntityFX.interpPosX = par1Entity.lastTickPosX
				+ (par1Entity.posX - par1Entity.lastTickPosX) * (double) par2;
		EntityFX.interpPosY = par1Entity.lastTickPosY
				+ (par1Entity.posY - par1Entity.lastTickPosY) * (double) par2;
		EntityFX.interpPosZ = par1Entity.lastTickPosZ
		...
		for (int i = 0; i < 3; i++) {
			...
			for (int k = 0; k < fxLayers[i].size(); k++) {
				...
				tessellator
						.setBrightness(entityfx.getBrightnessForRender(par2));
	...
	public void addBlockDestroyEffects(int par1, int par2, int par3, int par4,
			int par5) {
		if (par4 == 0) {
			return;
		}
		...
		for (int j = 0; j < i; j++) {
			for (int k = 0; k < i; k++) {
				for (int l = 0; l < i; l++) {
					...
					double d1 = (double) par2 + ((double) k + 0.5D)
							/ (double) i;
					double d2 = (double) par3 + ((double) l + 0.5D)
							/ (double) i;
					int i1 = rand.nextInt(6);
					addEffect((new EntityDiggingFX(worldObj, d, d1, d2, d
							- (double) par1 - 0.5D, d1 - (double) par2 - 0.5D,
							d2 - (double) par3 - 0.5D, block, i1, par5))
							.func_4041_a(par1, par2, par3));
				}
			}
		}
	}
	...
	public void addBlockHitEffects(int par1, int par2, int par3, int par4) {
		...
		float f = 0.1F;
		double d = (double) par1 + rand.nextDouble()
				* (block.maxX - block.minX - (double) (f * 2.0F)) + (double) f
				+ block.minX;
		double d1 = (double) par2 + rand.nextDouble()
				* (block.maxY - block.minY - (double) (f * 2.0F)) + (double) f
				+ block.minY;
		double d2 = (double) par3 + rand.nextDouble()
				* (block.maxZ - block.minZ - (double) (f * 2.0F)) + (double) f
				+ block.minZ;
		...
		addEffect((new EntityDiggingFX(worldObj, d, d1, d2, 0.0D, 0.0D, 0.0D,
				block, par4, worldObj.getBlockMetadata(par1, par2, par3)))
				.func_4041_a(par1, par2, par3).multiplyVelocity(0.2F)
				.func_405_d(0.6F));
	}
	...
	public String getStatistics() {
		return (new StringBuilder())
				.append("")
				.append(fxLayers[0].size() + fxLayers[1].size()
						+ fxLayers[2].size()).toString();
	}
}
