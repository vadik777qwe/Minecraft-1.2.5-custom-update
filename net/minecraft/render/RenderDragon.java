...
import net.minecraft.entity.models.ModelDragon;
...
public class RenderDragon extends RenderLiving {
	/**
	 * The entity instance of the dragon. Note: This is a static field in
	 * RenderDragon because there is only supposed to be one dragon
	 */
	 ...
	 * rotateCorpse method.
	 */
	protected void rotateDragonBody(EntityDragon par1EntityDragon, float par2,
			float par3, float par4) {
		float f = (float) par1EntityDragon.func_40160_a(7, par4)[0];
		float f1 = (float) (par1EntityDragon.func_40160_a(5, par4)[1] - par1EntityDragon
				.func_40160_a(10, par4)[1]);
		...
		if (par1EntityDragon.deathTime > 0) {
			...
			GL11.glRotatef(f2 * getDeathMaxRotation(par1EntityDragon), 0.0F,
					0.0F, 1.0F);
		}
	}

	protected void func_40280_a(EntityDragon par1EntityDragon, float par2,
		...
		if (par1EntityDragon.field_40178_aA > 0) {
			...
			loadDownloadableImageTexture(par1EntityDragon.skinUrl,
					"/mob/enderdragon/shuffle.png");
			mainModel.render(par1EntityDragon, par2, par3, par4, par5, par6,
					par7);
	 ...
	 */
	public void renderDragon(EntityDragon par1EntityDragon, double par2,
		...
		if (par1EntityDragon.healingEnderCrystal != null) {
			float f = (float) par1EntityDragon.healingEnderCrystal.innerRotation
					+ par9;
			float f1 = MathHelper.sin(f * 0.2F) / 2.0F + 0.5F;
			f1 = (f1 * f1 + f1) * 0.2F;
			float f2 = (float) (par1EntityDragon.healingEnderCrystal.posX
					- par1EntityDragon.posX - (par1EntityDragon.prevPosX - par1EntityDragon.posX)
					* (double) (1.0F - par9));
			float f3 = (float) (((double) f1 + par1EntityDragon.healingEnderCrystal.posY)
					- 1.0D - par1EntityDragon.posY - (par1EntityDragon.prevPosY - par1EntityDragon.posY)
					* (double) (1.0F - par9));
			float f4 = (float) (par1EntityDragon.healingEnderCrystal.posZ
					- par1EntityDragon.posZ - (par1EntityDragon.prevPosZ - par1EntityDragon.posZ)
					* (double) (1.0F - par9));
			...
			GL11.glRotatef(((float) (-Math.atan2(f4, f2)) * 180F)
					/ (float) Math.PI - 90F, 0.0F, 1.0F, 0.0F);
			GL11.glRotatef(((float) (-Math.atan2(f5, f3)) * 180F)
					/ (float) Math.PI - 90F, 1.0F, 0.0F, 0.0F);
			...
			float f8 = MathHelper.sqrt_float(f2 * f2 + f3 * f3 + f4 * f4) / 32F
			...
			for (int j = 0; j <= i; j++) {
				float f9 = MathHelper
						.sin(((float) (j % i) * (float) Math.PI * 2.0F)
								/ (float) i) * 0.75F;
				float f10 = MathHelper
						.cos(((float) (j % i) * (float) Math.PI * 2.0F)
								/ (float) i) * 0.75F;
				float f11 = ((float) (j % i) * 1.0F) / (float) i;
				tessellator.setColorOpaque_I(0);
				tessellator.addVertexWithUV(f9 * 0.2F, f10 * 0.2F, 0.0D, f11,
						f8);
	...
	protected void renderDragonDying(EntityDragon par1EntityDragon, float par2) {
		...
		if (par1EntityDragon.field_40178_aA > 0) {
			...
			for (int i = 0; (float) i < ((f + f * f) / 2.0F) * 60F; i++) {
				...
				GL11.glRotatef(random.nextFloat() * 360F, 1.0F, 0.0F, 0.0F);
				GL11.glRotatef(random.nextFloat() * 360F, 0.0F, 1.0F, 0.0F);
				GL11.glRotatef(random.nextFloat() * 360F + f * 90F, 0.0F, 0.0F,
						1.0F);
				tessellator.startDrawing(6);
				float f2 = random.nextFloat() * 20F + 5F + f1 * 10F;
				float f3 = random.nextFloat() * 2.0F + 1.0F + f1 * 2.0F;
				tessellator
						.setColorRGBA_I(0xffffff, (int) (255F * (1.0F - f1)));
				tessellator.addVertex(0.0D, 0.0D, 0.0D);
				tessellator.setColorRGBA_I(0xff00ff, 0);
				tessellator.addVertex(-0.86599999999999999D * (double) f3, f2,
						-0.5F * f3);
				tessellator.addVertex(0.86599999999999999D * (double) f3, f2,
						-0.5F * f3);
	...
	protected int func_40283_a(EntityDragon par1EntityDragon, int par2,
			float par3) {
		if (par2 == 1) {
			GL11.glDepthFunc(GL11.GL_LEQUAL);
		}
	...
	protected void rotateCorpse(EntityLiving par1EntityLiving, float par2,
			float par3, float par4) {
	...
	protected void renderModel(EntityLiving par1EntityLiving, float par2,
			float par3, float par4, float par5, float par6, float par7) {
		func_40280_a((EntityDragon) par1EntityLiving, par2, par3, par4, par5,
				par6, par7);
	}

	public void doRenderLiving(EntityLiving par1EntityLiving, double par2,
			double par4, double par6, float par8, float par9) {
		renderDragon((EntityDragon) par1EntityLiving, par2, par4, par6, par8,
				par9);
	}
