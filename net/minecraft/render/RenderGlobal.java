...
import net.minecraft.entity.EntityDropParticleFX;
import net.minecraft.entity.EntitySuspendFX;
import net.minecraft.entity.Entity;
import net.minecraft.entity.EntityHugeExplodeFX;
import net.minecraft.entity.EntitySorter;
import net.minecraft.entity.EntityPortalFX;
import net.minecraft.entity.EntityAuraFX;
import net.minecraft.entity.EntityRenderer;
import net.minecraft.entity.EntityFlameFX;
import net.minecraft.entity.EntitySpellParticleFX;
import net.minecraft.entity.EntityHeartFX;
import net.minecraft.entity.EntityFootStepFX;
import net.minecraft.entity.EntityCritFX;
import net.minecraft.entity.EntityBreakingFX;
...
import net.minecraft.entity.EntityEnchantmentTableParticleFX;
...
import net.minecraft.entity.EntityCloudFX;
import net.minecraft.entity.EntityLargeExplodeFX;
...
public class RenderGlobal implements IWorldAccess {
	...
	private RenderList allRenderLists[] = { new RenderList(), new RenderList(),
			new RenderList(), new RenderList() };
	...
	public RenderGlobal(Minecraft par1Minecraft, RenderEngine par2RenderEngine) {
		...
		byte byte0 = 34;
		byte byte1 = 32;
		glRenderListBase = GLAllocation.generateDisplayLists(byte0 * byte0
				* byte1 * 3);
		occlusionEnabled = OpenGlCapsChecker.checkARBOcclusion();

		if (occlusionEnabled) {
			occlusionResult.clear();
			glOcclusionQueryBase = GLAllocation.createDirectIntBuffer(byte0
					* byte0 * byte1);
	...
	public void loadRenderers() {
		...
		worldRenderers = new WorldRenderer[renderChunksWide * renderChunksTall
				* renderChunksDeep];
		...
		for (int j1 = 0; j1 < renderChunksWide; j1++) {
			for (int k1 = 0; k1 < renderChunksTall; k1++) {
				for (int l1 = 0; l1 < renderChunksDeep; l1++) {
					worldRenderers[(l1 * renderChunksTall + k1)
							* renderChunksWide + j1] = new WorldRenderer(
							worldObj, tileEntities, j1 * 16, k1 * 16, l1 * 16,
							glRenderListBase + k);

					if (occlusionEnabled) {
						worldRenderers[(l1 * renderChunksTall + k1)
								* renderChunksWide + j1].glOcclusionQuery = glOcclusionQueryBase
								.get(l);
					}

					worldRenderers[(l1 * renderChunksTall + k1)
							* renderChunksWide + j1].isWaitingOnOcclusionQuery = false;
					worldRenderers[(l1 * renderChunksTall + k1)
							* renderChunksWide + j1].isVisible = true;
					worldRenderers[(l1 * renderChunksTall + k1)
							* renderChunksWide + j1].isInFrustum = true;
					worldRenderers[(l1 * renderChunksTall + k1)
							* renderChunksWide + j1].chunkIndex = l++;
					worldRenderers[(l1 * renderChunksTall + k1)
							* renderChunksWide + j1].markDirty();
					sortedWorldRenderers[(l1 * renderChunksTall + k1)
							* renderChunksWide + j1] = worldRenderers[(l1
							* renderChunksTall + k1)
							* renderChunksWide + j1];
					worldRenderersToUpdate.add(worldRenderers[(l1
							* renderChunksTall + k1)
							* renderChunksWide + j1]);
					k += 3;
				}
		...
		if (worldObj != null) {
			...
			if (entityliving != null) {
				markRenderersForNewPosition(
						MathHelper.floor_double(((Entity) (entityliving)).posX),
	...
	public void renderEntities(Vec3D par1Vec3D, ICamera par2ICamera, float par3) {
		...
		TileEntityRenderer.instance.cacheActiveRenderInfo(worldObj,
				renderEngine, mc.fontRenderer, mc.renderViewEntity, par3);
		RenderManager.instance.cacheActiveRenderInfo(worldObj, renderEngine,
				mc.fontRenderer, mc.renderViewEntity, mc.gameSettings, par3);
		...
		RenderManager.renderPosX = ((Entity) (entityliving)).lastTickPosX
				+ (((Entity) (entityliving)).posX - ((Entity) (entityliving)).lastTickPosX)
				* (double) par3;
		RenderManager.renderPosY = ((Entity) (entityliving)).lastTickPosY
				+ (((Entity) (entityliving)).posY - ((Entity) (entityliving)).lastTickPosY)
				* (double) par3;
		...
		for (int j = 0; j < list.size(); j++) {
			...
			if (entity1.isInRangeToRenderVec3D(par1Vec3D)
					&& (entity1.ignoreFrustumCheck || par2ICamera
							.isBoundingBoxInFrustum(entity1.boundingBox))
					&& (entity1 != mc.renderViewEntity
							|| mc.gameSettings.thirdPersonView != 0 || mc.renderViewEntity
								.isPlayerSleeping())
					&& worldObj.blockExists(
							MathHelper.floor_double(entity1.posX), 0,
							MathHelper.floor_double(entity1.posZ))) {
				countEntitiesRendered++;
		...
		for (int k = 0; k < tileEntities.size(); k++) {
			TileEntityRenderer.instance.renderTileEntity(
					(TileEntity) tileEntities.get(k), par3);
		}
	...
	public String getDebugInfoRenders() {
		return (new StringBuilder()).append("C: ")
				.append(renderersBeingRendered).append("/")
				.append(renderersLoaded).append(". F: ")
				.append(renderersBeingClipped).append(", O: ")
				.append(renderersBeingOccluded).append(", E: ")
				.append(renderersSkippingRenderPass).toString();
	}
	...
	public String getDebugInfoEntities() {
		return (new StringBuilder())
				.append("E: ")
				.append(countEntitiesRendered)
				.append("/")
				.append(countEntitiesTotal)
				.append(". B: ")
				.append(countEntitiesHidden)
				.append(", I: ")
				.append(countEntitiesTotal - countEntitiesHidden
						- countEntitiesRendered).toString();
	}
		...
		for (int k = 0; k < renderChunksWide; k++) {
			...
			for (int j1 = 0; j1 < renderChunksDeep; j1++) {
				...
				for (int i2 = 0; i2 < renderChunksTall; i2++) {
					...
					WorldRenderer worldrenderer = worldRenderers[(j1
							* renderChunksTall + i2)
							* renderChunksWide + k];
	 ...
	 */
	public int sortAndRender(EntityLiving par1EntityLiving, int par2,
			double par3) {
		...
		for (int i = 0; i < 10; i++) {
			...
			if (worldrenderer.needsUpdate
					&& !worldRenderersToUpdate.contains(worldrenderer)) {
				worldRenderersToUpdate.add(worldrenderer);
			}
		}
		...
		double d = par1EntityLiving.lastTickPosX
				+ (par1EntityLiving.posX - par1EntityLiving.lastTickPosX)
				* par3;
		double d1 = par1EntityLiving.lastTickPosY
				+ (par1EntityLiving.posY - par1EntityLiving.lastTickPosY)
				* par3;
		double d2 = par1EntityLiving.lastTickPosZ
		...
		if (occlusionEnabled && mc.gameSettings.advancedOpengl
			...
			do {
				...
				for (int j1 = byte0; j1 < l; j1++) {
					...
					if (!sortedWorldRenderers[j1].isInFrustum
							|| sortedWorldRenderers[j1].isWaitingOnOcclusionQuery) {
						continue;
					}

					float f3 = MathHelper.sqrt_float(sortedWorldRenderers[j1]
					...
					ARBOcclusionQuery.glBeginQueryARB(
							...
							sortedWorldRenderers[j1].glOcclusionQuery);
			...
			Profiler.endStartSection("render");
			j += renderSortedRenderers(0, sortedWorldRenderers.length, par2,
					par3);
		}
	...
	private void checkOcclusionQueryResult(int par1, int par2) {
		for (int i = par1; i < par2; i++) {
			...
			ARBOcclusionQuery.glGetQueryObjectuARB(
					sortedWorldRenderers[i].glOcclusionQuery,
			...
			if (occlusionResult.get(0) != 0) {
				...
				ARBOcclusionQuery.glGetQueryObjectuARB(
						sortedWorldRenderers[i].glOcclusionQuery,
						ARBOcclusionQuery.GL_QUERY_RESULT_ARB, occlusionResult);
	...
	private int renderSortedRenderers(int par1, int par2, int par3, double par4) {
		...
		for (int j = par1; j < par2; j++) {
			if (par3 == 0) {
				...
				if (sortedWorldRenderers[j].skipRenderPass[par3]) {
					...
					renderersBeingClipped++;
						...
						&& !sortedWorldRenderers[j].isVisible) {
			...
			if (sortedWorldRenderers[j].skipRenderPass[par3]
					|| !sortedWorldRenderers[j].isInFrustum || occlusionEnabled
					&& !sortedWorldRenderers[j].isVisible) {
				continue;
			}
		...
		EntityLiving entityliving = mc.renderViewEntity;
		double d = entityliving.lastTickPosX
				+ (entityliving.posX - entityliving.lastTickPosX) * par4;
		double d1 = entityliving.lastTickPosY
				+ (entityliving.posY - entityliving.lastTickPosY) * par4;
		double d2 = entityliving.lastTickPosZ
				+ (entityliving.posZ - entityliving.lastTickPosZ) * par4;
		int l = 0;
		...
		for (int j1 = 0; j1 < glRenderLists.size(); j1++) {
			...
			for (int l1 = 0; l1 < l; l1++) {
				if (allRenderLists[l1].func_862_a(worldrenderer.posXMinus,
						worldrenderer.posYMinus, worldrenderer.posZMinus)) {
					k1 = l1;
				}
			}
			...
			if (k1 < 0) {
				k1 = l++;
				allRenderLists[k1].func_861_a(worldrenderer.posXMinus,
						worldrenderer.posYMinus, worldrenderer.posZMinus, d,
						d1, d2);
			}

			allRenderLists[k1].func_858_a(worldrenderer
	...
	public void renderSky(float par1) {
		...
		GL11.glBlendFunc(GL11.GL_SRC_ALPHA, GL11.GL_ONE_MINUS_SRC_ALPHA);
		RenderHelper.disableStandardItemLighting();
		float af[] = worldObj.worldProvider.calcSunriseSunsetColors(
				worldObj.getCelestialAngle(par1), par1);
		...
		if (af != null) {
			...
			GL11.glPushMatrix();
			GL11.glRotatef(90F, 1.0F, 0.0F, 0.0F);
			GL11.glRotatef(MathHelper.sin(worldObj
					.getCelestialAngleRadians(par1)) >= 0.0F ? 0.0F : 180F,
					0.0F, 0.0F, 1.0F);
			GL11.glRotatef(90F, 0.0F, 0.0F, 1.0F);
			float f6 = af[0];
			float f8 = af[1];
			...
			for (int k = 0; k <= j; k++) {
				...
				tessellator1.addVertex(f22 * 120F, f23 * 120F, -f23 * 40F
						* af[3]);
			}
		...
		GL11.glRotatef(worldObj.getCelestialAngle(par1) * 360F, 1.0F, 0.0F,
				0.0F);
		float f15 = 30F;
		GL11.glBindTexture(GL11.GL_TEXTURE_2D,
				renderEngine.getTexture("/terrain/sun.png"));
		...
		f15 = 20F;
		GL11.glBindTexture(GL11.GL_TEXTURE_2D,
				renderEngine.getTexture("/terrain/moon_phases.png"));
		...
		if (worldObj.worldProvider.isSkyColored()) {
			GL11.glColor3f(f * 0.2F + 0.04F, f1 * 0.2F + 0.04F,
					f2 * 0.6F + 0.1F);
		} else {
			GL11.glColor3f(f, f1, f2);
		}
	...
	public void renderClouds(float par1) {
		...
		float f = (float) (mc.renderViewEntity.lastTickPosY + (mc.renderViewEntity.posY - mc.renderViewEntity.lastTickPosY)
				* (double) par1);
		byte byte0 = 32;
		int i = 256 / byte0;
		Tessellator tessellator = Tessellator.instance;
		GL11.glBindTexture(GL11.GL_TEXTURE_2D,
				renderEngine.getTexture("/environment/clouds.png"));
		GL11.glEnable(GL11.GL_BLEND);
		GL11.glBlendFunc(GL11.GL_SRC_ALPHA, GL11.GL_ONE_MINUS_SRC_ALPHA);
		...
		double d1 = mc.renderViewEntity.prevPosX
				...
				* (double) par1 + d * 0.029999999329447746D;
		...
		for (int l = -byte0 * i; l < byte0 * i; l += byte0) {
			for (int i1 = -byte0 * i; i1 < byte0 * i; i1 += byte0) {
				tessellator.addVertexWithUV(l + 0, f8, i1 + byte0,
						(float) (l + 0) * f5 + f9, (float) (i1 + byte0) * f5
								+ f10);
				tessellator.addVertexWithUV(l + byte0, f8, i1 + byte0,
						(float) (l + byte0) * f5 + f9, (float) (i1 + byte0)
								* f5 + f10);
				tessellator.addVertexWithUV(l + byte0, f8, i1 + 0,
						(float) (l + byte0) * f5 + f9, (float) (i1 + 0) * f5
								+ f10);
				tessellator.addVertexWithUV(l + 0, f8, i1 + 0, (float) (l + 0)
						* f5 + f9, (float) (i1 + 0) * f5 + f10);
			}
		}
	...
	public void renderCloudsFancy(float par1) {
		...
		double d1 = (mc.renderViewEntity.prevPosX
				...
				* (double) par1 + d * 0.029999999329447746D)
				/ (double) f1;
		double d2 = (mc.renderViewEntity.prevPosZ + (mc.renderViewEntity.posZ - mc.renderViewEntity.prevPosZ)
				* (double) par1)
				/ (double) f1 + 0.33000001311302185D;
		...
		for (int l = 0; l < 2; l++) {
			...
			for (int i1 = -byte0 + 1; i1 <= byte0; i1++) {
				for (int j1 = -byte0 + 1; j1 <= byte0; j1++) {
					...
					if (f3 > -f2 - 1.0F) {
						tessellator.setColorRGBA_F(f4 * 0.7F, f5 * 0.7F,
								f6 * 0.7F, 0.8F);
						tessellator.setNormal(0.0F, -1F, 0.0F);
						tessellator.addVertexWithUV(f18 + 0.0F, f3 + 0.0F, f19
								+ (float) k, (f16 + 0.0F) * f12 + f8,
								(f17 + (float) k) * f12 + f10);
						tessellator.addVertexWithUV(f18 + (float) k, f3 + 0.0F,
								f19 + (float) k, (f16 + (float) k) * f12 + f8,
								(f17 + (float) k) * f12 + f10);
						tessellator.addVertexWithUV(f18 + (float) k, f3 + 0.0F,
								f19 + 0.0F, (f16 + (float) k) * f12 + f8,
								(f17 + 0.0F) * f12 + f10);
						tessellator.addVertexWithUV(f18 + 0.0F, f3 + 0.0F,
								f19 + 0.0F, (f16 + 0.0F) * f12 + f8,
								(f17 + 0.0F) * f12 + f10);
					}
					...
					if (f3 <= f2 + 1.0F) {
						...
						tessellator.addVertexWithUV(f18 + 0.0F,
								(f3 + f2) - f15, f19 + (float) k, (f16 + 0.0F)
										* f12 + f8, (f17 + (float) k) * f12
										+ f10);
						tessellator.addVertexWithUV(f18 + (float) k, (f3 + f2)
								- f15, f19 + (float) k, (f16 + (float) k) * f12
								+ f8, (f17 + (float) k) * f12 + f10);
						tessellator.addVertexWithUV(f18 + (float) k, (f3 + f2)
								- f15, f19 + 0.0F,
								(f16 + (float) k) * f12 + f8, (f17 + 0.0F)
										* f12 + f10);
						tessellator.addVertexWithUV(f18 + 0.0F,
								(f3 + f2) - f15, f19 + 0.0F, (f16 + 0.0F) * f12
										+ f8, (f17 + 0.0F) * f12 + f10);
					}

					tessellator.setColorRGBA_F(f4 * 0.9F, f5 * 0.9F, f6 * 0.9F,
							0.8F);
					...
					if (i1 > -1) {
						...
						for (int k1 = 0; k1 < k; k1++) {
							tessellator.addVertexWithUV(
									f18 + (float) k1 + 0.0F, f3 + 0.0F, f19
											+ (float) k,
									(f16 + (float) k1 + 0.5F) * f12 + f8,
									(f17 + (float) k) * f12 + f10);
							tessellator.addVertexWithUV(
									f18 + (float) k1 + 0.0F, f3 + f2, f19
											+ (float) k,
									(f16 + (float) k1 + 0.5F) * f12 + f8,
									(f17 + (float) k) * f12 + f10);
							tessellator.addVertexWithUV(
									f18 + (float) k1 + 0.0F, f3 + f2,
									f19 + 0.0F, (f16 + (float) k1 + 0.5F) * f12
											+ f8, (f17 + 0.0F) * f12 + f10);
							tessellator.addVertexWithUV(
									f18 + (float) k1 + 0.0F, f3 + 0.0F,
									f19 + 0.0F, (f16 + (float) k1 + 0.5F) * f12
											+ f8, (f17 + 0.0F) * f12 + f10);
						}
					}
					...
					if (i1 <= 1) {
						...
						for (int l1 = 0; l1 < k; l1++) {
							tessellator.addVertexWithUV(
									(f18 + (float) l1 + 1.0F) - f15, f3 + 0.0F,
									f19 + (float) k, (f16 + (float) l1 + 0.5F)
											* f12 + f8, (f17 + (float) k) * f12
											+ f10);
							tessellator.addVertexWithUV(
									(f18 + (float) l1 + 1.0F) - f15, f3 + f2,
									f19 + (float) k, (f16 + (float) l1 + 0.5F)
											* f12 + f8, (f17 + (float) k) * f12
											+ f10);
							tessellator.addVertexWithUV(
									(f18 + (float) l1 + 1.0F) - f15, f3 + f2,
									f19 + 0.0F, (f16 + (float) l1 + 0.5F) * f12
											+ f8, (f17 + 0.0F) * f12 + f10);
							tessellator.addVertexWithUV(
									(f18 + (float) l1 + 1.0F) - f15, f3 + 0.0F,
									f19 + 0.0F, (f16 + (float) l1 + 0.5F) * f12
					...
					tessellator.setColorRGBA_F(f4 * 0.8F, f5 * 0.8F, f6 * 0.8F,
							0.8F);
					...
					if (j1 > -1) {
						...
						for (int i2 = 0; i2 < k; i2++) {
							tessellator.addVertexWithUV(f18 + 0.0F, f3 + f2,
									f19 + (float) i2 + 0.0F, (f16 + 0.0F) * f12
											+ f8, (f17 + (float) i2 + 0.5F)
											* f12 + f10);
							tessellator.addVertexWithUV(f18 + (float) k, f3
									+ f2, f19 + (float) i2 + 0.0F,
									(f16 + (float) k) * f12 + f8, (f17
											+ (float) i2 + 0.5F)
											* f12 + f10);
							tessellator.addVertexWithUV(f18 + (float) k,
									f3 + 0.0F, f19 + (float) i2 + 0.0F,
									(f16 + (float) k) * f12 + f8, (f17
											+ (float) i2 + 0.5F)
											* f12 + f10);
							tessellator.addVertexWithUV(f18 + 0.0F, f3 + 0.0F,
					...
					if (j1 <= 1) {
						...
						for (int j2 = 0; j2 < k; j2++) {
							tessellator.addVertexWithUV(f18 + 0.0F, f3 + f2,
									(f19 + (float) j2 + 1.0F) - f15,
									(f16 + 0.0F) * f12 + f8,
									(f17 + (float) j2 + 0.5F) * f12 + f10);
							tessellator.addVertexWithUV(f18 + (float) k, f3
									+ f2, (f19 + (float) j2 + 1.0F) - f15,
									(f16 + (float) k) * f12 + f8, (f17
											+ (float) j2 + 0.5F)
											* f12 + f10);
							tessellator.addVertexWithUV(f18 + (float) k,
									f3 + 0.0F, (f19 + (float) j2 + 1.0F) - f15,
									(f16 + (float) k) * f12 + f8, (f17
											+ (float) j2 + 0.5F)
											* f12 + f10);
							tessellator.addVertexWithUV(f18 + 0.0F, f3 + 0.0F,
	...
	public boolean updateRenderers(EntityLiving par1EntityLiving, boolean par2) {
		...
		if (flag) {
			Collections.sort(worldRenderersToUpdate, new RenderSorter(
					par1EntityLiving));
			int i = worldRenderersToUpdate.size() - 1;
			int j = worldRenderersToUpdate.size();

			for (int k = 0; k < j; k++) {
				WorldRenderer worldrenderer = (WorldRenderer) worldRenderersToUpdate
						.get(i - k);
		...
		for (int j1 = 0; j1 < l; j1++) {
			WorldRenderer worldrenderer1 = (WorldRenderer) worldRenderersToUpdate
					.get(j1);
			...
			if (!par2) {
				if (worldrenderer1.distanceToEntitySquared(par1EntityLiving) > 256F) {
					...
					for (k2 = 0; k2 < byte0
							&& (aworldrenderer[k2] == null || rendersorter
									.doCompare(aworldrenderer[k2],
											worldrenderer1) <= 0); k2++) {
					}
		...
		if (arraylist != null) {
			...
			for (int k1 = arraylist.size() - 1; k1 >= 0; k1--) {
				WorldRenderer worldrenderer2 = (WorldRenderer) arraylist
						.get(k1);
		...
		for (int j3 = worldRenderersToUpdate.size(); j2 != j3; j2++) {
			WorldRenderer worldrenderer4 = (WorldRenderer) worldRenderersToUpdate
					.get(j2);
	...
	public void drawBlockBreaking(EntityPlayer par1EntityPlayer,
			MovingObjectPosition par2MovingObjectPosition, int par3,
			ItemStack par4ItemStack, float par5) {
		Tessellator tessellator = Tessellator.instance;
		GL11.glEnable(GL11.GL_BLEND);
		GL11.glEnable(GL11.GL_ALPHA_TEST);
		GL11.glBlendFunc(GL11.GL_SRC_ALPHA, GL11.GL_ONE);
		...
		if (par3 == 0) {
			if (damagePartialTime > 0.0F) {
				...
				GL11.glPushMatrix();
				int j = worldObj.getBlockId(par2MovingObjectPosition.blockX,
				...
				double d = par1EntityPlayer.lastTickPosX
				...
				globalRenderBlocks.renderBlockUsingTexture(block,
						par2MovingObjectPosition.blockX,
						par2MovingObjectPosition.blockY,
						par2MovingObjectPosition.blockZ,
						240 + (int) (damagePartialTime * 10F));
				tessellator.draw();
			...
			GL11.glColor4f(
					...
					f,
					f,
					MathHelper.sin((float) System.currentTimeMillis() / 200F) * 0.2F + 0.5F);
	...
	public void drawSelectionBox(EntityPlayer par1EntityPlayer,
		...
		if (par3 == 0
			...
			GL11.glDepthMask(false);
			float f = 0.002F;
			int i = worldObj.getBlockId(par2MovingObjectPosition.blockX,
					par2MovingObjectPosition.blockY,
					par2MovingObjectPosition.blockZ);

			if (i > 0) {
				Block.blocksList[i].setBlockBoundsBasedOnState(worldObj,
						par2MovingObjectPosition.blockX,
						par2MovingObjectPosition.blockY,
						par2MovingObjectPosition.blockZ);
				double d = par1EntityPlayer.lastTickPosX
				...
				double d2 = par1EntityPlayer.lastTickPosZ
						+ (par1EntityPlayer.posZ - par1EntityPlayer.lastTickPosZ)
						* (double) par5;
				drawOutlinedBoundingBox(Block.blocksList[i]
						.getSelectedBoundingBoxFromPool(worldObj,
								par2MovingObjectPosition.blockX,
								par2MovingObjectPosition.blockY,
								par2MovingObjectPosition.blockZ)
						.expand(f, f, f).getOffsetBoundingBox(-d, -d1, -d2));
			}
		...
		for (int k1 = i; k1 <= l; k1++) {
			...
			for (int i2 = j; i2 <= i1; i2++) {
				...
				for (int k2 = k; k2 <= j1; k2++) {
					...
					int i3 = (l2 * renderChunksTall + j2) * renderChunksWide
							+ l1;
	 ...
	 * (could be all the same renderer potentially) Args: x, y, z
	 */
	...
	/**
	 * As of mc 1.2.3 this method has exactly the same signature and does
	 * exactly the same as markBlockNeedsUpdate
	 */
	...
	/**
	 * Called across all registered IWorldAccess instances when a block range is
	 * invalidated. Args: minX, minY, minZ, maxX, maxY, maxZ
	 */
	public void markBlockRangeNeedsUpdate(int par1, int par2, int par3,
	...
	public void clipRenderersByFrustum(ICamera par1ICamera, float par2) {
		for (int i = 0; i < worldRenderers.length; i++) {
			if (!worldRenderers[i].skipAllRenderPasses()
					&& (!worldRenderers[i].isInFrustum || (i
							+ frustumCheckOffset & 0xf) == 0)) {
	...
	public void playRecord(String par1Str, int par2, int par3, int par4) {
		if (par1Str != null) {
			mc.ingameGUI.setRecordPlayingMessage((new StringBuilder())
					.append("C418 - ").append(par1Str).toString());
		}
	...
	public void playSound(String par1Str, double par2, double par4,
		...
		if (mc.renderViewEntity.getDistanceSq(par2, par4, par6) < (double) (f * f)) {
			mc.sndManager.playSound(par1Str, (float) par2, (float) par4,
					(float) par6, par8, par9);
		}
	}
	...
	public EntityFX func_40193_b(String par1Str, double par2, double par4,
			double par6, double par8, double par10, double par12) {
		...
		if (par1Str.equals("hugeexplosion")) {
			mc.effectRenderer
					.addEffect(((EntityFX) (obj = new EntityHugeExplodeFX(
							worldObj, par2, par4, par6, par8, par10, par12))));
		} else if (par1Str.equals("largeexplode")) {
			mc.effectRenderer
					.addEffect(((EntityFX) (obj = new EntityLargeExplodeFX(
							renderEngine, worldObj, par2, par4, par6, par8,
							par10, par12))));
		}
		...
		if (par1Str.equals("bubble")) {
			obj = new EntityBubbleFX(worldObj, par2, par4, par6, par8, par10,
					par12);
		} else if (par1Str.equals("suspended")) {
			obj = new EntitySuspendFX(worldObj, par2, par4, par6, par8, par10,
					par12);
		} else if (par1Str.equals("depthsuspend")) {
			obj = new EntityAuraFX(worldObj, par2, par4, par6, par8, par10,
					par12);
		} else if (par1Str.equals("townaura")) {
			obj = new EntityAuraFX(worldObj, par2, par4, par6, par8, par10,
					par12);
		} else if (par1Str.equals("crit")) {
			obj = new EntityCritFX(worldObj, par2, par4, par6, par8, par10,
					par12);
		} else if (par1Str.equals("magicCrit")) {
			obj = new EntityCritFX(worldObj, par2, par4, par6, par8, par10,
					par12);
			((EntityFX) (obj)).func_40097_b(
					((EntityFX) (obj)).func_40098_n() * 0.3F,
					((EntityFX) (obj)).func_40101_o() * 0.8F,
					((EntityFX) (obj)).func_40102_p());
			((EntityFX) (obj)).setParticleTextureIndex(((EntityFX) (obj))
					.getParticleTextureIndex() + 1);
		} else if (par1Str.equals("smoke")) {
			obj = new EntitySmokeFX(worldObj, par2, par4, par6, par8, par10,
					par12);
		} else if (par1Str.equals("mobSpell")) {
			obj = new EntitySpellParticleFX(worldObj, par2, par4, par6, 0.0D,
					0.0D, 0.0D);
			((EntityFX) (obj)).func_40097_b((float) par8, (float) par10,
					(float) par12);
		} else if (par1Str.equals("spell")) {
			obj = new EntitySpellParticleFX(worldObj, par2, par4, par6, par8,
					par10, par12);
			...
			obj = new EntityNoteFX(worldObj, par2, par4, par6, par8, par10,
					par12);
		} else if (par1Str.equals("portal")) {
			obj = new EntityPortalFX(worldObj, par2, par4, par6, par8, par10,
					par12);
		} else if (par1Str.equals("enchantmenttable")) {
			obj = new EntityEnchantmentTableParticleFX(worldObj, par2, par4,
					par6, par8, par10, par12);
		} else if (par1Str.equals("explode")) {
			obj = new EntityExplodeFX(worldObj, par2, par4, par6, par8, par10,
					par12);
		} else if (par1Str.equals("flame")) {
			obj = new EntityFlameFX(worldObj, par2, par4, par6, par8, par10,
					par12);
			...
			obj = new EntitySplashFX(worldObj, par2, par4, par6, par8, par10,
					par12);
		} else if (par1Str.equals("largesmoke")) {
			obj = new EntitySmokeFX(worldObj, par2, par4, par6, par8, par10,
					par12, 2.5F);
		} else if (par1Str.equals("cloud")) {
			obj = new EntityCloudFX(worldObj, par2, par4, par6, par8, par10,
					par12);
		} else if (par1Str.equals("reddust")) {
			obj = new EntityReddustFX(worldObj, par2, par4, par6, (float) par8,
					(float) par10, (float) par12);
		} else if (par1Str.equals("snowballpoof")) {
			obj = new EntityBreakingFX(worldObj, par2, par4, par6,
					Item.snowball);
		} else if (par1Str.equals("dripWater")) {
			obj = new EntityDropParticleFX(worldObj, par2, par4, par6,
					Material.water);
		} else if (par1Str.equals("dripLava")) {
			obj = new EntityDropParticleFX(worldObj, par2, par4, par6,
					Material.lava);
		} else if (par1Str.equals("snowshovel")) {
			obj = new EntitySnowShovelFX(worldObj, par2, par4, par6, par8,
					par10, par12);
		} else if (par1Str.equals("slime")) {
			obj = new EntityBreakingFX(worldObj, par2, par4, par6,
					Item.slimeBall);
		} else if (par1Str.equals("heart")) {
			obj = new EntityHeartFX(worldObj, par2, par4, par6, par8, par10,
					par12);
		} else if (par1Str.startsWith("iconcrack_")) {
			int j = Integer
					.parseInt(par1Str.substring(par1Str.indexOf("_") + 1));
			obj = new EntityBreakingFX(worldObj, par2, par4, par6, par8, par10,
					par12, Item.itemsList[j]);
		} else if (par1Str.startsWith("tilecrack_")) {
			int k = Integer
					.parseInt(par1Str.substring(par1Str.indexOf("_") + 1));
			obj = new EntityDiggingFX(worldObj, par2, par4, par6, par8, par10,
					par12, Block.blocksList[k], 0, 0);
		}
	 ...
	 */
	public void doNothingWithTileEntity(int i, int j, int k,
	...
	public void playAuxSFX(EntityPlayer par1EntityPlayer, int par2, int par3,
		...
		case 1001:
			worldObj.playSoundEffect(par3, par4, par5, "random.click", 1.0F,
					1.2F);
			break;
		...
		case 2000:
			int i = par6 % 3 - 1;
			int l = (par6 / 3) % 3 - 1;
			double d3 = (double) par3 + (double) i * 0.59999999999999998D
					+ 0.5D;
			double d7 = (double) par4 + 0.5D;
			double d11 = (double) par5 + (double) l * 0.59999999999999998D
					+ 0.5D;

			for (int l1 = 0; l1 < 10; l1++) {
				double d13 = random.nextDouble() * 0.20000000000000001D + 0.01D;
				double d14 = d3 + (double) i * 0.01D
						+ (random.nextDouble() - 0.5D) * (double) l * 0.5D;
				double d15 = d7 + (random.nextDouble() - 0.5D) * 0.5D;
				double d17 = d11 + (double) l * 0.01D
						+ (random.nextDouble() - 0.5D) * (double) i * 0.5D;
				double d19 = (double) i * d13 + random.nextGaussian() * 0.01D;
				double d21 = -0.029999999999999999D + random.nextGaussian()
						* 0.01D;
		...
		case 2003:
			...
			String s = (new StringBuilder()).append("iconcrack_")
					.append(Item.eyeOfEnder.shiftedIndex).toString();

			for (int i1 = 0; i1 < 8; i1++) {
				spawnParticle(s, d, d4, d8,
						random.nextGaussian() * 0.14999999999999999D,
						random.nextDouble() * 0.20000000000000001D,
						random.nextGaussian() * 0.14999999999999999D);
			}

			for (double d12 = 0.0D; d12 < (Math.PI * 2D); d12 += 0.15707963267948966D) {
				spawnParticle("portal", d + Math.cos(d12) * 5D,
						d4 - 0.40000000000000002D, d8 + Math.sin(d12) * 5D,
						Math.cos(d12) * -5D, 0.0D, Math.sin(d12) * -5D);
				spawnParticle("portal", d + Math.cos(d12) * 5D,
						d4 - 0.40000000000000002D, d8 + Math.sin(d12) * 5D,
						Math.cos(d12) * -7D, 0.0D, Math.sin(d12) * -7D);
			}
		...
		case 2002:
			...
			double d5 = par4;
			double d9 = par5;
			String s1 = (new StringBuilder()).append("iconcrack_")
					.append(Item.potion.shiftedIndex).toString();

			for (int j1 = 0; j1 < 8; j1++) {
				spawnParticle(s1, d1, d5, d9,
						random.nextGaussian() * 0.14999999999999999D,
						random.nextDouble() * 0.20000000000000001D,
						random.nextGaussian() * 0.14999999999999999D);
			}
			...
			for (int i2 = 0; i2 < 100; i2++) {
				...
				EntityFX entityfx = func_40193_b(s2, d1 + d20
						* 0.10000000000000001D, d5 + 0.29999999999999999D, d9
						+ d24 * 0.10000000000000001D, d20, d22, d24);
			...
			worldObj.playSoundEffect((double) par3 + 0.5D,
					(double) par4 + 0.5D, (double) par5 + 0.5D, "random.glass",
		...
		case 2001:
			...
			if (j > 0) {
				...
				mc.sndManager.playSound(block.stepSound.getBreakSound(),
						(float) par3 + 0.5F, (float) par4 + 0.5F,
						(float) par5 + 0.5F,
			...
			mc.effectRenderer.addBlockDestroyEffects(par3, par4, par5,
					par6 & 0xfff, par6 >> 12 & 0xff);
			break;

		case 2004:
			for (int k = 0; k < 20; k++) {
				double d2 = (double) par3 + 0.5D
						+ ((double) worldObj.rand.nextFloat() - 0.5D) * 2D;
				double d6 = (double) par4 + 0.5D
						+ ((double) worldObj.rand.nextFloat() - 0.5D) * 2D;
				double d10 = (double) par5 + 0.5D
						+ ((double) worldObj.rand.nextFloat() - 0.5D) * 2D;
		...
		case 1003:
			if (Math.random() < 0.5D) {
				worldObj.playSoundEffect((double) par3 + 0.5D,
						(double) par4 + 0.5D, (double) par5 + 0.5D,
		...
		case 1004:
			worldObj.playSoundEffect((float) par3 + 0.5F, (float) par4 + 0.5F,
