...
public class ModelBlaze extends ModelBase {
	...
	public void setRotationAngles(float par1, float par2, float par3,
		...
		for (int i = 0; i < 4; i++) {
			field_40323_a[i].rotationPointY = -2F
					+ MathHelper.cos(((float) (i * 2) + par3) * 0.25F);
		...
		for (int j = 4; j < 8; j++) {
			field_40323_a[j].rotationPointY = 2.0F + MathHelper
					.cos(((float) (j * 2) + par3) * 0.25F);
		...
		for (int k = 8; k < 12; k++) {
			field_40323_a[k].rotationPointY = 11F + MathHelper
					.cos(((float) k * 1.5F + par3) * 0.5F);
