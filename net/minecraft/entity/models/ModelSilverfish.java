package net.minecraft.entity.models;
...
public class ModelSilverfish extends ModelBase {
	...
	private static final int silverfishBoxLength[][] = { { 3, 2, 2 },
			{ 4, 3, 2 }, { 6, 4, 3 }, { 3, 3, 3 }, { 2, 2, 3 }, { 2, 1, 2 },
			{ 1, 1, 2 } };
	private static final int silverfishTexturePositions[][] = { { 0, 0 },
			{ 0, 4 }, { 0, 9 }, { 0, 16 }, { 0, 22 }, { 11, 0 }, { 13, 4 } };
	...
	public ModelSilverfish() {
		...
		for (int i = 0; i < silverfishBodyParts.length; i++) {
			silverfishBodyParts[i] = new ModelRenderer(this,
					silverfishTexturePositions[i][0],
					silverfishTexturePositions[i][1]);
			silverfishBodyParts[i].addBox((float) silverfishBoxLength[i][0]
					* -0.5F, 0.0F, (float) silverfishBoxLength[i][2] * -0.5F,
					silverfishBoxLength[i][0], silverfishBoxLength[i][1],
					silverfishBoxLength[i][2]);
			silverfishBodyParts[i].setRotationPoint(0.0F,
					24 - silverfishBoxLength[i][1], f);
			field_35399_c[i] = f;
		...
		silverfishWings[0].addBox(-5F, 0.0F, (float) silverfishBoxLength[2][2]
				* -0.5F, 10, 8, silverfishBoxLength[2][2]);
		silverfishWings[0].setRotationPoint(0.0F, 16F, field_35399_c[2]);
		silverfishWings[1] = new ModelRenderer(this, 20, 11);
		silverfishWings[1].addBox(-3F, 0.0F, (float) silverfishBoxLength[4][2]
				* -0.5F, 6, 4, silverfishBoxLength[4][2]);
		silverfishWings[1].setRotationPoint(0.0F, 20F, field_35399_c[4]);
		silverfishWings[2] = new ModelRenderer(this, 20, 18);
		silverfishWings[2].addBox(-3F, 0.0F, (float) silverfishBoxLength[4][2]
				* -0.5F, 6, 5, silverfishBoxLength[1][2]);
	...
	public void setRotationAngles(float par1, float par2, float par3,
		...
		for (int i = 0; i < silverfishBodyParts.length; i++) {
			silverfishBodyParts[i].rotateAngleY = MathHelper.cos(par3 * 0.9F
					+ (float) i * 0.15F * (float) Math.PI)
					* (float) Math.PI * 0.05F * (float) (1 + Math.abs(i - 2));
			silverfishBodyParts[i].rotationPointX = MathHelper.sin(par3 * 0.9F
					+ (float) i * 0.15F * (float) Math.PI)
					* (float) Math.PI * 0.2F * (float) Math.abs(i - 2);
		}
