...
public class ModelWolf extends ModelBase {
	...
	public ModelWolf() {
		...
		wolfHeadMain.setTextureOffset(16, 14)
				.addBox(-3F, -5F, 0.0F, 2, 2, 1, f);
		wolfHeadMain.setTextureOffset(16, 14).addBox(1.0F, -5F, 0.0F, 2, 2, 1,
				f);
		wolfHeadMain.setTextureOffset(0, 10).addBox(-1.5F, 0.0F, -5F, 3, 3, 4,
				f);
	}
	...
	public void setLivingAnimations(EntityLiving par1EntityLiving, float par2,
		...
		wolfHeadMain.rotateAngleZ = entitywolf.getInterestedAngle(par4)
		...
		if (entitywolf.getWolfShaking()) {
			float f = entitywolf.getBrightness(par4)
					* entitywolf.getShadingWhileShaking(par4);
			GL11.glColor3f(f, f, f);
		}

		float f1 = (0.5F + Math.max(par3,
				entitywolf.getInterestedAngle(par4) * 1.5F)) * 0.25F;
		...
		if (!entitywolf.isTamed()) {
			...
			for (int i = 0; i < tails.length; i++) {
				...
				tails[i].rotateAngleX = MathHelper.sin(f2 - (float) (i + 1)
						* 0.75F)
						* f1;
			}
		} else {
			...
			for (int j = 0; j < tails.length; j++) {
				...
				tails[j].rotateAngleZ = MathHelper.sin(f2 - (float) (j + 1)
						* 0.75F)
						* f1;
			}
		}
