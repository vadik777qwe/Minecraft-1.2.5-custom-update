...
public class EntityOtherPlayerMP extends EntityPlayer {
	...
	public EntityOtherPlayerMP(World par1World, String par2Str) {
		...
		if (par2Str != null && par2Str.length() > 0) {
			skinUrl = (new StringBuilder())
					...
					.append(par2Str).append(".png").toString();
		}
	...
	public void onLivingUpdate() {
		...
		if (otherPlayerMPPosRotationIncrements > 0) {
			double d = posX + (otherPlayerMPX - posX)
					/ (double) otherPlayerMPPosRotationIncrements;
			double d1 = posY + (otherPlayerMPY - posY)
					/ (double) otherPlayerMPPosRotationIncrements;
			double d2 = posZ + (otherPlayerMPZ - posZ)
					/ (double) otherPlayerMPPosRotationIncrements;
			double d3;
			...
			rotationPitch += (otherPlayerMPPitch - (double) rotationPitch)
					/ (double) otherPlayerMPPosRotationIncrements;
