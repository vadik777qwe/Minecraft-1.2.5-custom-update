...
public class EntityGhast extends EntityFlying implements IMob {
	...
	protected void updateEntityActionState() {
		...
		if (targetedEntity != null
				&& targetedEntity.getDistanceSqToEntity(this) < d4 * d4) {
			...
			renderYawOffset = rotationYaw = (-(float) Math.atan2(d5, d7) * 180F)
					/ (float) Math.PI;

			if (canEntityBeSeen(targetedEntity)) {
				if (attackCounter == 10) {
					worldObj.playAuxSFXAtEntity(null, 1007, (int) posX,
							(int) posY, (int) posZ, 0);
				}
				...
				if (attackCounter == 20) {
					worldObj.playAuxSFXAtEntity(null, 1008, (int) posX,
							(int) posY, (int) posZ, 0);
					EntityFireball entityfireball = new EntityFireball(
							worldObj, this, d5, d6, d7);
					double d8 = 4D;
					Vec3D vec3d = getLook(1.0F);
					entityfireball.posX = posX + vec3d.xCoord * d8;
					entityfireball.posY = posY + (double) (height / 2.0F)
							+ 0.5D;
