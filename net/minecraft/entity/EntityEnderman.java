...
public class EntityEnderman extends EntityMob {
	...
	protected Entity findPlayerToAttack() {
		EntityPlayer entityplayer = worldObj
				.getClosestVulnerablePlayerToEntity(this, 64D);
	...
	private boolean shouldAttackPlayer(EntityPlayer par1EntityPlayer) {
		...
		Vec3D vec3d1 = Vec3D.createVector(
				...
				(boundingBox.minY + (double) (height / 2.0F))
						- (par1EntityPlayer.posY + (double) par1EntityPlayer
								.getEyeHeight()), posZ - par1EntityPlayer.posZ);
	...
	public void onLivingUpdate() {
			...
			if (getCarried() == 0) {
				if (rand.nextInt(20) == 0) {
					int i = MathHelper.floor_double((posX - 2D)
							+ rand.nextDouble() * 4D);
					int l = MathHelper.floor_double(posY + rand.nextDouble()
							* 3D);
					int j1 = MathHelper.floor_double((posZ - 2D)
							+ rand.nextDouble() * 4D);
				...
				int j = MathHelper.floor_double((posX - 1.0D)
						+ rand.nextDouble() * 2D);
				int i1 = MathHelper.floor_double(posY + rand.nextDouble() * 2D);
				int k1 = MathHelper.floor_double((posZ - 1.0D)
						+ rand.nextDouble() * 2D);
				int i2 = worldObj.getBlockId(j, i1, k1);
				int j2 = worldObj.getBlockId(j, i1 - 1, k1);

				if (i2 == 0 && j2 > 0
						&& Block.blocksList[j2].renderAsNormalBlock()) {
					worldObj.setBlockAndMetadataWithNotify(j, i1, k1,
							getCarried(), getCarryingData());
					setCarried(0);
				}
			}
		}
		...
		for (int k = 0; k < 2; k++) {
			worldObj.spawnParticle("portal", posX + (rand.nextDouble() - 0.5D)
					* (double) width, (posY + rand.nextDouble()
					* (double) height) - 0.25D, posZ
	...
	protected boolean teleportToEntity(Entity par1Entity) {
		Vec3D vec3d = Vec3D
				.createVector(
						...
						((boundingBox.minY + (double) (height / 2.0F)) - par1Entity.posY)
								+ (double) par1Entity.getEyeHeight(), posZ
								- par1Entity.posZ);
	...
	protected boolean teleportTo(double par1, double par3, double par5) {
		...
		if (worldObj.blockExists(i, j, k)) {
			...
			for (flag1 = false; !flag1 && j > 0;) {
				...
				if (i1 == 0
						|| !Block.blocksList[i1].blockMaterial.blocksMovement()) {
					posY--;
					j--;
				} else {
					flag1 = true;
		...
		for (int j1 = 0; j1 < l; j1++) {
			...
			double d4 = d + (posX - d) * d3 + (rand.nextDouble() - 0.5D)
					* (double) width * 2D;
			double d5 = d1 + (posY - d1) * d3 + rand.nextDouble()
					* (double) height;
			double d6 = d2 + (posZ - d2) * d3 + (rand.nextDouble() - 0.5D)
					* (double) width * 2D;
