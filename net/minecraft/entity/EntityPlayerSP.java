...
public class EntityPlayerSP extends EntityPlayer {
	...
	public EntityPlayerSP(Minecraft par1Minecraft, World par2World,
		...
		if (par3Session != null && par3Session.username != null
			...
			skinUrl = (new StringBuilder())
					...
					.append(par3Session.username).append(".png").toString();
		}
	...
	public void onLivingUpdate() {
		...
		if (inPortal) {
			...
			if (timeInPortal >= 1.0F) {
				...
				if (!worldObj.isRemote) {
					timeUntilPortal = 10;
					mc.sndManager.playSoundFX("portal.travel", 1.0F,
							rand.nextFloat() * 0.4F + 0.8F);
					byte byte0 = 0;
		...
		pushOutOfBlocks(posX - (double) width * 0.34999999999999998D,
				boundingBox.minY + 0.5D, posZ + (double) width
		...
		if (onGround && !flag1 && movementInput.moveForward >= f
				&& !isSprinting() && flag2 && !isUsingItem()
		...
		if (isSprinting()
				&& (movementInput.moveForward < f || isCollidedHorizontally || !flag2)) {
			setSprinting(false);
		}
