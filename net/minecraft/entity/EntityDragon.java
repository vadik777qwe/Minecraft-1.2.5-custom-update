...
public class EntityDragon extends EntityDragonBase {
	...
	public EntityDragon(World par1World) {
		...
		dragonPartArray = (new EntityDragonPart[] {
				dragonPartHead = new EntityDragonPart(this, "head", 6F, 6F),
				dragonPartBody = new EntityDragonPart(this, "body", 8F, 8F),
				dragonPartTail1 = new EntityDragonPart(this, "tail", 4F, 4F),
				dragonPartTail2 = new EntityDragonPart(this, "tail", 4F, 4F),
				dragonPartTail3 = new EntityDragonPart(this, "tail", 4F, 4F),
				dragonPartWing1 = new EntityDragonPart(this, "wing", 4F, 4F),
	...
	public double[] func_40160_a(int par1, float par2) {
		...
		ad[2] = field_40162_d[i][2]
				+ (field_40162_d[j][2] - field_40162_d[i][2]) * (double) par2;
		return ad;
	}
	...
	public void onLivingUpdate() {
		...
		if (worldObj.isRemote) {
			if (newPosRotationIncrements > 0) {
				...
				double d2 = posY + (newPosY - posY)
						/ (double) newPosRotationIncrements;
				double d4 = posZ + (newPosZ - posZ)
						/ (double) newPosRotationIncrements;
				double d6;
			...
			if (field_40163_ay || d7 < 100D || d7 > 22500D
			...
			Vec3D vec3d1 = Vec3D.createVector(
					MathHelper.sin((rotationYaw * (float) Math.PI) / 180F),
					motionY,
					-MathHelper.cos((rotationYaw * (float) Math.PI) / 180F))
					.normalize();
		...
		float f3 = (((float) (func_40160_a(5, 1.0F)[1] - func_40160_a(10, 1.0F)[1]) * 10F) / 180F)
				* (float) Math.PI;
		...
		dragonPartBody.setLocationAndAngles(posX + (double) (f8 * 0.5F), posY,
				posZ - (double) (f9 * 0.5F), 0.0F, 0.0F);
		dragonPartWing1.onUpdate();
		dragonPartWing1.setLocationAndAngles(posX + (double) (f9 * 4.5F),
				posY + 2D, posZ + (double) (f8 * 4.5F), 0.0F, 0.0F);
		dragonPartWing2.onUpdate();
		dragonPartWing2.setLocationAndAngles(posX - (double) (f9 * 4.5F),
				posY + 2D, posZ - (double) (f8 * 4.5F), 0.0F, 0.0F);
		...
		if (!worldObj.isRemote && maxHurtTime == 0) {
			collideWithEntities(worldObj.getEntitiesWithinAABBExcludingEntity(
					this, dragonPartWing1.boundingBox.expand(4D, 2D, 4D)
							.offset(0.0D, -2D, 0.0D)));
			collideWithEntities(worldObj.getEntitiesWithinAABBExcludingEntity(
					this, dragonPartWing2.boundingBox.expand(4D, 2D, 4D)
							.offset(0.0D, -2D, 0.0D)));
		...
		float f11 = MathHelper.sin((rotationYaw * (float) Math.PI) / 180F
				- randomYawVelocity * 0.01F);
		float f12 = MathHelper.cos((rotationYaw * (float) Math.PI) / 180F
				- randomYawVelocity * 0.01F);
		dragonPartHead.onUpdate();
		dragonPartHead.setLocationAndAngles(posX + (double) (f11 * 5.5F * f5),
				posY + (ad1[1] - ad[1]) * 1.0D + (double) (f6 * 5.5F), posZ
		...
		for (int j = 0; j < 3; j++) {
			...
			float f13 = (rotationYaw * (float) Math.PI)
					/ 180F
					+ ((simplifyAngle(ad2[0] - ad[0]) * (float) Math.PI) / 180F)
					* 1.0F;
	...
	private void updateDragonEnderCrystal() {
		...
		if (rand.nextInt(10) == 0) {
			...
			List list = worldObj.getEntitiesWithinAABB(
					net.minecraft.entity.EntityEnderCrystal.class,
	...
	private void collideWithEntities(List par1List) {
		...
		do {
			...
			if (entity instanceof EntityLiving) {
				...
				entity.addVelocity((d2 / d4) * 4D, 0.20000000298023224D,
						(d3 / d4) * 4D);
			}
		} while (true);
	}
	...
	private boolean destroyBlocksInAABB(AxisAlignedBB par1AxisAlignedBB) {
		...
		if (flag1) {
			double d = par1AxisAlignedBB.minX
					+ (par1AxisAlignedBB.maxX - par1AxisAlignedBB.minX)
					* (double) rand.nextFloat();
			double d1 = par1AxisAlignedBB.minY
					+ (par1AxisAlignedBB.maxY - par1AxisAlignedBB.minY)
					* (double) rand.nextFloat();
			double d2 = par1AxisAlignedBB.minZ
					+ (par1AxisAlignedBB.maxZ - par1AxisAlignedBB.minZ)
					* (double) rand.nextFloat();
	...
	public boolean attackEntityFromPart(EntityDragonPart par1EntityDragonPart,
			DamageSource par2DamageSource, int par3) {
		...
		if ((par2DamageSource.getEntity() instanceof EntityPlayer)
				|| par2DamageSource == DamageSource.explosion) {
	...
	protected void onDeathUpdate() {
		...
		if (!worldObj.isRemote && field_40178_aA > 150
				&& field_40178_aA % 5 == 0) {
			for (int i = 1000; i > 0;) {
	...
	private void createEnderPortal(int par1, int par2) {
				...
				for (int l = par2 - i; l <= par2 + i; l++) {
					...
					if (j < byte0) {
						if (d2 <= (double) (i - 1) - 0.5D) {
							worldObj.setBlockWithNotify(k, j, l,
		...
		worldObj.setBlockWithNotify(par1, byte0 + 0, par2,
				Block.bedrock.blockID);
		worldObj.setBlockWithNotify(par1, byte0 + 1, par2,
				Block.bedrock.blockID);
		worldObj.setBlockWithNotify(par1, byte0 + 2, par2,
				Block.bedrock.blockID);
		worldObj.setBlockWithNotify(par1 - 1, byte0 + 2, par2,
				Block.torchWood.blockID);
		worldObj.setBlockWithNotify(par1 + 1, byte0 + 2, par2,
				Block.torchWood.blockID);
		worldObj.setBlockWithNotify(par1, byte0 + 2, par2 - 1,
				Block.torchWood.blockID);
		worldObj.setBlockWithNotify(par1, byte0 + 2, par2 + 1,
				Block.torchWood.blockID);
		worldObj.setBlockWithNotify(par1, byte0 + 3, par2,
				Block.bedrock.blockID);
		worldObj.setBlockWithNotify(par1, byte0 + 4, par2,
