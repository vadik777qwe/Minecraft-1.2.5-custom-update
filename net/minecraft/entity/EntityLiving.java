...
import net.minecraft.enums.EnumCreatureAttribute;
...
public abstract class EntityLiving extends Entity {
	...
	public boolean func_48100_a(Class par1Class) {
		return (net.minecraft.entity.EntityCreeper.class) != par1Class
				&& (net.minecraft.entity.EntityGhast.class) != par1Class;
	}
	...
	public boolean isWithinHomeDistance(int par1, int par2, int par3) {
		if (maximumHomeDistance == -1F) {
			...
			return homePosition.getDistanceSquared(par1, par2, par3) < maximumHomeDistance
					* maximumHomeDistance;
		}
	}
	...
	public void onEntityUpdate() {
		...
		if (isEntityAlive()
				...
				&& !activePotionsMap.containsKey(Integer
						.valueOf(Potion.waterBreathing.id))) {
			...
			if (getAir() == -20) {
				...
				for (int i = 0; i < 8; i++) {
					...
					worldObj.spawnParticle("bubble", posX + (double) f, posY
							+ (double) f1, posZ + (double) f2, motionX,
							motionY, motionZ);
				}
	...
	protected void onDeathUpdate() {
		...
		if (deathTime == 20) {
			if (!worldObj.isRemote && (recentlyHit > 0 || isPlayer())
					&& !isChild()) {
	...
	public boolean attackEntityFrom(DamageSource par1DamageSource, int par2) {
		...
		if (flag) {
			...
			if (entity != null) {
				...
				for (d1 = entity.posZ - posZ; d * d + d1 * d1 < 0.0001D; d1 = (Math
		...
		if (health <= 0) {
			if (flag) {
				worldObj.playSoundAtEntity(this, getDeathSound(),
	...
	public void onDeath(DamageSource par1DamageSource) {
		...
		if (!worldObj.isRemote) {
			...
			if (entity instanceof EntityPlayer) {
				i = EnchantmentHelper
						.getLootingModifier(((EntityPlayer) entity).inventory);
			}
	...
	public void moveEntityWithHeading(float par1, float par2) {
		if (isInWater()) {
			...
			if (isCollidedHorizontally
					&& isOffsetPositionInLiquid(motionX,
							((motionY + 0.60000002384185791D) - posY) + d,
							motionZ)) {
			...
			if (isCollidedHorizontally
					&& isOffsetPositionInLiquid(motionX,
							((motionY + 0.60000002384185791D) - posY) + d1,
							motionZ)) {
				motionY = 0.30000001192092896D;
			}
		} else {
	...
	public void writeEntityToNBT(NBTTagCompound par1NBTTagCompound) {
		...
		if (!activePotionsMap.isEmpty()) {
			...
			for (Iterator iterator = activePotionsMap.values().iterator(); iterator
					.hasNext(); nbttaglist.appendTag(nbttagcompound)) {
				PotionEffect potioneffect = (PotionEffect) iterator.next();
				nbttagcompound = new NBTTagCompound();
				nbttagcompound.setByte("Id", (byte) potioneffect.getPotionID());
				nbttagcompound.setByte("Amplifier",
						(byte) potioneffect.getAmplifier());
				nbttagcompound.setInteger("Duration",
	...
	public void readEntityFromNBT(NBTTagCompound par1NBTTagCompound) {
		...
		if (par1NBTTagCompound.hasKey("ActiveEffects")) {
			...
			for (int i = 0; i < nbttaglist.tagCount(); i++) {
				...
				activePotionsMap.put(Integer.valueOf(byte0), new PotionEffect(
						byte0, j, byte1));
			}
		}
	}
	...
	protected void jump() {
		...
		if (isPotionActive(Potion.jump)) {
			motionY += (float) (getActivePotionEffect(Potion.jump)
					.getAmplifier() + 1) * 0.1F;
		}
	...
	protected void despawnEntity() {
		...
		if (entityplayer != null) {
			...
			if (entityAge > 600 && rand.nextInt(800) == 0 && d3 > 1024D
					&& canDespawn()) {
				setDead();
			} else if (d3 < 1024D) {
				entityAge = 0;
	...
	/**
	 * Called when the entity vanishes after dies by damage (or other method
	 * that put health below or at zero).
	 */
	...
	protected void updatePotionEffects() {
		...
		do {
			...
			PotionEffect potioneffect = (PotionEffect) activePotionsMap
					.get(integer);
		...
		if (potionsNeedUpdate) {
			if (!worldObj.isRemote) {
				if (!activePotionsMap.isEmpty()) {
					int i = PotionHelper
							.func_40354_a(activePotionsMap.values());
		...
		if (rand.nextBoolean()) {
			...
			if (j > 0) {
				...
				worldObj.spawnParticle("mobSpell", posX
	...
	public void addPotionEffect(PotionEffect par1PotionEffect) {
		...
		if (activePotionsMap.containsKey(Integer.valueOf(par1PotionEffect
				.getPotionID()))) {
			((PotionEffect) activePotionsMap.get(Integer
					.valueOf(par1PotionEffect.getPotionID())))
					.combine(par1PotionEffect);
			onChangedPotionEffect((PotionEffect) activePotionsMap.get(Integer
					.valueOf(par1PotionEffect.getPotionID())));
		} else {
			activePotionsMap.put(
					Integer.valueOf(par1PotionEffect.getPotionID()),
					par1PotionEffect);
	...
	/**
	 * This method return a value to be applyed directly to entity speed, this
	 * factor is less than 1 when a slowdown potion effect is applyed, more than
	 * 1 when a haste potion effect is applyed and 2 for fleeing entities.
	 */
	protected float getSpeedModifier() {
		float f = 1.0F;

		if (isPotionActive(Potion.moveSpeed)) {
			f *= 1.0F + 0.2F * (float) (getActivePotionEffect(Potion.moveSpeed)
					.getAmplifier() + 1);
		}

		if (isPotionActive(Potion.moveSlowdown)) {
			f *= 1.0F - 0.15F * (float) (getActivePotionEffect(
					Potion.moveSlowdown).getAmplifier() + 1);
		}
