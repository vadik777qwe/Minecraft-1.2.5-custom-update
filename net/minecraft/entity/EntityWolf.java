...
public class EntityWolf extends EntityTameable {
	 ...
	 * with tilted head. This happens when tamed wolf is wound and player holds
	 * porkchop (raw or cooked), or when wild wolf sees bone in player's hands.
	 */
	...
	public EntityWolf(World par1World) {
		...
		tasks.addTask(9, new EntityAIWatchClosest(this,
				net.minecraft.entity.EntityPlayer.class, 8F));
		tasks.addTask(9, new EntityAILookIdle(this));
		targetTasks.addTask(1, new EntityAIOwnerHurtByTarget(this));
		targetTasks.addTask(2, new EntityAIOwnerHurtTarget(this));
		targetTasks.addTask(3, new EntityAIHurtByTarget(this, true));
		targetTasks.addTask(4, new EntityAITargetNonTamed(this,
				net.minecraft.entity.EntitySheep.class, 16F, 200, false));
	}
	...
	public void onLivingUpdate() {
		...
		if (!worldObj.isRemote && isShaking && !field_25052_g && !hasPath()
				&& onGround) {
			field_25052_g = true;
	...
	public void onUpdate() {
		...
		if (isWet()) {
			...
			if (timeWolfIsShaking == 0.0F) {
				worldObj.playSoundAtEntity(this, "mob.wolf.shake",
						getSoundVolume(),
			...
			if (timeWolfIsShaking > 0.4F) {
				...
				int i = (int) (MathHelper.sin((timeWolfIsShaking - 0.4F)
						* (float) Math.PI) * 7F);

				for (int j = 0; j < i; j++) {
					float f1 = (rand.nextFloat() * 2.0F - 1.0F) * width * 0.5F;
					float f2 = (rand.nextFloat() * 2.0F - 1.0F) * width * 0.5F;
					worldObj.spawnParticle("splash", posX + (double) f1,
							f + 0.8F, posZ + (double) f2, motionX, motionY,
							motionZ);
				}
			}
		}
	...
	public float getShakeAngle(float par1, float par2) {
		...
		return MathHelper.sin(f * (float) Math.PI)
				* MathHelper.sin(f * (float) Math.PI * 11F) * 0.15F
				* (float) Math.PI;
	}

	public float getInterestedAngle(float par1) {
		return (field_25054_c + (field_25048_b - field_25054_c) * par1) * 0.15F
				* (float) Math.PI;
	}
	...
	public boolean interact(EntityPlayer par1EntityPlayer) {
		...
		if (!isTamed()) {
			...
			if (par1EntityPlayer.username.equalsIgnoreCase(getOwnerName())
					&& !worldObj.isRemote && !isWheat(itemstack)) {
	...
	public float getTailRotation() {
		...
		if (isTamed()) {
			return (0.55F - (float) (20 - dataWatcher.getWatchableObjectInt(18)) * 0.02F)
					* (float) Math.PI;
		} else {
	...
	public boolean isWheat(ItemStack par1ItemStack) {
		...
		if (!(Item.itemsList[par1ItemStack.itemID] instanceof ItemFood)) {
			...
			return ((ItemFood) Item.itemsList[par1ItemStack.itemID])
					.isWolfsFavoriteMeat();
		}
	}
