...
public class EntitySilverfish extends EntityMob {
	...
	public boolean attackEntityFrom(DamageSource par1DamageSource, int par2) {
		if (allySummonCooldown <= 0
				&& (par1DamageSource instanceof EntityDamageSource)) {
	...
	protected void attackEntity(Entity par1Entity, float par2) {
		if (attackTime <= 0 && par2 < 1.2F
			...
			attackTime = 20;
			par1Entity.attackEntityFrom(DamageSource.causeMobDamage(this),
					attackStrength);
		}
	}
	...
	protected void updateEntityActionState() {
		...
		if (allySummonCooldown > 0) {
			...
			if (allySummonCooldown == 0) {
				...
				for (int l1 = 0; !flag && l1 <= 5 && l1 >= -5; l1 = l1 > 0 ? 0 - l1
						: 1 - l1) {
					for (int j2 = 0; !flag && j2 <= 10 && j2 >= -10; j2 = j2 > 0 ? 0 - j2
							: 1 - j2) {
						for (int k2 = 0; !flag && k2 <= 10 && k2 >= -10; k2 = k2 > 0 ? 0 - k2
								: 1 - k2) {
							int l2 = worldObj.getBlockId(i + j2, k + l1, i1
									+ k2);
							...
							worldObj.playAuxSFX(
									...
									Block.silverfish.blockID
											+ (worldObj.getBlockMetadata(
													i + j2, k + l1, i1 + k2) << 12));
							worldObj.setBlockWithNotify(i + j2, k + l1,
									i1 + k2, 0);
							Block.silverfish.onBlockDestroyedByPlayer(worldObj,
									i + j2, k + l1, i1 + k2, 0);
		...
		if (entityToAttack == null && !hasPath()) {
			...
			int i2 = worldObj.getBlockId(j + Facing.offsetsXForSide[k1], l
					+ Facing.offsetsYForSide[k1], j1
					+ Facing.offsetsZForSide[k1]);

			if (BlockSilverfish.getPosingIdByMetadata(i2)) {
				worldObj.setBlockAndMetadataWithNotify(j
						+ Facing.offsetsXForSide[k1], l
						+ Facing.offsetsYForSide[k1], j1
						+ Facing.offsetsZForSide[k1], Block.silverfish.blockID,
						BlockSilverfish.getMetadataForBlockType(i2));
