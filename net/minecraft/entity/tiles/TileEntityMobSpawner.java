...
public class TileEntityMobSpawner extends TileEntity {
	...
	/**
	 * The string ID of the mobs being spawned from this spawner. Defaults to
	 * pig, apparently.
	 */
	...
	public boolean anyPlayerInRange() {
		return worldObj.getClosestPlayer((double) xCoord + 0.5D,
				(double) yCoord + 0.5D, (double) zCoord + 0.5D, 16D) != null;
	}
		...
		if (!worldObj.isRemote) {
			...
			for (int i = 0; i < byte0; i++) {
				EntityLiving entityliving = (EntityLiving) EntityList
						.createEntityByName(mobID, worldObj);
