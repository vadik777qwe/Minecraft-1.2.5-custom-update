...
public class TileEntityPiston extends TileEntity {
	...
	public TileEntityPiston(int par1, int par2, int par3, boolean par4,
			boolean par5) {
		storedBlockID = par1;
		storedMetadata = par2;
	...
	public float getOffsetX(float par1) {
		if (extending) {
			return (getProgress(par1) - 1.0F)
					* (float) Facing.offsetsXForSide[storedOrientation];
		} else {
			return (1.0F - getProgress(par1))
					* (float) Facing.offsetsXForSide[storedOrientation];
		}
	}
	...
	public float getOffsetY(float par1) {
		if (extending) {
			return (getProgress(par1) - 1.0F)
					* (float) Facing.offsetsYForSide[storedOrientation];
		} else {
			return (1.0F - getProgress(par1))
					* (float) Facing.offsetsYForSide[storedOrientation];
		}
	...
	public float getOffsetZ(float par1) {
		if (extending) {
			return (getProgress(par1) - 1.0F)
					* (float) Facing.offsetsZForSide[storedOrientation];
		} else {
			return (1.0F - getProgress(par1))
					* (float) Facing.offsetsZForSide[storedOrientation];
		}
	...
	private void updatePushedObjects(float par1, float par2) {
		...
		if (axisalignedbb != null) {
			...
			if (!list.isEmpty()) {
				...
				for (Iterator iterator = pushedObjects.iterator(); iterator
						.hasNext(); entity
						.moveEntity(
								par2
										* (float) Facing.offsetsXForSide[storedOrientation],
