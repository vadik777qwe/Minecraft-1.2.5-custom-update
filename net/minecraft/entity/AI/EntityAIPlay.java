	...
	public boolean shouldExecute() {
		...
		do {
			...
			if (entity != villagerObj) {
				...
				if (!entityvillager.getIsPlayingFlag()
						&& entityvillager.getGrowingAge() < 0) {
					double d1 = entityvillager
							.getDistanceSqToEntity(villagerObj);
		...
		if (targetVillager == null) {
			Vec3D vec3d = RandomPositionGenerator.func_48622_a(villagerObj, 16,
					3);
	...
	public void updateTask() {
		...
		if (targetVillager != null) {
			if (villagerObj.getDistanceSqToEntity(targetVillager) > 4D) {
				villagerObj.getNavigator().func_48667_a(targetVillager,
						field_48358_c);
			}
			...
			villagerObj.getNavigator().func_48666_a(vec3d.xCoord, vec3d.yCoord,
					vec3d.zCoord, field_48358_c);
		}
	}
}
