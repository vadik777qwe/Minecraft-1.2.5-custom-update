...
public class EntityAIAttackOnCollide extends EntityAIBase {
	...
	public EntityAIAttackOnCollide(EntityLiving par1EntityLiving,
			Class par2Class, float par3, boolean par4) {
		this(par1EntityLiving, par3, par4);
		classTarget = par2Class;
	}

	public EntityAIAttackOnCollide(EntityLiving par1EntityLiving, float par2,
			boolean par3) {
		field_46091_d = 0;
	...
	public boolean shouldExecute() {
		...
		if (classTarget != null
				&& !classTarget.isAssignableFrom(entityliving.getClass())) {
			return false;
		} else {
	...
	public boolean continueExecuting() {
		...
		return attacker.isWithinHomeDistance(
				MathHelper.floor_double(entityTarget.posX),
				MathHelper.floor_double(entityTarget.posY),
				MathHelper.floor_double(entityTarget.posZ));
	}
	...
	public void updateTask() {
		attacker.getLookHelper().setLookPositionWithEntity(entityTarget, 30F,
				30F);

		if ((field_48264_f || attacker.func_48090_aM().canSee(entityTarget))
				&& --field_48269_i <= 0) {
		...
		if (attacker.getDistanceSq(entityTarget.posX,
				entityTarget.boundingBox.minY, entityTarget.posZ) > d) {
			return;
		}
