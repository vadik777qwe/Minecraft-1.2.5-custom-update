...
public class EntityAIFleeSun extends EntityAIBase {
	...
	public void startExecuting() {
		theCreature.getNavigator().func_48666_a(shelterX, shelterY, shelterZ,
				field_48299_e);
	}
	...
	private Vec3D findPossibleShelter() {
		...
		for (int i = 0; i < 10; i++) {
			int j = MathHelper.floor_double((theCreature.posX + (double) random
					.nextInt(20)) - 10D);
			int k = MathHelper
					.floor_double((theCreature.boundingBox.minY + (double) random
							.nextInt(6)) - 3D);
			int l = MathHelper.floor_double((theCreature.posZ + (double) random
					.nextInt(20)) - 10D);

			if (!theWorld.canBlockSeeTheSky(j, k, l)
					&& theCreature.getBlockPathWeight(j, k, l) < 0.0F) {
