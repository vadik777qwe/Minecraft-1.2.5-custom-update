...
public class EntityAITempt extends EntityAIBase {
	...
	public EntityAITempt(EntityCreature par1EntityCreature, float par2,
	...
	public boolean continueExecuting() {
		if (scaredByPlayerMovement) {
			if (temptedEntity.getDistanceSqToEntity(temptingPlayer) < 36D) {
				if (temptingPlayer.getDistanceSq(field_48276_c, field_48273_d,
						field_48274_e) > 0.010000000000000002D) {
					return false;
				}

				if (Math.abs((double) temptingPlayer.rotationPitch
						- field_48271_f) > 5D
						|| Math.abs((double) temptingPlayer.rotationYaw
								- field_48272_g) > 5D) {
					return false;
				}
			} else {
	...
	public void updateTask() {
		...
		if (temptedEntity.getDistanceSqToEntity(temptingPlayer) < 6.25D) {
			...
			temptedEntity.getNavigator().func_48667_a(temptingPlayer,
					field_48275_b);
		}
	}
