...
public class EntityBlaze extends EntityMob {
	...
	public void onLivingUpdate() {
		if (!worldObj.isRemote) {
			...
			if (getEntityToAttack() != null
					&& getEntityToAttack().posY
							+ (double) getEntityToAttack().getEyeHeight() > posY
							+ (double) getEyeHeight() + (double) heightOffset) {
				motionY = motionY + (0.30000001192092896D - motionY)
		...
		for (int i = 0; i < 2; i++) {
			worldObj.spawnParticle("largesmoke", posX
					...
					posZ + (rand.nextDouble() - 0.5D) * (double) width, 0.0D,
					0.0D, 0.0D);
		}
	...
	protected void attackEntity(Entity par1Entity, float par2) {
		if (attackTime <= 0 && par2 < 2.0F
			...
			double d1 = (par1Entity.boundingBox.minY + (double) (par1Entity.height / 2.0F))
			...
			if (attackTime == 0) {
				...
				if (field_40152_d > 1) {
					...
					worldObj.playAuxSFXAtEntity(null, 1009, (int) posX,
							(int) posY, (int) posZ, 0);
					...
					for (int i = 0; i < 1; i++) {
						EntitySmallFireball entitysmallfireball = new EntitySmallFireball(
								worldObj, this, d + rand.nextGaussian()
										* (double) f, d1, d2
										+ rand.nextGaussian() * (double) f);
						entitysmallfireball.posY = posY
								+ (double) (height / 2.0F) + 0.5D;
