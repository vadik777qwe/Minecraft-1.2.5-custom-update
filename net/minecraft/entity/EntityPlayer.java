...
public abstract class EntityPlayer extends EntityLiving {
	...
	public EntityPlayer(World par1World) {
		...
		setLocationAndAngles((double) chunkcoordinates.posX + 0.5D,
	...
	public void onUpdate() {
		...
		if (!worldObj.isRemote && craftingInventory != null
	...
	protected void updateItemUse(ItemStack par1ItemStack, int par2) {
		if (par1ItemStack.getItemUseAction() == EnumAction.drink) {
			worldObj.playSoundAtEntity(this, "random.drink", 0.5F,
		...
		if (par1ItemStack.getItemUseAction() == EnumAction.eat) {
			for (int i = 0; i < par2; i++) {
				...
				worldObj.spawnParticle(
						(new StringBuilder()).append("iconcrack_")
								.append(par1ItemStack.getItem().shiftedIndex)
								.toString(), vec3d1.xCoord, vec3d1.yCoord,
						vec3d1.zCoord, vec3d.xCoord,
						vec3d.yCoord + 0.050000000000000003D, vec3d.zCoord);
			}

			worldObj.playSoundAtEntity(this, "random.eat",
	...
	protected void onItemUseFinish() {
		if (itemInUse != null) {
			...
			if (itemstack != itemInUse || itemstack != null
	...
	public void updateCloak() {
		playerCloakUrl = (new StringBuilder())
				.append("http://s3.amazonaws.com/MinecraftCloaks/")
	...
	public void onDeath(DamageSource par1DamageSource) {
		...
		if (par1DamageSource != null) {
			motionX = -MathHelper
					.cos(((attackedAtYaw + rotationYaw) * (float) Math.PI) / 180F) * 0.1F;
			motionZ = -MathHelper
					.sin(((attackedAtYaw + rotationYaw) * (float) Math.PI) / 180F) * 0.1F;
		} else {
	...
	public EntityItem dropPlayerItemWithRandomChoice(ItemStack par1ItemStack,
		...
		if (par2) {
			...
			float f1 = 0.3F;
			entityitem.motionX = -MathHelper.sin((rotationYaw / 180F)
					* (float) Math.PI)
					* MathHelper.cos((rotationPitch / 180F) * (float) Math.PI)
					* f1;
			entityitem.motionZ = MathHelper.cos((rotationYaw / 180F)
					* (float) Math.PI)
					* MathHelper.cos((rotationPitch / 180F) * (float) Math.PI)
					* f1;
			entityitem.motionY = -MathHelper.sin((rotationPitch / 180F)
					* (float) Math.PI)
					* f1 + 0.1F;
			f1 = 0.02F;
	...
	public float getCurrentPlayerStrVsBlock(Block par1Block) {
		...
		if (isPotionActive(Potion.digSpeed)) {
			f1 *= 1.0F + (float) (getActivePotionEffect(Potion.digSpeed)
					.getAmplifier() + 1) * 0.2F;
		}

		if (isPotionActive(Potion.digSlowdown)) {
			f1 *= 1.0F - (float) (getActivePotionEffect(Potion.digSlowdown)
					.getAmplifier() + 1) * 0.2F;
		}
	...
	public boolean attackEntityFrom(DamageSource par1DamageSource, int par2) {
		...
		if ((entity1 instanceof EntityArrow)
				&& ((EntityArrow) entity1).shootingEntity != null) {
	 ...
	 */
	protected int applyPotionDamageCalculations(DamageSource par1DamageSource,
			int par2) {
		...
		int j = EnchantmentHelper.getEnchantmentModifierDamage(inventory,
				par1DamageSource);
	...
	protected void alertWolves(EntityLiving par1EntityLiving, boolean par2) {
		...
		if (par1EntityLiving instanceof EntityWolf) {
			...
			if (entitywolf.isTamed()
					&& username.equals(entitywolf.getOwnerName())) {
				return;
			}
		}
		...
		List list = worldObj.getEntitiesWithinAABB(
				net.minecraft.entity.EntityWolf.class,
				AxisAlignedBB.getBoundingBoxFromPool(posX, posY, posZ,
						posX + 1.0D, posY + 1.0D, posZ + 1.0D).expand(16D, 4D,
						16D));
		...
		do {
			...
			if (entitywolf1.isTamed()
					...
					&& username.equals(entitywolf1.getOwnerName())
					&& (!par2 || !entitywolf1.isSitting())) {
	...
	public void swingItem() {
		if (!isSwinging || swingProgressInt >= getSwingSpeedModifier() / 2
				|| swingProgressInt < 0) {
			swingProgressInt = -1;
			isSwinging = true;
		}
	...
	public void attackTargetEntityWithCurrentItem(Entity par1Entity) {
		...
		if (i > 0 || k > 0) {
			...
			i += k;
			boolean flag1 = par1Entity.attackEntityFrom(
					DamageSource.causePlayerDamage(this), i);
			...
			if (flag1) {
				if (j > 0) {
					par1Entity
							.addVelocity(
									-MathHelper
											.sin((rotationYaw * (float) Math.PI) / 180F)
											* (float) j * 0.5F,
									0.10000000000000001D,
									MathHelper
											.cos((rotationYaw * (float) Math.PI) / 180F)
											* (float) j * 0.5F);
	...
	/**
	 * is called when the player performs a critical hit on the Entity. Args:
	...
	public EnumStatus sleepInBedAt(int par1, int par2, int par3) {
		if (!worldObj.isRemote) {
			...
			if (Math.abs(posX - (double) par1) > 3D
					|| Math.abs(posY - (double) par2) > 2D
					|| Math.abs(posZ - (double) par3) > 3D) {
			...
			double d1 = 5D;
			List list = worldObj.getEntitiesWithinAABB(
					net.minecraft.entity.EntityMob.class, AxisAlignedBB
							.getBoundingBoxFromPool((double) par1 - d,
									(double) par2 - d1, (double) par3 - d,
									(double) par1 + d, (double) par2 + d1,
									(double) par3 + d));
		...
		if (worldObj.blockExists(par1, par2, par3)) {
			...
			func_22052_e(j);
			setPosition((float) par1 + f, (float) par2 + 0.9375F, (float) par3
					+ f1);
		} else {
			setPosition((float) par1 + 0.5F, (float) par2 + 0.9375F,
					(float) par3 + 0.5F);
		}
	...
	public void wakeUpPlayer(boolean par1, boolean par2, boolean par3) {
		...
		if (chunkcoordinates != null
				&& worldObj.getBlockId(chunkcoordinates.posX,
						chunkcoordinates.posY, chunkcoordinates.posZ) == Block.bed.blockID) {
			BlockBed.setBedOccupied(worldObj, chunkcoordinates.posX,
					chunkcoordinates.posY, chunkcoordinates.posZ, false);
			ChunkCoordinates chunkcoordinates2 = BlockBed
					.getNearestEmptyChunkCoordinates(worldObj,
							chunkcoordinates.posX, chunkcoordinates.posY,
							chunkcoordinates.posZ, 0);

			if (chunkcoordinates2 == null) {
				chunkcoordinates2 = new ChunkCoordinates(chunkcoordinates.posX,
						chunkcoordinates.posY + 1, chunkcoordinates.posZ);
			}

			setPosition((float) chunkcoordinates2.posX + 0.5F,
	...
	public static ChunkCoordinates verifyRespawnCoordinates(World par0World,
		...
		ichunkprovider.loadChunk(par1ChunkCoordinates.posX - 3 >> 4,
		...
		if (par0World.getBlockId(par1ChunkCoordinates.posX,
			...
			ChunkCoordinates chunkcoordinates = BlockBed
	...
	public float getBedOrientationInDegrees() {
		if (playerLocation != null) {
			int i = worldObj.getBlockMetadata(playerLocation.posX,
	...
	/**
	 * Returns the coordinates to respawn the player based on last bed that the
	 * player sleep.
	 */
	...
	public void addMovementStat(double par1, double par3, double par5) {
		...
		if (isInsideOfMaterial(Material.water)) {
			int i = Math.round(MathHelper.sqrt_double(par1 * par1 + par3 * par3
					+ par5 * par5) * 100F);
			...
			int j = Math.round(MathHelper
					.sqrt_double(par1 * par1 + par5 * par5) * 100F);
			...
			if (par3 > 0.0D) {
				addStat(StatList.distanceClimbedStat,
						(int) Math.round(par3 * 100D));
			}
		} else if (onGround) {
			int k = Math.round(MathHelper
					.sqrt_double(par1 * par1 + par5 * par5) * 100F);
					...
					if (startMinecartRidingCoordinate == null) {
						startMinecartRidingCoordinate = new ChunkCoordinates(
							...
							.getEuclideanDistanceTo(
									MathHelper.floor_double(posX),
									MathHelper.floor_double(posY),
									MathHelper.floor_double(posZ)) >= 1000D) {
	...
	protected void fall(float par1) {
		...
		if (par1 >= 2.0F) {
			addStat(StatList.distanceFallenStat,
					(int) Math.round((double) par1 * 100D));
		}
	...
	public int getItemIcon(ItemStack par1ItemStack, int par2) {
		...
		if (par1ItemStack.itemID == Item.fishingRod.shiftedIndex
				&& fishEntity != null) {
			i = par1ItemStack.getIconIndex() + 16;
		} else {
			if (par1ItemStack.getItem().func_46058_c()) {
				return par1ItemStack.getItem().func_46057_a(
	...
	/**
	 * Decrease the player level, used to pay levels for enchantments on items
	 * at enchanted table.
	 */
