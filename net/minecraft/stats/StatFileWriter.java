...
public class StatFileWriter {
	...
	public StatFileWriter(Session par1Session, File par2File) {
		...
		for (int j = 0; j < i; j++) {
			...
			if (!file1.getName().startsWith("stats_")
					|| !file1.getName().endsWith(".dat")) {
				continue;
			}
			...
			if (!file2.exists()) {
				System.out.println((new StringBuilder()).append("Relocating ")
						.append(file1.getName()).toString());
	...
	public void func_27179_a(Map par1Map) {
		...
		for (Iterator iterator = par1Map.keySet().iterator(); iterator
				.hasNext(); writeStatToMap(field_25102_a, statbase,
				((Integer) par1Map.get(statbase)).intValue())) {
			statbase = (StatBase) iterator.next();
			writeStatToMap(field_25101_b, statbase,
					((Integer) par1Map.get(statbase)).intValue());
		}
	}
	...
	public void func_27180_b(Map par1Map) {
		...
		for (Iterator iterator = par1Map.keySet().iterator(); iterator
				.hasNext(); field_25102_a.put(
				statbase,
				Integer.valueOf(((Integer) par1Map.get(statbase)).intValue()
						+ i))) {
			statbase = (StatBase) iterator.next();
	...
	public void func_27187_c(Map par1Map) {
		...
		for (Iterator iterator = par1Map.keySet().iterator(); iterator
				.hasNext(); writeStatToMap(field_25101_b, statbase,
				((Integer) par1Map.get(statbase)).intValue())) {
			statbase = (StatBase) iterator.next();
		}
	}
	...
	public static Map func_27177_a(String par0Str) {
		...
		try {
			...
			List list = jsonrootnode
					.getArrayNode(new Object[] { "stats-change" });

			for (Iterator iterator = list.iterator(); iterator.hasNext();) {
				JsonNode jsonnode = (JsonNode) iterator.next();
				Map map = jsonnode.getFields();
				java.util.Map.Entry entry = (java.util.Map.Entry) map
						.entrySet().iterator().next();
				int i = Integer.parseInt(((JsonStringNode) entry.getKey())
						.getText());
				int j = Integer.parseInt(((JsonNode) entry.getValue())
						.getText());
				StatBase statbase = StatList.getOneShotStat(i);

				if (statbase == null) {
					System.out.println((new StringBuilder()).append(i)
							.append(" is not a valid stat").toString());
				} else {
					stringbuilder.append(StatList.getOneShotStat(i).statGuid)
							.append(",");
			...
			if (!s1.equals(jsonrootnode
					.getStringValue(new Object[] { "checksum" }))) {
	...
	public static String func_27185_a(String par0Str, String par1Str,
			Map par2Map) {
		StringBuilder stringbuilder = new StringBuilder();
		...
		if (par0Str != null && par1Str != null) {
			...
			stringbuilder.append("    \"name\":\"").append(par0Str)
					.append("\",\r\n");
			stringbuilder.append("    \"sessionid\":\"").append(par1Str)
					.append("\"\r\n");
			stringbuilder.append("  },\r\n");
		}
		...
		for (Iterator iterator = par2Map.keySet().iterator(); iterator
				.hasNext(); stringbuilder1.append(par2Map.get(statbase))
				.append(",")) {
			statbase = (StatBase) iterator.next();
			...
			stringbuilder.append("\r\n    {\"").append(statbase.statId)
					.append("\":").append(par2Map.get(statbase));
		...
		stringbuilder.append("  \"checksum\":\"")
				.append(md5string.getMD5String(stringbuilder1.toString()))
				.append("\"\r\n");
		stringbuilder.append("}");
	...
	public boolean canUnlockAchievement(Achievement par1Achievement) {
		return par1Achievement.parentAchievement == null
				|| hasAchievementUnlocked(par1Achievement.parentAchievement);
	}
