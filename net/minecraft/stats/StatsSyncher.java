...
import net.minecraft.threads.ThreadStatSyncherReceive;
import net.minecraft.threads.ThreadStatSyncherSend;
import java.io.*;
...
public class StatsSyncher {
	...
	public volatile Map field_27437_b;
	public volatile Map field_27436_c;
	...
	public StatsSyncher(Session par1Session, StatFileWriter par2StatFileWriter,
			File par3File) {
		isBusy = false;
		field_27437_b = null;
		field_27436_c = null;
		field_27427_l = 0;
		field_27426_m = 0;
		unsentDataFile = new File(par3File, (new StringBuilder())
				.append("stats_").append(par1Session.username.toLowerCase())
				.append("_unsent.dat").toString());
		dataFile = new File(par3File, (new StringBuilder()).append("stats_")
				.append(par1Session.username.toLowerCase()).append(".dat")
				.toString());
		unsentOldFile = new File(par3File, (new StringBuilder())
				.append("stats_").append(par1Session.username.toLowerCase())
				.append("_unsent.old").toString());
		oldFile = new File(par3File, (new StringBuilder()).append("stats_")
				.append(par1Session.username.toLowerCase()).append(".old")
				.toString());
		unsentTempFile = new File(par3File, (new StringBuilder())
				.append("stats_").append(par1Session.username.toLowerCase())
				.append("_unsent.tmp").toString());
		tempFile = new File(par3File, (new StringBuilder()).append("stats_")
				.append(par1Session.username.toLowerCase()).append(".tmp")
				.toString());

		if (!par1Session.username.toLowerCase().equals(par1Session.username)) {
			func_28214_a(par3File, (new StringBuilder()).append("stats_")
					.append(par1Session.username).append("_unsent.dat")
					.toString(), unsentDataFile);
			func_28214_a(par3File, (new StringBuilder()).append("stats_")
					.append(par1Session.username).append(".dat").toString(),
					dataFile);
			func_28214_a(par3File, (new StringBuilder()).append("stats_")
					.append(par1Session.username).append("_unsent.old")
					.toString(), unsentOldFile);
			func_28214_a(par3File, (new StringBuilder()).append("stats_")
					.append(par1Session.username).append(".old").toString(),
					oldFile);
			func_28214_a(par3File, (new StringBuilder()).append("stats_")
					.append(par1Session.username).append("_unsent.tmp")
					.toString(), unsentTempFile);
			func_28214_a(par3File, (new StringBuilder()).append("stats_")
					.append(par1Session.username).append(".tmp").toString(),
					tempFile);
		}
		...
		if (unsentDataFile.exists()) {
			par2StatFileWriter.func_27179_a(func_27415_a(unsentDataFile,
	...
	private void func_27410_a(Map par1Map, File par2File, File par3File,
			File par4File) throws IOException {
		PrintWriter printwriter = new PrintWriter(new FileWriter(par3File,
				false));

		try {
			printwriter.print(StatFileWriter.func_27185_a(theSession.username,
					"local", par1Map));
		} finally {
			printwriter.close();
		}
	...
	public static Map func_27422_a(StatsSyncher par0StatsSyncher) {
		return par0StatsSyncher.field_27437_b;
	}

	public static File func_27423_b(StatsSyncher par0StatsSyncher) {
		return par0StatsSyncher.dataFile;
	}

	public static File func_27411_c(StatsSyncher par0StatsSyncher) {
		return par0StatsSyncher.tempFile;
	}

	public static File func_27413_d(StatsSyncher par0StatsSyncher) {
		return par0StatsSyncher.oldFile;
	}

	public static void func_27412_a(StatsSyncher par0StatsSyncher, Map par1Map,
			File par2File, File par3File, File par4File) throws IOException {
		par0StatsSyncher.func_27410_a(par1Map, par2File, par3File, par4File);
	}

	public static Map func_27421_a(StatsSyncher par0StatsSyncher, Map par1Map) {
		return par0StatsSyncher.field_27437_b = par1Map;
	}

	public static Map func_27409_a(StatsSyncher par0StatsSyncher, File par1File,
			File par2File, File par3File) {
		return par0StatsSyncher.func_27415_a(par1File, par2File, par3File);
	}

	public static boolean setBusy(StatsSyncher par0StatsSyncher, boolean par1) {
		return par0StatsSyncher.isBusy = par1;
	}

	public static File getUnsentDataFile(StatsSyncher par0StatsSyncher) {
		return par0StatsSyncher.unsentDataFile;
	}

	public static File getUnsentTempFile(StatsSyncher par0StatsSyncher) {
		return par0StatsSyncher.unsentTempFile;
	}

	public static File getUnsentOldFile(StatsSyncher par0StatsSyncher) {
