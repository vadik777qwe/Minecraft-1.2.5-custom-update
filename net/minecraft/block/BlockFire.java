...
public class BlockFire extends Block {
	...
	public void updateTick(World par1World, int par2, int par3, int par4,
		...
		if (!flag
				...
				&& (par1World.canLightningStrikeAt(par2, par3, par4)
						...
						|| par1World.canLightningStrikeAt(par2 + 1, par3, par4)
						|| par1World.canLightningStrikeAt(par2, par3, par4 - 1) || par1World
		...
		if (i < 15) {
			par1World.setBlockMetadata(par2, par3, par4,
					i + par5Random.nextInt(3) / 2);
		}

		par1World.scheduleBlockUpdate(par2, par3, par4, blockID, tickRate()
				+ par5Random.nextInt(10));
		...
		if (!flag && !canBlockCatchFire(par1World, par2, par3 - 1, par4)
				&& i == 15 && par5Random.nextInt(4) == 0) {
			par1World.setBlockWithNotify(par2, par3, par4, 0);
			return;
		}
		...
		tryToCatchBlockOnFire(par1World, par2 + 1, par3, par4, 300 + byte0,
				par5Random, i);
		tryToCatchBlockOnFire(par1World, par2 - 1, par3, par4, 300 + byte0,
				par5Random, i);
		tryToCatchBlockOnFire(par1World, par2, par3 - 1, par4, 250 + byte0,
				par5Random, i);
		tryToCatchBlockOnFire(par1World, par2, par3 + 1, par4, 250 + byte0,
				par5Random, i);
		tryToCatchBlockOnFire(par1World, par2, par3, par4 - 1, 300 + byte0,
				par5Random, i);
		tryToCatchBlockOnFire(par1World, par2, par3, par4 + 1, 300 + byte0,
				par5Random, i);
		...
		for (int j = par2 - 1; j <= par2 + 1; j++) {
			for (int k = par4 - 1; k <= par4 + 1; k++) {
				for (int l = par3 - 1; l <= par3 + 4; l++) {
					...
					int j1 = getChanceOfNeighborsEncouragingFire(par1World, j,
							l, k);
					...
					if (k1 <= 0 || par5Random.nextInt(i1) > k1
							|| par1World.isRaining()
							&& par1World.canLightningStrikeAt(j, l, k)
					...
					par1World.setBlockAndMetadataWithNotify(j, l, k, blockID,
							l1);
				}
			}
		}
	...
	public boolean canBlockCatchFire(IBlockAccess par1IBlockAccess, int par2,
			int par3, int par4) {
		return chanceToEncourageFire[par1IBlockAccess.getBlockId(par2, par3,
				par4)] > 0;
	}
	...
	public void randomDisplayTick(World par1World, int par2, int par3,
		...
		if (par1World.isBlockNormalCube(par2, par3 - 1, par4)
			...
			for (int i = 0; i < 3; i++) {
				...
				par1World.spawnParticle("largesmoke", f, f6, f12, 0.0D, 0.0D,
						0.0D);
			}
		} else {
			if (Block.fire.canBlockCatchFire(par1World, par2 - 1, par3, par4)) {
				for (int j = 0; j < 2; j++) {
					...
					par1World.spawnParticle("largesmoke", f1, f7, f13, 0.0D,
							0.0D, 0.0D);
				}
			}
			...
			if (Block.fire.canBlockCatchFire(par1World, par2 + 1, par3, par4)) {
				for (int k = 0; k < 2; k++) {
					...
					par1World.spawnParticle("largesmoke", f2, f8, f14, 0.0D,
							0.0D, 0.0D);
				}
			}
			...
			if (Block.fire.canBlockCatchFire(par1World, par2, par3, par4 - 1)) {
				for (int l = 0; l < 2; l++) {
					...
					par1World.spawnParticle("largesmoke", f3, f9, f15, 0.0D,
							0.0D, 0.0D);
				}
			}
			...
			if (Block.fire.canBlockCatchFire(par1World, par2, par3, par4 + 1)) {
				for (int i1 = 0; i1 < 2; i1++) {
					...
					float f16 = (float) (par4 + 1) - par5Random.nextFloat()
							* 0.1F;
					par1World.spawnParticle("largesmoke", f4, f10, f16, 0.0D,
							0.0D, 0.0D);
				}
			}
			...
			if (Block.fire.canBlockCatchFire(par1World, par2, par3 + 1, par4)) {
				for (int j1 = 0; j1 < 2; j1++) {
					...
					par1World.spawnParticle("largesmoke", f5, f11, f17, 0.0D,
							0.0D, 0.0D);
				}
			}
		}
