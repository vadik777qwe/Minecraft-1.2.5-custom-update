...
public class BlockDispenser extends BlockContainer {
	 ...
	 */
	private void setDispenserDefaultDirection(World par1World, int par2,
			int par3, int par4) {
	...
	public boolean blockActivated(World par1World, int par2, int par3,
		...
		TileEntityDispenser tileentitydispenser = (TileEntityDispenser) par1World
	...
	/**
	 * dispenses an item from a randomly selected item stack from the blocks
	 * inventory into the game world.
	 */
	private void dispenseItem(World par1World, int par2, int par3, int par4,
			Random par5Random) {
			...
			if (itemstack == null) {
				...
				if (itemstack.itemID == Item.arrow.shiftedIndex) {
					EntityArrow entityarrow = new EntityArrow(par1World, d, d1,
							d2);
					entityarrow.setArrowHeading(j, 0.10000000149011612D, k,
							1.1F, 6F);
					...
					EntitySnowball entitysnowball = new EntitySnowball(
							par1World, d, d1, d2);
					entitysnowball.setThrowableHeading(j, 0.10000000149011612D,
							k, 1.1F, 6F);
					...
					entitypotion.setThrowableHeading(j, 0.10000000149011612D,
							k, 1.375F, 3F);
					...
					ItemMonsterPlacer.func_48440_a(par1World,
							...
							d1 - 0.29999999999999999D, d2 + (double) k
									* 0.29999999999999999D);
					par1World.playAuxSFX(1002, par2, par3, par4, 0);
				} else if (itemstack.itemID == Item.fireballCharge.shiftedIndex) {
					EntitySmallFireball entitysmallfireball = new EntitySmallFireball(
							par1World, d + (double) j * 0.29999999999999999D,
							d1, d2 + (double) k * 0.29999999999999999D,
							(double) j + par5Random.nextGaussian()
									* 0.050000000000000003D,
							par5Random.nextGaussian() * 0.050000000000000003D,
							(double) k + par5Random.nextGaussian()
									* 0.050000000000000003D);
					par1World.spawnEntityInWorld(entitysmallfireball);
					par1World.playAuxSFX(1009, par2, par3, par4, 0);
				} else {
					EntityItem entityitem = new EntityItem(par1World, d,
							d1 - 0.29999999999999999D, d2, itemstack);
				...
				par1World.playAuxSFX(2000, par2, par3, par4, j + 1 + (k + 1)
						* 3);
			}
		}
	}
