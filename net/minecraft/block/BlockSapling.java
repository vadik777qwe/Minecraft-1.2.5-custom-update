...
public class BlockSapling extends BlockFlower {
	...
	public void growTree(World par1World, int par2, int par3, int par4,
		...
		if (i == 1) {
			...
			do {
				...
				do {
					...
					if (func_50076_f(par1World, par2 + j, par3, par4 + k, 3)
							&& func_50076_f(par1World, par2 + j + 1, par3, par4
									+ k, 3)
							&& func_50076_f(par1World, par2 + j, par3, par4 + k
									+ 1, 3)
							&& func_50076_f(par1World, par2 + j + 1, par3, par4
									+ k + 1, 3)) {
						obj = new WorldGenHugeTrees(true,
			...
			if (obj == null) {
				j = k = 0;
				obj = new WorldGenTrees(true, 4 + par5Random.nextInt(7), 3, 3,
						false);
			}
		} else {
		...
		if (!((WorldGenerator) (obj)).generate(par1World, par5Random, par2 + j,
				par3, par4 + k)) {
			if (flag) {
				par1World.setBlockAndMetadata(par2 + j, par3, par4 + k,
						blockID, i);
				par1World.setBlockAndMetadata(par2 + j + 1, par3, par4 + k,
						blockID, i);
				par1World.setBlockAndMetadata(par2 + j, par3, par4 + k + 1,
						blockID, i);
				par1World.setBlockAndMetadata(par2 + j + 1, par3, par4 + k + 1,
						blockID, i);
			} else {
	...
	public boolean func_50076_f(World par1World, int par2, int par3, int par4,
			int par5) {
		return par1World.getBlockId(par2, par3, par4) == blockID
