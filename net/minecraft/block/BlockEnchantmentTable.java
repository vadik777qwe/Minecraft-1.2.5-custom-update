		...
		for (int i = par2 - 2; i <= par2 + 2; i++) {
			for (int j = par4 - 2; j <= par4 + 2; j++) {
				...
				for (int k = par3; k <= par3 + 1; k++) {
					...
					if (!par1World.isAirBlock((i - par2) / 2 + par2, k,
							(j - par4) / 2 + par4)) {
						break;
					}

					par1World.spawnParticle("enchantmenttable",
							(double) par2 + 0.5D, (double) par3 + 2D,
							(double) par4 + 0.5D,
							(double) ((float) (i - par2) + par5Random
									.nextFloat()) - 0.5D, (float) (k - par3)
									- par5Random.nextFloat() - 1.0F,
							(double) ((float) (j - par4) + par5Random
									.nextFloat()) - 0.5D);
				}
			}
		}
