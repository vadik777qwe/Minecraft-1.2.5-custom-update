...
public class BlockMushroom extends BlockFlower {
	...
	public void updateTick(World par1World, int par2, int par3, int par4,
		...
		if (par5Random.nextInt(25) == 0) {
			...
			for (int j = par2 - byte0; j <= par2 + byte0; j++) {
				for (int l = par4 - byte0; l <= par4 + byte0; l++) {
					for (int j1 = par3 - 1; j1 <= par3 + 1; j1++) {
						if (par1World.getBlockId(j, j1, l) == blockID
								&& --i <= 0) {
							return;
						}
					}
			...
			for (int l1 = 0; l1 < 4; l1++) {
				if (par1World.isAirBlock(k, i1, k1)
						&& canBlockStay(par1World, k, i1, k1)) {
					par2 = k;
					par3 = i1;
					par4 = k1;
				}
	...
	public boolean canBlockStay(World par1World, int par2, int par3, int par4) {
		if (par3 < 0 || par3 >= 256) {
			...
			return i == Block.mycelium.blockID
					|| par1World.getFullBlockLightValue(par2, par3, par4) < 13
					&& canThisPlantGrowOnThisBlockID(i);
		}
	}
	...
	public boolean fertilizeMushroom(World par1World, int par2, int par3,
		...
		if (worldgenbigmushroom == null
				|| !worldgenbigmushroom.generate(par1World, par5Random, par2,
						par3, par4)) {
