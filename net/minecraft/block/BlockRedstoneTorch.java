...
public class BlockRedstoneTorch extends BlockTorch {
	...
	/**
	 * An array of when redstone torches became active. Used for redstone
	 * torches to burn out.
	 */
	...
	private boolean checkForBurnout(World par1World, int par2, int par3,
			int par4, boolean par5) {
		if (par5) {
			torchUpdates.add(new RedstoneUpdateInfo(par2, par3, par4, par1World
					.getWorldTime()));
		}
		...
		for (int j = 0; j < torchUpdates.size(); j++) {
			RedstoneUpdateInfo redstoneupdateinfo = (RedstoneUpdateInfo) torchUpdates
					.get(j);

			if (redstoneupdateinfo.x == par2 && redstoneupdateinfo.y == par3
					&& redstoneupdateinfo.z == par4 && ++i >= 8) {
				return true;
			}
		}
	...
	public void updateTick(World par1World, int par2, int par3, int par4,
		...
		for (; torchUpdates.size() > 0
				&& par1World.getWorldTime()
						- ((RedstoneUpdateInfo) torchUpdates.get(0)).updateTime > 60L; torchUpdates
				.remove(0)) {
		}
		...
		if (torchActive) {
			if (flag) {
				...
				if (checkForBurnout(par1World, par2, par3, par4, true)) {
					...
					for (int i = 0; i < 5; i++) {
						double d = (double) par2 + par5Random.nextDouble()
								* 0.59999999999999998D + 0.20000000000000001D;
						double d1 = (double) par3 + par5Random.nextDouble()
								* 0.59999999999999998D + 0.20000000000000001D;
						double d2 = (double) par4 + par5Random.nextDouble()
								* 0.59999999999999998D + 0.20000000000000001D;
						par1World.spawnParticle("smoke", d, d1, d2, 0.0D, 0.0D,
								0.0D);
					}
				}
			}
	...
	public void randomDisplayTick(World par1World, int par2, int par3,
		...
		int i = par1World.getBlockMetadata(par2, par3, par4);
		double d = (double) ((float) par2 + 0.5F)
				+ (double) (par5Random.nextFloat() - 0.5F)
		...
		if (i == 1) {
			par1World.spawnParticle("reddust", d - d4, d1 + d3, d2, 0.0D, 0.0D,
					0.0D);
		} else if (i == 2) {
			par1World.spawnParticle("reddust", d + d4, d1 + d3, d2, 0.0D, 0.0D,
					0.0D);
		} else if (i == 3) {
			par1World.spawnParticle("reddust", d, d1 + d3, d2 - d4, 0.0D, 0.0D,
					0.0D);
		} else if (i == 4) {
			par1World.spawnParticle("reddust", d, d1 + d3, d2 + d4, 0.0D, 0.0D,
					0.0D);
