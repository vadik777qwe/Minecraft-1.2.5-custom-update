...
public class BlockFenceGate extends BlockDirectional {
	...
	public AxisAlignedBB getCollisionBoundingBoxFromPool(World par1World,
		...
		if (i == 2 || i == 0) {
			return AxisAlignedBB.getBoundingBoxFromPool(par2, par3,
					(float) par4 + 0.375F, par2 + 1, (float) par3 + 1.5F,
					(float) par4 + 0.625F);
		} else {
			return AxisAlignedBB.getBoundingBoxFromPool((float) par2 + 0.375F,
					par3, par4, (float) par2 + 0.625F, (float) par3 + 1.5F,
					par4 + 1);
		}
	}
	...
	public boolean getBlocksMovement(IBlockAccess par1IBlockAccess, int par2,
			int par3, int par4) {
		return isFenceGateOpen(par1IBlockAccess.getBlockMetadata(par2, par3,
				par4));
	}
	...
	public void onBlockPlacedBy(World par1World, int par2, int par3, int par4,
		...
		int i = (MathHelper
				.floor_double((double) ((par5EntityLiving.rotationYaw * 4F) / 360F) + 0.5D) & 3) % 4;
	...
	public boolean blockActivated(World par1World, int par2, int par3,
		...
		if (isFenceGateOpen(i)) {
			...
			int j = (MathHelper
					.floor_double((double) ((par5EntityPlayer.rotationYaw * 4F) / 360F) + 0.5D) & 3) % 4;
