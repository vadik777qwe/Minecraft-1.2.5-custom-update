...
public class BlockFlowing extends BlockFluid {
	 ...
	 * count. Only source blocks of the same liquid as the block using the field
	 * are counted.
	 */
	...
	public void updateTick(World par1World, int par2, int par3, int par4,
		...
		if (blockMaterial == Material.lava
				&& !par1World.worldProvider.isHellWorld) {
			byte0 = 2;
		}
		...
		if (i > 0) {
			...
			if (blockMaterial == Material.lava && i < 8 && k < 8 && k > i
		...
		if (liquidCanDisplaceBlock(par1World, par2, par3 - 1, par4)) {
			...
			if (i >= 8) {
				par1World.setBlockAndMetadataWithNotify(par2, par3 - 1, par4,
						blockID, i);
			} else {
				par1World.setBlockAndMetadataWithNotify(par2, par3 - 1, par4,
						blockID, i + 8);
				...
				&& (i == 0 || blockBlocksFlow(par1World, par2, par3 - 1, par4))) {
			boolean aflag[] = getOptimalFlowDirections(par1World, par2, par3,
					par4);
			int l = i + byte0;
		...
		if (liquidCanDisplaceBlock(par1World, par2, par3, par4)) {
			...
			if (i > 0) {
				if (blockMaterial == Material.lava) {
					...
					Block.blocksList[i].dropBlockAsItem(par1World, par2, par3,
							par4, par1World.getBlockMetadata(par2, par3, par4),
							0);
				}
			}

			par1World.setBlockAndMetadataWithNotify(par2, par3, par4, blockID,
					par5);
		}
	}
	...
	private int calculateFlowCost(World par1World, int par2, int par3,
		...
		for (int j = 0; j < 4; j++) {
			...
			if (blockBlocksFlow(par1World, k, l, i1)
					|| par1World.getBlockMaterial(k, l, i1) == blockMaterial
					&& par1World.getBlockMetadata(k, l, i1) == 0) {
				continue;
			}
	...
	private boolean[] getOptimalFlowDirections(World par1World, int par2,
		...
		for (int i = 0; i < 4; i++) {
			...
			if (blockBlocksFlow(par1World, k, j1, k1)
					|| par1World.getBlockMaterial(k, j1, k1) == blockMaterial
					&& par1World.getBlockMetadata(k, j1, k1) == 0) {
				continue;
			}
	...
	private boolean blockBlocksFlow(World par1World, int par2, int par3,
		...
		if (i == Block.doorWood.blockID || i == Block.doorSteel.blockID
				|| i == Block.signPost.blockID || i == Block.ladder.blockID
				|| i == Block.reed.blockID) {
			return true;
		}
