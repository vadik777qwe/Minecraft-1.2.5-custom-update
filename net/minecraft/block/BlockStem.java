...
public class BlockStem extends BlockFlower {
				...
				if (i < 7) {
					...
					if (par1World.getBlockId(k, par3, l) == 0
							&& (i1 == Block.tilledField.blockID
									|| i1 == Block.dirt.blockID || i1 == Block.grass.blockID)) {
						par1World.setBlockWithNotify(k, par3, l,
								fruitType.blockID);
					}
				}
			}
	...
	private float getGrowthModifier(World par1World, int par2, int par3,
			int par4) {
		float f = 1.0F;
	...
	public void setBlockBoundsBasedOnState(IBlockAccess par1IBlockAccess,
		...
		float f = 0.125F;
		setBlockBounds(0.5F - f, 0.0F, 0.5F - f, 0.5F + f, (float) maxY,
				0.5F + f);
	}
	...
	public int func_35296_f(IBlockAccess par1IBlockAccess, int par2, int par3,
			int par4) {
		...
		return par1IBlockAccess.getBlockId(par2, par3, par4 + 1) != fruitType.blockID ? -1
				: 3;
	}
	...
	public void dropBlockAsItemWithChance(World par1World, int par2, int par3,
		...
		for (int i = 0; i < 3; i++) {
			if (par1World.rand.nextInt(15) <= par5) {
				...
				EntityItem entityitem = new EntityItem(par1World, (float) par2
						+ f1, (float) par3 + f2, (float) par4 + f3,
						new ItemStack(item));
