...
public class BlockPortal extends BlockBreakable {
	...
	public void setBlockBoundsBasedOnState(IBlockAccess par1IBlockAccess,
		...
		if (par1IBlockAccess.getBlockId(par2 - 1, par3, par4) == blockID
			...
			float f1 = 0.125F;
			float f3 = 0.5F;
			setBlockBounds(0.5F - f1, 0.0F, 0.5F - f3, 0.5F + f1, 1.0F,
					0.5F + f3);
		}
	}
	...
	public boolean tryToCreatePortal(World par1World, int par2, int par3,
		...
		for (int k = -1; k <= 2; k++) {
			for (int i1 = -1; i1 <= 3; i1++) {
				...
				int k1 = par1World.getBlockId(par2 + i * k, par3 + i1, par4 + j
						* k);
		...
		for (int l = 0; l < 2; l++) {
			for (int j1 = 0; j1 < 3; j1++) {
				par1World.setBlockWithNotify(par2 + i * l, par3 + j1, par4 + j
						* l, Block.portal.blockID);
			}
		}
	...
	public void onNeighborBlockChange(World par1World, int par2, int par3,
		...
		if (l != 3
				|| par1World.getBlockId(par2, k + l, par4) != Block.obsidian.blockID) {
			par1World.setBlockWithNotify(par2, par3, par4, 0);
			return;
		}
		...
		if ((par1World.getBlockId(par2 + i, par3, par4 + j) != Block.obsidian.blockID || par1World
				.getBlockId(par2 - i, par3, par4 - j) != blockID)
				&& (par1World.getBlockId(par2 - i, par3, par4 - j) != Block.obsidian.blockID || par1World
						.getBlockId(par2 + i, par3, par4 + j) != blockID)) {
			par1World.setBlockWithNotify(par2, par3, par4, 0);
			return;
		} else {
