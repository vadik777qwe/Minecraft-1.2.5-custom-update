			...
			for (int k = 0; k <= 3; k++) {
				...
				if ((i & l) != 0
						&& !canBePlacedOn(par1World.getBlockId(par2
								+ Direction.offsetX[k], par3, par4
								+ Direction.offsetZ[k]))
						&& (par1World.getBlockId(par2, par3 + 1, par4) != blockID || (par1World
								.getBlockMetadata(par2, par3 + 1, par4) & l) == 0)) {
					j &= ~l;
				}
			}
		}
	...
	public void updateTick(World par1World, int par2, int par3, int par4,
		...
		if (!par1World.isRemote && par1World.rand.nextInt(4) == 0) {
			...
			do {
				...
				for (int k = par4 - byte0; k <= par4 + byte0; k++) {
					...
					do {
						...
						if (par1World.getBlockId(j, i1, k) == blockID
								&& --i <= 0) {
							flag = true;
							break label0;
						}
			...
			if (l == 1 && par3 < 255
				...
				if (k1 > 0) {
					for (int j2 = 0; j2 <= 3; j2++) {
						if (!canBePlacedOn(par1World.getBlockId(par2
								+ Direction.offsetX[j2], par3 + 1, par4
								+ Direction.offsetZ[j2]))) {
							k1 &= ~(1 << j2);
						}
					}

					if (k1 > 0) {
						par1World.setBlockAndMetadataWithNotify(par2, par3 + 1,
								par4, blockID, k1);
					}
				}
				...
				int l1 = par1World.getBlockId(par2 + Direction.offsetX[j1],
						par3, par4 + Direction.offsetZ[j1]);
				...
				if (l1 == 0 || Block.blocksList[l1] == null) {
					...
					if ((j & 1 << k2) != 0
							&& canBePlacedOn(par1World.getBlockId(par2
									+ Direction.offsetX[j1]
									+ Direction.offsetX[k2], par3, par4
									+ Direction.offsetZ[j1]
									+ Direction.offsetZ[k2]))) {
						par1World.setBlockAndMetadataWithNotify(par2
								+ Direction.offsetX[j1], par3, par4
								+ Direction.offsetZ[j1], blockID, 1 << k2);
					} else if ((j & 1 << j3) != 0
							&& canBePlacedOn(par1World.getBlockId(par2
									+ Direction.offsetX[j1]
									+ Direction.offsetX[j3], par3, par4
									+ Direction.offsetZ[j1]
									+ Direction.offsetZ[j3]))) {
						par1World.setBlockAndMetadataWithNotify(par2
								+ Direction.offsetX[j1], par3, par4
								+ Direction.offsetZ[j1], blockID, 1 << j3);
					} else if ((j & 1 << k2) != 0
						...
						par1World.setBlockAndMetadataWithNotify(
								par2 + Direction.offsetX[j1]
										+ Direction.offsetX[k2], par3, par4
										+ Direction.offsetZ[j1]
										+ Direction.offsetZ[k2], blockID,
								1 << (j1 + 2 & 3));
					} else if ((j & 1 << j3) != 0
							&& par1World.isAirBlock(par2
									+ Direction.offsetX[j1]
						...
						par1World.setBlockAndMetadataWithNotify(
								par2 + Direction.offsetX[j1]
										+ Direction.offsetX[j3], par3, par4
										+ Direction.offsetZ[j1]
										+ Direction.offsetZ[j3], blockID,
								1 << (j1 + 2 & 3));
					} else if (canBePlacedOn(par1World.getBlockId(par2
							+ Direction.offsetX[j1], par3 + 1, par4
							+ Direction.offsetZ[j1]))) {
						par1World.setBlockAndMetadataWithNotify(par2
								+ Direction.offsetX[j1], par3, par4
								+ Direction.offsetZ[j1], blockID, 0);
					}
				} else if (Block.blocksList[l1].blockMaterial.isOpaque()
						&& Block.blocksList[l1].renderAsNormalBlock()) {
					par1World.setBlockMetadataWithNotify(par2, par3, par4, j
							| 1 << j1);
				}
			} else if (par3 > 1) {
				...
				if (i2 == 0) {
					...
					if (k3 != (k3 | i3)) {
						par1World.setBlockMetadataWithNotify(par2, par3 - 1,
								par4, k3 | i3);
					}
				}
			}
