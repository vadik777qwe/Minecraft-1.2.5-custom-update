...
public class BlockFarmland extends Block {
	...
	public AxisAlignedBB getCollisionBoundingBoxFromPool(World par1World,
		...
		return AxisAlignedBB.getBoundingBoxFromPool(par2 + 0, par3 + 0,
				par4 + 0, par2 + 1, par3 + 1, par4 + 1);
	}
	...
	private boolean isCropsNearby(World par1World, int par2, int par3, int par4) {
		...
		for (int j = par2 - i; j <= par2 + i; j++) {
			for (int k = par4 - i; k <= par4 + i; k++) {
				...
				if (l == Block.crops.blockID || l == Block.melonStem.blockID
						|| l == Block.pumpkinStem.blockID) {
					return true;
				}
			}
		}
