...
public class BlockTNT extends Block {
	...
	public void onBlockDestroyedByExplosion(World par1World, int par2,
		...
		if (par1World.isRemote) {
			...
			EntityTNTPrimed entitytntprimed = new EntityTNTPrimed(par1World,
					(float) par2 + 0.5F, (float) par3 + 0.5F,
					(float) par4 + 0.5F);
			entitytntprimed.fuse = par1World.rand
					.nextInt(entitytntprimed.fuse / 4)
	...
	public void onBlockDestroyedByPlayer(World par1World, int par2, int par3,
		...
		if ((par5 & 1) == 0) {
			dropBlockAsItem_do(par1World, par2, par3, par4, new ItemStack(
					Block.tnt.blockID, 1, 0));
		} else {
			EntityTNTPrimed entitytntprimed = new EntityTNTPrimed(par1World,
					(float) par2 + 0.5F, (float) par3 + 0.5F,
					(float) par4 + 0.5F);
			par1World.spawnEntityInWorld(entitytntprimed);
			par1World.playSoundAtEntity(entitytntprimed, "random.fuse", 1.0F,
					1.0F);
		}
	}
	...
	public boolean blockActivated(World par1World, int par2, int par3,
		...
		if (par5EntityPlayer.getCurrentEquippedItem() != null
			...
			return super.blockActivated(par1World, par2, par3, par4,
					par5EntityPlayer);
		}
	}
