...
public class BlockDragonEgg extends Block {
	...
	private void fallIfPossible(World par1World, int par2, int par3, int par4) {
		...
		if (BlockSand.canFallBelow(par1World, i, j - 1, k) && j >= 0) {
			...
			if (BlockSand.fallInstantly
					|| !par1World.checkChunksExist(par2 - byte0, par3 - byte0,
							par4 - byte0, par2 + byte0, par3 + byte0, par4
									+ byte0)) {
				par1World.setBlockWithNotify(par2, par3, par4, 0);

				for (; BlockSand.canFallBelow(par1World, par2, par3 - 1, par4)
						&& par3 > 0; par3--) {
				}
				...
				EntityFallingSand entityfallingsand = new EntityFallingSand(
						par1World, (float) par2 + 0.5F, (float) par3 + 0.5F,
						(float) par4 + 0.5F, blockID);
	...
	private void teleportNearby(World par1World, int par2, int par3, int par4) {
		...
		for (int i = 0; i < 1000; i++) {
			int j = (par2 + par1World.rand.nextInt(16))
					- par1World.rand.nextInt(16);
			int k = (par3 + par1World.rand.nextInt(8))
					- par1World.rand.nextInt(8);
			int l = (par4 + par1World.rand.nextInt(16))
					- par1World.rand.nextInt(16);

			if (par1World.getBlockId(j, k, l) == 0) {
				par1World.setBlockAndMetadataWithNotify(j, k, l, blockID,
				...
				for (int i1 = 0; i1 < c; i1++) {
					...
					double d1 = (double) j + (double) (par2 - j) * d
							+ (par1World.rand.nextDouble() - 0.5D) * 1.0D
							+ 0.5D;
					double d2 = ((double) k + (double) (par3 - k) * d + par1World.rand
							.nextDouble() * 1.0D) - 0.5D;
					double d3 = (double) l + (double) (par4 - l) * d
							+ (par1World.rand.nextDouble() - 0.5D) * 1.0D
							+ 0.5D;
