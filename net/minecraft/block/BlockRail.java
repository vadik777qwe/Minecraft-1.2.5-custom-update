...
public class BlockRail extends Block {
	 ...
	 */
	public static final boolean isRailBlockAt(World par0World, int par1,
			int par2, int par3) {
		int i = par0World.getBlockId(par1, par2, par3);
		return i == Block.rail.blockID || i == Block.railPowered.blockID
				|| i == Block.railDetector.blockID;
	}
	...
	public void onNeighborBlockChange(World par1World, int par2, int par3,
		...
		if (flag) {
			...
			boolean flag1 = par1World.isBlockIndirectlyGettingPowered(par2,
					par3, par4);
			flag1 = flag1
					|| isNeighborRailPowered(par1World, par2, par3, par4, i,
							true, 0)
					|| isNeighborRailPowered(par1World, par2, par3, par4, i,
							false, 0);
			boolean flag2 = false;
			...
			if (flag2) {
				...
				if (j == 2 || j == 3 || j == 4 || j == 5) {
					par1World.notifyBlocksOfNeighborChange(par2, par3 + 1,
							par4, blockID);
				}
			}
		} else if (par5 > 0
				...
				&& !isPowered
				&& RailLogic.getNAdjacentTracks(new RailLogic(this, par1World,
						par2, par3, par4)) == 3) {
	 ...
	 */
	private void refreshTrackShape(World par1World, int par2, int par3,
			int par4, boolean par5) {
		if (par1World.isRemote) {
			return;
		} else {
			(new RailLogic(this, par1World, par2, par3, par4))
					.refreshTrackShape(par1World
	...
	/**
	 * Powered minecart rail is conductive like wire, so check for powered
	 * neighbors
	 */
	private boolean isNeighborRailPowered(World par1World, int par2, int par3,
			int par4, int par5, boolean par6, int par7) {
		if (par7 >= 8) {
			return false;
		}
		...
		return flag
				&& isRailPassingPower(par1World, par2, par3 - 1, par4, par6,
						par7, i);
	}
	...
	private boolean isRailPassingPower(World par1World, int par2, int par3,
			int par4, boolean par5, int par6, int par7) {
		...
		if (i == Block.railPowered.blockID) {
			...
			if ((j & 8) != 0) {
				if (par1World.isBlockIndirectlyGettingPowered(par2, par3, par4)) {
					...
					return isNeighborRailPowered(par1World, par2, par3, par4,
							j, par5, par6 + 1);
				}
			}
		}
	 ...
	 */
	public static boolean isPoweredBlockRail(BlockRail par0BlockRail) {
