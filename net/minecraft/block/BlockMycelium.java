...
public class BlockMycelium extends Block {
	...
	public int getBlockTexture(IBlockAccess par1IBlockAccess, int par2,
		...
		Material material = par1IBlockAccess.getBlockMaterial(par2, par3 + 1,
				par4);
		return material != Material.snow && material != Material.craftedSnow ? 77
				: 68;
	}
	...
	public void updateTick(World par1World, int par2, int par3, int par4,
		...
		if (par1World.getBlockLightValue(par2, par3 + 1, par4) < 4
				&& Block.lightOpacity[par1World
						.getBlockId(par2, par3 + 1, par4)] > 2) {
			...
			for (int i = 0; i < 4; i++) {
				...
				if (par1World.getBlockId(j, k, l) == Block.dirt.blockID
						&& par1World.getBlockLightValue(j, k + 1, l) >= 4
						&& Block.lightOpacity[i1] <= 2) {
	...
	public void randomDisplayTick(World par1World, int par2, int par3,
		...
		if (par5Random.nextInt(10) == 0) {
			par1World.spawnParticle("townaura",
					(float) par2 + par5Random.nextFloat(), (float) par3 + 1.1F,
					(float) par4 + par5Random.nextFloat(), 0.0D, 0.0D, 0.0D);
		}
	}
