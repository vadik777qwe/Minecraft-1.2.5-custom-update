...
public class BlockPistonMoving extends BlockContainer {
	...
	public void dropBlockAsItemWithChance(World par1World, int par2, int par3,
		...
		TileEntityPiston tileentitypiston = getTileEntityAtLocation(par1World,
				par2, par3, par4);

		if (tileentitypiston == null) {
			return;
		} else {
			Block.blocksList[tileentitypiston.getStoredBlockID()]
					.dropBlockAsItem(par1World, par2, par3, par4,
							tileentitypiston.getBlockMetadata(), 0);
			return;
		}
	}
	 ...
	 */
	public static TileEntity getTileEntity(int par0, int par1, int par2,
	...
	public AxisAlignedBB getCollisionBoundingBoxFromPool(World par1World,
		...
		return getAxisAlignedBB(par1World, par2, par3, par4,
				tileentitypiston.getStoredBlockID(), f,
				tileentitypiston.getPistonOrientation());
	}
	...
	public void setBlockBoundsBasedOnState(IBlockAccess par1IBlockAccess,
			int par2, int par3, int par4) {
		TileEntityPiston tileentitypiston = getTileEntityAtLocation(
		...
		if (tileentitypiston != null) {
			...
			minX = block.minX
					- (double) ((float) Facing.offsetsXForSide[i] * f);
			minY = block.minY
					- (double) ((float) Facing.offsetsYForSide[i] * f);
			minZ = block.minZ
					- (double) ((float) Facing.offsetsZForSide[i] * f);
			maxX = block.maxX
					- (double) ((float) Facing.offsetsXForSide[i] * f);
			maxY = block.maxY
					- (double) ((float) Facing.offsetsYForSide[i] * f);
	...
	public AxisAlignedBB getAxisAlignedBB(World par1World, int par2, int par3,
			int par4, int par5, float par6, int par7) {
		if (par5 == 0 || par5 == blockID) {
			return null;
		}

		AxisAlignedBB axisalignedbb = Block.blocksList[par5]
