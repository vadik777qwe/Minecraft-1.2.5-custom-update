...
public class BlockSnow extends Block {
		...
		if (i >= 3) {
			return AxisAlignedBB.getBoundingBoxFromPool((double) par2 + minX,
					(double) par3 + minY, (double) par4 + minZ, (double) par2
							+ maxX, (float) par3 + 0.5F, (double) par4 + maxZ);
		} else {
			return null;
		}
	}
	...
	public boolean canPlaceBlockAt(World par1World, int par2, int par3, int par4) {
		...
		if (i == 0 || i != Block.leaves.blockID
				&& !Block.blocksList[i].isOpaqueCube()) {
			return false;
		} else {
			return par1World.getBlockMaterial(par2, par3 - 1, par4)
					.blocksMovement();
		}
	}
	...
	public void harvestBlock(World par1World, EntityPlayer par2EntityPlayer,
		...
		double d2 = (double) (par1World.rand.nextFloat() * f)
				+ (double) (1.0F - f) * 0.5D;
		EntityItem entityitem = new EntityItem(par1World, (double) par3 + d,
				(double) par4 + d1, (double) par5 + d2, new ItemStack(i, 1, 0));
