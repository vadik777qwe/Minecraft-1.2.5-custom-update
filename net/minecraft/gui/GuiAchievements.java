...
public class GuiAchievements extends GuiScreen {
	...
	public GuiAchievements(StatFileWriter par1StatFileWriter) {
		...
		char c = '\215';
		char c1 = '\215';
		field_27116_m = guiMapX = field_27112_q = AchievementList.openInventory.displayColumn
				* 24 - c / 2 - 12;
		field_27115_n = guiMapY = field_27111_r = AchievementList.openInventory.displayRow
				* 24 - c1 / 2;
	}
	...
	public void initGui() {
		controlList.clear();
		controlList.add(new GuiSmallButton(1, width / 2 + 24, height / 2 + 74,
	...
	public void drawScreen(int par1, int par2, float par3) {
		if (Mouse.isButtonDown(0)) {
			...
			if ((isMouseButtonDown == 0 || isMouseButtonDown == 1) && par1 >= k
					&& par1 < k + 224 && par2 >= l && par2 < l + 155) {
	...
	protected void genAchievementBackground(int par1, int par2, float par3) {
		int i = MathHelper.floor_double(field_27116_m
				+ (guiMapX - field_27116_m) * (double) par3);
		int j = MathHelper.floor_double(field_27115_n
				+ (guiMapY - field_27115_n) * (double) par3);
		...
		for (int i3 = 0; i3 * 16 - l2 < 155; i3++) {
			...
			for (int k3 = 0; k3 * 16 - k2 < 224; k3++) {
				...
				drawTexturedModalRect((k1 + k3 * 16) - k2, (l1 + i3 * 16) - l2,
						l4 % 16 << 4, (l4 >> 4) << 4, 16, 16);
			}
		}
		...
		for (int j3 = 0; j3 < AchievementList.achievementList.size(); j3++) {
			Achievement achievement1 = (Achievement) AchievementList.achievementList
					.get(j3);
			...
			int i5 = (achievement1.parentAchievement.displayColumn * 24 - i)
					+ 11 + k1;
			int l5 = (achievement1.parentAchievement.displayRow * 24 - j) + 11
					+ l1;
			...
			char c = Math
					.sin(((double) (System.currentTimeMillis() % 600L) / 600D)
							* Math.PI * 2D) <= 0.59999999999999998D ? '\202'
					: '\377';
			int i8 = 0xff000000;
		...
		for (int i4 = 0; i4 < AchievementList.achievementList.size(); i4++) {
			Achievement achievement2 = (Achievement) AchievementList.achievementList
					.get(i4);
			...
			if (statFileWriter.hasAchievementUnlocked(achievement2)) {
				...
				float f2 = Math
						.sin(((double) (System.currentTimeMillis() % 600L) / 600D)
								* Math.PI * 2D) >= 0.59999999999999998D ? 0.8F
						: 0.6F;
			...
			renderitem.renderItemIntoGUI(mc.fontRenderer, mc.renderEngine,
					achievement2.theItemStack, k6 + 3, j7 + 3);
			GL11.glDisable(GL11.GL_LIGHTING);
			...
			if (par1 >= k1 && par2 >= l1 && par1 < k1 + 224 && par2 < l1 + 155
					&& par1 >= k6 && par1 <= k6 + 22 && par2 >= j7
					&& par2 <= j7 + 22) {
		...
		GL11.glColor4f(1.0F, 1.0F, 1.0F, 1.0F);
		mc.renderEngine.bindTexture(l);
		drawTexturedModalRect(i1, j1, 0, 0, achievementsPaneWidth,
		...
		if (achievement != null) {
			...
			if (statFileWriter.canUnlockAchievement(achievement)) {
				...
				drawGradientRect(k5 - 3, j6 - 3, k5 + l6 + 3, j6 + k7 + 3 + 12,
						0xc0000000, 0xc0000000);
				fontRenderer.drawSplitString(s1, k5, j6 + 12, l6, 0xffa0a0a0);

				if (statFileWriter.hasAchievementUnlocked(achievement)) {
					fontRenderer
							.drawStringWithShadow(StatCollector
									.translateToLocal("achievement.taken"), k5,
									j6 + k7 + 4, 0xff9090ff);
				}
			} else {
				int i7 = Math.max(fontRenderer.getStringWidth(s), 120);
				String s2 = StatCollector.translateToLocalFormatted(
						"achievement.requires", new Object[] { StatCollector
								.translateToLocal(achievement.parentAchievement
										.getName()) });
				int l7 = fontRenderer.splitStringWidth(s2, i7);
				drawGradientRect(k5 - 3, j6 - 3, k5 + i7 + 3, j6 + l7 + 12 + 3,
						0xc0000000, 0xc0000000);
			...
			fontRenderer
					.drawStringWithShadow(
							...
							k5,
							j6,
							statFileWriter.canUnlockAchievement(achievement) ? achievement
									.getSpecial() ? -128 : -1 : achievement
									.getSpecial() ? 0xff808040 : 0xff808080);
		}
