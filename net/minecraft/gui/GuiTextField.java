...
public class GuiTextField extends Gui {
	...
	public void func_50031_b(String par1Str) {
		...
		if (text.length() > 0) {
			s = (new StringBuilder()).append(s).append(text.substring(0, i))
					.toString();
		}

		if (k < s1.length()) {
			s = (new StringBuilder()).append(s).append(s1.substring(0, k))
					.toString();
			l = k;
		} else {
		...
		if (text.length() > 0 && j < text.length()) {
			s = (new StringBuilder()).append(s).append(text.substring(j))
					.toString();
		}
	...
	public void mouseClicked(int par1, int par2, int par3) {
		boolean flag = par1 >= xPos && par1 < xPos + width && par2 >= yPos
				&& par2 < yPos + height;
		...
		if (isFocused && par3 == 0) {
			...
			String s = fontRenderer.func_50107_a(text.substring(field_50041_n),
					func_50019_l());
			func_50030_e(fontRenderer.func_50107_a(s, i).length()
					+ field_50041_n);
		}
	}
	...
	public void drawTextBox() {
		...
		boolean flag2 = field_50042_o < text.length()
				|| text.length() >= func_50040_g();
		int k1 = j1;
		...
		if (flag1) {
			if (flag2) {
				Gui.drawRect(k1, i1 - 1, k1 + 1, i1 + 1
						+ fontRenderer.FONT_HEIGHT, 0xffd0d0d0);
			} else {
	...
	public void func_50032_g(int par1) {
		...
		if (fontRenderer != null) {
			...
			if (par1 == field_50041_n) {
				field_50041_n -= fontRenderer.func_50104_a(text, j, true)
						.length();
			}
