...
import net.minecraft.render.RenderItem;
import net.minecraft.render.RenderDragon;
...
public class GuiIngame extends Gui {
	...
	public void renderGameOverlay(float par1, boolean par2, int par3, int par4) {
		...
		if (!mc.thePlayer.isPotionActive(Potion.confusion)) {
			float f = mc.thePlayer.prevTimeInPortal
		...
		if (!mc.playerController.func_35643_e()) {
			...
			drawTexturedModalRect((i / 2 - 91 - 1)
					+ inventoryplayer.currentItem * 20, j - 22 - 1, 0, 22, 24,
					22);
			GL11.glBindTexture(GL11.GL_TEXTURE_2D,
			...
			if (mc.playerController.shouldDrawHUD()) {
				...
				for (int j10 = 0; j10 < 10; j10++) {
					...
					drawTexturedModalRect(l12, j13, 16 + i12 * 9, 9 * byte3, 9,
							9);

					if (flag) {
						if (j10 * 2 + 1 < i2) {
							drawTexturedModalRect(l12, j13, j11 + 54,
									9 * byte3, 9, 9);
						}

						if (j10 * 2 + 1 == i2) {
							drawTexturedModalRect(l12, j13, j11 + 63,
									9 * byte3, 9, 9);
						}
					}

					if (j10 * 2 + 1 < i1) {
						drawTexturedModalRect(l12, j13, j11 + 36, 9 * byte3, 9,
								9);
					}

					if (j10 * 2 + 1 == i1) {
						drawTexturedModalRect(l12, j13, j11 + 45, 9 * byte3, 9,
								9);
					}
				}
				...
				if (mc.thePlayer.isInsideOfMaterial(Material.water)) {
					...
					int l11 = (int) Math
							.ceil(((double) (l10 - 2) * 10D) / 300D);
					int k12 = (int) Math.ceil(((double) l10 * 10D) / 300D)
							- l11;

					for (int i13 = 0; i13 < l11 + k12; i13++) {
						if (i13 < l11) {
							drawTexturedModalRect(i6 - i13 * 8 - 9, k8, 16, 18,
									9, 9);
						} else {
							drawTexturedModalRect(i6 - i13 * 8 - 9, k8, 25, 18,
									9, 9);
						}
					}
				}
		...
		if (mc.playerController.func_35642_f()
			...
			String s = (new StringBuilder()).append("")
					.append(mc.thePlayer.experienceLevel).toString();
		...
		if (mc.gameSettings.showDebugInfo) {
			...
			fontrenderer.drawStringWithShadow(
					(new StringBuilder()).append("Minecraft 1.2.5 (")
							.append(mc.debug).append(")").toString(), 2, 2,
					0xffffff);
			fontrenderer.drawStringWithShadow(mc.debugInfoRenders(), 2, 12,
					0xffffff);
			fontrenderer.drawStringWithShadow(mc.getEntityDebug(), 2, 22,
					0xffffff);
			fontrenderer.drawStringWithShadow(mc.debugInfoEntities(), 2, 32,
					0xffffff);
			fontrenderer.drawStringWithShadow(mc.getWorldProviderName(), 2, 42,
					0xffffff);
			long l = Runtime.getRuntime().maxMemory();
			long l2 = Runtime.getRuntime().totalMemory();
			long l3 = Runtime.getRuntime().freeMemory();
			long l5 = l2 - l3;
			String s1 = (new StringBuilder()).append("Used memory: ")
					.append((l5 * 100L) / l).append("% (")
					.append(l5 / 1024L / 1024L).append("MB) of ")
					.append(l / 1024L / 1024L).append("MB").toString();
			drawString(fontrenderer, s1, i - fontrenderer.getStringWidth(s1)
					- 2, 2, 0xe0e0e0);
			s1 = (new StringBuilder()).append("Allocated memory: ")
					.append((l2 * 100L) / l).append("% (")
					.append(l2 / 1024L / 1024L).append("MB)").toString();
			drawString(fontrenderer, s1, i - fontrenderer.getStringWidth(s1)
					- 2, 12, 0xe0e0e0);
			drawString(fontrenderer, (new StringBuilder()).append("x: ")
					.append(mc.thePlayer.posX).toString(), 2, 64, 0xe0e0e0);
			drawString(fontrenderer, (new StringBuilder()).append("y: ")
					.append(mc.thePlayer.posY).toString(), 2, 72, 0xe0e0e0);
			drawString(fontrenderer, (new StringBuilder()).append("z: ")
					.append(mc.thePlayer.posZ).toString(), 2, 80, 0xe0e0e0);
			drawString(
					fontrenderer,
					(new StringBuilder())
							.append("f: ")
			...
			if (mc.theWorld != null && mc.theWorld.blockExists(l7, l8, k9)) {
				...
				drawString(
						...
						(new StringBuilder())
								.append("lc: ")
								.append(chunk.getTopFilledSegment() + 15)
								.append(" b: ")
								.append(chunk.func_48490_a(l7 & 0xf, k9 & 0xf,
										mc.theWorld.getWorldChunkManager()).biomeName)
								.append(" bl: ")
								.append(chunk.getSavedLightValue(
										EnumSkyBlock.Block, l7 & 0xf, l8,
										k9 & 0xf))
								.append(" sl: ")
								.append(chunk.getSavedLightValue(
										EnumSkyBlock.Sky, l7 & 0xf, l8,
										k9 & 0xf))
								.append(" rl: ")
								.append(chunk.getBlockLightValue(l7 & 0xf, l8,
										k9 & 0xf, 0)).toString(), 2, 96,
						0xe0e0e0);
			}

			if (!mc.theWorld.isRemote) {
				drawString(fontrenderer, (new StringBuilder()).append("Seed: ")
						.append(mc.theWorld.getSeed()).toString(), 2, 112,
						0xe0e0e0);
			}
		...
		if (recordPlayingUpFor > 0) {
			...
			if (l1 > 0) {
				...
				fontrenderer.drawString(recordPlaying,
						-fontrenderer.getStringWidth(recordPlaying) / 2, -4, j2
								+ (l1 << 24));
				GL11.glDisable(GL11.GL_BLEND);
				GL11.glPopMatrix();
			}
		...
		if ((mc.thePlayer instanceof EntityClientPlayerMP)
			...
			int i5 = (i - i4 * k4) / 2;
			byte byte0 = 10;
			drawRect(i5 - 1, byte0 - 1, i5 + k4 * i4, byte0 + 9 * j3,
					0x80000000);
			...
			for (int k6 = 0; k6 < k2; k6++) {
				...
				fontrenderer.drawStringWithShadow(guiplayerinfo.name, j7, i8,
						0xffffff);
				mc.renderEngine.bindTexture(mc.renderEngine
						.getTexture("/gui/icons.png"));
				int l9 = 0;
				byte byte1 = 0;
				...
				zLevel += 100F;
				drawTexturedModalRect((j7 + k4) - 12, i8, 0 + l9 * 10,
						176 + byte1 * 8, 10, 8);
				zLevel -= 100F;
			}
		}
	...
	private void func_50010_a(FontRenderer par1FontRenderer) {
		...
		for (int k = 0; k + field_50017_n < chatMessageList.size() && k < byte0; k++) {
			if (((ChatLine) chatMessageList.get(k)).updateCounter >= 200
					&& !flag) {
				continue;
			}

			ChatLine chatline = (ChatLine) chatMessageList.get(k
					+ field_50017_n);
			...
			if (l1 > 2) {
				...
				GL11.glEnable(GL11.GL_BLEND);
				par1FontRenderer.drawStringWithShadow(s, byte1, j2,
						0xffffff + (l1 << 24));
			}
		}
	...
	private void renderBossHealth() {
		...
		char c = '\266';
		int j = i / 2 - c / 2;
		int k = (int) (((float) entitydragon.func_41010_ax() / (float) entitydragon
				.getMaxHealth()) * (float) (c + 1));
		byte byte0 = 12;
		...
		fontrenderer.drawStringWithShadow(s,
				i / 2 - fontrenderer.getStringWidth(s) / 2, byte0 - 10,
				0xff00ff);
		GL11.glColor4f(1.0F, 1.0F, 1.0F, 1.0F);
		GL11.glBindTexture(GL11.GL_TEXTURE_2D,
				mc.renderEngine.getTexture("/gui/icons.png"));
	...
	public void addChatMessage(String par1Str) {
		...
		for (Iterator iterator = mc.fontRenderer.func_50108_c(par1Str, 320)
				.iterator(); iterator.hasNext(); chatMessageList.add(0,
				new ChatLine(s))) {
		...
		for (; chatMessageList.size() > 100; chatMessageList
				.remove(chatMessageList.size() - 1)) {
		}
	}
	...
	public ChatClickData func_50012_a(int par1, int par2) {
		...
		if (par1 <= 320 && par2 < mc.fontRenderer.FONT_HEIGHT * i + i) {
			...
			return new ChatClickData(mc.fontRenderer,
					(ChatLine) chatMessageList.get(j), par1,
					(par2 - (j - field_50017_n) * mc.fontRenderer.FONT_HEIGHT)
							+ j);
		} else {
			return null;
		}
	...
	public void setRecordPlayingMessage(String par1Str) {
		recordPlaying = (new StringBuilder()).append("Now playing: ")
	...
	/**
	 * Adds the string to chat message after translate it with the language
	 * file.
	 */
