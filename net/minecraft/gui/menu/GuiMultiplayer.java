...
import net.minecraft.gui.GuiScreenAddServer;
import net.minecraft.gui.GuiScreenServerList;
...
public class GuiMultiplayer extends GuiScreen {
	...
	private void loadServerList() {
		try {
			...
			for (int i = 0; i < nbttaglist.tagCount(); i++) {
				serverList.add(ServerNBTStorage
						.createServerNBTStorage((NBTTagCompound) nbttaglist
								.tagAt(i)));
			}
	...
	private void saveServerList() {
		try {
			...
			for (int i = 0; i < serverList.size(); i++) {
				nbttaglist.appendTag(((ServerNBTStorage) serverList.get(i))
						.getCompoundTag());
			}

			NBTTagCompound nbttagcompound = new NBTTagCompound();
			nbttagcompound.setTag("servers", nbttaglist);
			CompressedStreamTools.safeWrite(nbttagcompound, new File(
	...
	public void initGuiControls() {
		...
		controlList.add(buttonEdit = new GuiButton(7, width / 2 - 154,
				height - 28, 70, 20, stringtranslate
						.translateKey("selectServer.edit")));
		controlList.add(buttonDelete = new GuiButton(2, width / 2 - 74,
				height - 28, 70, 20, stringtranslate
						.translateKey("selectServer.delete")));
		controlList.add(buttonSelect = new GuiButton(1, width / 2 - 154,
				height - 52, 100, 20, stringtranslate
						.translateKey("selectServer.select")));
		controlList.add(new GuiButton(4, width / 2 - 50, height - 52, 100, 20,
				stringtranslate.translateKey("selectServer.direct")));
		controlList.add(new GuiButton(3, width / 2 + 4 + 50, height - 52, 100,
				20, stringtranslate.translateKey("selectServer.add")));
		controlList.add(new GuiButton(8, width / 2 + 4, height - 28, 70, 20,
				stringtranslate.translateKey("selectServer.refresh")));
		controlList.add(new GuiButton(0, width / 2 + 4 + 76, height - 28, 75,
				20, stringtranslate.translateKey("gui.cancel")));
		boolean flag = selectedServer >= 0
				&& selectedServer < serverSlotContainer.getSize();
	...
	protected void actionPerformed(GuiButton par1GuiButton) {
		...
		if (par1GuiButton.id == 2) {
			...
			if (s != null) {
				deleteClicked = true;
				StringTranslate stringtranslate = StringTranslate.getInstance();
				String s1 = stringtranslate
						.translateKey("selectServer.deleteQuestion");
				String s2 = (new StringBuilder())
						.append("'")
						.append(s)
						.append("' ")
						.append(stringtranslate
								.translateKey("selectServer.deleteWarning"))
						.toString();
				String s3 = stringtranslate
						.translateKey("selectServer.deleteButton");
				String s4 = stringtranslate.translateKey("gui.cancel");
				GuiYesNo guiyesno = new GuiYesNo(this, s1, s2, s3, s4,
						selectedServer);
			...
			mc.displayGuiScreen(new GuiScreenServerList(this,
					tempServer = new ServerNBTStorage(StatCollector
							.translateToLocal("selectServer.defaultName"), "")));
			...
			editClicked = true;
			ServerNBTStorage servernbtstorage = (ServerNBTStorage) serverList
					.get(selectedServer);
			mc.displayGuiScreen(new GuiScreenAddServer(this,
					tempServer = new ServerNBTStorage(servernbtstorage.name,
							servernbtstorage.host)));
	...
	public void drawScreen(int par1, int par2, float par3) {
		...
		drawCenteredString(fontRenderer,
				stringtranslate.translateKey("multiplayer.title"), width / 2,
				20, 0xffffff);
	...
	private void joinServer(ServerNBTStorage par1ServerNBTStorage) {
		...
		mc.displayGuiScreen(new GuiConnecting(mc, as[0], as.length <= 1 ? 25565
				: parseIntWithDefault(as[1], 25565)));
	}

	/**
	 * Poll server for MOTD, lag, and player count/max
	 */
	private void pollServer(ServerNBTStorage par1ServerNBTStorage)
			throws IOException {
		String s = par1ServerNBTStorage.host;
		String as[] = s.split(":");
		...
		try {
			...
			par1ServerNBTStorage.motd = (new StringBuilder()).append("\2477")
					.append(s4).toString();

			if (l >= 0 && i1 > 0) {
				par1ServerNBTStorage.playerCount = (new StringBuilder())
						.append("\2477").append(l).append("\2478/\2477")
						.append(i1).toString();
			} else {
	...
	protected void func_35325_a(String par1Str, int par2, int par3) {
		if (par1Str == null) {
			...
			drawGradientRect(i - 3, j - 3, i + k + 3, j + 8 + 3, 0xc0000000,
					0xc0000000);
	 ...
	 * Poll server for MOTD, lag, and player count/max
	 */
	public static void pollServer(GuiMultiplayer par0GuiMultiplayer,
			ServerNBTStorage par1ServerNBTStorage) throws IOException {
	 ...
	 */
	static String setTooltipText(GuiMultiplayer par0GuiMultiplayer,
			String par1Str) {
