...
class GuiWorldSlot extends GuiSlot {
	...
	public GuiWorldSlot(GuiSelectWorld par1GuiSelectWorld) {
		super(par1GuiSelectWorld.mc, par1GuiSelectWorld.width,
				par1GuiSelectWorld.height, 32, par1GuiSelectWorld.height - 64,
				36);
	...
	protected void elementClicked(int par1, boolean par2) {
		...
		boolean flag = GuiSelectWorld.getSelectedWorld(parentWorldGui) >= 0
				&& GuiSelectWorld.getSelectedWorld(parentWorldGui) < getSize();
	...
	protected void drawSlot(int par1, int par2, int par3, int par4,
		...
		SaveFormatComparator saveformatcomparator = (SaveFormatComparator) GuiSelectWorld
				.getSize(parentWorldGui).get(par1);
		String s = saveformatcomparator.getDisplayName();

		if (s == null || MathHelper.stringNullOrLengthZero(s)) {
			s = (new StringBuilder())
					.append(GuiSelectWorld
							.getLocalizedWorldName(parentWorldGui)).append(" ")
					.append(par1 + 1).toString();
		}
		...
		s1 = (new StringBuilder())
				.append(s1)
				.append(" (")
				.append(GuiSelectWorld.getDateFormatter(parentWorldGui).format(
						new Date(saveformatcomparator.getLastTimePlayed())))
				.toString();
		...
		if (saveformatcomparator.requiresConversion()) {
			s2 = (new StringBuilder())
					.append(GuiSelectWorld
							.getLocalizedMustConvert(parentWorldGui))
					.append(" ").append(s2).toString();
		} else {
			s2 = GuiSelectWorld.getLocalizedGameMode(parentWorldGui)[saveformatcomparator
					.getGameType()];
			...
			if (saveformatcomparator.isHardcoreModeEnabled()) {
				s2 = (new StringBuilder())
						...
						.append("\2478").toString();
			}
		}

		parentWorldGui.drawString(parentWorldGui.fontRenderer, s, par2 + 2,
				par3 + 1, 0xffffff);
		parentWorldGui.drawString(parentWorldGui.fontRenderer, s1, par2 + 2,
				par3 + 12, 0x808080);
		parentWorldGui.drawString(parentWorldGui.fontRenderer, s2, par2 + 2,
				par3 + 12 + 10, 0x808080);
	}
}
