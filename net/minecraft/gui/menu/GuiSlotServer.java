...
public class GuiSlotServer extends GuiSlot {
	...
	public GuiSlotServer(GuiMultiplayer par1GuiMultiplayer) {
		super(par1GuiMultiplayer.mc, par1GuiMultiplayer.width,
				par1GuiMultiplayer.height, 32, par1GuiMultiplayer.height - 64,
				36);
	...
	protected void elementClicked(int par1, boolean par2) {
		...
		boolean flag = GuiMultiplayer.getSelectedServer(parentGui) >= 0
				&& GuiMultiplayer.getSelectedServer(parentGui) < getSize();
	...
	protected void drawSlot(int par1, int par2, int par3, int par4,
		...
		ServerNBTStorage servernbtstorage = (ServerNBTStorage) GuiMultiplayer
				.getServerList(parentGui).get(par1);

		synchronized (GuiMultiplayer.getLock()) {
			if (GuiMultiplayer.getThreadsPending() < 5
					&& !servernbtstorage.polled) {
		...
		parentGui.drawString(parentGui.fontRenderer, servernbtstorage.name,
				par2 + 2, par3 + 1, 0xffffff);
		parentGui.drawString(parentGui.fontRenderer, servernbtstorage.motd,
				par2 + 2, par3 + 12, 0x808080);
		parentGui.drawString(
				parentGui.fontRenderer,
				servernbtstorage.playerCount,
				(par2 + 215)
						- parentGui.fontRenderer
								.getStringWidth(servernbtstorage.playerCount),
				par3 + 12, 0x808080);
		parentGui.drawString(parentGui.fontRenderer, servernbtstorage.host,
				par2 + 2, par3 + 12 + 11, 0x303030);
		GL11.glColor4f(1.0F, 1.0F, 1.0F, 1.0F);
		parentGui.mc.renderEngine.bindTexture(parentGui.mc.renderEngine
		...
		if (servernbtstorage.polled && servernbtstorage.lag != -2L) {
			...
			if (servernbtstorage.lag < 0L) {
				...
				s = (new StringBuilder()).append(servernbtstorage.lag)
						.append("ms").toString();
			}
		} else {
			i = 1;
		...
		parentGui.drawTexturedModalRect(par2 + 205, par3, 0 + i * 10,
				176 + j * 8, 10, 8);
		byte byte0 = 4;

		if (mouseX >= (par2 + 205) - byte0 && mouseY >= par3 - byte0
				&& mouseX <= par2 + 205 + 10 + byte0
				&& mouseY <= par3 + 8 + byte0) {
