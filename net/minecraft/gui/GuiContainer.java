...
public abstract class GuiContainer extends GuiScreen {
	...
	public void drawScreen(int par1, int par2, float par3) {
		...
		for (int l = 0; l < inventorySlots.inventorySlots.size(); l++) {
			...
			if (isMouseOverSlot(slot1, par1, par2)) {
				...
				drawGradientRect(j1, k1, j1 + 16, k1 + 16, 0x80ffffff,
						0x80ffffff);
		...
		if (inventoryplayer.getItemStack() != null) {
			...
			itemRenderer.renderItemIntoGUI(fontRenderer, mc.renderEngine,
					inventoryplayer.getItemStack(), par1 - i - 8, par2 - j - 8);
			itemRenderer.renderItemOverlayIntoGUI(fontRenderer,
					mc.renderEngine, inventoryplayer.getItemStack(), par1 - i
							- 8, par2 - j - 8);
			zLevel = 0.0F;
			...
			if (list.size() > 0) {
				...
				for (int j4 = 0; j4 < list.size(); j4++) {
					...
					if (j4 == 0) {
						s = (new StringBuilder())
								.append("\247")
								.append(Integer.toHexString(itemstack
										.getRarity().nameColor)).append(s)
								.toString();
					} else {
	...
	private void drawSlotInventory(Slot par1Slot) {
		...
		if (itemstack == null) {
			...
			if (i1 >= 0) {
				...
				drawTexturedModalRect(k, l, (i1 % 16) * 16, (i1 / 16) * 16, 16,
						16);
				GL11.glEnable(GL11.GL_LIGHTING);
				flag = true;
			}
	...
	protected void mouseClicked(int par1, int par2, int par3) {
		...
		if (par3 == 0 || par3 == 1) {
			...
			int i = guiLeft;
			int j = guiTop;
			boolean flag = par1 < i || par2 < j || par1 >= i + xSize
					|| par2 >= j + ySize;
			int k = -1;
	...
	public void onGuiClosed() {
		if (mc.thePlayer == null) {
			...
			mc.playerController.func_20086_a(inventorySlots.windowId,
					mc.thePlayer);
			return;
		}
	}
