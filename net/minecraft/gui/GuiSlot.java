...
public abstract class GuiSlot {
	...
	public GuiSlot(Minecraft par1Minecraft, int par2, int par3, int par4,
			int par5, int par6) {
		initialClickY = -2F;
		selectedElement = -1;
		lastClicked = 0L;
	...
	protected abstract void drawSlot(int i, int j, int k, int l,
	...
	public void drawScreen(int par1, int par2, float par3) {
		...
		if (Mouse.isButtonDown(0)) {
			if (initialClickY == -1F) {
				...
				if (par2 >= top && par2 <= bottom) {
					...
					if (par1 >= i1 && par1 <= j1 && j2 >= 0 && l1 >= 0
							&& j2 < i) {
						boolean flag1 = j2 == selectedElement
								&& System.currentTimeMillis() - lastClicked < 250L;
						elementClicked(j2, flag1);
						selectedElement = j2;
						lastClicked = System.currentTimeMillis();
					} else if (par1 >= i1 && par1 <= j1 && l1 < 0) {
						func_27255_a(par1 - i1,
								((par2 - top) + (int) amountScrolled) - 4);
						flag = false;
					}
					...
					if (par1 >= j && par1 <= k) {
						...
						scrollMultiplier /= (float) (bottom - top - k3)
								/ (float) l2;
					} else {
				...
				amountScrolled -= ((float) par2 - initialClickY)
						* scrollMultiplier;
				initialClickY = par2;
			}
		} else {
		...
		GL11.glBindTexture(GL11.GL_TEXTURE_2D,
				mc.renderEngine.getTexture("/gui/background.png"));
		GL11.glColor4f(1.0F, 1.0F, 1.0F, 1.0F);
		float f = 32F;
		tessellator.startDrawingQuads();
		tessellator.setColorOpaque_I(0x202020);
		tessellator.addVertexWithUV(left, bottom, 0.0D, (float) left / f,
				(float) (bottom + (int) amountScrolled) / f);
		tessellator.addVertexWithUV(right, bottom, 0.0D, (float) right / f,
				(float) (bottom + (int) amountScrolled) / f);
		tessellator.addVertexWithUV(right, top, 0.0D, (float) right / f,
				(float) (top + (int) amountScrolled) / f);
		tessellator.addVertexWithUV(left, top, 0.0D, (float) left / f,
				(float) (top + (int) amountScrolled) / f);
		tessellator.draw();
		...
		for (int k2 = 0; k2 < i; k2++) {
			...
			if (field_25123_p && isSelected(k2)) {
				...
				tessellator.addVertexWithUV(j4 + 1, i3 + l3 + 1, 0.0D, 0.0D,
						1.0D);
				tessellator.addVertexWithUV(l4 - 1, i3 + l3 + 1, 0.0D, 1.0D,
						1.0D);
	...
	private void overlayBackground(int par1, int par2, int par3, int par4) {
		...
		tessellator.addVertexWithUV(width, par2, 0.0D, (float) width / f,
				(float) par2 / f);
		tessellator.setColorRGBA_I(0x404040, par3);
		tessellator.addVertexWithUV(width, par1, 0.0D, (float) width / f,
				(float) par1 / f);
