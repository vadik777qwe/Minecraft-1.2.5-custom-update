...
public abstract class GuiSlotStats extends GuiSlot {
	...
	protected GuiSlotStats(GuiStats par1GuiStats) {
		super(GuiStats.getMinecraft1(par1GuiStats), par1GuiStats.width,
				par1GuiStats.height, 32, par1GuiStats.height - 64, 20);
	...
	protected void func_27260_a(int par1, int par2, Tessellator par3Tessellator) {
		...
		if (field_27268_b == 0) {
			GuiStats.drawSprite(field_27269_g, (par1 + 115) - 18, par2 + 1, 0,
					0);
		} else {
			GuiStats.drawSprite(field_27269_g, (par1 + 115) - 18, par2 + 1, 0,
					18);
		...
		if (field_27268_b == 1) {
			GuiStats.drawSprite(field_27269_g, (par1 + 165) - 18, par2 + 1, 0,
					0);
		} else {
			GuiStats.drawSprite(field_27269_g, (par1 + 165) - 18, par2 + 1, 0,
					18);
		...
		if (field_27268_b == 2) {
			GuiStats.drawSprite(field_27269_g, (par1 + 215) - 18, par2 + 1, 0,
					0);
		} else {
			GuiStats.drawSprite(field_27269_g, (par1 + 215) - 18, par2 + 1, 0,
					18);
	...
	protected void func_27255_a(int par1, int par2) {
		...
		if (field_27268_b >= 0) {
			...
			GuiStats.getMinecraft2(field_27269_g).sndManager.playSoundFX(
	...
	protected void func_27265_a(StatCrafting par1StatCrafting, int par2,
			int par3, boolean par4) {
		if (par1StatCrafting != null) {
			String s = par1StatCrafting.func_27084_a(GuiStats
					.getStatsFileWriter(field_27269_g).writeStat(
							par1StatCrafting));
			field_27269_g.drawString(GuiStats.getFontRenderer4(field_27269_g),
					s, par2
							- GuiStats.getFontRenderer5(field_27269_g)
									.getStringWidth(s), par3 + 5,
					par4 ? 0xffffff : 0x909090);
		} else {
			String s1 = "-";
			field_27269_g.drawString(GuiStats.getFontRenderer6(field_27269_g),
					s1, par2
							- GuiStats.getFontRenderer7(field_27269_g)
									.getStringWidth(s1), par3 + 5,
					par4 ? 0xffffff : 0x909090);
		}
	}
	...
	protected void func_27257_b(int par1, int par2) {
		...
		if (i >= 0) {
			...
			s = (new StringBuilder()).append("")
					.append(StringTranslate.getInstance().translateKey(s))
					.toString().trim();

			if (s.length() > 0) {
				int k = par1 + 12;
				int l = par2 - 12;
				int i1 = GuiStats.getFontRenderer8(field_27269_g)
						.getStringWidth(s);
				GuiStats.drawGradientRect(field_27269_g, k - 3, l - 3, k + i1
						+ 3, l + 8 + 3, 0xc0000000, 0xc0000000);
				GuiStats.getFontRenderer9(field_27269_g).drawStringWithShadow(
						s, k, l, -1);
			}
		}
	}

	protected void func_27267_a(StatCrafting par1StatCrafting, int par2,
			int par3) {
		if (par1StatCrafting == null) {
			return;
		}
		...
		String s = (new StringBuilder())
				...
				.append(StringTranslate.getInstance().translateNamedKey(
						item.getItemName())).toString().trim();

		if (s.length() > 0) {
			int i = par2 + 12;
			int j = par3 - 12;
			int k = GuiStats.getFontRenderer10(field_27269_g).getStringWidth(s);
			GuiStats.drawGradientRect1(field_27269_g, i - 3, j - 3, i + k + 3,
					j + 8 + 3, 0xc0000000, 0xc0000000);
			GuiStats.getFontRenderer11(field_27269_g).drawStringWithShadow(s,
					i, j, -1);
		}
	}
