...
public class GuiContainerCreative extends GuiContainer {
	...
	protected void handleMouseClick(Slot par1Slot, int par2, int par3,
		...
		if (par1Slot != null) {
			if (par1Slot.inventory == inventory) {
				...
				inventorySlots.slotClick(par1Slot.slotNumber, par3, par4,
						mc.thePlayer);
				ItemStack itemstack = inventorySlots.getSlot(
						par1Slot.slotNumber).getStack();
				mc.playerController.sendSlotPacket(itemstack,
						(par1Slot.slotNumber - inventorySlots.inventorySlots
								.size()) + 9 + 36);
			}
		} else {
			...
			if (inventoryplayer1.getItemStack() != null) {
				if (par3 == 0) {
					mc.thePlayer
							.dropPlayerItem(inventoryplayer1.getItemStack());
					mc.playerController.func_35639_a(inventoryplayer1
							.getItemStack());
					inventoryplayer1.setItemStack(null);
				}

				if (par3 == 1) {
					ItemStack itemstack3 = inventoryplayer1.getItemStack()
							.splitStack(1);
	...
	protected void drawGuiContainerForegroundLayer() {
		fontRenderer.drawString(
				StatCollector.translateToLocal("container.creative"), 8, 6,
				0x404040);
	}
	...
	public void drawScreen(int par1, int par2, float par3) {
		...
		if (!wasClicking && flag && par1 >= k && par2 >= l && par1 < i1
				&& par2 < j1) {
			isScrolling = true;
		}
	...
	protected void drawGuiContainerBackgroundLayer(float par1, int par2,
		...
		int i1 = k + 17;
		int j1 = i1 + 160 + 2;
		drawTexturedModalRect(j + 154, k + 17
				+ (int) ((float) (j1 - i1 - 17) * currentScroll), 0, 208, 16,
				16);
	}
