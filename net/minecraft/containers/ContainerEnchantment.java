...
import net.minecraft.enchants.EnchantmentData;
...
import net.minecraft.inventory.slot.SlotEnchantment;
import net.minecraft.inventory.slot.SlotEnchantmentTable;
...
public class ContainerEnchantment extends Container {
				...
				if (!worldPointer.isRemote) {
					...
					for (int k = -1; k <= 1; k++) {
						for (int i1 = -1; i1 <= 1; i1++) {
							if (k == 0
									&& i1 == 0
									|| !worldPointer.isAirBlock(posX + i1,
											posY, posZ + k)
									|| !worldPointer.isAirBlock(posX + i1,
											posY + 1, posZ + k)) {
								continue;
							...
							if (worldPointer.getBlockId(posX + i1 * 2, posY,
									posZ + k * 2) == Block.bookShelf.blockID) {
								j++;
							}

							if (worldPointer.getBlockId(posX + i1 * 2,
									posY + 1, posZ + k * 2) == Block.bookShelf.blockID) {
								j++;
							}
							...
							if (worldPointer.getBlockId(posX + i1 * 2, posY,
									posZ + k) == Block.bookShelf.blockID) {
								j++;
							}

							if (worldPointer.getBlockId(posX + i1 * 2,
									posY + 1, posZ + k) == Block.bookShelf.blockID) {
								j++;
							}

							if (worldPointer.getBlockId(posX + i1, posY, posZ
									+ k * 2) == Block.bookShelf.blockID) {
								j++;
							}

							if (worldPointer.getBlockId(posX + i1, posY + 1,
									posZ + k * 2) == Block.bookShelf.blockID) {
								j++;
							}
						}
					...
					for (int l = 0; l < 3; l++) {
						enchantLevels[l] = EnchantmentHelper
								.calcItemStackEnchantability(rand, l, j,
										itemstack);
					}
	...
	public boolean enchantItem(EntityPlayer par1EntityPlayer, int par2) {
		...
		if (enchantLevels[par2] > 0
				&& itemstack != null
				&& (par1EntityPlayer.experienceLevel >= enchantLevels[par2] || par1EntityPlayer.capabilities.isCreativeMode)) {
			if (!worldPointer.isRemote) {
				List list = EnchantmentHelper.buildEnchantmentList(rand,
						itemstack, enchantLevels[par2]);
				...
				if (list != null) {
					...
					for (Iterator iterator = list.iterator(); iterator
							.hasNext(); itemstack.addEnchantment(
