...
public abstract class Container {
	...
	/**
	 * list of all people that need to be notified when this craftinventory
	 * changes
	 */
	...
	public void updateCraftingResults() {
		for (int i = 0; i < inventorySlots.size(); i++) {
			...
			for (int j = 0; j < crafters.size(); j++) {
				((ICrafting) crafters.get(j)).updateCraftingInventorySlot(this,
						i, itemstack1);
			}
		}
	}
	...
	public ItemStack slotClick(int par1, int par2, boolean par3,
		...
		if (par2 == 0 || par2 == 1) {
			...
			if (par1 == -999) {
				if (inventoryplayer.getItemStack() != null && par1 == -999) {
					if (par2 == 0) {
						par4EntityPlayer.dropPlayerItem(inventoryplayer
								.getItemStack());
				...
				if (slot != null) {
					...
					if (itemstack2 == null) {
						...
						int k = par2 != 0 ? (itemstack2.stackSize + 1) / 2
						...
						if (itemstack2.itemID != itemstack4.itemID
								|| itemstack2.getHasSubtypes()
								&& itemstack2.getItemDamage() != itemstack4
										.getItemDamage()
								|| !ItemStack.func_46154_a(itemstack2,
										itemstack4)) {
							if (itemstack4.stackSize <= slot
									.getSlotStackLimit()) {
							...
							if (l > itemstack4.getMaxStackSize()
									- itemstack2.stackSize) {
								l = itemstack4.getMaxStackSize()
										- itemstack2.stackSize;
							}
					...
					} else if (itemstack2.itemID == itemstack4.itemID
							&& itemstack4.getMaxStackSize() > 1
							&& (!itemstack2.getHasSubtypes() || itemstack2
									.getItemDamage() == itemstack4
									.getItemDamage())
							&& ItemStack.func_46154_a(itemstack2, itemstack4)) {
						int i1 = itemstack2.stackSize;

						if (i1 > 0
								&& i1 + itemstack4.stackSize <= itemstack4
										.getMaxStackSize()) {
							...
							slot.onPickupFromSlot(inventoryplayer
									.getItemStack());
						}
					}
				}
	...
	protected boolean mergeItemStack(ItemStack par1ItemStack, int par2,
		...
		if (par1ItemStack.isStackable()) {
			while (par1ItemStack.stackSize > 0
					&& (!par4 && i < par3 || par4 && i >= par2)) {
				Slot slot = (Slot) inventorySlots.get(i);
				ItemStack itemstack = slot.getStack();

				if (itemstack != null
						&& itemstack.itemID == par1ItemStack.itemID
						&& (!par1ItemStack.getHasSubtypes() || par1ItemStack
								.getItemDamage() == itemstack.getItemDamage())
						&& ItemStack.func_46154_a(par1ItemStack, itemstack)) {
					...
					if (k <= par1ItemStack.getMaxStackSize()) {
						...
						slot.onSlotChanged();
						flag = true;
					} else if (itemstack.stackSize < par1ItemStack
							.getMaxStackSize()) {
						par1ItemStack.stackSize -= par1ItemStack
								.getMaxStackSize() - itemstack.stackSize;
