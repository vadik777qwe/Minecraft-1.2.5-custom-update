...
public class MapGenCaves extends MapGenBase {
	...
	protected void generateLargeCaveNode(long par1, int par3, int par4,
		...
		generateCaveNode(par1, par3, par4, par5ArrayOfByte, par6, par8, par10,
	...
	protected void generateCaveNode(long par1, int par3, int par4,
		...
		for (; par15 < par16; par15++) {
			double d2 = 1.5D + (double) (MathHelper
					.sin(((float) par15 * (float) Math.PI) / (float) par16)
					* par12 * 1.0F);
			double d3 = d2 * par17;
			...
			if (!flag && par15 == j && par12 > 1.0F && par16 > 0) {
				generateCaveNode(random.nextLong(), par3, par4,
						par5ArrayOfByte, par6, par8, par10,
						random.nextFloat() * 0.5F + 0.5F, par13
								- ((float) Math.PI / 2F), par14 / 3F, par15,
						par16, 1.0D);
				generateCaveNode(random.nextLong(), par3, par4,
						par5ArrayOfByte, par6, par8, par10,
						random.nextFloat() * 0.5F + 0.5F, par13
			...
			if (par6 < d - 16D - d2 * 2D || par10 < d1 - 16D - d2 * 2D
					|| par6 > d + 16D + d2 * 2D || par10 > d1 + 16D + d2 * 2D) {
				continue;
			}
			...
			for (int j1 = (int) d4; !flag2 && j1 < k; j1++) {
				for (int l1 = (int) d6; !flag2 && l1 < i1; l1++) {
					for (int i2 = l + 1; !flag2 && i2 >= d5 - 1; i2--) {
						...
						if (par5ArrayOfByte[j2] == Block.waterMoving.blockID
								|| par5ArrayOfByte[j2] == Block.waterStill.blockID) {
							flag2 = true;
						}
			...
			for (int k1 = (int) d4; k1 < k; k1++) {
				...
				for (int k2 = (int) d6; k2 < i1; k2++) {
					double d9 = (((double) (k2 + par4 * 16) + 0.5D) - par10)
							/ d2;
						...
						if (d10 > -0.69999999999999996D
							...
							if (byte0 == Block.stone.blockID
								...
								if (i3 < 10) {
									...
									if (flag3
											&& par5ArrayOfByte[l2 - 1] == Block.dirt.blockID) {
										par5ArrayOfByte[l2 - 1] = worldObj
												.getBiomeGenForCoords(k1 + par3
														* 16, k2 + par4 * 16).topBlock;
									}
								}
							}
						}
	...
	protected void recursiveGenerate(World par1World, int par2, int par3,
		...
		for (int j = 0; j < i; j++) {
			...
			if (rand.nextInt(4) == 0) {
				generateLargeCaveNode(rand.nextLong(), par4, par5,
						par6ArrayOfByte, d, d1, d2);
				k += rand.nextInt(4);
			}
			...
			for (int l = 0; l < k; l++) {
				...
				generateCaveNode(rand.nextLong(), par4, par5, par6ArrayOfByte,
