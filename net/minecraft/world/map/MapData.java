...
public class MapData extends WorldSavedData {
	...
	public void func_28169_a(EntityPlayer par1EntityPlayer,
		...
		for (int i = 0; i < field_28174_h.size(); i++) {
			...
			if (mapinfo1.entityplayerObj.isDead
					|| !mapinfo1.entityplayerObj.inventory
							.hasItemStack(par2ItemStack)) {
			...
			float f = (float) (mapinfo1.entityplayerObj.posX - (double) xCenter)
					/ (float) (1 << scale);
			float f1 = (float) (mapinfo1.entityplayerObj.posZ - (double) zCenter)
					/ (float) (1 << scale);
			int j = 64;
			int k = 64;

			if (f < (float) (-j) || f1 < (float) (-k) || f > (float) j
					|| f1 > (float) k) {
				continue;
			}
			...
			if (mapinfo1.entityplayerObj.dimension == dimension) {
				playersVisibleOnMap.add(new MapCoord(this, byte0, byte1, byte2,
						byte3));
			}
		}
	}
	...
	public void func_28170_a(int par1, int par2, int par3) {
		...
		for (int i = 0; i < field_28174_h.size(); i++) {
			...
			if (mapinfo.field_28119_b[par1] < 0
					|| mapinfo.field_28119_b[par1] > par2) {
				mapinfo.field_28119_b[par1] = par2;
			}

			if (mapinfo.field_28124_c[par1] < 0
