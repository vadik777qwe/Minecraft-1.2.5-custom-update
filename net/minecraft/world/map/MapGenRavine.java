...
public class MapGenRavine extends MapGenBase {
	...
	protected void generateRavine(long par1, int par3, int par4,
			...
			for (int k1 = (int) d4; k1 < k; k1++) {
				...
				for (int k2 = (int) d6; k2 < i1; k2++) {
					...
					do {
						...
						if ((d8 * d8 + d9 * d9) * (double) field_35627_a[i3]
								+ (d10 * d10) / 6D < 1.0D) {
	...
	protected void recursiveGenerate(World par1World, int par2, int par3,
		...
		for (int j = 0; j < i; j++) {
			...
			generateRavine(rand.nextLong(), par4, par5, par6ArrayOfByte, d, d1,
					d2, f2, f, f1, 0, 0, 3D);
		}
	}
}
