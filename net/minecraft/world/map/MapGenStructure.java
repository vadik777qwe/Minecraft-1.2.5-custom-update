...
public abstract class MapGenStructure extends MapGenBase {
	...
	public void generate(IChunkProvider par1IChunkProvider, World par2World,
			int par3, int par4, byte par5ArrayOfByte[]) {
		super.generate(par1IChunkProvider, par2World, par3, par4,
				par5ArrayOfByte);
	}
	...
	protected void recursiveGenerate(World par1World, int par2, int par3,
		...
		if (coordMap.containsKey(Long.valueOf(ChunkCoordIntPair.chunkXZ2Int(
				par2, par3)))) {
			return;
		}
		...
		if (canSpawnStructureAtCoords(par2, par3)) {
			...
			coordMap.put(
					Long.valueOf(ChunkCoordIntPair.chunkXZ2Int(par2, par3)),
					structurestart);
		}
	}
	...
	public boolean generateStructuresInChunk(World par1World,
			Random par2Random, int par3, int par4) {
		int i = (par3 << 4) + 8;
		int j = (par4 << 4) + 8;
		boolean flag = false;
		...
		do {
			...
			if (structurestart.isSizeableStructure()
					&& structurestart.getBoundingBox().intersectsWith(i, j,
							i + 15, j + 15)) {
				structurestart.generateStructure(par1World, par2Random,
						new StructureBoundingBox(i, j, i + 15, j + 15));
				flag = true;
			}
		} while (true);
	...
	public boolean func_40483_a(int par1, int par2, int par3) {
		...
		do {
			if (iterator.hasNext()) {
				StructureStart structurestart = (StructureStart) iterator
						.next();

				if (!structurestart.isSizeableStructure()
						|| !structurestart.getBoundingBox().intersectsWith(
								par1, par3, par1, par3)) {
					continue;
				}
				...
				} while (!structurecomponent.getBoundingBox().isVecInside(par1,
						par2, par3));
	...
	public ChunkPosition getNearestInstance(World par1World, int par2,
			int par3, int par4) {
		worldObj = par1World;
		...
		do {
			...
			StructureStart structurestart = (StructureStart) ((Iterator) (obj))
					.next();

			if (structurestart.isSizeableStructure()) {
				StructureComponent structurecomponent = (StructureComponent) structurestart
						.getComponents().get(0);
