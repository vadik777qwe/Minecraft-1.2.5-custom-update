...
import net.minecraft.structures.StructureStrongholdStart;
...
public class MapGenStronghold extends MapGenStructure {
	...
	public MapGenStronghold() {
		allowedBiomeGenBases = (new BiomeGenBase[] { BiomeGenBase.desert,
	...
	protected boolean canSpawnStructureAtCoords(int par1, int par2) {
		if (!ranBiomeCheck) {
			...
			for (int k = 0; k < structureCoords.length; k++) {
				...
				ChunkPosition chunkposition = worldObj.getWorldChunkManager()
						.findBiomePosition((l << 4) + 8, (i1 << 4) + 8, 112,
								arraylist, random);
				...
				if (chunkposition != null) {
					...
					System.out.println((new StringBuilder())
							...
							.append(l).append(", ").append(i1).append(")")
							.toString());
				}
		...
		for (int j = 0; j < i; j++) {
			...
			if (par1 == chunkcoordintpair.chunkXPos
					&& par2 == chunkcoordintpair.chunkZPos) {
				System.out.println((new StringBuilder()).append(par1)
						.append(", ").append(par2).toString());
				return true;
			}
		}
	...
	protected StructureStart getStructureStart(int par1, int par2) {
		...
		for (structurestrongholdstart = new StructureStrongholdStart(worldObj,
				rand, par1, par2); structurestrongholdstart.getComponents()
				.isEmpty()
				|| ((ComponentStrongholdStairs2) structurestrongholdstart
						.getComponents().get(0)).portalRoom == null; structurestrongholdstart = new StructureStrongholdStart(
				worldObj, rand, par1, par2)) {
		}
