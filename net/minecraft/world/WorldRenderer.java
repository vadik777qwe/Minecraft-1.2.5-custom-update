...
public class WorldRenderer {
	...
	public void setPosition(int par1, int par2, int par3) {
		if (par1 == posX && par2 == posY && par3 == posZ) {
			...
			rendererBoundingBox = AxisAlignedBB.getBoundingBox(
					(float) par1 - f, (float) par2 - f, (float) par3 - f,
					(float) (par1 + 16) + f, (float) (par2 + 16) + f,
					(float) (par3 + 16) + f);
			GL11.glNewList(glRenderList + 2, GL11.GL_COMPILE);
			RenderItem.renderAABB(AxisAlignedBB.getBoundingBoxFromPool(
					(float) posXClip - f, (float) posYClip - f,
					(float) posZClip - f, (float) (posXClip + 16) + f,
					(float) (posYClip + 16) + f, (float) (posZClip + 16) + f));
			GL11.glEndList();
			markDirty();
			return;
		}
	...
	public void updateRenderer() {
		...
		int l1 = 1;
		ChunkCache chunkcache = new ChunkCache(worldObj, i - l1, j - l1,
				k - l1, l + l1, i1 + l1, j1 + l1);
				...
				for (int j2 = j; j2 < i1; j2++) {
					for (int k2 = k; k2 < j1; k2++) {
						for (int l2 = i; l2 < l; l2++) {
							...
							if (!flag2) {
								flag2 = true;
								GL11.glNewList(glRenderList + i2,
										GL11.GL_COMPILE);
								GL11.glPushMatrix();
								setupGLTranslation();
								float f = 1.000001F;
							...
							if (i2 == 0 && Block.blocksList[i3].hasTileEntity()) {
								TileEntity tileentity = chunkcache
										.getBlockTileEntity(l2, j2, k2);

								if (TileEntityRenderer.instance
										.hasSpecialRenderer(tileentity)) {
							...
							if (j3 == i2) {
								flag1 |= renderblocks.renderBlockByRenderType(
										block, l2, j2, k2);
							}
						}
					}
