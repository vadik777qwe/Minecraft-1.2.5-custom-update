...
import net.minecraft.world.save.SaveHandlerMP;
import net.minecraft.src.WorldSettings;

public class WorldClient extends World {
	/**
	 * Contains a list of blocks to to receive and process after they've been
	 * 'accepted' by the client (i.e., not invalidated).
	 */
	...
	public WorldClient(NetClientHandler par1NetClientHandler,
			WorldSettings par2WorldSettings, int par3, int par4) {
	...
	public void tick() {
		...
		for (int j = 0; j < blocksToReceive.size(); j++) {
			WorldBlockPositionType worldblockpositiontype = (WorldBlockPositionType) blocksToReceive
					.get(j);

			if (--worldblockpositiontype.acceptCountdown == 0) {
				super.setBlockAndMetadata(worldblockpositiontype.posX,
						worldblockpositiontype.posY,
						worldblockpositiontype.posZ,
						worldblockpositiontype.blockID,
						worldblockpositiontype.metadata);
				super.markBlockNeedsUpdate(worldblockpositiontype.posX,
						worldblockpositiontype.posY,
						worldblockpositiontype.posZ);
	 ...
	 * receive ticks.
	 */
	public void invalidateBlockReceiveRegion(int par1, int par2, int par3,
			int par4, int par5, int par6) {
		for (int i = 0; i < blocksToReceive.size(); i++) {
			WorldBlockPositionType worldblockpositiontype = (WorldBlockPositionType) blocksToReceive
					.get(i);

			if (worldblockpositiontype.posX >= par1
					&& worldblockpositiontype.posY >= par2
					&& worldblockpositiontype.posZ >= par3
					&& worldblockpositiontype.posX <= par4
					&& worldblockpositiontype.posY <= par5
					&& worldblockpositiontype.posZ <= par6) {
	...
	public void doPreChunk(int par1, int par2, boolean par3) {
		...
		if (!par3) {
			markBlocksDirty(par1 * 16, 0, par2 * 16, par1 * 16 + 15, 256,
					par2 * 16 + 15);
		}
	}
