...
public class VillageSiege {
	...
	private boolean func_48574_b() {
		...
		for (Iterator iterator = list.iterator(); iterator.hasNext();) {
			...
			field_48576_f = field_48582_a.villageCollectionObj
					.findNearestVillage((int) entityplayer.posX,
							(int) entityplayer.posY, (int) entityplayer.posZ, 1);

			if (field_48576_f != null
					&& field_48576_f.getNumVillageDoors() >= 10
					&& field_48576_f.getTicksSinceLastDoorAdding() >= 20
					&& field_48576_f.getNumVillagers() >= 20) {
				...
				do {
					...
					field_48577_g = chunkcoordinates.posX
							+ (int) ((double) (MathHelper
									.cos(field_48582_a.rand.nextFloat()
											* (float) Math.PI * 2.0F) * f) * 0.90000000000000002D);
					field_48583_h = chunkcoordinates.posY;
					field_48584_i = chunkcoordinates.posZ
							+ (int) ((double) (MathHelper
									.sin(field_48582_a.rand.nextFloat()
											* (float) Math.PI * 2.0F) * f) * 0.90000000000000002D);
					flag = false;
					Iterator iterator1 = field_48582_a.villageCollectionObj
							.func_48554_b().iterator();
					...
					do {
						...
						if (village == field_48576_f
								|| !village.isInRange(field_48577_g,
										field_48583_h, field_48584_i)) {
							continue;
						}
				...
				Vec3D vec3d = func_48572_a(field_48577_g, field_48583_h,
						field_48584_i);
	...
	private boolean func_48575_c() {
		...
		entityzombie.setLocationAndAngles(vec3d.xCoord, vec3d.yCoord,
				vec3d.zCoord, field_48582_a.rand.nextFloat() * 360F, 0.0F);
		field_48582_a.spawnEntityInWorld(entityzombie);
		ChunkCoordinates chunkcoordinates = field_48576_f.getCenter();
		entityzombie.setHomeArea(chunkcoordinates.posX, chunkcoordinates.posY,
				chunkcoordinates.posZ, field_48576_f.getVillageRadius());
		return true;
	}
	...
	private Vec3D func_48572_a(int par1, int par2, int par3) {
		for (int i = 0; i < 10; i++) {
			...
			if (field_48576_f.isInRange(j, k, l)
					&& SpawnerAnimals.canCreatureTypeSpawnAtLocation(
							EnumCreatureType.monster, field_48582_a, j, k, l)) {
