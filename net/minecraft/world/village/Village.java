...
public class Village {
	...
	public void tick(int par1) {
		...
		if (numIronGolems < i && villageDoorInfoList.size() > 20
				&& worldObj.rand.nextInt(7000) == 0) {
			Vec3D vec3d = tryGetIronGolemSpawningLocation(
			...
			if (vec3d != null) {
				...
				entityirongolem.setPosition(vec3d.xCoord, vec3d.yCoord,
						vec3d.zCoord);
	 ...
	 */
	private Vec3D tryGetIronGolemSpawningLocation(int par1, int par2, int par3,
		...
		for (int i = 0; i < 10; i++) {
			...
			if (isInRange(j, k, l)
					&& isValidIronGolemSpawningLocation(j, k, l, par4, par5,
							par6)) {
	...
	private void updateNumIronGolems() {
		List list = worldObj.getEntitiesWithinAABB(
				net.minecraft.entity.EntityIronGolem.class, AxisAlignedBB
						.getBoundingBoxFromPool(center.posX - villageRadius,
	...
	private void updateNumVillagers() {
		List list = worldObj.getEntitiesWithinAABB(
				net.minecraft.entity.EntityVillager.class, AxisAlignedBB
						.getBoundingBoxFromPool(center.posX - villageRadius,
								center.posY - 4, center.posZ - villageRadius,
								center.posX + villageRadius, center.posY + 4,
								center.posZ + villageRadius));
	...
	public VillageDoorInfo findNearestDoor(int par1, int par2, int par3) {
		...
		do {
			...
			VillageDoorInfo villagedoorinfo1 = (VillageDoorInfo) iterator
					.next();
	...
	public VillageDoorInfo getVillageDoorAt(int par1, int par2, int par3) {
		...
		for (Iterator iterator = villageDoorInfoList.iterator(); iterator
				.hasNext();) {
			VillageDoorInfo villagedoorinfo = (VillageDoorInfo) iterator.next();

			if (villagedoorinfo.posX == par1 && villagedoorinfo.posZ == par3
					&& Math.abs(villagedoorinfo.posY - par2) <= 1) {
				return villagedoorinfo;
			}
		}
	...
	public void addOrRenewAgressor(EntityLiving par1EntityLiving) {
		for (Iterator iterator = villageAgressors.iterator(); iterator
				.hasNext();) {
		...
		villageAgressors.add(new VillageAgressor(this, par1EntityLiving,
				tickCounter));
	}

	public EntityLiving findNearestVillageAggressor(
			EntityLiving par1EntityLiving) {
		double d = Double.MAX_VALUE;
		VillageAgressor villageagressor = null;

		for (int i = 0; i < villageAgressors.size(); i++) {
			VillageAgressor villageagressor1 = (VillageAgressor) villageAgressors
					.get(i);
			double d1 = villageagressor1.agressor
					.getDistanceSqToEntity(par1EntityLiving);
	...
	private void removeDeadAndOutOfRangeDoors() {
		...
		do {
			...
			if (!isBlockDoor(villagedoorinfo.posX, villagedoorinfo.posY,
					villagedoorinfo.posZ)
					|| Math.abs(tickCounter
							- villagedoorinfo.lastActivityTimestamp) > 1200) {
	...
	private void updateVillageRadiusAndCenter() {
		...
		center.set(centerHelper.posX / i, centerHelper.posY / i,
				centerHelper.posZ / i);
		int j = 0;
		...
		for (Iterator iterator = villageDoorInfoList.iterator(); iterator
				.hasNext();) {
			VillageDoorInfo villagedoorinfo = (VillageDoorInfo) iterator.next();
			j = Math.max(villagedoorinfo.getDistanceSquared(center.posX,
					center.posY, center.posZ), j);
		}
