	...
	public boolean generate(World par1World, Random par2Random, int par3,
		...
		for (int l = par3 - i - 1; l <= par3 + i + 1; l++) {
			for (int k1 = par4 - 1; k1 <= par4 + byte0 + 1; k1++) {
				for (int j2 = par5 - j - 1; j2 <= par5 + j + 1; j2++) {
					...
					if ((l == par3 - i - 1 || l == par3 + i + 1
							|| j2 == par5 - j - 1 || j2 == par5 + j + 1)
							&& k1 == par4
							&& par1World.isAirBlock(l, k1, j2)
							&& par1World.isAirBlock(l, k1 + 1, j2)) {
						k++;
					}
				}
			}
		...
		for (int i1 = par3 - i - 1; i1 <= par3 + i + 1; i1++) {
			for (int l1 = par4 + byte0; l1 >= par4 - 1; l1--) {
				for (int k2 = par5 - j - 1; k2 <= par5 + j + 1; k2++) {
					if (i1 == par3 - i - 1 || l1 == par4 - 1
							|| k2 == par5 - j - 1 || i1 == par3 + i + 1
							|| l1 == par4 + byte0 + 1 || k2 == par5 + j + 1) {
						if (l1 >= 0
								&& !par1World.getBlockMaterial(i1, l1 - 1, k2)
										.isSolid()) {
						...
						if (l1 == par4 - 1 && par2Random.nextInt(4) != 0) {
							par1World.setBlockWithNotify(i1, l1, k2,
		...
		for (int j1 = 0; j1 < 2; j1++) {
			...
			for (int i2 = 0; i2 < 3; i2++) {
				...
				do {
					...
					if (itemstack != null) {
						tileentitychest.setInventorySlotContents(par2Random
		...
		if (tileentitymobspawner != null) {
			...
			System.err.println((new StringBuilder())
					.append("Failed to fetch mob spawner entity at (")
					.append(par3).append(", ").append(par4).append(", ")
					.append(par5).append(")").toString());
		}
