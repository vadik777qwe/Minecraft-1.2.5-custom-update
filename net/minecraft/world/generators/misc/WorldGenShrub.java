...
public class WorldGenShrub extends WorldGenerator {
	...
	public boolean generate(World par1World, Random par2Random, int par3,
			int par4, int par5) {
		for (int i = 0; ((i = par1World.getBlockId(par3, par4, par5)) == 0 || i == Block.leaves.blockID)
				&& par4 > 0; par4--) {
		}
		...
		if (j == Block.dirt.blockID || j == Block.grass.blockID) {
			...
			setBlockAndMetadata(par1World, par3, par4, par5,
					Block.wood.blockID, field_48196_b);
			...
			for (int k = par4; k <= par4 + 2; k++) {
				...
				for (int j1 = par3 - i1; j1 <= par3 + i1; j1++) {
					...
					for (int l1 = par5 - i1; l1 <= par5 + i1; l1++) {
						...
						if ((Math.abs(k1) != i1 || Math.abs(i2) != i1 || par2Random
								.nextInt(2) != 0)
								&& !Block.opaqueCubeLookup[par1World
										.getBlockId(j1, k, l1)]) {
							setBlockAndMetadata(par1World, j1, k, l1,
									Block.leaves.blockID, field_48197_a);
						}
					}
				}
			}
