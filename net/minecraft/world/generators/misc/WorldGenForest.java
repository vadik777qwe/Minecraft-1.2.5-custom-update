	...
	public boolean generate(World par1World, Random par2Random, int par3,
		...
		for (int l = (par4 - 3) + i; l <= par4 + i; l++) {
			...
			for (int l2 = par3 - j2; l2 <= par3 + j2; l2++) {
				...
				for (int j3 = par5 - j2; j3 <= par5 + j2; j3++) {
					...
					if ((Math.abs(i3) != j2 || Math.abs(k3) != j2 || par2Random
							.nextInt(2) != 0 && k1 != 0)
							&& !Block.opaqueCubeLookup[par1World.getBlockId(l2,
									l, j3)]) {
						setBlockAndMetadata(par1World, l2, l, j3,
		...
		for (int i1 = 0; i1 < i; i1++) {
			...
			if (l1 == 0 || l1 == Block.leaves.blockID) {
				setBlockAndMetadata(par1World, par3, par4 + i1, par5,
