package net.minecraft.world.region;
...
public class RegionFile {
	...
	public RegionFile(File par1File) {
		lastModified = 0L;
		fileName = par1File;
		debugln((new StringBuilder()).append("REGION LOAD ").append(fileName)
				.toString());
		sizeDelta = 0;
	...
	private void debug(String par1Str, int par2, int par3, String par4Str) {
		debug((new StringBuilder()).append("REGION ").append(par1Str)
				.append(" ").append(fileName.getName()).append("[")
				.append(par2).append(",").append(par3).append("] = ")
				.append(par4Str).toString());
	}

	private void debug(String par1Str, int par2, int par3, int par4,
			String par5Str) {
		debug((new StringBuilder()).append("REGION ").append(par1Str)
				.append(" ").append(fileName.getName()).append("[")
				.append(par2).append(",").append(par3).append("] ")
				.append(par4).append("B = ").append(par5Str).toString());
	}

	private void debugln(String par1Str, int par2, int par3, String par4Str) {
		debug(par1Str, par2, par3, (new StringBuilder()).append(par4Str)
				.append("\n").toString());
	}
	...
	public synchronized DataInputStream getChunkDataInputStream(int par1,
		...
		try {
			...
			if (l > 4096 * k) {
				debugln("READ", par1, par2,
						(new StringBuilder()).append("invalid length: ")
								.append(l).append(" > 4096 * ").append(k)
								.toString());
				return null;
			}
			...
			if (byte0 == 2) {
				...
				debugln("READ", par1, par2,
						(new StringBuilder()).append("unknown version ")
	...
	public DataOutputStream getChunkDataOutputStream(int par1, int par2) {
		if (outOfBounds(par1, par2)) {
			...
			return new DataOutputStream(new DeflaterOutputStream(
					new RegionFileChunkBuffer(this, par1, par2)));
		}
	}
			...
			if (j != 0 && i1 == j1) {
				...
				if (l1 != -1) {
					...
					do {
						...
						} else if (((Boolean) sectorFree.get(j2))
								.booleanValue()) {
							l1 = j2;
							i2 = 1;
						}
