...
public class AnvilChunkLoader implements IThreadedFileIO, IChunkLoader {
	...
	public Chunk loadChunk(World par1World, int par2, int par3)
		...
		synchronized (field_48450_c) {
			if (field_48449_b.contains(chunkcoordintpair)) {
				...
				do {
					...
					if (((AnvilChunkLoaderPending) field_48451_a.get(i)).field_48427_a
							.equals(chunkcoordintpair)) {
						nbttagcompound = ((AnvilChunkLoaderPending) field_48451_a
								.get(i)).field_48426_b;
						break;
					}
		...
		if (nbttagcompound == null) {
			java.io.DataInputStream datainputstream = RegionFileCache
					.getChunkInputStream(chunkSaveLocation, par2, par3);
	...
	protected Chunk func_48443_a(World par1World, int par2, int par3,
			NBTTagCompound par4NBTTagCompound) {
		if (!par4NBTTagCompound.hasKey("Level")) {
			System.out.println((new StringBuilder()).append("Chunk file at ")
					.append(par2).append(",").append(par3)
		...
		Chunk chunk = func_48444_a(par1World,
				par4NBTTagCompound.getCompoundTag("Level"));

		if (!chunk.isAtLocation(par2, par3)) {
			System.out
					.println((new StringBuilder())
							.append("Chunk file at ")
							.append(par2)
							.append(",")
							.append(par3)
							.append(" is in the wrong location; relocating. (Expected ")
							.append(par2).append(", ").append(par3)
							.append(", got ").append(chunk.xPosition)
							.append(", ").append(chunk.zPosition).append(")")
							.toString());
			par4NBTTagCompound.setInteger("xPos", par2);
			par4NBTTagCompound.setInteger("zPos", par3);
			chunk = func_48444_a(par1World,
					par4NBTTagCompound.getCompoundTag("Level"));
		}
	...
	protected void func_48446_a(ChunkCoordIntPair par1ChunkCoordIntPair,
		...
		synchronized (field_48450_c) {
			if (field_48449_b.contains(par1ChunkCoordIntPair)) {
				for (int i = 0; i < field_48451_a.size(); i++) {
					if (((AnvilChunkLoaderPending) field_48451_a.get(i)).field_48427_a
						...
						field_48451_a.set(i, new AnvilChunkLoaderPending(
			...
			field_48451_a.add(new AnvilChunkLoaderPending(
					par1ChunkCoordIntPair, par2NBTTagCompound));
	...
	public boolean writeNextIO() {
		...
		synchronized (field_48450_c) {
			if (field_48451_a.size() > 0) {
				anvilchunkloaderpending = (AnvilChunkLoaderPending) field_48451_a
						.remove(0);
	...
	private void func_48447_a(
			AnvilChunkLoaderPending par1AnvilChunkLoaderPending)
			throws IOException {
		DataOutputStream dataoutputstream = RegionFileCache
				.getChunkOutputStream(chunkSaveLocation,
						par1AnvilChunkLoaderPending.field_48427_a.chunkXPos,
						par1AnvilChunkLoaderPending.field_48427_a.chunkZPos);
		CompressedStreamTools.write(par1AnvilChunkLoaderPending.field_48426_b,
				dataoutputstream);
	...
	private void func_48445_a(Chunk par1Chunk, World par2World,
		...
		ExtendedBlockStorage aextendedblockstorage[] = par1Chunk
		...
		for (int k = 0; k < i; k++) {
			...
			if (extendedblockstorage == null
					|| extendedblockstorage.func_48700_f() == 0) {
				continue;
			}

			NBTTagCompound nbttagcompound = new NBTTagCompound();
			nbttagcompound.setByte("Y",
					(byte) (extendedblockstorage.getYLocation() >> 4 & 0xff));
			nbttagcompound.setByteArray("Blocks",
					extendedblockstorage.func_48692_g());

			if (extendedblockstorage.getBlockMSBArray() != null) {
				nbttagcompound.setByteArray("Add",
						extendedblockstorage.getBlockMSBArray().data);
			}

			nbttagcompound.setByteArray("Data",
					extendedblockstorage.func_48697_j().data);
			nbttagcompound.setByteArray("SkyLight",
					extendedblockstorage.getSkylightArray().data);
			nbttagcompound.setByteArray("BlockLight",
					extendedblockstorage.getBlocklightArray().data);
		...
		for (Iterator iterator1 = par1Chunk.chunkTileEntityMap.values()
				.iterator(); iterator1.hasNext(); nbttaglist2
				.appendTag(nbttagcompound2)) {
		...
		if (list != null) {
			...
			for (Iterator iterator2 = list.iterator(); iterator2.hasNext(); nbttaglist3
					.appendTag(nbttagcompound3)) {
				NextTickListEntry nextticklistentry = (NextTickListEntry) iterator2
						.next();
				...
				nbttagcompound3.setInteger("t",
						(int) (nextticklistentry.scheduledTime - l));
			}
	...
	private Chunk func_48444_a(World par1World,
			NBTTagCompound par2NBTTagCompound) {
		...
		chunk.isTerrainPopulated = par2NBTTagCompound
		...
		for (int k = 0; k < nbttaglist.tagCount(); k++) {
			...
			ExtendedBlockStorage extendedblockstorage = new ExtendedBlockStorage(
					byte1 << 4);
			extendedblockstorage.setBlockLSBArray(nbttagcompound
					.getByteArray("Blocks"));

			if (nbttagcompound.hasKey("Add")) {
				extendedblockstorage.setBlockMSBArray(new NibbleArray(
						nbttagcompound.getByteArray("Add"), 4));
			}

			extendedblockstorage.setBlockMetadataArray(new NibbleArray(
					nbttagcompound.getByteArray("Data"), 4));
			extendedblockstorage.setSkylightArray(new NibbleArray(
					nbttagcompound.getByteArray("SkyLight"), 4));
			extendedblockstorage.setBlocklightArray(new NibbleArray(
					nbttagcompound.getByteArray("BlockLight"), 4));
		...
		if (nbttaglist1 != null) {
			for (int l = 0; l < nbttaglist1.tagCount(); l++) {
				NBTTagCompound nbttagcompound1 = (NBTTagCompound) nbttaglist1
						.tagAt(l);
				Entity entity = EntityList.createEntityFromNBT(nbttagcompound1,
						par1World);
		...
		if (nbttaglist2 != null) {
			for (int i1 = 0; i1 < nbttaglist2.tagCount(); i1++) {
				NBTTagCompound nbttagcompound2 = (NBTTagCompound) nbttaglist2
						.tagAt(i1);
				TileEntity tileentity = TileEntity
						.createAndLoadEntity(nbttagcompound2);
