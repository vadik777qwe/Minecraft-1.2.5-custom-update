...
import net.minecraft.world.WorldChunkManagerHell;
...
import net.minecraft.world.save.SaveFormatOld;
...
public class AnvilSaveConverter extends SaveFormatOld {
	...
	public List getSaveList() {
		...
		for (int j = 0; j < i; j++) {
			...
			if (worldinfo == null || worldinfo.getSaveVersion() != 19132
					&& worldinfo.getSaveVersion() != 19133) {
				continue;
			}
			...
			long l = 0L;
			arraylist.add(new SaveFormatComparator(s, s1, worldinfo
					.getLastTimePlayed(), l, worldinfo.getGameType(), flag,
	...
	public boolean isOldMapFormat(String par1Str) {
		...
		return worldinfo != null
				&& worldinfo.getSaveVersion() != func_48431_c();
	}
	...
	public boolean convertMapFormat(String par1Str,
		...
		System.out.println((new StringBuilder())
				.append("Total conversion count is ").append(i).toString());
		WorldInfo worldinfo = getWorldInfo(par1Str);
		Object obj = null;
		...
		if (worldinfo.getTerrainType() == WorldType.FLAT) {
			...
			obj = new WorldChunkManager(worldinfo.getSeed(),
					worldinfo.getTerrainType());
		}

		func_48428_a(new File(file, "region"), arraylist,
				((WorldChunkManager) (obj)), 0, i, par2IProgressUpdate);
		func_48428_a(new File(file1, "region"), arraylist1,
				new WorldChunkManagerHell(BiomeGenBase.hell, 1.0F, 0.0F),
				arraylist.size(), i, par2IProgressUpdate);
		func_48428_a(new File(file2, "region"), arraylist2,
				new WorldChunkManagerHell(BiomeGenBase.sky, 0.5F, 0.0F),
				arraylist.size() + arraylist1.size(), i, par2IProgressUpdate);
	...
	private void func_48428_a(File par1File, ArrayList par2ArrayList,
			WorldChunkManager par3WorldChunkManager, int par4, int par5,
			IProgressUpdate par6IProgressUpdate) {
		int i;

		for (Iterator iterator = par2ArrayList.iterator(); iterator.hasNext(); par6IProgressUpdate
				.setLoadingProgress(i)) {
			File file = (File) iterator.next();
			func_48430_a(par1File, file, par3WorldChunkManager, par4, par5,
					par6IProgressUpdate);
			par4++;
	...
	private void func_48430_a(File par1File, File par2File,
			WorldChunkManager par3WorldChunkManager, int par4, int par5,
			IProgressUpdate par6IProgressUpdate) {
		try {
			String s = par2File.getName();
			RegionFile regionfile = new RegionFile(par2File);
			RegionFile regionfile1 = new RegionFile(
					new File(par1File,
							(new StringBuilder())
			...
			for (int i = 0; i < 32; i++) {
				for (int j = 0; j < 32; j++) {
					if (!regionfile.isChunkSaved(i, j)
							|| regionfile1.isChunkSaved(i, j)) {
						continue;
					}

					DataInputStream datainputstream = regionfile
							.getChunkDataInputStream(i, j);
					...
					if (datainputstream == null) {
						...
						AnvilConverterData anvilconverterdata = ChunkLoader
								.load(nbttagcompound1);
						NBTTagCompound nbttagcompound2 = new NBTTagCompound();
						NBTTagCompound nbttagcompound3 = new NBTTagCompound();
						nbttagcompound2.setTag("Level", nbttagcompound3);
						ChunkLoader.convertToAnvilFormat(anvilconverterdata,
								nbttagcompound3, par3WorldChunkManager);
						DataOutputStream dataoutputstream = regionfile1
								.getChunkDataOutputStream(i, j);
						CompressedStreamTools.write(nbttagcompound2,
								dataoutputstream);
						dataoutputstream.close();
					}
				}

				int k = (int) Math.round((100D * (double) (par4 * 1024))
						/ (double) (par5 * 1024));
				int l = (int) Math
						.round((100D * (double) ((i + 1) * 32 + par4 * 1024))
								/ (double) (par5 * 1024));
