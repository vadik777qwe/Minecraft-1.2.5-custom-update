...
import net.minecraft.threads.ThreadedFileIOBase;
...
public class World implements IBlockAccess {
	...
	/**
	 * Contains a timestamp from when the World object was created. Is used in
	...
	/**
	 * Used to differentiate between a newly generated world and an already
	 * existing world.
	 */
	...
	/**
	 * entities within AxisAlignedBB excluding one, set and returned in
	 * getEntitiesWithinAABBExcludingEntity(Entity var1, AxisAlignedBB var2)
	 */
	...
	public BiomeGenBase getBiomeGenForCoords(int par1, int par2) {
		if (blockExists(par1, 0, par2)) {
			...
			if (chunk != null) {
				return chunk.func_48490_a(par1 & 0xf, par2 & 0xf,
	...
	public World(ISaveHandler par1ISaveHandler, String par2Str,
		...
		this(par1ISaveHandler, par2Str, par3WorldSettings,
				((WorldProvider) (null)));
	}
	...
	public World(ISaveHandler par1ISaveHandler, String par2Str,
		...
		if (par4WorldProvider != null) {
			...
			worldProvider = WorldProvider.getProviderForDimension(worldInfo
					.getDimension());
		} else {
	...
	protected IChunkProvider createChunkProvider() {
		...
		return new ChunkProvider(this, ichunkloader,
	...
	protected void generateSpawnPoint() {
		if (!worldProvider.canRespawnHere()) {
			worldInfo.setSpawnPosition(0,
					worldProvider.getAverageGroundLevel(), 0);
			return;
		}
		...
		ChunkPosition chunkposition = worldchunkmanager.findBiomePosition(0, 0,
				256, list, random);
		int i = 0;
	...
	/**
	 * spawns a player, load data from level.dat if needed and loads surrounding
	 * chunks
	 */
	...
	/**
	 * Saves the data for this World. If passed true, then only save up to 2
	 * chunks, otherwise, save all chunks.
	 */
	...
	/**
	 * Saves the world and all chunk data without displaying any progress
	 * message. If passed 0, then save player info and metadata as well.
	 */
	...
	public int getBlockId(int par1, int par2, int par3) {
		if (par1 < 0xfe363c80 || par3 < 0xfe363c80 || par1 >= 0x1c9c380
				|| par3 >= 0x1c9c380) {
			return 0;
		}
		...
		if (par2 >= 256) {
			...
			return getChunkFromChunkCoords(par1 >> 4, par3 >> 4).getBlockID(
					par1 & 0xf, par2, par3 & 0xf);
		}
	}
	...
	public int func_48462_d(int par1, int par2, int par3) {
		...
		if (par2 >= 256) {
			...
			return getChunkFromChunkCoords(par1 >> 4, par3 >> 4)
					.getBlockLightOpacity(par1 & 0xf, par2, par3 & 0xf);
		}
	}
	...
	public boolean setBlockAndMetadata(int par1, int par2, int par3, int par4,
		...
		if (par2 >= 256) {
			...
			boolean flag = chunk.setBlockIDWithMetadata(par1 & 0xf, par2,
					par3 & 0xf, par4, par5);
	...
	/**
	 * Sets the block to the specified blockID at the block coordinates Args x,
	 * y, z, blockID
	 */
	 ...
	 * block changed. Args: x, y, z, metadata
	 */
	...
	/**
	 * Sets the block ID and metadata, then notifies neighboring blocks of the
	 * change Params: x, y, z, BlockID, Metadata
	 */
	...
	public void markBlockNeedsUpdate(int par1, int par2, int par3) {
		for (int i = 0; i < worldAccesses.size(); i++) {
			((IWorldAccess) worldAccesses.get(i)).markBlockNeedsUpdate(par1,
					par2, par3);
		}
	}
	...
	public void markBlockAsNeedsUpdate(int par1, int par2, int par3) {
		for (int i = 0; i < worldAccesses.size(); i++) {
			((IWorldAccess) worldAccesses.get(i)).markBlockRangeNeedsUpdate(
	 ...
	 * Args: x, y, z, blockID
	 */
	private void notifyBlockOfNeighborChange(int par1, int par2, int par3,
			int par4) {
	...
	public int getFullBlockLightValue(int par1, int par2, int par3) {
		...
		return getChunkFromChunkCoords(par1 >> 4, par3 >> 4)
				.getBlockLightValue(par1 & 0xf, par2, par3 & 0xf, 0);
	}
	...
	public int getBlockLightValue_do(int par1, int par2, int par3, boolean par4) {
		...
		if (par4) {
			...
			if (i == Block.stairSingle.blockID
					...
					|| i == Block.stairCompactCobblestone.blockID
					|| i == Block.stairCompactPlanks.blockID) {
	...
	public int getHeightValue(int par1, int par2) {
		if (par1 < 0xfe363c80 || par2 < 0xfe363c80 || par1 >= 0x1c9c380
				|| par2 >= 0x1c9c380) {
			return 0;
		}
	...
	public int getSkyBlockTypeBrightness(EnumSkyBlock par1EnumSkyBlock,
		...
		if (par2 < 0xfe363c80 || par4 < 0xfe363c80 || par2 >= 0x1c9c380
				|| par4 >= 0x1c9c380) {
			return par1EnumSkyBlock.defaultLightValue;
		}
		...
		if (Block.useNeighborBrightness[getBlockId(par2, par3, par4)]) {
			...
			return chunk.getSavedLightValue(par1EnumSkyBlock, par2 & 0xf, par3,
					par4 & 0xf);
		}
	}
	...
	public void setLightValue(EnumSkyBlock par1EnumSkyBlock, int par2,
		...
		chunk.setLightValue(par1EnumSkyBlock, par2 & 0xf, par3, par4 & 0xf,
				par5);

		for (int i = 0; i < worldAccesses.size(); i++) {
			((IWorldAccess) worldAccesses.get(i)).markBlockNeedsUpdate2(par2,
					par3, par4);
		}
	}
	...
	public void func_48464_p(int par1, int par2, int par3) {
		for (int i = 0; i < worldAccesses.size(); i++) {
			((IWorldAccess) worldAccesses.get(i)).markBlockNeedsUpdate2(par1,
					par2, par3);
		}
	}
	...
	public float getLightBrightness(int par1, int par2, int par3) {
		return worldProvider.lightBrightnessTable[getBlockLightValue(par1,
				par2, par3)];
	}
	...
	public MovingObjectPosition rayTraceBlocks_do_do(Vec3D par1Vec3D,
		...
		if ((!par4 || block == null || block.getCollisionBoundingBoxFromPool(
			...
			MovingObjectPosition movingobjectposition = block
					.collisionRayTrace(this, l, i1, j1, par1Vec3D, par2Vec3D);
		...
		for (int l1 = 200; l1-- >= 0;) {
			...
			Vec3D vec3d = Vec3D.createVector(par1Vec3D.xCoord,
			...
			if ((!par4 || block1 == null || block1
					.getCollisionBoundingBoxFromPool(this, l, i1, j1) != null)
					&& j2 > 0 && block1.canCollideCheck(k2, par3)) {
				MovingObjectPosition movingobjectposition1 = block1
						.collisionRayTrace(this, l, i1, j1, par1Vec3D,
								par2Vec3D);
	...
	/**
	 * Plays a sound at the entity's position. Args: entity, sound, unknown1,
	 * volume (relative to 1.0)
	 */
	public void playSoundAtEntity(Entity par1Entity, String par2Str,
			float par3, float par4) {
		for (int i = 0; i < worldAccesses.size(); i++) {
			((IWorldAccess) worldAccesses.get(i)).playSound(par2Str,
					par1Entity.posX, par1Entity.posY
							- (double) par1Entity.yOffset, par1Entity.posZ,
					par3, par4);
		}
	}
	...
	public void playSoundEffect(double par1, double par3, double par5,
		...
		for (int i = 0; i < worldAccesses.size(); i++) {
			((IWorldAccess) worldAccesses.get(i)).playSound(par7Str, par1,
					par3, par5, par8, par9);
		}
	}
	...
	public void playRecord(String par1Str, int par2, int par3, int par4) {
		for (int i = 0; i < worldAccesses.size(); i++) {
			((IWorldAccess) worldAccesses.get(i)).playRecord(par1Str, par2,
					par3, par4);
		}
	}
	...
	public void spawnParticle(String par1Str, double par2, double par4,
		...
		for (int i = 0; i < worldAccesses.size(); i++) {
			((IWorldAccess) worldAccesses.get(i)).spawnParticle(par1Str, par2,
	...
	/**
	 * Not sure what this does 100%, but from the calling methods this method
	...
	public List getCollidingBoundingBoxes(Entity par1Entity,
		...
		for (int k1 = i; k1 < j; k1++) {
			for (int l1 = i1; l1 < j1; l1++) {
				...
				for (int i2 = k - 1; i2 < l; i2++) {
					...
					if (block != null) {
						block.getCollidingBoundingBoxes(this, k1, i2, l1,
								par2AxisAlignedBB, collidingBoundingBoxes);
					}
				}
			}
		}
		...
		double d = 0.25D;
		List list = getEntitiesWithinAABBExcludingEntity(par1Entity,
				par2AxisAlignedBB.expand(d, d, d));

		for (int j2 = 0; j2 < list.size(); j2++) {
			AxisAlignedBB axisalignedbb = ((Entity) list.get(j2))
					.getBoundingBox();

			if (axisalignedbb != null
					&& axisalignedbb.intersectsWith(par2AxisAlignedBB)) {
	...
	public float getCelestialAngle(float par1) {
		return worldProvider.calculateCelestialAngle(worldInfo.getWorldTime(),
				par1);
	}
	...
	public int getTopSolidOrLiquidBlock(int par1, int par2) {
		...
		while (i > 0) {
			...
			if (j == 0 || !Block.blocksList[j].blockMaterial.blocksMovement()
					|| Block.blocksList[j].blockMaterial == Material.leaves) {
				i--;
			} else {
				return i + 1;
			}
	...
	public void scheduleBlockUpdate(int par1, int par2, int par3, int par4,
			int par5) {
		NextTickListEntry nextticklistentry = new NextTickListEntry(par1, par2,
				par3, par4);
		byte byte0 = 8;

		if (scheduledUpdatesAreImmediate) {
			if (checkChunksExist(nextticklistentry.xCoord - byte0,
					nextticklistentry.yCoord - byte0, nextticklistentry.zCoord
							- byte0, nextticklistentry.xCoord + byte0,
					nextticklistentry.yCoord + byte0, nextticklistentry.zCoord
							+ byte0)) {
				int i = getBlockId(nextticklistentry.xCoord,
						nextticklistentry.yCoord, nextticklistentry.zCoord);

				if (i == nextticklistentry.blockID && i > 0) {
					Block.blocksList[i].updateTick(this,
		...
		if (checkChunksExist(par1 - byte0, par2 - byte0, par3 - byte0, par1
				+ byte0, par2 + byte0, par3 + byte0)) {
			if (par4 > 0) {
				nextticklistentry.setScheduledTime((long) par5
	...
	public void updateEntities() {
		...
		for (int l = 0; l < loadedEntityList.size(); l++) {
			...
			if (entity2.ridingEntity != null) {
				if (!entity2.ridingEntity.isDead
						&& entity2.ridingEntity.riddenByEntity == entity2) {
					continue;
				}
		...
		do {
			...
			if (!tileentity.isInvalid()
					...
					&& blockExists(tileentity.xCoord, tileentity.yCoord,
							tileentity.zCoord)) {
			...
			if (tileentity.isInvalid()) {
				...
				if (chunkExists(tileentity.xCoord >> 4, tileentity.zCoord >> 4)) {
					...
					if (chunk != null) {
						chunk.removeChunkBlockTileEntity(
								tileentity.xCoord & 0xf, tileentity.yCoord,
								tileentity.zCoord & 0xf);
					}
				}
			}
		} while (true);
		...
		if (!addedTileEntityList.isEmpty()) {
			...
			do {
				...
				if (!tileentity1.isInvalid()) {
					...
					if (chunkExists(tileentity1.xCoord >> 4,
							tileentity1.zCoord >> 4)) {
						Chunk chunk1 = getChunkFromChunkCoords(
								tileentity1.xCoord >> 4,
								tileentity1.zCoord >> 4);

						if (chunk1 != null) {
							chunk1.setChunkBlockTileEntity(
									tileentity1.xCoord & 0xf,
									tileentity1.yCoord,
									tileentity1.zCoord & 0xf, tileentity1);
						}
					}

					markBlockNeedsUpdate(tileentity1.xCoord,
	...
	public void updateEntityWithOptionalForce(Entity par1Entity, boolean par2) {
		...
		if (par2
				&& !checkChunksExist(i - byte0, 0, j - byte0, i + byte0, 0, j
						+ byte0)) {
			return;
		}
		...
		if (!par1Entity.addedToChunk || par1Entity.chunkCoordX != k
				|| par1Entity.chunkCoordY != l || par1Entity.chunkCoordZ != i1) {
			if (par1Entity.addedToChunk
				...
				getChunkFromChunkCoords(par1Entity.chunkCoordX,
						par1Entity.chunkCoordZ).removeEntityAtIndex(par1Entity,
						par1Entity.chunkCoordY);
			}
	...
	public boolean checkIfAABBIsClear(AxisAlignedBB par1AxisAlignedBB) {
		List list = getEntitiesWithinAABBExcludingEntity(null,
				par1AxisAlignedBB);
		...
		for (int k1 = i; k1 < j; k1++) {
			for (int l1 = k; l1 < l; l1++) {
				for (int i2 = i1; i2 < j1; i2++) {
					...
					double d1 = (float) (l1 + 1)
							- BlockFluid
									.getFluidHeightPercent(getBlockMetadata(k1,
											l1, i2));

					if ((double) l >= d1) {
						flag = true;
						block.velocityToAddToEntity(this, k1, l1, i2,
								par3Entity, vec3d);
					}
				}
			}
	...
	public Explosion newExplosion(Entity par1Entity, double par2, double par4,
			double par6, float par8, boolean par9) {
		Explosion explosion = new Explosion(this, par1Entity, par2, par4, par6,
				par8);
		...
		for (float f = 0.0F; f <= 1.0F; f = (float) ((double) f + d)) {
			for (float f1 = 0.0F; f1 <= 1.0F; f1 = (float) ((double) f1 + d1)) {
				for (float f2 = 0.0F; f2 <= 1.0F; f2 = (float) ((double) f2 + d2)) {
					double d3 = par2AxisAlignedBB.minX
							+ (par2AxisAlignedBB.maxX - par2AxisAlignedBB.minX)
							* (double) f;
					double d4 = par2AxisAlignedBB.minY
							+ (par2AxisAlignedBB.maxY - par2AxisAlignedBB.minY)
							* (double) f1;
					double d5 = par2AxisAlignedBB.minZ
							+ (par2AxisAlignedBB.maxZ - par2AxisAlignedBB.minZ)
							* (double) f2;

					if (rayTraceBlocks(Vec3D.createVector(d3, d4, d5),
							par1Vec3D) == null) {
						i++;
					}
	...
	public boolean func_48457_a(EntityPlayer par1EntityPlayer, int par2,
			int par3, int par4, int par5) {
		if (par5 == 0) {
			par3--;
		}
	...
	public String getDebugLoadedEntities() {
		return (new StringBuilder()).append("All: ")
				.append(loadedEntityList.size()).toString();
	}
	...
	public TileEntity getBlockTileEntity(int par1, int par2, int par3) {
		label0: {
			...
			label1: {
				...
				tileentity = chunk.getChunkBlockTileEntity(par1 & 0xf, par2,
						par3 & 0xf);
				...
				} while (tileentity1.isInvalid() || tileentity1.xCoord != par1
						|| tileentity1.yCoord != par2
						|| tileentity1.zCoord != par3);
	...
	public void setBlockTileEntity(int par1, int par2, int par3,
		...
		if (par4TileEntity != null && !par4TileEntity.isInvalid()) {
			if (scanningTileEntities) {
				...
				if (chunk != null) {
					chunk.setChunkBlockTileEntity(par1 & 0xf, par2, par3 & 0xf,
							par4TileEntity);
				}
			}
		}
	...
	public void tick() {
		...
		SpawnerAnimals.performSpawning(this, spawnHostileMobs,
				spawnPeacefulMobs && worldInfo.getWorldTime() % 400L == 0L);
	...
	protected void func_48461_r() {
		...
		if (!playerEntities.isEmpty()) {
			...
			int l = (MathHelper.floor_double(entityplayer1.posX) + rand
					.nextInt(11)) - 5;
			int j1 = (MathHelper.floor_double(entityplayer1.posY) + rand
					.nextInt(11)) - 5;
			int k1 = (MathHelper.floor_double(entityplayer1.posZ) + rand
					.nextInt(11)) - 5;
	...
	protected void func_48458_a(int par1, int par2, Chunk par3Chunk) {
		...
		if (ambientTickCountdown == 0) {
			...
			if (i1 == 0 && getFullBlockLightValue(j, l, k) <= rand.nextInt(8)
					&& getSavedLightValue(EnumSkyBlock.Sky, j, l, k) <= 0) {
				EntityPlayer entityplayer = getClosestPlayer((double) j + 0.5D,
						(double) l + 0.5D, (double) k + 0.5D, 8D);

				if (entityplayer != null
						&& entityplayer.getDistanceSq((double) j + 0.5D,
								(double) l + 0.5D, (double) k + 0.5D) > 4D) {
					playSoundEffect((double) j + 0.5D, (double) l + 0.5D,
							(double) k + 0.5D, "ambient.cave.cave", 0.7F,
	...
	protected void tickBlocksAndAmbiance() {
		...
		for (Iterator iterator = activeChunkSet.iterator(); iterator.hasNext(); Profiler
				.endSection()) {
			ChunkCoordIntPair chunkcoordintpair = (ChunkCoordIntPair) iterator
					.next();
			int k = chunkcoordintpair.chunkXPos * 16;
			int l = chunkcoordintpair.chunkZPos * 16;
			Profiler.startSection("getChunk");
			Chunk chunk = getChunkFromChunkCoords(chunkcoordintpair.chunkXPos,
			...
			if (rand.nextInt(16) == 0) {
				...
				if (isBlockHydratedIndirectly(l1 + k, j3 - 1, k2 + l)) {
					setBlockWithNotify(l1 + k, j3 - 1, k2 + l,
							Block.ice.blockID);
				}
			...
			for (int l2 = 0; l2 < i2; l2++) {
				...
				for (int k3 = 0; k3 < 3; k3++) {
					...
					if (block != null && block.getTickRandomly()) {
						i++;
						block.updateTick(this, i4 + k, k4
								+ extendedblockstorage.getYLocation(), j4 + l,
								rand);
					}
				}
			}
	...
	public boolean isBlockHydrated(int par1, int par2, int par3, boolean par4) {
		...
		if (par2 >= 0
			...
			if ((i == Block.waterStill.blockID || i == Block.waterMoving.blockID)
	...
	public boolean canSnowAt(int par1, int par2, int par3) {
		...
		if (par2 >= 0
			...
			if (j == 0 && Block.snow.canPlaceBlockAt(this, par1, par2, par3)
					&& i != 0 && i != Block.ice.blockID
					&& Block.blocksList[i].blockMaterial.blocksMovement()) {
				return true;
			}
		}
	...
	private int computeSkyLightValue(int par1, int par2, int par3, int par4,
			int par5, int par6) {
		int i = 0;
		...
		if (canBlockSeeTheSky(par2, par3, par4)) {
			...
			int j = getSavedLightValue(EnumSkyBlock.Sky, par2 - 1, par3, par4)
					- par6;
			int k = getSavedLightValue(EnumSkyBlock.Sky, par2 + 1, par3, par4)
					- par6;
			int l = getSavedLightValue(EnumSkyBlock.Sky, par2, par3 - 1, par4)
					- par6;
			int i1 = getSavedLightValue(EnumSkyBlock.Sky, par2, par3 + 1, par4)
					- par6;
			int j1 = getSavedLightValue(EnumSkyBlock.Sky, par2, par3, par4 - 1)
					- par6;
			int k1 = getSavedLightValue(EnumSkyBlock.Sky, par2, par3, par4 + 1)
					- par6;
	...
	private int computeBlockLightValue(int par1, int par2, int par3, int par4,
			int par5, int par6) {
		int i = Block.lightValue[par5];
		int j = getSavedLightValue(EnumSkyBlock.Block, par2 - 1, par3, par4)
				- par6;
		int k = getSavedLightValue(EnumSkyBlock.Block, par2 + 1, par3, par4)
				- par6;
		int l = getSavedLightValue(EnumSkyBlock.Block, par2, par3 - 1, par4)
				- par6;
		int i1 = getSavedLightValue(EnumSkyBlock.Block, par2, par3 + 1, par4)
				- par6;
		int j1 = getSavedLightValue(EnumSkyBlock.Block, par2, par3, par4 - 1)
				- par6;
		int k1 = getSavedLightValue(EnumSkyBlock.Block, par2, par3, par4 + 1)
				- par6;
					...
					if (k4 > 0) {
						...
						if (l5 + j6 + l6 < 17) {
							...
							while (j7 < 6) {
								...
								int j5 = getSavedLightValue(par1EnumSkyBlock,
										l7, i8, j8);
								int k8 = Block.lightOpacity[getBlockId(l7, i8,
										j8)];
								...
								if (j5 == k4 - k8
										&& j < lightUpdateBlockList.length) {
									lightUpdateBlockList[j++] = (l7 - par2)
											+ 32 + ((i8 - par3) + 32 << 6)
											+ ((j8 - par4) + 32 << 12)
											+ (k4 - k8 << 18);
								}
		...
		do {
			...
			if (k5 != k3) {
				...
				if (k5 > k3) {
					...
					if (i6 + k6 + i7 < 17
							&& j < lightUpdateBlockList.length - 6) {
						if (getSavedLightValue(par1EnumSkyBlock, j1 - 1, i2, l2) < k5) {
							lightUpdateBlockList[j++] = (j1 - 1 - par2) + 32
									+ ((i2 - par3) + 32 << 6)
									+ ((l2 - par4) + 32 << 12);
						}
						...
						if (getSavedLightValue(par1EnumSkyBlock, j1, i2 - 1, l2) < k5) {
							lightUpdateBlockList[j++] = (j1 - par2) + 32
									+ ((i2 - 1 - par3) + 32 << 6)
									+ ((l2 - par4) + 32 << 12);
						}
	...
	public boolean tickUpdates(boolean par1) {
		...
		for (int j = 0; j < i; j++) {
			NextTickListEntry nextticklistentry = (NextTickListEntry) scheduledTickTreeSet
					.first();

			if (!par1
					&& nextticklistentry.scheduledTime > worldInfo
							.getWorldTime()) {
				break;
			}
			...
			int k = getBlockId(nextticklistentry.xCoord,
					nextticklistentry.yCoord, nextticklistentry.zCoord);

			if (k == nextticklistentry.blockID && k > 0) {
				Block.blocksList[k].updateTick(this, nextticklistentry.xCoord,
						nextticklistentry.yCoord, nextticklistentry.zCoord,
						rand);
			}
		}
	...
	public List getPendingBlockUpdates(Chunk par1Chunk, boolean par2) {
		...
		do {
			...
			NextTickListEntry nextticklistentry = (NextTickListEntry) iterator
					.next();
	...
	/**
	 * Randomly will call the random display update on a 1000 blocks within 16
	 * units of the specified position. Args: x, y, z
	 */
	public void randomDisplayUpdates(int par1, int par2, int par3) {
		...
		for (int i = 0; i < 1000; i++) {
			...
			if (i1 == 0 && rand.nextInt(8) > k
					&& worldProvider.getWorldHasNoSky()) {
				spawnParticle("depthsuspend", (float) j + rand.nextFloat(),
						(float) k + rand.nextFloat(),
						(float) l + rand.nextFloat(), 0.0D, 0.0D, 0.0D);
				continue;
			}
	 ...
	 */
	public void updateTileEntityChunkAndDoNothing(int par1, int par2, int par3,
			TileEntity par4TileEntity) {
		...
		for (int i = 0; i < worldAccesses.size(); i++) {
			((IWorldAccess) worldAccesses.get(i)).doNothingWithTileEntity(par1,
					par2, par3, par4TileEntity);
		}
	}
	...
	/**
	 * Returns true if the specified block can be placed at the given
	 * coordinates, optionally making sure there are no entities in the way.
	 * Args: blockID, x, y, z, ignoreEntities
	 */
	public boolean canBlockBePlacedAt(int par1, int par2, int par3, int par4,
			boolean par5, int par6) {
		int i = getBlockId(par2, par3, par4);
		Block block = Block.blocksList[i];
		Block block1 = Block.blocksList[par1];
		AxisAlignedBB axisalignedbb = block1.getCollisionBoundingBoxFromPool(
				this, par2, par3, par4);
		...
		if (block != null
				&& (block == Block.waterMoving || block == Block.waterStill
						|| block == Block.lavaMoving
						|| block == Block.lavaStill || block == Block.fire || block.blockMaterial
							.isGroundCover())) {
			block = null;
		}

		return par1 > 0 && block == null
				&& block1.canPlaceBlockOnSide(this, par2, par3, par4, par6);
	}
	...
	public PathEntity getPathEntityToEntity(Entity par1Entity,
		...
		PathEntity pathentity = (new PathFinder(chunkcache, par4, par5, par6,
				par7)).createEntityPathTo(par1Entity, par2Entity, par3);
		Profiler.endSection();
		return pathentity;
	}
	...
	public PathEntity getEntityPathToXYZ(Entity par1Entity, int par2, int par3,
		...
		int j2 = k + l;
		ChunkCache chunkcache = new ChunkCache(this, i1, j1, k1, l1, i2, j2);
		PathEntity pathentity = (new PathFinder(chunkcache, par6, par7, par8,
				par9)).createEntityPathTo(par1Entity, par2, par3, par4, par5);
		Profiler.endSection();
		return pathentity;
	}
	 ...
	 * direction
	 */
	public boolean isBlockProvidingPowerTo(int par1, int par2, int par3,
			int par4) {
		int i = getBlockId(par1, par2, par3);
		...
		if (i == 0) {
			...
			return Block.blocksList[i].isIndirectlyPoweringTo(this, par1, par2,
					par3, par4);
		}
	}
	...
	/**
	 * Is a block next to you getting powered (if its an attachable block) or is
	 * it providing power directly to you. Args: x, y, z, direction
	 */
	public boolean isBlockIndirectlyProvidingPowerTo(int par1, int par2,
			int par3, int par4) {
		...
		if (i == 0) {
			...
			return Block.blocksList[i].isPoweringTo(this, par1, par2, par3,
					par4);
		}
	}
	...
	public EntityPlayer func_48456_a(double par1, double par3, double par5) {
		...
		for (int i = 0; i < playerEntities.size(); i++) {
			EntityPlayer entityplayer1 = (EntityPlayer) playerEntities.get(i);
			double d1 = entityplayer1.getDistanceSq(par1, entityplayer1.posY,
					par3);
	...
	public void setSpawnPoint(ChunkCoordinates par1ChunkCoordinates) {
		worldInfo.setSpawnPosition(par1ChunkCoordinates.posX,
	 ...
	 */
	public boolean canMineBlock(EntityPlayer par1EntityPlayer, int par2,
			int par3, int i) {
		return true;
	}
	...
	public void updateEntityList() {
		...
		for (int k = 0; k < loadedEntityList.size(); k++) {
			...
			if (entity1.ridingEntity != null) {
				if (!entity1.ridingEntity.isDead
						&& entity1.ridingEntity.riddenByEntity == entity1) {
					continue;
				}
	...
	public boolean isAllPlayersFullyAsleep() {
		if (allPlayersSleeping && !isRemote) {
			for (Iterator iterator = playerEntities.iterator(); iterator
					.hasNext();) {
				EntityPlayer entityplayer = (EntityPlayer) iterator.next();
	...
	public void playAuxSFXAtEntity(EntityPlayer par1EntityPlayer, int par2,
		...
		for (int i = 0; i < worldAccesses.size(); i++) {
			((IWorldAccess) worldAccesses.get(i)).playAuxSFX(par1EntityPlayer,
	...
	public Random setRandomSeed(int par1, int par2, int par3) {
		long l = (long) par1 * 0x4f9939f508L + (long) par2 * 0x1ef1565bd5L
				+ getWorldInfo().getSeed() + (long) par3;
		rand.setSeed(l);
		return rand;
	}

	/**
	 * Updates lighting. Returns true if there are more lighting updates to
	 * update
	 */
	 ...
	 */
	public SpawnListEntry getRandomMob(EnumCreatureType par1EnumCreatureType,
			int par2, int par3, int par4) {
		List list = getChunkProvider().getPossibleCreatures(
