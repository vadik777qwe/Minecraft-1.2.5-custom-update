package net.minecraft.world.save;
...
public class SaveFormatOld implements ISaveFormat {
	...
	public List getSaveList() {
		...
		for (int i = 0; i < 5; i++) {
			String s = (new StringBuilder()).append("World").append(i + 1)
					.toString();
			WorldInfo worldinfo = getWorldInfo(s);

			if (worldinfo != null) {
				arraylist.add(new SaveFormatComparator(s, "", worldinfo
						.getLastTimePlayed(), worldinfo.getSizeOnDisk(),
						worldinfo.getGameType(), false, worldinfo
		...
		if (file1.exists()) {
			try {
				NBTTagCompound nbttagcompound = CompressedStreamTools
						.readCompressed(new FileInputStream(file1));
	...
	protected static void deleteFiles(File par0ArrayOfFile[]) {
		for (int i = 0; i < par0ArrayOfFile.length; i++) {
			if (par0ArrayOfFile[i].isDirectory()) {
				System.out.println((new StringBuilder()).append("Deleting ")
						.append(par0ArrayOfFile[i]).toString());
