...
public class Chunk {
				...
				do {
					...
					if (j1 > 0) {
						...
						if (extendedblockstorage != null) {
							extendedblockstorage.setExtSkylightValue(j,
									k1 & 0xf, l, j1);
							worldObj.func_48464_p((xPosition << 4) + j, k1,
									(zPosition << 4) + l);
						}
					}
				} while (--k1 > 0 && j1 > 0);
	...
	private void updateSkylightNeighborHeight(int par1, int par2, int par3,
			int par4) {
		...
		if (!worldObj.worldProvider.hasNoSky) {
			if (j < i) {
				for (int i1 = j; i1 < i; i1++) {
					...
					if (extendedblockstorage != null) {
						extendedblockstorage.setExtSkylightValue(par1,
								i1 & 0xf, par3, 15);
						worldObj.func_48464_p((xPosition << 4) + par1, i1,
								(zPosition << 4) + par3);
					}
				}
			} else {
				for (int j1 = i; j1 < j; j1++) {
					...
					if (extendedblockstorage1 != null) {
						extendedblockstorage1.setExtSkylightValue(par1,
								j1 & 0xf, par3, 0);
						worldObj.func_48464_p((xPosition << 4) + par1, j1,
								(zPosition << 4) + par3);
					}
				}
			}
			...
			do {
				...
				if (extendedblockstorage2 != null) {
					extendedblockstorage2.setExtSkylightValue(par1, j & 0xf,
							par3, k1);
				}
			} while (true);
		}
	...
	public int getBlockMetadata(int par1, int par2, int par3) {
		...
		if (extendedblockstorage != null) {
			return extendedblockstorage.getExtBlockMetadata(par1, par2 & 0xf,
					par3);
		} else {
			return 0;
		}
	 ...
	 * blockID, metadata
	 */
	public boolean setBlockIDWithMetadata(int par1, int par2, int par3,
			int par4, int par5) {
		int i = par3 << 4 | par1;
		...
		if (extendedblockstorage == null) {
			...
			extendedblockstorage = storageArrays[par2 >> 4] = new ExtendedBlockStorage(
					(par2 >> 4) << 4);
			flag = par2 >= j;
		}
		...
		if (k != 0) {
			...
			} else if ((Block.blocksList[k] instanceof BlockContainer)
					&& k != par4) {
		...
		if (par4 != 0) {
			...
			if (Block.blocksList[par4] instanceof BlockContainer) {
				TileEntity tileentity = getChunkBlockTileEntity(par1, par2,
						par3);

				if (tileentity == null) {
					tileentity = ((BlockContainer) Block.blocksList[par4])
							.getBlockEntity();
	...
	public boolean setBlockMetadata(int par1, int par2, int par3, int par4) {
		...
		int i = extendedblockstorage
				.getExtBlockMetadata(par1, par2 & 0xf, par3);
	...
	public int getSavedLightValue(EnumSkyBlock par1EnumSkyBlock, int par2,
		...
		if (par1EnumSkyBlock == EnumSkyBlock.Sky) {
			return extendedblockstorage.getExtSkylightValue(par2, par3 & 0xf,
					par4);
		}

		if (par1EnumSkyBlock == EnumSkyBlock.Block) {
			return extendedblockstorage.getExtBlocklightValue(par2, par3 & 0xf,
					par4);
		} else {
			return par1EnumSkyBlock.defaultLightValue;
		}
	...
	public void setLightValue(EnumSkyBlock par1EnumSkyBlock, int par2,
		...
		if (extendedblockstorage == null) {
			extendedblockstorage = storageArrays[par3 >> 4] = new ExtendedBlockStorage(
					(par3 >> 4) << 4);
			generateSkylightMap();
		}
		...
		if (par1EnumSkyBlock == EnumSkyBlock.Sky) {
			if (!worldObj.worldProvider.hasNoSky) {
				extendedblockstorage.setExtSkylightValue(par2, par3 & 0xf,
						par4, par5);
			}
		} else if (par1EnumSkyBlock == EnumSkyBlock.Block) {
			extendedblockstorage.setExtBlocklightValue(par2, par3 & 0xf, par4,
					par5);
		} else {
			return;
		}
	...
	public int getBlockLightValue(int par1, int par2, int par3, int par4) {
		...
		if (extendedblockstorage == null) {
			if (!worldObj.worldProvider.hasNoSky
					&& par4 < EnumSkyBlock.Sky.defaultLightValue) {
		...
		int i = worldObj.worldProvider.hasNoSky ? 0 : extendedblockstorage
				.getExtSkylightValue(par1, par2 & 0xf, par3);
		...
		i -= par4;
		int j = extendedblockstorage.getExtBlocklightValue(par1, par2 & 0xf,
				par3);
	...
	public void addEntity(Entity par1Entity) {
		...
		if (i != xPosition || j != zPosition) {
			System.out.println((new StringBuilder()).append("Wrong location! ")
					.append(par1Entity).toString());
			Thread.dumpStack();
		}
	...
	public TileEntity getChunkBlockTileEntity(int par1, int par2, int par3) {
		...
		TileEntity tileentity = (TileEntity) chunkTileEntityMap
				.get(chunkposition);
		...
		if (tileentity == null) {
			...
			if (tileentity == null) {
				tileentity = ((BlockContainer) Block.blocksList[i])
						.getBlockEntity();
				worldObj.setBlockTileEntity(xPosition * 16 + par1, par2,
						zPosition * 16 + par3, tileentity);
			}
	 ...
	 */
	public void setChunkBlockTileEntity(int par1, int par2, int par3,
		...
		if (getBlockID(par1, par2, par3) == 0
				|| !(Block.blocksList[getBlockID(par1, par2, par3)] instanceof BlockContainer)) {
			return;
		} else {
	...
	public void onChunkUnload() {
		...
		for (Iterator iterator = chunkTileEntityMap.values().iterator(); iterator
				.hasNext(); worldObj.markTileEntityForDespawn(tileentity)) {
		...
		for (int k = i; k <= j; k++) {
			...
			for (int l = 0; l < list.size(); l++) {
				...
				if (par1Class.isAssignableFrom(entity.getClass())
						&& entity.boundingBox.intersectsWith(par2AxisAlignedBB)) {
					par3List.add(entity);
				}
			}
		}
	...
	public boolean needsSaving(boolean par1) {
		if (par1) {
			if (hasEntities && worldObj.getWorldTime() != lastSaveTime) {
				...
				&& worldObj.getWorldTime() >= lastSaveTime + 600L) {
			return true;
		}
	...
	public void populateChunk(IChunkProvider par1IChunkProvider,
			IChunkProvider par2IChunkProvider, int par3, int par4) {
	...
	public int getPrecipitationHeight(int par1, int par2) {
		...
		if (j == -999) {
			...
			for (j = -1; k > 0 && j == -1;) {
				...
				Material material = l != 0 ? Block.blocksList[l].blockMaterial
						: Material.air;
	...
	public boolean getAreLevelsEmpty(int par1, int par2) {
		...
		for (int i = par1; i <= par2; i += 16) {
			...
			if (extendedblockstorage != null
					&& !extendedblockstorage.getIsEmpty()) {
				return false;
			}
		}
	...
	public void setStorageArrays(
			ExtendedBlockStorage par1ArrayOfExtendedBlockStorage[]) {
		storageArrays = par1ArrayOfExtendedBlockStorage;
	}

	public void func_48494_a(byte par1ArrayOfByte[], int par2, int par3,
			boolean par4) {
		int i = 0;
		...
		for (int k = 0; k < storageArrays.length; k++) {
			if ((par2 & 1 << k) != 0 && storageArrays[k] != null) {
				...
				System.arraycopy(par1ArrayOfByte, i, nibblearray.data, 0,
						nibblearray.data.length);
		...
		for (int l = 0; l < storageArrays.length; l++) {
			if ((par2 & 1 << l) != 0 && storageArrays[l] != null) {
				NibbleArray nibblearray1 = storageArrays[l]
						.getBlocklightArray();
				System.arraycopy(par1ArrayOfByte, i, nibblearray1.data, 0,
						nibblearray1.data.length);
		...
		for (int i1 = 0; i1 < storageArrays.length; i1++) {
			if ((par2 & 1 << i1) != 0 && storageArrays[i1] != null) {
				...
				System.arraycopy(par1ArrayOfByte, i, nibblearray2.data, 0,
						nibblearray2.data.length);
		...
		for (int j1 = 0; j1 < storageArrays.length; j1++) {
			if ((par3 & 1 << j1) != 0) {
				...
				System.arraycopy(par1ArrayOfByte, i, nibblearray3.data, 0,
						nibblearray3.data.length);
				i += nibblearray3.data.length;
				continue;
			}

			if (par4 && storageArrays[j1] != null
					&& storageArrays[j1].getBlockMSBArray() != null) {
				storageArrays[j1].func_48715_h();
			}
		}

		if (par4) {
			System.arraycopy(par1ArrayOfByte, i, blockBiomeArray, 0,
		...
		for (Iterator iterator = chunkTileEntityMap.values().iterator(); iterator
				.hasNext(); tileentity.updateContainingBlockInfo()) {
			tileentity = (TileEntity) iterator.next();
		}
	}

	public BiomeGenBase func_48490_a(int par1, int par2,
			WorldChunkManager par3WorldChunkManager) {
		int i = blockBiomeArray[par2 << 4 | par1] & 0xff;

		if (i == 255) {
			BiomeGenBase biomegenbase = par3WorldChunkManager.getBiomeGenAt(
	...
	public void enqueueRelightChecks() {
		for (int i = 0; i < 8; i++) {
			...
			for (int k1 = 0; k1 < 16; k1++) {
				...
				if ((storageArrays[j] != null || k1 != 0 && k1 != 15 && k != 0
						&& k != 15 && l != 0 && l != 15)
						&& (storageArrays[j] == null || storageArrays[j]
								.getExtBlockID(k, k1, l) != 0)) {
					continue;
				}
