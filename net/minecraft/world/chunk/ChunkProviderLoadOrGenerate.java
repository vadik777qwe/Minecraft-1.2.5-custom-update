...
public class ChunkProviderLoadOrGenerate implements IChunkProvider {
	/**
	 * A completely empty Chunk, used by ChunkProviderLoadOrGenerate when
	 * there's no ChunkProvider.
	 */
	...
	/**
	 * The last X position of a chunk that was returned from
	 * setRecordPlayingMessage
	 */
	int lastQueriedChunkXPos;

	/**
	 * The last Z position of a chunk that was returned from
	 * setRecordPlayingMessage
	 */
	...
	/**
	 * This is the chunk that the player is currently standing over. Args:
	 * chunkX, chunkZ
	 */
	...
	/**
	 * Checks if the chunk coordinate could actually be stored within the chunk
	 * cache. Args: chunkX, chunkZ
	 */
	public boolean canChunkExist(int par1, int par2) {
		byte byte0 = 15;
		return par1 >= curChunkX - byte0 && par2 >= curChunkY - byte0
				&& par1 <= curChunkX + byte0 && par2 <= curChunkY + byte0;
	}
	...
	public boolean chunkExists(int par1, int par2) {
		...
		if (par1 == lastQueriedChunkXPos && par2 == lastQueriedChunkZPos
				&& lastQueriedChunk != null) {
			return true;
		} else {
			int i = par1 & 0x1f;
			int j = par2 & 0x1f;
			int k = i + j * 32;
			return chunks[k] != null
					&& (chunks[k] == blankChunk || chunks[k].isAtLocation(par1,
							par2));
		}
	}
	...
	public String makeString() {
		return (new StringBuilder()).append("ChunkCache: ")
				.append(chunks.length).toString();
	}
	...
	public List getPossibleCreatures(EnumCreatureType par1EnumCreatureType,
		...
		return chunkProvider.getPossibleCreatures(par1EnumCreatureType, par2,
				par3, par4);
	}
	...
	public ChunkPosition findClosestStructure(World par1World, String par2Str,
		...
		return chunkProvider.findClosestStructure(par1World, par2Str, par3,
				par4, par5);
	}
}
