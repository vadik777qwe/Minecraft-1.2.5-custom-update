...
public class ChunkProvider implements IChunkProvider {
	...
	public ChunkProvider(World par1World, IChunkLoader par2IChunkLoader,
	...
	public void dropChunk(int par1, int par2) {
		...
		if (i < -c || i > c || j < -c || j > c) {
			droppedChunksSet.add(Long.valueOf(ChunkCoordIntPair.chunkXZ2Int(
					par1, par2)));
		}
	}
	...
	public Chunk provideChunk(int par1, int par2) {
		Chunk chunk = (Chunk) chunkMap.getValueByKey(ChunkCoordIntPair
	...
	/**
	 * Attemps to load the chunk from the save file, returns null if the chunk
	 * is not available.
	 */
	...
	public boolean unload100OldestChunks() {
		for (int i = 0; i < 100; i++) {
			if (!droppedChunksSet.isEmpty()) {
				...
				Chunk chunk1 = (Chunk) chunkMap
						.getValueByKey(long1.longValue());
		...
		for (int j = 0; j < 10; j++) {
			...
			EntityPlayer entityplayer = worldObj.func_48456_a(
					(double) (chunk.xPosition << 4) + 8D,
					(double) (chunk.zPosition << 4) + 8D, 288D);
	...
	public String makeString() {
		return (new StringBuilder()).append("ServerChunkCache: ")
				.append(chunkMap.getNumHashElements()).append(" Drop: ")
				.append(droppedChunksSet.size()).toString();
	}
