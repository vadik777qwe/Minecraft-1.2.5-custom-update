...
import net.minecraft.world.generators.misc.WorldGenGlowStone2;
import net.minecraft.world.generators.misc.WorldGenHellLava;
import net.minecraft.world.generators.misc.WorldGenFlowers;
import net.minecraft.world.generators.misc.WorldGenFire;
import net.minecraft.world.generators.misc.WorldGenGlowStone1;
...
public class ChunkProviderHell implements IChunkProvider {
	 ...
	 */
	public void generateNetherTerrain(int par1, int par2,
		...
		byte byte2 = 17;
		int j = byte0 + 1;
		field_4163_o = func_4057_a(field_4163_o, par1 * byte0, 0, par2 * byte0,
				i, byte2, j);
		...
		for (int k = 0; k < byte0; k++) {
			for (int l = 0; l < byte0; l++) {
				for (int i1 = 0; i1 < 16; i1++) {
					double d = 0.125D;
					double d1 = field_4163_o[((k + 0) * j + (l + 0)) * byte2
							+ (i1 + 0)];
					double d2 = field_4163_o[((k + 0) * j + (l + 1)) * byte2
							+ (i1 + 0)];
					double d3 = field_4163_o[((k + 1) * j + (l + 0)) * byte2
							+ (i1 + 0)];
					double d4 = field_4163_o[((k + 1) * j + (l + 1)) * byte2
							+ (i1 + 0)];
					double d5 = (field_4163_o[((k + 0) * j + (l + 0)) * byte2
							+ (i1 + 1)] - d1)
							* d;
					double d6 = (field_4163_o[((k + 0) * j + (l + 1)) * byte2
							+ (i1 + 1)] - d2)
							* d;
					double d7 = (field_4163_o[((k + 1) * j + (l + 0)) * byte2
							+ (i1 + 1)] - d3)
							* d;
					double d8 = (field_4163_o[((k + 1) * j + (l + 1)) * byte2
							+ (i1 + 1)] - d4)
							* d;
	...
	public void func_4058_b(int par1, int par2, byte par3ArrayOfByte[]) {
		...
		slowsandNoise = slowsandGravelNoiseGen.generateNoiseOctaves(
				slowsandNoise, par1 * 16, par2 * 16, 0, 16, 16, 1, d, d, 1.0D);
		gravelNoise = slowsandGravelNoiseGen.generateNoiseOctaves(gravelNoise,
				par1 * 16, 109, par2 * 16, 16, 1, 16, d, 1.0D, d);
		netherrackExclusivityNoise = netherrackExculsivityNoiseGen
				.generateNoiseOctaves(netherrackExclusivityNoise, par1 * 16,
						par2 * 16, 0, 16, 16, 1, d * 2D, d * 2D, d * 2D);

		for (int i = 0; i < 16; i++) {
			for (int j = 0; j < 16; j++) {
				boolean flag = slowsandNoise[i + j * 16] + hellRNG.nextDouble()
						* 0.20000000000000001D > 0.0D;
				boolean flag1 = gravelNoise[i + j * 16] + hellRNG.nextDouble()
						* 0.20000000000000001D > 0.0D;
				int k = (int) (netherrackExclusivityNoise[i + j * 16] / 3D + 3D + hellRNG
						.nextDouble() * 0.25D);
				int l = -1;
	...
	public Chunk provideChunk(int par1, int par2) {
		hellRNG.setSeed((long) par1 * 0x4f9939f508L + (long) par2
				* 0x1ef1565bd5L);
	...
	private double[] func_4057_a(double par1ArrayOfDouble[], int par2,
	...
	public void populate(IChunkProvider par1IChunkProvider, int par2, int par3) {
		...
		for (int k = 0; k < 8; k++) {
			...
			(new WorldGenHellLava(Block.lavaMoving.blockID)).generate(worldObj,
					hellRNG, i1, k2, i4);
		}
		...
		if (hellRNG.nextInt(1) == 0) {
			...
			(new WorldGenFlowers(Block.mushroomBrown.blockID)).generate(
					worldObj, hellRNG, i2, k3, i5);
		}
	...
	public List getPossibleCreatures(EnumCreatureType par1EnumCreatureType,
		...
		if (par1EnumCreatureType == EnumCreatureType.monster
				&& genNetherBridge.func_40483_a(par2, par3, par4)) {
