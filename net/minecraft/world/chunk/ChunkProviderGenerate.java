...
import net.minecraft.world.map.MapGenStronghold;
import net.minecraft.world.map.MapGenCaves;
import net.minecraft.world.map.MapGenMineshaft;
import net.minecraft.world.map.MapGenVillage;
import net.minecraft.world.map.MapGenRavine;
import net.minecraft.enums.EnumCreatureType;
import net.minecraft.world.generators.misc.WorldGenLakes;
import net.minecraft.world.generators.misc.WorldGenDungeons;
...
public class ChunkProviderGenerate implements IChunkProvider {
	...
	public void generateTerrain(int par1, int par2, byte par3ArrayOfByte[]) {
		...
		biomesForGeneration = worldObj.getWorldChunkManager()
				.getBiomesForGeneration(biomesForGeneration, par1 * 4 - 2,
						par2 * 4 - 2, i + 5, j + 5);
		noiseArray = initializeNoiseField(noiseArray, par1 * byte0, 0, par2
				* byte0, i, byte3, j);
		...
		for (int k = 0; k < byte0; k++) {
			for (int l = 0; l < byte0; l++) {
				for (int i1 = 0; i1 < byte1; i1++) {
					double d = 0.125D;
					double d1 = noiseArray[((k + 0) * j + (l + 0)) * byte3
							+ (i1 + 0)];
					double d2 = noiseArray[((k + 0) * j + (l + 1)) * byte3
							+ (i1 + 0)];
					double d3 = noiseArray[((k + 1) * j + (l + 0)) * byte3
							+ (i1 + 0)];
					double d4 = noiseArray[((k + 1) * j + (l + 1)) * byte3
							+ (i1 + 0)];
					double d5 = (noiseArray[((k + 0) * j + (l + 0)) * byte3
							+ (i1 + 1)] - d1)
							* d;
					double d6 = (noiseArray[((k + 0) * j + (l + 1)) * byte3
							+ (i1 + 1)] - d2)
							* d;
					double d7 = (noiseArray[((k + 1) * j + (l + 0)) * byte3
							+ (i1 + 1)] - d3)
							* d;
					double d8 = (noiseArray[((k + 1) * j + (l + 1)) * byte3
							+ (i1 + 1)] - d4)
							* d;
	...
	public void replaceBlocksForBiome(int par1, int par2,
		...
		for (int i = 0; i < 16; i++) {
			for (int j = 0; j < 16; j++) {
				...
				int k = (int) (stoneNoise[i + j * 16] / 3D + 3D + rand
						.nextDouble() * 0.25D);
				int l = -1;
	...
	private double[] initializeNoiseField(double par1ArrayOfDouble[], int par2,
		...
		if (field_35388_l == null) {
			...
			for (int i = -2; i <= 2; i++) {
				for (int j = -2; j <= 2; j++) {
					float f = 10F / MathHelper.sqrt_float((float) (i * i + j
							* j) + 0.2F);
		...
		for (int i1 = 0; i1 < par5; i1++) {
			for (int j1 = 0; j1 < par7; j1++) {
				...
				float f3 = 0.0F;
				byte byte0 = 2;
				BiomeGenBase biomegenbase = biomesForGeneration[i1 + 2
						+ (j1 + 2) * (par5 + 5)];

				for (int k1 = -byte0; k1 <= byte0; k1++) {
					for (int l1 = -byte0; l1 <= byte0; l1++) {
						BiomeGenBase biomegenbase1 = biomesForGeneration[i1
								+ k1 + 2 + (j1 + l1 + 2) * (par5 + 5)];
						float f4 = field_35388_l[k1 + 2 + (l1 + 2) * 5]
	...
	public void populate(IChunkProvider par1IChunkProvider, int par2, int par3) {
		...
		int i = par2 * 16;
		int j = par3 * 16;
		BiomeGenBase biomegenbase = worldObj.getBiomeGenForCoords(i + 16,
				j + 16);
		...
		SpawnerAnimals.performWorldGenSpawning(worldObj, biomegenbase, i + 8,
				j + 8, 16, 16, rand);
		i += 8;
		j += 8;
		...
		for (int k1 = 0; k1 < 16; k1++) {
			for (int l2 = 0; l2 < 16; l2++) {
				...
				if (worldObj.isBlockHydratedDirectly(k1 + i, l3 - 1, l2 + j)) {
					worldObj.setBlockWithNotify(k1 + i, l3 - 1, l2 + j,
							Block.ice.blockID);
				}

				if (worldObj.canSnowAt(k1 + i, l3, l2 + j)) {
					worldObj.setBlockWithNotify(k1 + i, l3, l2 + j,
							Block.snow.blockID);
				}
			}
		}
	...
	public ChunkPosition findClosestStructure(World par1World, String par2Str,
		...
		if ("Stronghold".equals(par2Str) && strongholdGenerator != null) {
			return strongholdGenerator.getNearestInstance(par1World, par3,
					par4, par5);
		} else {
			return null;
		}
