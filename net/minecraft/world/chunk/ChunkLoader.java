...
public class ChunkLoader {
	...
	public static AnvilConverterData load(NBTTagCompound par0NBTTagCompound) {
		...
		anvilconverterdata.data = new NibbleArrayReader(
				par0NBTTagCompound.getByteArray("Data"), 7);
		anvilconverterdata.skyLight = new NibbleArrayReader(
				par0NBTTagCompound.getByteArray("SkyLight"), 7);
		anvilconverterdata.blockLight = new NibbleArrayReader(
				par0NBTTagCompound.getByteArray("BlockLight"), 7);
		anvilconverterdata.heightmap = par0NBTTagCompound
				.getByteArray("HeightMap");
		anvilconverterdata.terrainPopulated = par0NBTTagCompound
				.getBoolean("TerrainPopulated");
		anvilconverterdata.entities = par0NBTTagCompound.getTagList("Entities");
		anvilconverterdata.tileEntities = par0NBTTagCompound
				.getTagList("TileEntities");
		anvilconverterdata.tileTicks = par0NBTTagCompound
				.getTagList("TileTicks");

		try {
			anvilconverterdata.lastUpdated = par0NBTTagCompound
					.getLong("LastUpdate");
	...
	public static void convertToAnvilFormat(
			AnvilConverterData par0AnvilConverterData,
				...
				for (int j2 = 0; j2 < 16; j2++) {
					for (int k2 = 0; k2 < 16; k2++) {
						...
						nibblearray.set(
								...
								j2,
								k2,
								par0AnvilConverterData.data.get(i2, j2
										+ (j << 4), k2));
						nibblearray1.set(
								i2,
								j2,
								k2,
								par0AnvilConverterData.skyLight.get(i2, j2
										+ (j << 4), k2));
						nibblearray2.set(
								i2,
								j2,
								k2,
								par0AnvilConverterData.blockLight.get(i2, j2
										+ (j << 4), k2));
					}
				}
			}
		...
		for (int k = 0; k < 16; k++) {
			for (int i1 = 0; i1 < 16; i1++) {
				abyte0[i1 << 4 | k] = (byte) (par2WorldChunkManager
						.getBiomeGenAt(par0AnvilConverterData.x << 4 | k,
								par0AnvilConverterData.z << 4 | i1).biomeID & 0xff);
			}
		}
